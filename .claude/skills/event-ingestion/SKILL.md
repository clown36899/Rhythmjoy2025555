...
description: "Event Ingestion Recipe: 인스타그램 및 네이버를 통한 이벤트 포스터 정밀 수집 및 데이터 파싱. '수집 시작', '이벤트 긁어줘', '스윙 이벤트 수집해라' 요청 시 실행. 앵커 게시물(첫 번째 수집 대상) 방식으로 신규 이벤트 판별."
---
...
> [!IMPORTANT]
> **앵커 게시물(Anchor Post) 룰 — 기존 Fast-Fail 대체**
>
> 각 소스에서의 수집 흐름은 다음과 같다:
>
> 1. **첫 번째 수집 대상 게시물 = 앵커**: 최신 게시물부터 순서대로 열어보되, DJ 소셜 파티에 해당하는 **첫 번째 게시물**을 '앵커'로 지정한다. (강습/워크숍 등 관련 없는 게시물은 건너뛴다.)
> 2. **앵커는 날짜 무관 무조건 수집**: 앵커 게시물은 과거 이벤트이더라도 **날짜와 관계없이 반드시 수집**한다. 이 게시물이 다음 수집 시 '이미 수집 완료' 판별의 기준점(북마크) 역할을 한다.
> 3. **앵커가 이미 수집된 경우 = 즉시 스킵**: 앵커 게시물을 파싱한 결과가 `scraped_events.json`에 이미 존재(`[DUPLICATE]`)하면, 이 소스에는 지난 수집 이후 새 이벤트가 추가되지 않은 것이므로 **나머지 게시물 확인 없이 즉시 다음 소스로 이동**한다.
> 4. **앵커가 신규인 경우**: 앵커를 수집한 뒤, 그 다음 게시물부터 기존 중복 검사 룰을 적용한다. 중복(`[DUPLICATE]`)이 나오면 그 시점에서 해당 소스 탐색을 종료하고 다음 소스로 이동한다.
...
4. [필터] DJ 소셜 파티만 수집. 강습, 워크숍, 공연, 후기 발견 시 건너뛰고 다음 게시물 확인.
5. **[앵커 게시물 룰]** 최신순으로 열람하여 DJ 소셜 파티에 해당하는 **첫 번째 게시물(앵커)**은 날짜 무관 무조건 수집. 앵커가 이미 수집 완료된 중복이면 이 소스에는 신규 이벤트가 없으므로 즉시 다음 소스로. 앵커가 신규면 수집 후 다음 게시물로 진행하되, 중복 발견 시 해당 소스 탐색 종료.
```
...
> 🚨 **[에이전트 행동 강령 - 앵커 게시물(Anchor Post)]**: 각 소스에서 최신순으로 DJ 소셜 파티에 해당하는 **첫 번째 게시물(앵커)**은 날짜 무관 무조건 수집한다. 앵커가 이미 `scraped_events.json`에 존재(DUPLICATE)하면 신규 이벤트 없음 → **즉시 다음 소스로**. 앵커가 신규면 수집 후 다음 게시물 진행, 중복 발견 시 해당 소스 종료.
...
- [ ] 최근 6개 게시물 순차 진입 (최신순)
- [ ] **[1단계: 파싱]** 게시물 캡처 (`verify_...`) 및 상세 정보(날짜/시간/DJ/장소) 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
- [ ] **[4단계: 다운로드]** (중복이 아닐 경우) 원본(CDN) URL 확보 및 저장 (`public/scraped/poster_...`)
- [ ] **[5단계: 무결성 검사]** 파일 용량 확인 (`ls -lh`)

### 소스 2. 스윙프렌즈
- [ ] (주소 미지정 시 스킵)

### 소스 3. 해피홀
- [ ] 인스타그램 접속: https://www.instagram.com/happyhall2004/ (최근 6개 최신순 진입)
- [ ] 로그인 팝업(가입 모달) 발생 시 즉시 닫기 (비로그인 유지)
- [ ] **[1단계: 파싱]** 게시물 진입 후 스샷 캡처 (`verify_...`) 및 캡션 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
- [ ] **[4단계: 다운로드]** (중복이 아닐 경우) 원본 CDN 직접 다운로드 (`public/scraped/poster_...`)
- [ ] **[5단계: 무결성 검사]** 파일 용량 확인 (`ls -lh`)

### 소스 4. 스윙타임
- [ ] 인스타그램 접속: https://www.instagram.com/swingtimebar/ (최근 6개 최신순 진입)
- [ ] 로그인 팝업(가입 모달) 발생 시 즉시 닫기 (비로그인 유지)
- [ ] **[1단계: 파싱]** 게시물 진입 후 스샷 캡처 (`verify_...`) 및 캡션 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
...
### 소스 5. 경성홀
- [ ] 인스타그램 접속: https://www.instagram.com/kyungsunghall/ (최근 6개 최신순 진입)
- [ ] 로그인 팝업(가입 모달) 발생 시 즉시 닫기 (비로그인 유지)
- [ ] **[1단계: 파싱]** 게시물 진입 후 스샷 캡처 (`verify_...`) 및 캡션 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
...
### 소스 6. 박쥐스윙
- [ ] 인스타그램 접속: https://www.instagram.com/batswing2003/ (최근 6개 최신순 진입)
- [ ] 로그인 팝업(가입 모달) 발생 시 즉시 닫기 (비로그인 유지)
- [ ] **[1단계: 파싱]** 게시물 진입 후 스샷 캡처 (`verify_...`) 및 캡션 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
...
### 소스 7. 대전스윙피버
- [ ] 인스타그램 접속: https://www.instagram.com/daejeon.swingfever/ (최근 6개 최신순 진입)
- [ ] 로그인 팝업(가입 모달) 발생 시 즉시 닫기 (비로그인 유지)
- [ ] **[1단계: 파싱]** 게시물 진입 후 스샷 캡처 (`verify_...`) 및 캡션 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
...
### 소스 8. 스윙홀릭
- [ ] 인스타그램 접속: https://www.instagram.com/swingholic/ (최근 6개 최신순 진입)
- [ ] 로그인 팝업(가입 모달) 발생 시 즉시 닫기 (비로그인 유지)
- [ ] **[1단계: 파싱]** 게시물 진입 후 스샷 캡처 (`verify_...`) 및 캡션 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
...
### 소스 9. 네오스윙
- [ ] 인스타그램 접속: https://www.instagram.com/neo_swing/ (최근 6개 최신순 진입)
- [ ] 로그인 팝업(가입 모달) 발생 시 즉시 닫기 (비로그인 유지)
- [ ] **[1단계: 파싱]** 게시물 진입 후 스샷 캡처 (`verify_...`) 및 캡션 파싱
- [ ] **[2단계: 중복 검사]** 파싱 데이터로 `node .claude/skills/event-ingestion/scripts/check-duplicate.js` 실행
- [ ] **[3단계: 앵커 판별]** ⚠️ **(생략금지)** 첫 번째 수집 대상(앵커)이 DUPLICATE면 즉시 다음 소스로 이동. 신규면 수집 후 다음 게시물 진행, 이후 중복 발견 시 소스 종료.
