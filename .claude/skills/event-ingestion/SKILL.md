...
4.  **중복 확인 및 무결성 검사**:
    ...
    *   **[중복 대응 룰]**: 
        - `[DUPLICATE]` (85점 이상): 수집을 중단하고 해당 이벤트 스킵.
        - **[빠른 종료(Fast-Fail) 룰]**: 최신 게시물부터 순차적으로 확인(스샷->파싱->중복검증)을 진행하다가, **어느 게시물이든 중복(DUPLICATE)으로 판명되는 즉시**, 그보다 과거에 작성된 남은 게시물들 역시 모두 이미 수집되었을 것이므로 해당 소스에 대한 추가 확인을 전면 중단하고 **곧바로 다음 소스(주소)로 넘어간다.**
...
5. **[빠른 스킵]** 게시물을 최신순으로 열람 및 검증하다가 이미 수집된 중복(DUPLICATE) 데이터를 하나라도 발견하면, 그 이전(과거) 게시물들도 모두 중복일 것이므로 추가 탐색을 즉각 중단하고 다음 주소(소스)로 넘어간다.
```
...
> 🚨 **[에이전트 행동 강령 - 빠른 스킵(Fast-Fail)]**: 각 사이트에서 최신 게시물부터 순서대로 작업을 진행(스샷 -> 파싱 -> 중복검사)하되, **진행 도중 어느 단계에서든 중복(DUPLICATE)인 게시물이 확인되는 즉시**, (첫 번째든 세 번째든 관계없이) 남은 과거 게시물들에 대한 확인 작업을 즉각 중단하고 **곧바로 다음 소스로 넘어간다**.
...
- [ ] **[3단계: 빠른 스킵]** ⚠️ **(생략금지)** 중복(DUPLICATE) 판별 시, 남은 게시물 검증을 전면 취소하고 즉시 다음 소스로 이동
