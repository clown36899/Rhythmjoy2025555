...
export const excludedSourceRules = [
  {
    ...
    pattern: /(cafe\.naver\.com\/swingfamily(?:\/|$)|linktr\.ee\/swingfamily(?:\/|$))/i,
...
export const collectionSources = [
  ...
  source({ id: 'sosyalclub_swing', name: 'The Social Club 수요 발보아', scope: 'swing', genre: 'balboa', type: 'instagram', url: 'https://www.instagram.com/thesocialcluba/', venue: '소셜클럽', autoRegistrationPolicy: 'shadow', autoRegistrationAllowedActivityTypes: ['social'], allowedActivityTypes: ['social'], allowedWeekdays: [3], requiredEventPattern: /balboa\s+in\s+(?:the\s+)?social\s*club|수요(?:일)?\s*(?:발보아|소셜)|수쏘/i, priority: 1, runOrder: -0.9, notes: '공식 계정의 수요일 Balboa in Social Club만 수집한다. 날짜·장소·DJ·소셜 근거와 AI 98% 재검증을 모두 통과한 후보만 자동등록한다.' }),
  ...
  source({ id: 'swingtown-schedule-cafe', name: '스윙타운 월간 일정', scope: 'swing', genre: 'swing', type: 'naver_cafe', url: 'https://cafe.naver.com/f-e/cafes/10342583/menus/233?viewType=L', venue: '봉천살롱', autoRegistrationPolicy: 'shadow', autoRegistrationAllowedActivityTypes: ['social'], priority: 2, runOrder: -0.44, notes: '스윙타운 공식 카페의 월간 일정 게시판. 정확한 날짜·날짜별 DJ·소셜 근거가 한 구간에 함께 있는 정규 소셜만 자동등록하며, 휴강·강습·졸업파티·외부 행사가 섞인 구간은 수동 검수한다.' }),
  source({ id: 'swingtown-lessons-cafe', name: '스윙타운 외부 강습 원장', scope: 'swing', genre: 'swing', type: 'naver_cafe', url: 'https://cafe.naver.com/f-e/cafes/10342583/menus/13?viewType=L', autoRegistrationPolicy: 'shadow', autoRegistrationVenuePolicy: 'explicit', autoRegistrationAllowedActivityTypes: ['class'], allowedActivityTypes: ['class'], priority: 2, runOrder: -0.43, notes: '사용자 지정 강습 수집 원장. 외강&행사&연습실 게시판의 개별 강습만 수집한다. 장소는 각 원문에서 명시적으로 확인하며 카페 소재지로 대입하지 않는다. 미래 첫 회 날짜·원문 장소와 AI 재검증을 통과한 강습을 자동등록한다. 포스터는 선택이며 원본이 있으면 보존한다.' }),
  ...
  source({ id: 'hongdae-bonita-kakao', name: '홍대보니따 공식 카카오 채널', scope: 'salsa', genre: 'salsa', type: 'website', url: 'https://pf.kakao.com/_RIMtM', match: /^https:\/\/pf\.kakao\.com\/_RIMtM(?:\/\d+)?\/?(?:[?#].*)?$/, priority: 1, venue: '홍대 보니따', allowedActivityTypes: ['social'], autoRegistrationPolicy: 'manual', notes: '2026-09-11 공개 주간 원문·날짜별 DJ·휴무·홀별 살사 편성 확인. 본문/포스터 DJ 충돌은 검수 보류. 후보 수집 허용이며 예약 실행·공개 자동등록은 별도 검증 필요.' }),
  source({ id: 'dsn-crew-meetup', name: 'DSN Crew 공식 Meetup 소셜', scope: 'salsa', genre: 'salsa', type: 'meetup', url: 'https://www.meetup.com/ko-kr/dsn-crew/', match: /^https:\/\/(?:www\.)?meetup\.com\/(?:[a-z]{2}-[a-z]{2}\/)?dsn-crew\/events\/(?!calendar(?:\/|[?#]|$))[a-z0-9]+\/?(?:[?#].*)?$/i, priority: 1, allowedActivityTypes: ['social'], autoRegistrationPolicy: 'manual', autoRegistrationVenuePolicy: 'explicit', notes: '2026-09-11 운영자 그룹·개별 행사 날짜·목요 DJ MAX 및 클럽 라틴 확인. 수요 소셜은 강남턴으로 장소가 다름. 부속 강습의 과거 날짜/서로 다른 장소를 소셜로 가져오지 않음. 후보 수집만 허용.' }),
