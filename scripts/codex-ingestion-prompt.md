...
- Do not insert directly into `scraped_events` or `events`. Candidate inserts must go through the configured ingestor endpoint.
- Default candidate endpoint is `https://swingenjoy.com/api/scraped-events`. When `INGESTOR_V3=1`, use `https://swingenjoy.com/api/ingestor-v3/candidates`.
- The ingestor endpoint is the canonical duplicate gate. It may read live `events` for duplicate detection, but automation must never write to `events`.
