...
function filterEvents(allEvents: any[], settings: any): any[] {
  ...
  try {
    ...
    if (settingsError || !settings) {
      ...
      return {
        statusCode: 404,
        headers,
        body: JSON.stringify({ error: '빌보드 설정을 찾을 수 없습니다' })
      };
    }
    ...
    if (eventsError) {
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: '이벤트를 불러올 수 없습니다' })
      };
    }
    ...
    const billboardUrl = `${protocol}://${host}/billboard/${userId}`;
