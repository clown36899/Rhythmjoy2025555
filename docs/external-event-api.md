...
### 같은 장르 이름이 여러 분류에 있을 때

실제 사이트는 `genre` 이름만 비교하지 않고 `category + genre` 조합으로 구분합니다.

| 처리 | 실제 사이트 기준 |
|---|---|
| 중복 이름 허용 | `린디합`, `솔로재즈`, `발보아`, `블루스`, `팀원모집`은 `class`와 `club`에서 각각 사용할 수 있습니다. |
| 중복 이름 허용 | `기타`는 `event`, `class`, `club`에서 각각 사용할 수 있습니다. |
| 허용하지 않음 | 표에 없는 교차 조합: `social + 린디합`, `event + 정규강습`, `class + 파티` 등 |
| 한 번만 저장 | 관리자가 동일한 `category + genre` 권한을 중복 선택한 경우 |
...
일정 요청 한 건에는 알맞은 `category` 1개와 그 분류에 속한 `genre` 1개를 함께 보내 주세요. 관리자가 파트너에게 여러 조합을 허용할 수 있지만, 한 요청에서 여러 장르를 동시에 보내는 방식은 지원하지 않습니다. 관리자가 아무 조합도 제한하지 않은 키는 표의 모든 조합을 사용할 수 있습니다.
