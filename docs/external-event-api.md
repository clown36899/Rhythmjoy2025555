...
외부 사이트가 자기 일정 등록과 동시에 Swing Enjoy에도 같은 일정을 등록하기 위한 서버 간 API다. 조회·수정·삭제 기능은 제공하지 않는다.
...
`external_id`, `title`, `source_url`, `event_dates`는 항상 필수다. 파트너에 기본 분류가 설정되어 있으면 `category`와 `genre`는 생략할 수 있다. `external_id`가 같은 요청을 다시 보내면 새 일정을 만들지 않고 최초 `event_id`를 반환한다.
