...
- 계산·전달: 전역 관리자 연결의 고정점 반복 스캔을 동일 연결 그래프 순회로 변경해 행 순서에 따른 반복 비용을 없앴다. 반복 Locale formatter 생성을 없애고 KST 날짜/시각 분류를 유지했다. 요약 응답에서는 보이지 않는 상세 방문자/일별 항목을 제외하고, 목록을 열 때 기존 RPC에서 25명씩 읽는다. 기간 결과는 서비스 안에서 최대 3개·60초만 공유해 상세 페이지마다 재계산하지 않는다. 요약의 고유 방문자 수는 일별 고유 인원을 더하지 않고 기존 기간 중복 제거를 유지한다.
