...
- 검증: 행사 검색 정규화 회귀 7건, 대상 ESLint 오류 0건, 프로덕션 빌드와 변경 공백 검사가 통과했다. 캘린더 페이지의 헤더 연결만 변경되므로 장소·쇼핑·게시판 검색 데이터와 페이지 내부 달력 이동·강조 경로는 변경 경계에서 제외했다.
