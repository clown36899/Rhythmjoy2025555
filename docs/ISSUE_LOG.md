...
- 검증: 최초 DOM 존재 검사만으로 시각적 잘림을 놓쳐 화면 검토에서 발견했다. 실제 DB와 카카오 지도를 연결한 배포 전 빌드로 6개 검사 통과: 1440px·390px의 뱃지 전체 영역이 지도 안에 들어오는지, 서울 링크 수·클릭, 기존 스윙 다중 지역 및 빈 장르를 확인했다. 빌드·대상 ESLint 오류 0도 확인했다. 수집·회원 원장과 무관하여 운영 데이터는 다시 쓰지 않는다.
