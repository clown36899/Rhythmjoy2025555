...
- 검증: 머릿말 DOM 구조와 이미지 비드래그 회귀 테스트, 상세 수정·숨김·삭제 목록 동기화 테스트를 추가했다. 전체 Vitest 66개 파일·376개 테스트, Node 테스트 25개, 대상 ESLint와 Cafe24 운영 빌드가 통과했다. 운영 DB 연결 로컬 사이트의 433×938 모바일 렌더에서 배지는 제목 위 30×18px, 제목 간격 4px, 첫 행 높이 약 90px로 표시됐고 드래그 가능한 목록 썸네일은 0건이었다.
