...
- 상태: 강습 원문 6건 등록 준비 완료, 출처 연결 검증·배포 후 실제 등록 확인 예정.
...
- 보호: 게시판을 수집 위치 링크로 보존, 개별 글을 원문 근거로 보존. class만 허용하며 명시된 장소·원문 근거·AI 검증·완료 재등록 차단을 유지한다. 포스터 유무는 등록 조건이 아니며 실제 원본은 보존한다. 스윙 정기 실행 범위는 불변이다.
...
- 조사: SDA·라파·보스톤·라틴속으로·에버라틴·라스트댄스의 실제 카페 공지와 현재 회원 활동을 확인. 고정 공지의 본문 수정, 에버라틴 원장 이전, 동일 장소 중복 공지, 장소 미공개·기간 충돌, SA 과거 공지 등 판정 위험을 구체화했다. 카페 부재나 카카오 일변도라는 결론은 근거 없음. 신규 DB 저장·등록·새 정기수집 활성화는 이번 조사에서 실행하지 않았다.
- 기록: docs/salsa-cafe-source-research-2026-09-11.md, docs/decisions/2026-09-11-public-dance-availability.md. 조사 원문/익명 브라우저 화면 및 UI 검증 결과는 /tmp/salsa-source-expansion-20260911에 보관. 개인 신청 내역·연락처·계좌·쿠키는 Git 기록에서 제외했다.
