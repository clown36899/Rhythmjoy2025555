...
- 검증: 알림 출처·과거 URL·타 알림 보존·캘린더 재진입·알림 payload·서버 큐·기존 캘린더 레이아웃 회귀 테스트 103개가 통과했다. 날짜 고정값 때문에 자정 이후 실패하던 알림함 테스트는 실행 시점의 KST 날짜를 사용하도록 안정화했다. 최종 전체 Vitest 60개 파일·356개 테스트와 Node 24개 테스트, 대상 ESLint 오류 0건, Cafe24 단일 운영 감사 및 프런트·서버 함수 프로덕션 빌드가 통과했다. 운영 배포는 수행하지 않았다.
...
- 상태: 원인 검증·구조 개선·운영 데이터 대조 완료, 배포 전
...
- 검증: 시간대 비교, 서버 세션 집계, 분석 순수성, Android 인계, 스윙씬 개최일·제외·중복·장르·리드타임 계약의 대상 테스트 18개가 통과했다. Cafe24 프런트·서버 함수 빌드, 대상 ESLint 오류 0건, 변경 공백 검사가 통과했다. 알림 진입 후속 작업까지 포함한 최종 전체 Vitest는 60개 파일·356개 테스트가 모두 통과했고 Node 테스트 24개와 Cafe24 단일 운영 감사도 통과했다. 첫 전체 실행에서 기존 키오스크 테스트의 teardown 후 timer 오류가 한 번 관찰됐지만 단독 재현과 전체 재실행은 모두 통과했다. 운영 DB나 배포 서버에는 아직 쓰기·배포를 수행하지 않았다.
