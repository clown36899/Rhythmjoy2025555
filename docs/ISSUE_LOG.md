...
- 검증: 수집 휴무의 생성본 대체·원문 링크·사유 보존, 같은 입력 재시도 무변경, 실제 소셜 충돌 시 미생성, 공식 API 휴무 링크, 캘린더 DJ 슬롯과 리스트 배지 `휴무`, 기존 졸공·일반 소셜 경계를 검증했다. 정규 소셜·외부 API·캘린더 대상 Vitest 29건과 수집 표준 검사, 대상 ESLint 오류 0건이 통과했다. 수집 파서는 변경하지 않아 실제 원문 재수집은 중복 검증에서 제외하고 기존 후보 원장을 운영 조정 입력으로 사용한다.
