...
- 검증: 실제 운영 형태처럼 첫 항목 자체 이미지·둘째 항목 이미지 없음·요약 대표 이미지가 함께 있는 회귀 사례를 기존 알림함 테스트에 확장했다. 이미지 없는 둘째 카드가 대표 이미지를 공유하지 않고 기본 아이콘을 쓰며, 단일 알림 자체 이미지는 유지됨을 확인했다. 전체 Vitest 제품 스위트 74개 파일·469개 테스트와 Node 전용 35개 테스트, 대상 ESLint·TypeScript 무출력 검사, Cafe24 프로덕션 빌드와 `git diff --check`가 통과했다. 서버 생성 데이터는 운영 원장에서 항목별 값이 정상임을 확인했고 생성·전송 코드는 변경하지 않았으므로 중복 서버 테스트를 새로 추가하지 않았다.
- 관련 커밋: `pending`
