...
## 2026-08-22 경성홀 날짜별 소셜·올어바웃스윙 졸업공연 누락

- 상태: 근본 원인 수정·회귀 검증 완료, 운영 반영 진행 중
...
- 근본 원인: 날짜별 소셜 분해는 문서 전체 금지어 검사보다 먼저 성공했지만 뒤의 전역 `MT` 검사 결과가 분해된 후보까지 폐기했다. 장소 판별은 알려진 별칭 또는 출처 기본 장소만 인정해 일반 `📍/장소:` 라벨의 새 장소를 누락했다. 수집 링크나 Instagram 접근 실패는 아니었다.
- 수정: 완전한 날짜·DJ 소셜 구간이 존재할 때는 문서 뒤쪽 MT·휴관 문맥을 해당 구간에 전파하지 않고 날짜별 소셜만 보존한다. 순수 MT 게시물과 날짜·DJ가 불완전한 게시물은 계속 제외한다. `📍`, `장소:`, `개최지:`, `venue/location:`의 명시적 한 줄 장소를 원문 근거로 보존하고, 미정·추후 공지·강습별 상이·URL/DM 값은 거부한다. 원문 장소는 출처 기본 장소보다 우선한다. 잘린 이모지 surrogate가 DJ 이름에 저장되지 않도록 추출·검증도 보강했다.
- 유지한 불변조건: 순수 MT·대관·과거 일정 차단, 날짜·DJ·장소·이미지 필수, 같은 원문·날짜 후보 ID와 실제 일정 중복 방지, 수동 출처의 자동등록 금지를 유지했다. 새 DB·큐·상태값과 특정 게시물 ID 예외는 추가하지 않았다.
- 검증: 수집 표준 회귀에서 혼합 공지의 세 소셜이 각각 `8/22 제니스`, `8/25 BBB`, `9/1 해림`으로 분리되고 MT 문맥이 각 구간에 섞이지 않으며 순수 MT는 계속 차단됨을 확인했다. 실제 경성홀 원문 무저장 실행은 `발견 1 → 판별 1 → 분해 3`, 자동등록 준비 3건, 오류·차단 0건이었다. 실제 졸업공연 원문은 `행사 / 2026-08-22 / 마포구청 대강당` 후보 1건으로 통과하고 기존 수동 출처 정책에서만 멈췄다. 자동등록·분류 서버 회귀 29개도 통과했다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`
