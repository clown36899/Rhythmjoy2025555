...
- 배포·데이터 영향: 운영 데이터 수정과 배포는 수행하지 않았다. 스키마·마이그레이션·새 상태값이 없다.
...
- 배포·데이터 영향: 운영 일정 등록·수정과 배포는 수행하지 않았다. DB 마이그레이션이 없으며 롤백 시 UI 연결과 로컬 저장 키만 제거하면 된다.
...
- 배포·데이터 영향: 운영 게시글 등록·수정과 배포는 수행하지 않았다. 기존 JSON 저장 구조라 마이그레이션이 없고, 필드가 없는 기존 글은 일반 공개 글로 처리한다.
