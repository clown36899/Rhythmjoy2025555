...
- 검증: 실제 운영 데이터와 같은 `네오 8/23 졸업파티 / NEO SWING 140기 / social / 졸공`이 `졸공 140회`로 표시되고, 회차 없는 졸공은 `졸공`, 일반 소셜은 `DJ 메이저`, 미정은 공백으로 남는 회귀를 추가했다. 캘린더 종류·레이아웃 Vitest 10건, 대상 ESLint 오류 0건과 Cafe24 프로덕션 빌드가 통과했다. 구현 커밋 `bc71db85`를 푸시한 뒤 Cafe24 공개 빌드 `1787413881060` (`2026-08-22T15:51:24.450Z`)을 배포했고 외부 헬스가 정상이다. 운영 Chromium에서 이벤트 `844a4dcb-3878-4562-b16b-ec77dd110590`의 두 줄이 `해피홀 / 졸공 140회`이며 `DJ` 접두어가 없음을 확인했고, 일반 소셜 이벤트는 `DJ 충하`로 유지됨을 함께 확인했다.
- 관련 커밋: `bc71db85`
