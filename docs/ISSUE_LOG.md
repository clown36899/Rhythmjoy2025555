...
- 검증: `NewEventsBanner` 독립 아이콘·배지·클릭 확인 콜백과 `HomeV2MenuPanel` 하단 미고정 제외·고정 포함·개인/관리자 우선순위, 혜택 미확인 ID 안정성을 함께 검증한 대상 Vitest 3개 파일 8건이 통과했다. 빌드·운영 검증은 진행 중이다.
- 배포·데이터 영향: 1차 수정 커밋 `87a11553`과 개인 설정 우선순위 보완 `a8b8d357`은 운영 빌드 `1787389701811`에 포함됐다. 독립 아이콘 교정은 배포 전이며 DB·스키마·메뉴 설정·일정 원장을 수정하지 않는다.
