...
- 상태: 공통 원인 수정·운영 데이터 읽기 전용 미리보기 완료·배포 전
...
- 검증: 기존 휴무 재시도 테스트를 날짜 경과와 표시 행 삭제 복구 경계까지 확장해 수집 휴무 보존·복구, 일반 과거 생성본 삭제, 다음 미래 회차 생성을 한 테스트에서 확인했다. 정규 소셜 14건과 외부 API 인접 3건의 Vitest, 대상 ESLint가 통과했다. 수정 코드를 운영 DB에 읽기 전용 `dryRun`으로 적용한 결과 `creates: 3`, `removes: 0`이었고 대상 `regular-social:kyungsung-sat:2026-08-29`와 근거 원장이 남은 과거 휴무 2건만 복구 대상으로 나왔다. 운영 DB 쓰기·실제 복구·배포·공개 UI 확인은 아직 배포 지시가 없어 제외했다.
- 관련 결정: `docs/decisions/2026-08-23-regular-social-closure-visibility.md`
