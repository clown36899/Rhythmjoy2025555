...
- 상태: 구현·로컬 검증, 배포 확인 대기. 앞선 살사 별도 안내 화면은 요구 오해로 철회한다.
...
- 검증: 운영 DB 연결 데스크탑/390px 모바일의 원래 화면·장르별 조회·바로가기·되돌아오기 15개 검사, 데이터가 있는 살사 샘플 표시/분리/비드래그 8개 검사 통과. 선택한 기존·인접 Vitest 8개 파일 47건 통과. 대상 ESLint 오류 0건(기존 경고 87건), Cafe24 로컬 빌드와 diff 공백 검사 통과. 배포 결과는 완료 후 보강한다. 수집/알림 코드는 변경하지 않아 그 외부 부작용 경로는 재실행하지 않는다.
