...
- 상태: 재발 원인 재조사와 구조 변경·로컬 검증 완료, 과거 backfill 포함 재배포 준비.
...
- 검증: 회귀 5파일 28개 통과(서버 일별 마감·복구·중복·보호 11, UI 읽기/명시 재생성/실패·시간 제한 7, 기존 순수성·세션·기기 10), 서버 TypeScript 검사와 서버 묶음 빌드 통과, 대상 ESLint 오류 0(기존 이동 코드의 any 등 경고). 앱 프로젝트 타입 검사는 통계 변경 파일의 진단은 없으나 수정하지 않은 다른 파일의 기존 오류가 남아 전체 성공으로 보고하지 않는다. 운영 원본 읽기 전용 대조: 9/22 회원1·게스트20·활동145·세션25, 9/20 회원0·게스트12·활동71·세션14와 상세 목록 수 일치. 실제 운영 의존 모듈/실제 일별 보고서로 Chromium 390px 확인: 날짜 이동 포함 raw query 0, browser write 0, 페이지 오류·가로 넘침 0.
