...
- 기존 구현 판정: `일부 있음`. 전역·사용자 메뉴 고정/순서 원장, `benefits` 메뉴 ID·경로, 숨김 판정, 혜택 미확인 배지와 목록 진입 확인 처리가 이미 있었다. 정확한 빈틈은 2026-07-26 혜택 진입점을 추가하면서 이 원장을 재사용하지 않고 두 곳에 강제 노출을 추가한 것이다. 또한 개인 행이 있으면 개인값, 없으면 관리자 기본값을 읽는 경로는 있었지만 개인값이 로컬 초기 배치와 같다는 이유만으로 기본값으로 추정하는 과거 호환 분기가 남아 있었다. 새 필드·테이블·상태값은 필요하지 않다.
- 기존 보호 목적: 혜택 목록은 확장 메뉴에서 언제든 찾을 수 있고, 관리자가 고정하면 메인 컴팩트 메뉴에 놓을 수 있어야 한다. 개인 설정 행이 있는 사용자는 그 값을 유지하고, 행이 없는 사용자·비로그인 방문자는 관리자 기본값을 공유해야 한다. 새 혜택 수 배지와 혜택 목록 진입 시 확인 처리, 직접 URL 진입, 원데이·연습실 빠른 진입도 유지해야 한다.
- 수정: 컴팩트 메뉴는 기존 `pinnedMenuIds`의 최대 5개만 렌더하고 `benefits` 강제 추가를 제거했다. 광고·레거시 홈 카드의 별도 혜택 진입점과 반응형·키오스크 잔존 스타일도 제거했다. 기존 미확인 계산은 메뉴 소유자인 `HomeV2MenuPanel`에 재연결해, 혜택을 실제로 고정했을 때만 메인 아이콘과 배지가 함께 보이고 누르면 기존처럼 확인 처리한다. 레이아웃 선택은 값 비교 추정을 없애고 `user_home_menu_settings` 행 존재 시 개인값, 행 부재 시 `home_menu_layout_defaults`만 적용한다.
- 유지한 불변조건: 고정하지 않은 메뉴는 확장 메뉴의 순서에만 남고 메인에 자동 승격하지 않는다. 최대 5개 고정, 개인값 우선·미설정 사용자 관리자 기본값 공유, 전역 숨김, 관리자 편집, 혜택 분류·목록·읽음 원장, 원데이·연습실·자유게시판 동작은 변경하지 않았다. 기본값과 같게 저장할 때 개인 행을 지우고 리셋 시 개인 행을 지우는 기존 상속 전환도 유지한다. 스키마·운영 데이터 쓰기·새 드래그 동작은 없다.
- 검증: 고정 미포함·포함·배지 확인과 광고 영역 독립 진입점 미생성, 혜택 목록 직접 진입 확인 처리, 로그인 사용자의 개인 행 부재 시 관리자 기본값 적용, 개인값이 로컬 초기 배치와 같아도 개인값 유지 경계를 회귀검사로 확장했다. Vitest 제품 단언 449건과 Node 전용 4건, TypeScript 무출력 검사, 대상 ESLint 오류 0건, Cafe24 프로덕션 빌드, `git diff --check`가 통과했다. Node 기본 러너 파일 2개를 Vitest가 수집하는 기존 혼선은 해당 파일만 원래 러너로 분리해 실제 성공을 확인했고, 이 UI 변경과 무관해 설정 수정에서 제외했다. 운영 DB 읽기 전용 감사에서 관리자 기본 행 1건과 개인 행 1건을 확인했고 개인 행은 현재 기본값과 다른 배치라 행 존재 우선 전환 뒤에도 기존 결과를 보존한다. 운영 DB 연결 로컬의 390×844·1512×982에서 기본 고정 5개에만 메인 아이콘이 나오고, 별도 혜택 버튼 0개·확장 메뉴 혜택 1개·원데이 1개·연습실 1개·page error 0건을 확인했다. 쓰기 없는 고정 설정 응답 대체로 `benefits`를 고정한 경우에만 메인 아이콘·배지 `2`가 보이고 클릭 후 혜택 목록으로 이동하며 브라우저 오류가 없음을 확인했다. 배포 후 캐시를 차단한 새 브라우저의 390×844·1512×982에서도 고정 메뉴는 관리자 기본 5개, 메인 혜택 아이콘 0개, 광고 옆 혜택 버튼 0개, 확장 메뉴 혜택 1개였고 page error·예상 밖 HTTP 오류는 모두 0건이었다.
- 배포·데이터 영향: 구현 커밋 `87a11553`과 개인 설정 우선순위 보완 커밋 `a8b8d357`을 푸시하고 Cafe24 공개 빌드 `1787389701811`로 배포했다. 서비스 `active`, 내·외부 헬스 `ok`, 공개 버전을 확인했다. DB·스키마·메뉴 설정·일정 원장은 수정하지 않았다.
