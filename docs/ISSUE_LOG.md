...
- 조치: 머릿말을 데스크톱과 모바일 모두 제목 바로 위의 30×18px 소형 배지 행으로 옮기고 기존 머릿말 열과 모바일 메타데이터 들여쓰기를 제거했다. 상세 모달에 목록 변경 콜백을 연결해 수정 저장 시 상세와 목록 조회를 함께 기다린 뒤 편집창을 닫는다. 삭제는 목록 갱신 후 상세창을 닫고, 숨김 전환과 댓글 추가·삭제도 성공 후 목록을 다시 조회하도록 통합했다.
- 검증: 머릿말 DOM 구조와 이미지 비드래그 회귀 테스트, 상세 수정·숨김·삭제 목록 동기화 테스트를 추가했다. 전체 Vitest 66개 파일·376개 테스트, Node 테스트 25개, 대상 ESLint와 Cafe24 운영 빌드가 통과했다. 운영 DB 연결 로컬 사이트의 433×938 모바일 렌더에서 배지는 제목 위 30×18px, 제목 간격 4px, 첫 행 높이 약 90px로 표시됐다. 커밋 `c867db26`을 푸시하고 Cafe24에 배포했으며 서비스와 외부 헬스가 정상이고 공개 빌드는 `1786589380072` (`2026-08-13T02:49:44.717Z`)다. 운영 모바일·데스크톱 목록 모두 제목 위 배지와 제거된 기존 머릿말 열을 확인했고 드래그 가능한 목록 썸네일은 0건이었다.
- 관련 커밋: `c867db26`
