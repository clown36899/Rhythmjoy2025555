...
- 회귀 범위: 수집 표준 통과. 같은 유료 수강 조건을 스윙/살사에 공통 적용, 명시 프로모션 보존, 선택적 유료 후속 강습이 있는 무료 체험 보존, 재처리 시 기존 잘못된 혜택 필드 제거·포스터 없는 일반 강습 수집 유지 검사. 실제 기존 5개 payload를 prepareCandidate로 재처리해 모두 유효한 class/혜택 미지정 확인. 기존 AI 심사 검사 28건 통과. 최초 테스트 실행기는 잘못 골랐으나 해당 파일의 기존 Vitest로 재실행해 통과했다.
