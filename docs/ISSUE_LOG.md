...
- 상태: 근본 원인 수정·회귀 검증 완료, 운영 배포·재수집 대기
...
- 검증: `node scripts/test-ingestion-standards.mjs`와 자동등록·정규 소셜 회귀 38개가 통과했다. 두 공식 원문의 무저장 실행에서 발견 9문서, 판별 9문서, 날짜별 후보 4건을 출력했고 해피홀 8월 21일 DJ가 `테일`, 스윙타운 8월 22·25일이 자동등록 준비 상태로 확인됐다. 8월 29일 졸업파티는 수동 검수 차단 회귀로 검증했다.
