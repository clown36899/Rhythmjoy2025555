## 2026-09-25 — 소셜 달력·상세 가독성 개선 및 분리 배포 준비

- 상태: 화면 수정과 실제 공개 데이터를 이용한 320/390/1440px 검증 완료. 커밋·푸시 후 프런트 배포 예정.
...
- 상세 결정: [가독성과 배포 경계](decisions/2026-09-25-social-readable-ui.md). 운영 entry 사본 보존 후 배포하고 공개 검증 결과를 후속 기록한다.

## 2026-09-25 — 소셜 자동수집 재활성화 및 새 공지 탐색 시간대 복구

- 상태: 사용자 후속 지시에 따라 중단 결정을 철회하고 Mini PC·등록 서버의 소셜 자동수집/등록을 재활성화했다. 공식 공지 연결은 수집 일정과 함께 제공한다. UI는 로컬 실데이터 미리보기·빌드 검증 완료, 공개 UI 미배포.
