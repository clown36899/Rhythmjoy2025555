...
- 상태: 공통 원인 수정·회귀 및 운영 DB 읽기 검증 완료·배포 전
...
- 검증: 붙여 쓴/띄어 쓴 날짜·분류, 공백·구분 기호, 구조화 날짜·분류, 모든 키워드 필수, 설명 속 다른 날짜·분류 오탐 경계 회귀 7건이 통과했다. 대상 ESLint 오류 0건과 Cafe24 프로덕션 프런트 빌드가 통과했다. 운영 DB를 읽기 전용으로 조회해 `28일소셜`과 `28일 소셜`이 동일한 8월 28일 소셜 3건을 포함하고, `29일소셜`이 28일 해피홀 행을 제외하며 `28일강습`이 소셜 행을 제외하는 것을 확인했다. 배포 요청은 없어 운영 반영은 제외했다.
