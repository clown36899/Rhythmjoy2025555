...
- 상태: 공통 원인 수정·회귀 검증 완료, 운영 원장 보정·배포 대기
...
- 검증: 미오새와 동일한 운영 본문 포함 중복, same-ID pending 재판정, 후보 간 다른 원본 중복, 소셜 DJ 불일치, 정규 생성 소셜 교체, 2018년 플랫폼 게시일, 다음카페 2자리 작성일, 실제 본문 날짜와 다른 글 날짜 경계를 추가했다. 자동등록 연결 Vitest 32건과 전체 수집 표준 검사가 통과했다.
