...
- 검증: 서버 중복·정규 소셜 대상 테스트 28개가 통과했다. 새 규칙을 운영 미래 소셜 199행에 읽기 전용 적용한 결과 중복 후보는 신고된 스윙타임 2행으로 구성된 1그룹뿐이었고, DJ 미정 정규 일정과 다른 일정은 포함되지 않았다. 운영 중복 행과 후보 상태의 실제 정리는 배포 후 수행한다.
