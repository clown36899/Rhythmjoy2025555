## 2026-09-25 — 소셜 달력·상세 가독성 개선 및 공개 배포

- 상태: 커밋·푸시 후 공개 UI 배포 완료. 실제 공개 사이트의 320/390/1440px 검증 통과.
- 경로/기준: 긴 장소·DJ → 48px 너비에 맞춘 최소 4.8px 자동 축소 → 28px 카드와 긴 안내 상자 → 장소·상태·공식 공지를 알아보기 어려움. 기존 구현은 일부 있음. 기존 달력 카드/높이 상수·상태/출처 유틸리티·상세 날짜/장소/링크를 재사용했다. 직접 변경 이력 `6be6fa60`/`37932ebc`의 작은 칸에 내용을 맞추던 목적을 확인했다.
- 수정: 글씨 축소를 제거하고 장소 11.5px·상태 11px, 줄바꿈과 최소 64px 자동 높이를 적용. 등록·정규 요일·휴무를 글자/색으로 구분하고 상단 안내를 간결하게 정리. 상세는 날짜·장소를 먼저 표시하고 공식 공지 18px 주 버튼/해당 일정 원문 보조 버튼으로 분리. 긴 본문은 접고 펼칠 수 있게 했다. 포스터 없는 빈 칸과 내부 전사 표식의 공개 노출을 정리했다.
- 유지한 보호: 모든 날짜 셀의 전체 일정 표시, 정규 회차/휴무/실제 DJ 구별, 정확한 출처 링크, 비드래그, 별도 시간 입력/영역 금지, 기존 저장값·관리자 편집·자동/수동 등록·완료/중복/동일 장소 충돌·삭제 보호. 새 DB·상태·등록 경로 없이 기존 소유자만 수정했다.
- 선택 검증: 배포 작업본 관련 11개 Vitest 파일 210개, 알림 정책·배포 범위 Node 검사 11개, 수집 표준과 프런트 빌드 통과. 생성된 통계 서버 모듈이 없어서 첫 2개 suite 로딩이 실패했으나 기존 생성 명령 후 해당 64개 검사를 재실행해 통과했다. 실제 공개 API 화면의 카드 75개, 최소 글씨 크기, 잘림/가로 넘침 0, 날짜/버튼 순서, 정확한 두 링크, 본문 펼치기, 페이지 오류 0을 확인했다.
- 배포 경계: 운영에 이미 공개된 UI와 통계 이력을 통합한 깨끗한 작업본을 사용한다. 기존 배포기의 검증·전송·버전 마지막 공개 경로에 `--frontend-only`를 연결해 API·DB·cron·수집기 덮어쓰기와 서비스 재시작을 제외했다. 기존 전체 배포와 변경 미커밋 차단도 같은 실행형 검사로 유지 확인했다. 다른 미완료 UI/관리자 DJ 편집은 포함하지 않았다. 데이터/수집 동작은 이미 별도로 검증·반영되어 UI 배포를 위해 원장 재등록·전체 재수집·마이그레이션을 반복하지 않는다.
- 상세 결정: [가독성과 배포 경계](decisions/2026-09-25-social-readable-ui.md).
- 배포 결과: `56a85735`를 `origin/codex/social-ingestion-links-20260925`에 먼저 커밋·푸시한 뒤 기존 명령 `npm run deploy:cafe24 -- --frontend-only`로 공개 UI 배포 완료. 공개/빌드 version `1790336697731` 일치, entry 3파일 SHA 일치, health 정상·서비스 active. 공개 320/390/1440px에서도 카드 75개·최소 글씨 11.5/11px·잘림/가로 넘침/페이지 오류 0·정확한 두 공지 링크·본문 펼치기 검증 통과.
- 운영 보존: 서버 API/통계 모듈/레지스트리/패키지/통계 cron 7개 해시 불변, 서비스 시작 시각 19:07:35 KST로 이번 배포 재시작 없음 확인. Mini PC 두 모듈 해시 기존 적용값 일치, 6개 timer와 키오스크 2개 서비스 active. 수집 출처별 접근 실패를 이번 UI 배포로 해결했다고 주장하지 않는다. 배포 전 entry 사본은 `/opt/swingenjoy/backups/social-readable-ui-20260925-56a85735/`. 구형 systemctl의 `--value` 미지원으로 PID 기록만 재실행했으며 entry 사본/해시 보존은 이미 완료돼 있었다.

## 2026-09-25 — 소셜 자동수집 재활성화 및 새 공지 탐색 시간대 복구

- 상태: 사용자 후속 지시에 따라 중단 결정을 철회하고 Mini PC·등록 서버의 소셜 자동수집/등록을 재활성화했다. 공식 공지 연결은 수집 일정과 함께 제공한다. UI는 같은 날 가독성 개선 후 `56a85735` 공개 배포 완료(상단 기록 참조).
- 경로/원인: 예약 31개는 복구돼 있었지만 `isSupplementalRecoveryRun`이 `lastDiscoveryAt`의 KST 날짜만 비교 → 오전 한 번 탐색 후 오후·저녁의 설정된 탐색 시간도 당일 보충 모드 → 오늘 미확정 소셜이 없으면 브라우저 0 → 새 미래 공지 발견이 다음 날까지 막혔다. Mini PC의 실제 priority1/2 마지막 탐색은 9/25 08:00/09:00이었다. 이전 사보이 날짜 오추출·해피 URL 완료 오판까지 이 한 원인으로 설명하지 않는다.
- 기존 구현 **일부 있음**: 기존 탐색 시간 설정·`lastDiscoveryAt`·완료/보류 항목·오늘 공개 원장 선정자를 재사용했다. 하루 비교를 현재 탐색 시간대 비교로 고쳤으며 소셜 중단 상수는 true로 복구했다. 관련 미커밋 줄의 최초 목적은 9/23 결정 기록, `dcd369e0`/`fe2b77c6`의 완료·재개 보호는 scoped git diff로 확인했다. 새 DB·큐·필드·작업·등록 경로 없이 기존 소유자만 수정했다.
- 보호 목적/불변조건: 성공 항목 재추출·재저장 방지, 동일 내용 미래 실패는 당일까지 보류, 당일 실패만 다음 예약 재시도, 지난 회차 종료, 등록·휴무·관리자 삭제 후 중단을 유지한다. 원문 날짜/장소 근거·AI 검증·동일 장소 충돌 검토·쓰기 잠금도 그대로다. 지연 기동은 현재 시간대 한 번으로 따라잡고 놓친 예약을 연속 실행하지 않는다.
- 운영: 서버는 운영 레지스트리의 정책/설명만 변경하고 health 정상·서비스 active 확인(SHA `58649af20c6ac7445b26814f8304962767883b6496f966c17996e586aa1332fb`). 첫 준비 확인 대기가 짧아 자동 원복 후 제한된 health 대기로 재적용했다. Mini PC는 유휴 상태에서 레지스트리·진행 모듈 두 개만 교체, 6개 진행 파일 해시 불변 및 표준 검사 확인, 기존 6개 timer 재개. 진행 모듈 SHA `8f2a5c23342e2bde05ca158c30632307171167d38aa95a901d47d4d0ec3086bf`.
- 실제 실행: `20260925_191012_3158247`는 저장된 오전 상태에서 `mode=new-discovery`로 진입했다. 360초 제한 시험의 예상 작업시간 보호에 의해 조기 종료(exit 75)했으며, 루나/비밥/스윙잇 계정 목록은 로그인 화면·공개 fallback 불가, 신규 저장/등록 0이었다. 전체 성공으로 보고하지 않는다. `20260925_191236_3159125`는 같은 전용 브라우저로 네오스윙 이전 성공 글 `DdDYinBzasB`와 현 주간 글 `Ddkbx2cTSNq`를 읽고 휴무 2개를 읽기 전용으로 추출(exit 0, 쓰기/AI 0). `20260925_191410_3159653`는 네오 공식 계정에서 게시물 12개 발견·1개 읽기 성공, 접근 실패 0. 특정 출처 실패를 전체 로그인 장애로 확대하지 않았다.
- 실제 반복 억제: `20260925_191435_3159965`는 같은 탐색 시간대의 정상 wrapper 실행에서 `today-only-retry`, 오늘 미확정 0·브라우저/AI/저장/등록 0·exit 0. 기존 실패 출처 전체를 재시도하지 않았다. 공개 API에서 해피 9/25 휴무·9/26 소셜 기존 ID·9/27 휴무를 재확인했다. 새 운영 등록 성공을 이번 검증 성과로 주장하지 않는다.
- UI: 자동 갱신 중단 문구를 제거하고 수집 일정+공식 공지 안내를 표시했다. 상세의 큰 공식 공지 버튼과 해당 일정 원문 링크를 유지하고, 정규 요일 참고 안내/등록 일정/휴무를 구분한다. 실제 공개 API를 읽는 1440/390/320px 미리보기에서 정확한 두 링크, 비드래그, 가로 넘침 0·페이지 오류 0 및 고정 안내 노출을 확인했다.
- 선택 검증: 관련 8파일 129개 및 알림 정책 8개, 로컬/Mini PC 표준 검사, 프런트 빌드 통과. 기존 메인 시험을 실제 시간대 판정에 연결해 422/503 실패→다음 예약 등록→후속 브라우저 0을 검증했다. 다음 시간대 새 미래 글 탐색, 기존 성공 미재저장, 미래 실패 보류, 같은 시간대 무탐색, 지연/자정/레거시를 함께 확인했다. 구조 변경이 없어 DB 마이그레이션·원장 초기화·전체 재수집·다른 미배포 서버/UI 배포는 제외했다.
- 정책/롤백: [탐색 시간대 복구 결정](decisions/2026-09-25-social-discovery-windows.md). 중단 결정 기록·운영 README·수집 스킬의 현재 정책을 함께 수정해 이전 중단/하루 한 번 규칙이 재적용되지 않도록 했다. Mini PC/서버 백업은 결정 기록 참조.

## 2026-09-25 — 소셜 자동수집 중단 및 공식 공지 중심 안내

- 상태: **이 중단 결정은 같은 날 사용자 후속 지시로 철회됐다. 현재 정책은 상단 복구 기록을 따른다.** 아래는 당시 적용 결과다. 소셜 자동수집/등록 중단을 Mini PC·운영 서버에 적용했다. UI는 로컬 구현·실데이터 미리보기 완료, 공개 사이트에는 미배포. 기존 일정·원장 삭제 없음, 강습/행사/혜택과 관리자 수동 등록 유지.
- 경로/기준: DJ 없는 등록 소셜과 정규 생성본 → 빈 달력 보조 문구/제목에 숨겨진 출처 링크 → 운영 확정과 원문 근거 구별 어려움 → 기존 `link1`, `automation` 생성/예외 표식, 원문 출처·주최 ID 및 레지스트리를 연결. `bc71db85`의 미정 DJ 비노출과 `a2ca355e`의 휴무 표시, `ed4182c16`의 출처별 활동, 서버 자동/수동 보호 목적을 확인했다.
- 기존 구현 **일부 있음**: 공개 표시는 기존 달력 유틸리티·상세·출처 레지스트리를 재사용했다. 출처별 허용만으로 혼합 글·남은 후보·휴무 별도 경로를 함께 중단할 수 없어 기존 레지스트리에 전역 소셜 정책과 활동 허용 함수를 두었다. 새 DB·큐·원장 상태·등록 경로는 없다.
- UI: 정규 생성본은 ‘정규 요일’/점선, DJ 없는 등록본은 ‘소셜’. 휴무/졸공·기존 명시 DJ는 보존. 캘린더 고정 영역에 공식 공지 연결과 자동 갱신 중단을 표시한다. 상세는 주 동작 ‘공식 공지 확인’과 저장된 원문 링크를 분리하며, 프로필을 개별 원문으로 표시하지 않는다. 원문 없는 수동 일정에는 링크를 만들지 않는다. 정규 생성본의 기존 ‘공지 확인 시 갱신’ 설명은 공개 보기에서 숨기되 저장값/관리자 편집은 유지한다.
- 운영: 소셜 AI/휴무 추출, 후보 저장, 서버 자동등록과 당일 보충 재시도를 차단했다. 소셜 전용 출처는 제외하고 혼합 출처의 강습/행사는 유지한다. 소셜 지도 감사도 정책 중단을 누락으로 경고하지 않는다. Mini PC `20260925_185154_3155615` exit 0·브라우저/AI/자동등록 0, 모듈 해시 일치, 기존 6 timer와 키오스크 active 확인. 서버의 실제 기존 후보 자동등록 요청은 422와 정책 중단 사유를 반환하고 이벤트는 쓰지 않았다. 강습/행사 허용은 유지됨을 확인했다.
- 인접 수정: 지도 SDK 객체만 있고 `maps.load`가 없을 때 상세 전체가 실패했다. `3d33d060`의 기존 미준비 보호를 같은 조건문에서 확장했다. 새 지도/외부 호출 경로는 없다.
- 검증: 관련 8개 Vitest 파일 128개 및 알림 정책 8개, 수집 표준 검사 통과. 이전 자동등록·충돌/삭제/동일 이벤트 보호 검사는 이전 정책을 주입해 보존했고, 현재 기본 정책에서는 보충 실행의 원장/브라우저 미접근, 혼합 문서의 강습만 저장/완료, 소셜 AI·휴무 추출 미실행, 서버 intake/잔여 후보 차단과 관리자 수동 등록을 검사했다. 1440/390/320px 실제 공개 API 미리보기에서 공식 계정/개별 원문 URL 분리·비드래그·가로 넘침 0·페이지 오류 0, 오늘로 스크롤한 뒤 정책 안내 노출을 확인했다. 프런트 빌드 통과. DB/일정 데이터 변경이 없어 마이그레이션·전체 재수집·무관한 서버 전체 배포는 제외했다.
- 범위/복구: 서버 운영 SHA 비교 후 이번 정책 부분만 반영, 최초 잘못된 health 경로는 자동 원복 후 실제 `/__health`로 재확인해 200. 예약 재개 때 unloaded unit glob이 매칭되지 않아 확인 후 6개 정확한 timer 이름으로 재개했다. 백업/책임/롤백은 [결정 기록](decisions/2026-09-25-social-official-links.md). 로컬 UI 확인은 기존 실제 API와 이미지의 읽기 프록시를 사용하며 운영 데이터 쓰기 없음.

## 2026-09-25 — 해피홀 주간 공지 미수집과 과거 휴무 오추출

- 상태: 원인 수정·Mini PC/등록 서버 반영 완료. 공개 9/25 휴무, 9/26 토요 소셜, 9/27 휴무 확인. 후속 정상 실행은 오늘 대상 0·브라우저 미기동. 기존 6타이머와 키오스크 active 확인.
- 증상 → 판정 → 부작용 → 원장/공개: 9/25 08:00~17:30 매시간 neo_swing 선정 → 예전 URL 열람 기록만 보고 12개 글 전부 건너뜀 → 당일 회차는 미확정인데 재추출 없이 재시도 종료 → 공개는 DJ 미정 placeholder. 완료 문서/후보 기록은 비어 있었다. 9/22 원문을 실제 Mini PC에서 다시 읽자 `9월 4주`가 9/4 휴무로 재현돼, 실행 중단/링크 부재와 구분했다.
- 성공 대조: 같은 공식 계정 `/neo_swing/p/DdDYinBzasB`의 9/11 DJ 쓴귤 후보 `162e4122046da88d` → 이벤트 `261114d5-5258-4dbc-be94-319c4dc5e398`, 9/13 DJ 아홉 후보 `0fb96803b8819601` → `ed2f0a36-7ce5-4ee7-9ab8-4447f5d24177`. 이번 `/neo_swing/p/Ddkbx2cTSNq/`는 수집 브라우저의 게시 시각 9/22 09:20 KST, 본문 금햅/일햅 추석 휴무, 포스터 09.25~09.27·금요일/일요일 휴무. 별도 `/happyhall2004/p/Ddk16JhRVkq/`는 9/26 토요 정모·소셜과 금/일 휴무를 명시한다.
- 기존 구현 **일부 있음**: URL 체크포인트(`dcd369e0`)를 항목 완료 원장과 연결하고, 기존 문서 지문에서 CDN 서명 변경을 무시한다. 예외 날짜 함수(`6a49f44b7`)의 과거 조사용 fallback을 일반 수집과 분리하고, 공통 휴무 추출에 게시 주/명시 요일 목록을 연결했다. 주차를 날짜로 읽지 않는다. 기존 하루 주기 정규 일정 반영(`ba466398f`)을 휴무 intake 성공 전에 호출한다. 공식 해피홀 계정을 기존 수집기/서버 자동등록 허용 목록에 연결했다. 새 DB·상태·큐·예약은 없다.
- 불변조건: 이미 완료한 후보의 추출/쓰기 억제, 미래 실패의 당일까지 보류, 과거 회차 종료, 명시 원문·원본 포스터·AI 검증, 같은 날/장소 충돌 검토, 관리자 삭제·휴무·정규 placeholder 보호를 유지한다. 재조회는 기존 한 번의 제한된 묶음이며 실행 중 무한 반복하지 않는다.
- 실검증: 수정 전 dry-run `20260925_180035_3142887`은 잘못된 9/4 후보. 수정 후 `20260925_180703_3144201`은 9/25·27 휴무 두 건. 정상 재시도 `20260925_181128_3145767`은 구형 열람 기록의 첫 두 글을 검증하고 미완료를 유지했고, 다음 `20260925_181327_3147229`는 오늘 휴무를 실제 반영한 뒤 대체 출처 접근을 중단했다. 두 실행의 exit 75는 다른 잔여 문서/미래 회차가 남은 부분 실행이며 전체 성공으로 해석하지 않는다. 원문 두 개로 한정한 주간 복구 `20260925_181449_3148284`는 exit 0, 토요 후보 `71cc4d9f0b97c8ab`를 공개 이벤트 `82713f17-7c11-4358-89ac-966692f77cac`에 collected 연결, 9/27 휴무도 공개 확인. `20260925_181709_3150802`는 exit 0·수집 0·브라우저 미기동.
- 운영 정정: 9/4의 잘못된 휴무 원장도 두 기존 행 백업 후 수정했다. 당시 `/happyhall2004/p/DcxbYy0RWoN/` 포스터/본문의 DJ 호두를 확인해 같은 공개 ID를 정정하고 오류 후보의 closure 적용을 해제했다. 기존 삭제/쓰기 잠금 및 오류 ID 비교 사용, 신규 과거 후보·이미지 삭제·새 알림 없음.
- 회귀: 기존/인접 Vitest 7파일 122개와 알림 정책 8개, 수집 표준 검사 통과. 실제 main→Instagram 선택→구형 기록→실패/재시도/공개 성공/중단 연결, 날짜/주차/KST·연말, 혼합 강습/소셜, 미래 휴무 보류, 실제 포스터 파일 변경과 CDN 서명 변경, 휴무 공개 쓰기 실패 후 재시도, DJ 없는 공식 계정의 공개 충돌을 검증했다. UI·스키마·Naver 목록·인증·예약 빈도는 이번 최초 실패 경계가 아니므로 변경/전체 재검사를 제외했다.
- 운영 범위: Mini PC 네 모듈·표준 검사 파일 갱신, 등록 서버는 원본 SHA 확인 후 이번 휴무 반영 연결/공식 계정 한 행만 반영. health 200, 서버 SHA `a7a7ffc3be470277c3e946698d0e17c7b854d34a1e97b9168f9e9c909ac2408d`. 다른 로컬 미배포 변경은 유지. 백업/롤백과 상세 책임은 [결정 기록](decisions/2026-09-25-weekly-social-completion.md)에 남겼다.
- 후속 화면 확인: 9/26 미반영 제보 후 공개 캘린더 카드 클릭과 새 직접 링크에서 해피홀 토요 소셜·원본 포스터·9/26 날짜 표시까지 확인(18:26). DJ 미발표 원문이라 달력 카드는 장소만 표시한다. 제보 화면/캐시 원인은 미확인으로 구분하고, 무근거 DJ 추가나 중복 등록은 하지 않았다.

## 2026-09-24 — 사보이 소셜의 원문 오추출과 미등록 중복 후보 차단

- 상태: 공통 원인 수정 및 Mini PC·등록 서버 운영 반영 완료. 9/24 사보이 DJ 해림 실제 공개 등록, 이후 재실행의 대상 0·브라우저 미기동 확인. 인접 9/26 기존 DJ 오등록도 원문/저장 포스터 대조 후 같은 이벤트 ID로 정정. 관련 회귀 159개·표준 검사 통과.
- 증상 → 판정 → 부작용 → 원장: 9/22 게시된 9/24 사보이 공지 미등록 → Naver 바깥 영역에서 작성자 혼입, AI fallback이 원문 제목 날짜 우선 규칙을 건너뜀 → 지난주 글로 만든 DJ 미상·9/24 잘못된 후보가 일반 제목/장소 유사도로 정상 후보를 duplicate 처리 → 오늘 10:30 AI 승인에도 저장 API terminal skip, 공개는 정규 placeholder만 유지. 11:30에도 같은 원인으로 실패했다. 전원·예약 미실행이 원인은 아니었다.
- 성공 기준: 지난주 글 102745·후보 d98608881745bf55·공개 이벤트 c6d91fe4-bcc3-47d7-bc50-597234de1eca. 오늘 글 102764는 9/22 11:31 게시, 본문/포스터 DJ 해림. 9/22 16:36 생성된 정상 후보 7b7cc6de0c02240d를 차단한 512deb57259d153b는 미등록이며, 저장 포스터는 DJ 몽룡·날짜 없음인데 AI 전사는 9.24였다. 실제 링크 오류나 원문 부재로 단정하지 않았다.
- 기존 구현 **일부 있음**: 준비 대기/본문 읽기를 기존 utility 한 곳으로 이동; 기존 selectCandidateDates의 제목 날짜 권위를 AI fallback에 연결; 기존 canReopenScrapedCandidateDuplicate와 공개 충돌 검토를 재사용. 일반 유사 제목이 소셜의 엄격한 동일성 규칙을 무효화하는 공통 경로만 제거했다. 새 DB·진행 필드·큐·예약·중복 소유자는 없다.
- 보호 목적/이력: d61966c8·d8ab0f9e·bcf7015e의 Naver 신/구형 본문/제목 보호, 4913535f의 지난 제목 날짜 차단, e3f76dc3의 엄격한 중복, ec0a7be1의 검증 후보 재개를 해당 함수·호출부·기존 검사와 대조했다. 같은 원문/날짜 호환 내용·확정 DJ 중복·공식 API 우선·등록 완료·제외/관리자 삭제·휴무를 유지한다. 같은 날/장소 공개 충돌은 pending 검토 및 쓰기 잠금 안 재확인으로 보호한다.
- 검증: 본문/작성자/댓글/이미지 소유, 구형 DOM, 빈 페이지·이미지 글, 지난 명시 날짜의 AI 호출 방지, AI 날짜 충돌/정상/월간 글, intake→잘못된 duplicate 재개→등록→같은 이벤트 재시도, 기존 당일 재시도 중단·휴무·삭제·공개 충돌·수동·잠금 검사 등 7파일 159개 및 표준 수집 검사 통과. 원문 제목→수집기 호출 경계도 검사해 화면용 cleanTitle이 앞의 날짜를 지우기 전에 원문 제목을 보존하고, 일반 추출과 AI 추출 모두 같은 날짜 제한을 쓰도록 확인했다. 운영 원장 전체를 읽기 전용으로 대입해 정상 후보 검증 통과·재개 가능·확정 중복/공개 충돌 없음을 확인했다.
- 운영 반영: Mini PC 두 변경 모듈의 수정 전 사본과 운영 코드가 일치함을 확인하고 기존 6타이머만 잠시 중단해 백업 후 갱신. dry-run 20260924_114507_2899910에서 작성자 혼입 없는 9/24·해림을 원문/포스터로 재검증했다. 서버의 다른 미배포 목록 변경은 포함하지 않고 운영 원본에 중복 판정 변경만 적용, 동시 변경 SHA 검사·백업·문법 검사·health 확인. 첫 확인 명령은 구형 curl 옵션 미지원으로 안전 원복했고, 지원 Node health 확인으로 다시 반영해 내/외부 ok 및 서비스 active 확인. 후보/진행 원장은 초기화하거나 직접 수정하지 않았다.
- 추가 공통 원인: 운영 재검증에서 원문 작성자 혼입은 없어졌지만 영문 장소 인용을 AI 검증기가 거부했다. c3250dac5의 장소 동의어 보호는 AI/서버에 별도 문자열 치환으로 남아 있었고 기존 공통 장소 별칭을 사용하지 않았다. `venueNormalization.mjs`의 동일 별칭을 근거 문장에도 검사하는 함수 한 곳으로 연결했다. 별칭 테이블을 복제하거나 장소 근거 필수·원문 인용 일치·출처 고정 장소 검사를 해제하지 않았다. 영문/한글 일치와 장소 누락/다른 장소/거짓 인용을 검사했다.
- 실제 복구: 20260924_114743_2903165는 서버 재시작 확인과 겹쳐 HTTP 503으로 쓰기 실패했고 당일 미확정으로 남았다. 20260924_114946_2905578은 장소 근거 검증 실패로 안전 보류했다. 두 경우 모두 완료로 기록하지 않았다. 장소 공통 규칙 연결 후 20260924_115601_2907917에서 정상 후보 7b7cc6de0c02240d 재개→공개 이벤트 513d8f90-2dd0-42eb-a368-527c7de18ed4 등록→공개확인 1/누락 0, 오류 0. 저장 요청 ID 25e9662a5482c96c가 실제 응답의 기존 콘텐츠 ID 7b7cc6de0c02240d로 연결되는 것도 확인했다. 20260924_115903_2909829는 selectedSources/pendingSources 모두 빈 목록, 브라우저 미기동·AI 호출 0·등록 0이었다. 6타이머 재개, 키오스크 Chrome/화면 서비스 PID 1365388/1864 유지.
- 인접 잘못된 내용 정정: 9/26 글 102765는 본문과 원본/저장 포스터 모두 DJ 테일인데 공개 이벤트 9b42fd60-9cd7-4cb3-a23f-ae1c0d9f6df2는 쓴귤이었다. 원문과 다른 모델 출력이 정상 등록까지 통과한 이력이다. 새 판정 예외를 추가하지 않고 검토한 운영 데이터만 기존 saveCafe24TableRow로 수정했다. 원문·날짜·장소·기존 등록 후보 연결 검사, 보관 사본, 기존 이벤트 변경 잠금 안 원장 동일성 검사 뒤 제목/설명을 원문으로 정정했다. ID/포스터/날짜/장소/소유자 유지, 신규 이벤트·재알림 없음. 원래 잘못 추출한 수집 후보는 조사 이력으로 보존했다. 최종 공개 API에서 9/24 해림, 9/26 테일 각 1건 확인. 운영 백업: 서버 backups/savoy-evidence-20260924, Mini PC ingestion-runs/savoy-root-20260924.
- 제외/한계: 전체 UI/통계 배포, SNS 전체 재탐색, 빈도 증감, 같은 장소/날짜만으로 후보 삭제·합치기 제외. 인식 불가/근거 충돌은 보류하고 이미지 인식의 완전 무오류를 주장하지 않는다. 상세 판단·롤백은 [결정 기록](decisions/2026-09-24-social-evidence-and-duplicate-recovery.md).


# Project Issue Log

## 2026-09-15 — 경성홀 재수집 반복 실패: 날짜 표기 비호환 및 보충 재수집

- 예약 적용 완료: 기존 실행 종료 후 priority1/2 모두 재로드, 각 13개 예약 트리거·전체 수집 설정 확인. db1ec557의 로컬 조회 실패 종료 상태 보완도 적용·검증했다. 서버 함수 변경이 없어 이 후속만을 위한 웹 재배포는 하지 않았다.
- 운영 반영: 661fbd6f 커밋/푸시 후 Cafe24 배포, buildTime 1789443495777·health 정상·운영 파일 해시 일치. 기존 자동등록 API로 오늘 경성홀 DJ Deniz 1건 복구(23cbf65d-4af9-4c70-b7f9-d20f22277239). 공개 실제 일정 1건/기본 미정 교체·포스터 HTTP 200 확인.
- 08:00/12:30 당일 재수집은 실제 실행됐지만 경성홀 포스터의 `2026 09 15`/대체 텍스트 `20260915`를 날짜 근거로 인정하지 못해 등록 차단. 미완료 큐는 정상 유지됐다. 기존 AI 날짜 보호 목적(환각·시각 오인식 차단)을 확인하고 수집/AI/서버의 기존 판정 함수에 명시 연월일 표기 호환을 추가했다. 날짜 힌트 없는 추출 실패도 기존 AI 재검증 1회에 연결했다.
- 기존 구현 일부 있음 판정에 따라 새 큐/상태/수집기를 만들지 않았다. priority1/2 기존 예약을 매시 보충하고, 보충 시간대에는 당일 미확정 소스만 확인한다. 다른 대기 원장/전체 완료 시각, 관리자 삭제·휴무·중복/0.98 근거 검증 보존.
- 기존/인접 회귀 110개·표준 검사 및 실제 native 모의 외부 경계 4종 통과. 실제 원문/포스터 재입력에서 9/15 경성홀 DJ Deniz 1건이 근거 인용을 바꾸지 않고 AI 0.99 승인. 전역 신뢰도 실측이 아니다. UI/DB 구조 변경 없음; 세부 근거·범위/제외·롤백은 docs/decisions/2026-09-15-social-date-evidence-recovery.md 참조.

## 2026-09-13 — 해피홀 당일 누락 복구 및 미확정 소셜 자동 재수집

- 운영 적용 완료: aca1e9e0 커밋 및 origin/codex/social-retention-recovery-20260910 푸시. Mac 예약 실행 코드 동기화·검사 후 기존 priority1/2 LaunchAgent 재로드, 각 5개 트리거 확인. 웹사이트 번들 변경 없음.
- 증상→판정→부작용→원장/외부 결과: 아침 실행이 네오/해피홀 공식 글을 확인 완료 URL로 건너뜀 → 이번 실행 후보만 완결성 검사 → 당일 누락이 재시도 큐에 안 남음 → 공개 해피홀 DJ 미정 생성본 유지. 기존 구현 일부 있음; 체크포인트와 잔여 소스, 공식 소스 레지스트리, 등록 후 공개 검증 및 LaunchAgent를 재사용해 누락 연결만 보완.
- 기존 보호 목적은 반복 읽기 비용/중복 방지. 당일 미확정 소스만 우선 재독하고 최대 2개 확인 완료 Instagram 글을 재확인한다. 실패/미반영은 기존 remainingSources에 유지하며 공개 조회 실패는 완료 처리하지 않는다. 관리자 삭제·공식 휴무·정규 기본값 보존·날짜 기준·AI 98% 및 서버 등록 제한은 유지한다.
- 기존 priority1/2 예약에 낮·오후 재확인 창을 추가한다. 새 큐·필드·테이블·예약 작업 소유자는 없다. 실행기는 Mac이며 서버/UI 변경이 없어 웹 번들 재배포는 제외한다.
- 실제 원문 재수집으로 오늘 9/13 해피홀 DJ 아홉 근거 확인 및 기존 자동등록 성공(이벤트 ed2f0a36-7ce5-4ee7-9ab8-4447f5d24177). 공개 실제 공지 1건/해피홀 미정 0건/관리자 삭제한 오늘 경성홀 0건 확인. 출처별 당일 미확정 대조 잔여 0개.
- 표준 검사 확장, native 실제 흐름의 모의 외부 경계 3종, AI/DOM/삭제 보호/정규 소셜 63개, 잠금 3개 통과. 무관한 UI/DB 전체 검사는 제외. 전체 정확도 99%를 실측했다는 뜻이 아니다. 운영 설치 및 세부 제약은 docs/decisions/2026-09-13-same-day-social-recovery.md 참조.

## 2026-09-13 광고 대체 카드·관리자 삭제 우선순위 운영 반영 완료

- 커밋/배포: 542e6e7d를 origin/codex/social-retention-recovery-20260910에 선푸시한 뒤 기존 Cafe24 배포 명령 성공. 공개 version buildTime=1789289048441 (2026-09-13T08:44:12.298Z), __health 정상, swingenjoy/crond/httpd 모두 active. 서버 핵심 변경 파일 4개의 SHA-256이 배포 작업본과 일치한다.
- 검증: 관련 10개 파일 114개 테스트·빌드 통과. 실제 운영 메인의 포스터 없는 골든린더벅 강습에서 제목·설명·날짜·장소 카드 표시, 기본 포스터 img 0, draggable=false, 390px/1440px에서 카드 내용 범위·가로 넘침 없음·브라우저 오류 0 확인. 기존 포스터 광고도 같은 목록에 유지된다.
- 경성홀 운영 정정: 사용자 확인(RSF로 정규 소셜 휴무)에 따라 9/12·9/13의 정확한 regular-social:kyungsung-sat:2026-09-12 및 regular-social:kyungsung-sun:2026-09-13 두 생성본만 제거했다. 삭제 전 생성 출처/분류/DJ 미정/이미지 없음/예외 없음 검사를 수행했고 서버 backups/data-fixes/kyungsung-rsf-admin-delete-20260913-1789289115634.json(0600)에 원본을 보존했다. 명시적 유지보수 작업자 maintenance:codex-user-request-20260913으로 관리자 공통 원장+삭제 트랜잭션을 호출했으며 다른 사용자 세션을 사용하지 않았다.
- 결과: 공개 API 대상 0건, 제외 원장 2건, 다른 이벤트 전 필드 비교 동일. 운영 MySQL 연결 잠금 및 실제 삭제 원장 조회로 같은 ID와 새로운 DJ 후보의 저장 차단 확인. 정규 조정 dry-run creates=0/removes=0/retained=196, 두 회차 재생성 없음. 데이터 복구는 제외 원장 해제와 백업 원본 복원으로 가능하며 새 DB 스키마는 없다. 실제 화면 삭제 경로의 관리자 성공/실패/소유자/타인 권한 경계는 핸들러 회귀검사로 확인했다.

## 2026-09-13 관리자 삭제 우선순위 및 메인 광고 무포스터 카드 배포

- 범위: 관리자 삭제 후 정규 소셜 재생성 차단, 포스터 없는 메인 광고 제목·설명 카드. 운영과 해시가 일치한 b0918457 작업본에 관련 변경만 통합하며 별도 키오스크·실험실 및 기존 작업 폴더의 무관한 변경은 포함하지 않는다.
- 삭제 경로/기존 목적: events 물리 삭제 후 반복 규칙·수집 등록이 삭제 의사를 몰라 회차를 재생성했다. 0960c01ca의 소유자/관리자 삭제, bfd54fc99의 이미지 정리, 6a49f44b7의 정규 생성 및 fa36fb821의 excluded 후보 원장 억제를 조사했다. 기존 구현 일부 있음: excluded/_exclusion 원장을 재사용하고 관리자 삭제 ID·날짜/장소 소셜 회차를 연결했다. 시간/이미지는 차단 기준으로 사용하지 않는다.
- 삭제 수정: 관리자 삭제 3경로(직접 이벤트 API, generic 삭제, 실제 화면의 /api/delete-event)가 같은 원장+삭제 트랜잭션을 사용한다. 후보 중복·등록, 정규 생성/휴무/과거 보존, 두 이벤트 저장기가 공통 차단 기준을 읽는다. 저장과 삭제는 MySQL 연결 잠금으로 직렬화하며 외부 API는 커밋/롤백까지 유지한다. 비관리자 소유자 삭제·내부 조정 정리는 제외 원장을 생성하지 않는다. 날짜/장소·다른 댄스 범위·행사/강습/졸공을 보존하고 명시적 제외 해제 전 자동 복원하지 않는다. 새 테이블·큐·상태/스키마 없음.
- 배포 전 연결 보완: 앞 로컬 검사에서 두 삭제 API만 연결했고 실제 useEventModal/useEventActions/useEventSelection 호출부의 /api/delete-event가 빠졌음을 발견했다. 같은 저장기로 연결하고 실제 핸들러의 관리자 성공/실패, 소유자 삭제, 타인 권한 거부 경계를 추가 검증한다. 경성홀 전용 예외를 추가하지 않았다.
- 광고 경로/수정: 이미지 없음/로딩 실패 → getMainAdImage의 기본 이미지 → 소셜 전용 대체 조건 때문에 행사·강습 포스터 영역에 정보 없음. 7f49380da의 소셜 fallback과 e7056d0bc 사진형 카드를 모든 광고 분류로 확장하고 제목·HTML 제거 설명·날짜·장소를 표시한다. 실제 포스터는 유지하며 작은 카드 줄 제한, 텍스트 인디케이터, 드래그 차단을 적용한다. 광고 선정/수집 데이터 구조는 바꾸지 않는다.
- 검증 계획/경계: 삭제 원장 실패·롤백·잠금 실패·저장 직전 차단, 실제 삭제 핸들러, 정규 생성/과거 보존/휴무/재시도, 후보 등록, 외부 API·권한, 광고 번역/바로가기/무이미지·로딩 오류/HTML/클릭을 검사한다. 앞 로컬 Chromium fixture의 360x640·390x844·1440x1000에서 카드 내용 범위·가로 넘침 없음·페이지 오류 0 확인. 전체 무관한 UI/수집 재실행은 제외하며 실제 운영 DB 상태와 배포 결과는 아래 후속 기록에 남긴다.
- 운영 정책 대안: 이번 회차 완전 삭제와 휴무 안내(RSF 대관 사유 표시)를 구분한 관리자 선택을 제안했으나 별도 UI는 구현하지 않았다. 사용자는 9/12·9/13 경성홀 정규 소셜은 RSF 때문에 휴무라고 확인했다. 원문 부재만으로 휴무를 추정하지 않는다.
- 배포 전 검증 완료: 운영 기반 작업본의 관련 Vitest 10개 파일 114개 통과, 대상 ESLint 오류 0(기존 광고 경고 2), diff 공백 검사 통과. 기존 실행 중인 서버 핵심 파일 4개 해시가 기준 작업본과 일치함을 확인했다.

## 2026-09-11 살사 혜택 재오분류: 일반 유료 강습의 조건을 별도 행사로 취급

- 상태: 운영 살사 혜택 5건 해제, 공통 판정 수정·검증 및 배포 완료. c9a38b5d/34bf3ba9를 origin/codex/social-retention-recovery-20260910에 선푸시 후 배포(version 1789114118412, health ok, service active). 직전 “무료 → 할인” 교정은 사용자 의도에 맞지 않는 잘못된 해결이었다.
- 재발 조사 경로: 일반 강습 원문/생성된 강습 제목 → prepareCandidate의 본문 전체 할인 단어·금액 매칭 → 일반 강습 등록과 함께 benefit_eligible 전달 → 무료·할인 목록에 SDA 1건 및 에버라틴 4건 잔존. 일반 등록의 AI 날짜·장소 검증은 혜택 행사의 성격을 확인하는 검증이 아니다. 직전 금액 표현 보완은 오히려 평상시 수강 조건을 혜택으로 유지시켰다.
- 최초 목적 재조사: f6711d21d/57dbc9e63의 명시 혜택 수집, 2bb4b35c1 및 docs/decisions/2026-08-13-benefit-classification-evidence.md의 일반 요금과 혜택 행사 구분, 기존 prepareCandidate → benefitFieldsFromStructuredData → 공개 목록과 benefit AI 심사 경로를 다시 확인했다. 기존 보호 목적은 일반 요금 안내를 혜택으로 승격하지 않고, 실제 무료 참여·할인 행사·정기권 판매를 구분하는 것이다. 기존 8/13의 명시 할인 단어 기준만으로는 평상시 수강 조건까지 통과하는 빈틈이 있었다.
- 기존 구현 판정: 일부 있음. 기존 분류기·일반/혜택 필드·혜택 AI 심사 prompt를 재사용. 제목에서 혜택을 주제로 공지하거나 본문에 별도 할인 행사/캠페인을 공지한 근거가 있어야 할인 판정을 진행하도록 공통 분류 경계를 보완했다. 조건별 단어 삭제 예외나 살사/동호회/행사 ID 전용 분기를 만들지 않았다. AI 심사 설명도 같은 기준에 맞췄으며 새 AI 호출·필드·테이블·큐·분류 함수 없음.
- 원장 확인: 9/11 두 공개 원문 재열람. SDA는 수강료 95,000원 및 선입금/재수강/동반 조건, 에버라틴은 수강료 8만원 및 동시 신청/재수강 조건을 포함한 일반 강습 공지다. 현재 5건을 무료·할인에서 제외하라는 사용자 정정을 반영했다. 구체적 근거 링크는 직전 항목과 동일하며 원문 전체/연락처/계좌는 Git에 복사하지 않는다.
- 복구: 후보 3822692f5149a5fc와 직전 에버라틴 후보 4건의 연결 이벤트를 benefit_eligible=false/benefit_kind=null로, 후보는 혜택 필드·lifecycle 미지정으로 정정. 운영 backups/data-fixes/ordinary-class-benefit-*-20260911.json에 원본 0600 보관, dry-run·행 잠금·동시 변경 검사·단일 트랜잭션·쓰기 후 비교로 5쌍 수정. 완료 상태와 원문·ID·일정·작성자·포스터·일반 강습 등록은 보존했다. 공개 API 전체 대상 비교에서 혜택 필드/updated_at 외 변경 없음, 살사 혜택 0건·일반 강습 8건 확인.
- 회귀 범위: 수집 표준 통과. 같은 유료 수강 조건을 스윙/살사에 공통 적용, 명시 프로모션 보존, 선택적 유료 후속 강습이 있는 무료 체험 보존, 재처리 시 기존 잘못된 혜택 필드 제거·포스터 없는 일반 강습 수집 유지 검사. 실제 기존 5개 payload를 prepareCandidate로 재처리해 모두 유효한 class/혜택 미지정 확인. 기존 AI 심사 검사 28건 통과. 최초 테스트 실행기는 잘못 골랐으나 해당 파일의 기존 Vitest로 재실행해 통과했다. 재처리 검사의 짧은 원문은 개강과 신청 날짜 구분이 없어 기존 날짜 보호에 막혔으므로, fixture에 실제 개강일을 명시하고 날짜 보호 코드는 유지했다.
- 운영 최종검증: 복구 재실행 5건 모두 alreadyApplied. 배포된 서버 분류기로 실제 원래 5개 payload를 다시 평가해 정상 class/혜택 미지정, 별도 무료 체험·명시 할인 이벤트는 유지됨을 확인. 외부 version과 서버 health 확인. 원래 작업 폴더에는 이번 변경만 동기화했다.
- 유지/제외: 원문 근거·무료 부정형·유효기간·정기권·수동 명시 혜택 편집·등록 권한·collected 재등록 거부·알림 규칙은 유지. UI·스키마·스케줄·서버 저장은 수정하지 않아 해당 전체 회귀는 반복하지 않았다. 공개 운영 PC/모바일에서 살사 빈 혜택 목록과 기존 스윙 6건 분리 확인. 스윙 운영 혜택 데이터는 수정하지 않는다.

## 2026-09-11 무료·할인 목록 장르 누락과 살사 재수강 무료 오분류

- 경로: 메인의 선택 장르 → 무료 바로가기에서 dance 누락 / BenefitEventsPage의 전체 benefit_eligible 필터 / 원데이 변환에서 dance_scope 누락 → 두 장르의 목록·새 글 숫자·읽음 ID가 혼합. 별도 경로는 원문 “8만원, 재수강 무료, 5천원 추가할인” → classifyConfirmedBenefitEvent가 재수강의 부분 문자열 “수강 무료”를 무료로 판정하고 한국어 단위 할인을 놓침 → prepareCandidate 및 기존 서버 benefit 필드 전달 → 에버라틴 강습 4건이 free_event로 공개됨.
- 기존 구현 판정: 일부 있음. 공통 isEventInDanceScope/normalizeVisibleDanceScope, 기존 원데이 dance_scope, 기존 benefit 목록·ID 기반 읽음 저장·분류기·서버 whitelist를 재사용. 장르별 새 페이지/DB/상태/큐/파서 없음. 페이지·두 메인 진입점·읽음 훅만 공통 장르 경계에 연결했다. 원데이 데이터도 기존 필드를 보존한다.
- 최초 목적/이력: f6711d21d·cd085c191의 혜택 분류와 936f3ecf4의 목록, 40c426539의 원데이 병합, e3a069f30의 안정된 읽음 ID, 2bb4b35c1의 단순 얼리버드 오탐 방지, 43965c42c 및 8/22 이슈의 부정형 무료 배제가 직접 관련된다. 무료 주차·음료·경품·종료/부정 표현을 실제 무료 참여로 오인하지 않는 보호 목적을 유지한다. 같은 오분류 증상에 행사명/ID 예외를 추가하지 않고 한국어 활동명 경계 및 금액 표기의 공통 원인을 수정했다.
- 수정: 유료 재수강/재입장을 무료 참여로 부분 매칭하지 않게 하고, 5천원/1만원 및 추가·중복 할인 표현을 인식. 기존 무료 라인강습·살사강습·무료 체험·정기권·할인 우선순위를 유지. 공개 스윙·살사만 활성, 나머지 준비중 및 메인 장르 왕복 유지. 명시 장르 없는 기존 스윙 데이터, 실제 포스터, 날짜/정렬·지난 일정 표시·회원/비회원 읽음 저장은 유지한다.
- 운영 근거/복구: 9/11 공개 원문 https://m.cafe.naver.com/ca-fe/web/cafes/16855256/articles/2039 재열람에서 수강료 8만원, 2인 이상 동시 신청 합계 5천원 추가할인, 수료까지 재수강 무료 확인. 후보 258e5afae6991b07 / b6a0000f29f797c6 / d980da88e567e7ce / ee11203d67220d4e와 연결 이벤트 4건을 기존 8/22 복구 절차(잠금·사전 백업·동시 변경 검사·트랜잭션·검증)에 따라 discount_event로 정정. 백업은 운영 backups/data-fixes/everlatin-benefit-*-20260911.json(0600). collected 상태·ID·날짜·장소·본문·포스터·소유자는 그대로이며 삭제·재등록·알림 재발송 없음. 복구 재실행은 4건 alreadyApplied. 활성 원데이 중 혜택 지정 원장은 현재 0건으로 추가 복구 대상 없음. SDA는 별도의 유효한 동반/재수강 할인이 있어 유지했다.
- 검증: 기존 UI/읽음/메인 진입/공통 taxonomy 검사 29건 통과(추가 네비게이션 단언 포함 7건 재확인). 수집 표준에 유료 재수강·재수강료·재입장/한국어 금액 할인/종료 할인/기존 무료 강습 경계를 확장해 통과. 현재 원문 전체도 discount_event 판정. 운영 DB 연결 로컬 1440px/390px에서 살사 할인 5건, 스윙 6건(미분류 발보아 포함) 분리·메인 왕복·이미지 유지·드래그 불가·가로 넘침 없음·브라우저 오류 0 확인. 화면 검사 초기 실패는 영어 제목과 React 화면 반영 전 읽기였으며 검증 선택자/대기를 바로잡았다. 빌드 통과, 대상 ESLint 오류 0(기존 export 경고 1). 공개 API 비교에서 대상 4건은 혜택 종류/updated_at 외 모든 필드 동일.
- 제외 범위: 수집 스케줄·등록 권한·자동/수동 저장 경계·알림·DB 스키마는 변경하지 않아 전체 등록/인증 검사를 반복하지 않았다. 기존 수집 표준의 혜택 전달/날짜/완료 재시도 검사를 유지했고, 수동 명시 혜택을 새로 재해석하지 않는다. 과거 잘못 일괄 읽음 처리된 사용자 기록은 근거 없이 초기화하지 않는다.
- 배포: 3f772ef1을 origin/codex/social-retention-recovery-20260910에 먼저 커밋·푸시 후 Cafe24 배포 완료. version 1789113180641, 배포 health 검사 성공 및 service active. 운영 1440px/390px에서도 장르별 목록·메인 왕복·포스터·가로 넘침/브라우저 오류 검증 통과. 운영 서버에 배포된 공통 분류기를 직접 실행해 유료 재수강 null / 동시 신청 discount_event / 무료 라인강습 free_event 확인. 원래 작업 폴더에는 이번 패치와 이 기록만 동기화했다.

## 2026-09-11 일반 사용자 살사 등록 연결과 추가 원장 조사

- 경로: 일반 회원이 행사/강습 등록의 살사 범위를 선택 → EditableEventDetail.allowedGenreOptions가 swing/unknown만 허용, EventRegistrationModal 효과가 비관리자 salsa를 swing으로 복원 → 기존 저장 payload의 장르 선택이 막히거나 불일치 → events.dance_scope/genre 및 공개 캘린더 반영 누락. 기준 데이터는 공통 publicAvailable, 로그인/관리자 상태, 분류 선택값, 저장 payload다.
- 기존 구현 판정: 일부 있음. 345d2b961의 관리자 외 확장 장르 보호가 두 하드코딩에 남았고 d8a02eff의 공통 공개 목록은 이미 스윙·살사를 허용한다. 기존 scopeOptions와 normalizeVisibleDanceScope로 누락된 연결만 수정했다. 일반 회원은 두 장르, 관리자는 기존 다섯 장르, 레거시 unknown 선택·로그인·작성자 필터·기존 이미지 정책 유지. 새 화면/DB/등록 API/권한 우회 없음.
- 조사 경계: 등록창과 EditableEventDetail, calendar/v2/events/MyActivities/ModalRegistry 직접 호출부, shared taxonomy, generic-data-api의 로그인·작성자 보호 및 schema.sql의 events.dance_scope를 확인. 최초 공개 보호 이력과 해당 직접 변경만 조사했으며 스키마 변경은 필요 없었다.
- 수집 연결 판정: 일부 있음. 수라댄 QdX3 공개 강습 게시판은 기존 레지스트리에 없고 같은 책임의 class-only/explicitVenue 설정과 서버 등록 경로는 7e303341에 있다. 이를 재사용해 게시판 한 곳을 연결. 두 실제 공지는 포스터에서 날짜·장소를 직접 읽고 poster_text 근거를 보존했다. 새 함수/큐/테이블/파서 예외 없음. 신규 정기 탐색 활성화와 등록 허용 설정은 구분한다.
- 검사: 분류창 기존 검사 확장(일반/관리자·취소·스윙·레거시), 실제 등록 부모의 행사/강습/동호회 생성 payload·기존 살사 소유자 편집·비로그인 차단, 기존 서버 등록/메타데이터 보호를 합쳐 69건 통과. 수집 표준 통과. 소스 허용·활동/명시 장소/AI/날짜/완료 재시도 보호는 기존 매개변수 검사에 확장해 중복 테스트를 피했다. DB 구조·알림·스윙 스케줄은 변경하지 않아 해당 전체 회귀는 제외.
- 후속 공개 대조에서 강습 category는 맞지만 event_type이 파티/행사인 불일치 발견. 경로: 별도 event_type 없는 class 후보 → 2d4e23c38의 social 외 일괄 파티/행사 fallback → events 및 collected 후보에 저장. bfd54fc99의 eventTypeFromEventData가 이미 올바른 class 변환을 소유하므로 해당 기존 함수로 fallback만 연결. 명시된 event_type·AI/출처/날짜/장소·완료 재등록 거부는 유지하며 새 분류 함수나 사례 전용 예외 없음.
- 화면 검증: 운영 DB 연결 로컬 1440px/390px에서 실제 일정 등록 진입 → 행사/외강/동호회 분류 → 살사 선택·저장·재열기 모두 통과, 장르 범위 2개·오류 0. 일반 회원 인증 응답만 모의했고 쓰기 요청은 차단하여 가짜 운영 이벤트를 만들지 않았다. 실제 저장 payload/소유권은 위 단위검사로 검증. 아이콘 폰트가 접근성 이름에 포함되는 차이는 검증 선택자를 실제 요소로 보완했으며 UI 구조를 변경하지 않았다. 빌드 통과, 대상 ESLint 오류 0(기존 경고), 새 테스트 경고 0.
- 운영 결과: cf8e3438 커밋·푸시 후 일반 회원 등록 UI 배포, 이어 67b89b88 공통 보조 분류 fallback 수정을 커밋·푸시 후 재배포. 최종 version 1789064921834/health ok/service active. 대상 push는 origin/codex/social-retention-recovery-20260910. 운영 UI 1440px/390px에서 모의 일반 회원의 세 분류 선택·저장·재열기 통과. 새 함수/권한/스키마 중복 없음.
- 실제 등록: 수라댄 9/11 특강(9e0d4fa8-e610-453d-b2cf-6edeac4f8519), 9/18 반곡반(ba2d4924-988f-484e-ae69-67439e6c9685)을 기존 서버 수집/등록 API로 HTTP 201 반영. 앞선 6건 포함 강습 8건. 보조 분류 복구는 원본 payload의 명시 class/미지정 event_type과 collected 연결을 확인한 2쌍만 운영 사본·트랜잭션·동시 변경 검사로 수정. 후보를 pending으로 되돌리거나 재등록·알림 재발송하지 않았다. 백업은 운영 backups/data-fixes. 원본 ID/소유자/날짜/장소/포스터 유지, 원장 재검사는 alreadyApplied. 공통 코드 회귀는 서버 인접 52건 통과.
- 공개 데이터 검증: 신규 2건 API salsa/class/강습·날짜 일치, 포스터 HTTP 200, PC/모바일 모두 포스터 표시·드래그 불가·브라우저 오류 0. 기존 살사 일정 모두 보존, 중복/소셜 대체 삭제 0. 자세한 원장/등록 결과는 docs/salsa-class-registration-2026-09-11.md. 다른 작업이 섞인 원래 작업 폴더에는 이번 패치와 기록만 동기화했다.
- 추가 조사: 앞선 조사와 별개로 카페 14곳 및 마얀의 이전 후 원장 1곳 확인. 수라댄의 개강반/특강, 베스트라틴의 실제 신청·운영자 확인, 라틴팩토리의 기시작 4주 과정, 마얀의 현 원장, 회원 제한/외부 홍보/과거 영상 혼재를 구분. 근거는 docs/salsa-cafe-source-research-2026-09-11.md §11. 원본 회원 정보·계좌·쿠키는 Git에 저장하지 않는다.
## 2026-09-23 — 방문 통계 화면의 정보 우선순위·표현 정리

- 상태: 구현·회귀·반응형 검증·통계 전용 운영 배포 완료.
- 증상 → 판정 → 부작용 → 결과: 날짜/연간 탭에 중복 JSX와 여러 KPI가 반복됨 → 동일 summary를 서로 다른 이름(클릭/PV/활동, 회원 전환)으로 표시 → 숨겨진 1년 조회와 의미가 다른 수치가 운영 판단을 어렵게 함 → 저장 보고서 값은 유지하고 표시 계층을 정리한다.
- 기존 구현 판정: 일부 있음. SiteAnalyticsModal, 기존 report RPC/상세 25명 페이징/CSV/유형·메뉴 상세를 재사용한다. 새 데이터 저장소·계산·요청 경로는 없다. 62011d200의 운영 인사이트 표시, dbc5d4854의 모바일 분기, d013b0a7의 지표 순수성, 07115c26/72be39c1의 저장 결과 읽기와 지연 상세 로드 목적을 관련 blame/log·호출부·테스트에서 확인했다. 로그인 비율을 전환으로 해석하는 표현과 35% 양호 판정은 원장에 전환 근거가 없으므로 제거했다.
- 변경: 같은 반응형 화면에서 고유 방문자/방문 횟수/사이트 활동/평균 활성 체류를 먼저 표시한다. 실제 조회 날짜와 저장 상태, 일별 방문 추이·수치 표, 인기 콘텐츠와 유입을 보여준다. 활동 종류·메뉴/접속 패턴·앱/집계 기준은 접어서 제공한다. 중복 세션 카드·임의 품질 판정·설치 기록 수/현재 접속 추정·반복 개인 로그 요약은 제거하고 기존 방문자 상세 조회와 진단 자료는 유지한다. 비로그인 목록의 IP·식별값은 별도 운영 확인 항목으로 접는다. 최근 1년을 명시하고 CSV 파일명도 실제 조회 범위를 사용한다. 날짜 입력에 이름·범위, 이동·닫기 버튼에 접근성 이름, 비드래그 정책을 유지한다.
- 불변조건: 서버 저장값·과거 확정·기간 전체 중복 제거·관리자/봇 제외·명시 갱신만 mutation·상세 요청 시 페이징은 유지한다. 매일 방문자 수의 합이 기간 고유 방문자와 다를 수 있음을 설명한다. 화면 조회 후 자동 실시간 갱신으로 오해하지 않도록 집계 기준을 명시한다.
- 검증: 기존/인접 6파일 41개 통과. 기존 날짜 이동/지연 응답/오류·pending/수동 재생성/상세 페이징 검사를 유지하고 연간 CSV 범위·미래 날짜 이동 제한·지표 이름을 확장했다. 실제 운영 의존 번들을 사용하는 합성 데이터 Chromium 화면에서 1440px/390px/320px, 오늘/7일·접속 상세·비로그인 팝업을 확인했다. 화면 요청은 요약→팝업 상세→기간 요약 3회, pageerror 0, 320px 본문 가로 넘침 0. 집계 코드/DB 변경이 없으므로 원장 재집계·운영 데이터 변경 검사는 제외했다. 앱 전체 TS는 기존 타 파일 오류가 있으나 변경 파일 진단은 없다.
- 배포 경계: 기존 --analytics-only 빌더를 재사용해 스타일을 통계 모듈과 함께 포함한다. .analytics-review 범위의 우선순위로 늦게 로드되는 기존 CSS와 충돌을 방지한다. 현재 운영 generic/stats SHA를 검증한 638f4325 기준을 사용하며 이번 서버 delta는 0이다. 기존 롤백은 이전 index/module로 돌아가도록 유지한다.

- 운영 확인: 24c57a599f089b1bb195530ea53a0ff703783575을 origin/fix/analytics-closed-reports-20260923에 먼저 푸시하고 --analytics-only 배포 성공. 공개 version 일치, health ok, 실제 브라우저의 새 모듈/동반 스타일 로드 성공·pageerror 0. generic-data-api/stats-api/events-api SHA 모두 배포 전후 동일. 배포 중 과거 보고서 재생성 0건·오늘 live만 갱신했다. 관리자 인증 화면은 합성 데이터로 검증했고 실제 운영에서는 공개 자산/모듈/상태와 서버 해시를 확인했다.

## 2026-09-23 — 방문자 통계 수집·오늘 조회·기간 전달 비용 재검토

- 상태: 전 경로 조사·수정·통계 전용 운영 배포 완료. 최초 연간 계산의 잔여 비용은 아래에 명시.
- 증상 → 판정 → 부작용 → 원장/결과: 오늘 조회 → 종료일이 오늘이면 loadSources → 활동 26,799건·세션 9,881건 및 회원/설치 전체 읽기, 전역 관리자 기기 반복 판정 → 오늘 응답 3,202ms(별도 측정 로딩 1,169ms·핵심 계산 624ms·상세 191ms). 연간 화면은 일별 저장 약 42.4MB를 읽고 전 방문자 상세도 보내 27,820ms·16,925,662바이트였다. 수집 경로도 findSessionLog가 한 세션을 찾으려고 전체 9,881건을 읽었다.
- 기존 구현 판정: 일부 있음. 기존 site_usage_stats·report 서비스·stats cron·키/JSON 필드 단건 조회를 재사용한다. 수집 저장 키는 session_id이지만 운영 7,105건은 레거시 UUID 키이므로 단건 키 조회만으로 대체하지 않는다. 기존 JSON 필드 조회의 구형 MySQL fallback을 연결해 레거시 보호를 유지한다. 새 DB 테이블·큐·공개 API는 없다.
- 원래 보호 목적과 이력: 0960c01c의 레거시 세션 찾기, 5ef3786f/2e8134d3/2ab3ae40의 기간 밖 관리자·기기 연결, 72b94a84의 회원 ID 정규화, d013b0a7의 KST·논리 세션을 확인했다. 최근 07115c26 전환은 과거 조회만 해결하고 오늘 전체 읽기·연간 상세 전송 비용을 남겼다. 저장된 과거 값을 다시 계산하는 예외는 추가하지 않는다.
- 수정: 같은 저장소의 단일 live ID에 오늘을 finalized=false로 1분마다 갱신하고 일반 조회는 오늘도 저장값만 읽는다. KST 00:35 마감 때 finalized=true로 전환한다. 갱신 실패 시 이전 정상값을 유지하고 2분 이상 지연이면 화면에 표시한다. 명시 오늘 갱신은 한 번 읽은 원본으로 오늘만 갱신하며 같은 기간의 과거를 재계산하지 않는다. 전체 일별 무결성 확인은 일 1회, 매분 작업은 coverage와 오늘만 처리한다. 기존 비압축 v2는 수치를 바꾸지 않고 압축 저장으로 전환한다.
- 계산·전달: 전역 관리자 연결의 고정점 반복 스캔을 동일 연결 그래프 순회로 변경해 행 순서에 따른 반복 비용을 없앴다. 반복 Locale formatter 생성을 없애고 KST 날짜/시각 분류를 유지했다. 요약 응답에서는 보이지 않는 상세 방문자/일별 항목을 제외하고, 목록을 열 때 기존 RPC에서 25명씩 읽는다. 기간 결과는 서비스 안에서 최대 1개·60초만 공유해 상세 페이지마다 재계산하지 않는다. 요약의 고유 방문자 수는 일별 고유 인원을 더하지 않고 기존 기간 중복 제거를 유지한다.
- 검증: 대상 6파일 40개 통과. 자동 오늘 갱신/읽기 무계산/지연 표시/실패 보존/오늘만 명시 갱신, 마감·일별 감사·압축, 기간 중복/상세 페이지, 레거시·신규 세션 조회/실패, 관리자 연결·KST 경계, UI 상세 지연 로드·오류·날짜 경쟁을 선택했다. 운영 원본을 읽기 전용으로 고정한 오늘·1년 입력에서 수정 전후 core와 전체 report JSON이 완전 일치(활동 97·26,108). 서버 TS 검사·서버 빌드 통과, 대상 ESLint 오류 0. 앱 전체 타입 검사는 관련 파일 진단은 없지만 다른 기존 오류가 남았다. 실제 운영 의존 모듈과 원본 기반 보고서로 Chromium 390px에서 요약 1회→Guest 상세 1회, 페이지 오류·가로 넘침 0 확인.
- 배포 중 추가 성능 검증: 72be39c1 통계 전용 배포로 오늘 29ms·어제 52ms, 각 site_usage_stats SELECT 1회만 실행했다. 요약 174KB, 상세 25명 조회는 같은 기간 계산 재사용으로 추가 DB 읽기 0회였다. 323개 비압축 일별 행을 값 변경 없이 압축해 총 저장량 42.4MB→7.7MB로 줄였고 자동 매분 실행·health·실제 운영 모듈 로드·비인증 읽기/쓰기 401을 확인했다. 연간 최초 계산은 20.9초가 남아 수정 작업을 멈추고 CPU/메모리를 추가 측정했다. DB 읽기 0.2초, 압축 해제 등 1.8초, 핵심 계산 4.8초, 상세 계산 7.3초였다. 기간 합산에서 각 일별 상세 객체를 즉시 해제하고 IP 제외·방문자 ID·행 포함 판정을 한 계산 안에서만 재사용하도록 공통 원인을 줄였다. 운영 파일을 바꾸지 않는 임시 읽기 전용 실행에서 12.3초·동일 활동 26,108·전체 결과 동등성을 확인했다. 1GB 운영 서버의 연간 최초 계산은 여전히 오늘보다 무겁다는 한계를 남기며 즉시 응답이라고 보고하지 않는다.
- 최종 배포 확인: 638f43253d27f25709085529e75ca08ad4836870을 origin/fix/analytics-closed-reports-20260923에 먼저 커밋·푸시한 뒤 --analytics-only 배포 성공. 366일 확정 보고서는 재계산 0건, 오늘 live만 갱신했다. 최종 운영 함수 측정은 오늘 92ms·9,552바이트, 연간 최초 11,879ms·173,638바이트, 60초 캐시 내 같은 연간 재조회 0ms였다. 각 최초 요청은 site_usage_stats SELECT 1회이며 원장 읽기/쓰기가 없고 연간 재조회는 DB 요청도 없다. 공개 version.json 커밋 일치, health HTTP 200/ok, 이전 UI 모듈과 같은 새 모듈 SHA 유지 확인. 자동 매분 갱신 정상, events-api 해시 불변. 동등성 대조에 사용한 운영 개인정보 임시 파일과 서버의 읽기 전용 후보 실행 폴더는 삭제했다.
- 제외·남은 비용: 수집 정책·기존 원장 수정/삭제와 무관한 이벤트·게시판 배포는 제외한다. 자동 갱신은 관리자 과거 기기 보호 때문에 서버에서 전체 원본을 읽는 작업을 아직 1분에 한 번 수행한다. 이를 화면 요청에서는 제거했다. 상세는 누락 없이 페이지로 제공하며 스냅샷/레거시 행은 보존한다. 운영 결과는 배포 후 아래에 기록한다.

## 2026-09-23 — 방문자 통계를 조회 캐시에서 서버 일별 마감으로 변경

- 상태: 재발 원인 재조사·구조 변경·과거 366일 저장·통계 전용 운영 재배포 완료.
- 증상 → 판정 → 부작용 → 원장/결과: 어제 이동에도 ‘데이터 분석 중’ → v1은 최초 조회 시 저장 여부를 확인하고 없으면 브라우저 전체 집계 → 조회 후에야 보고서 upsert → 지난 날짜가 미리 준비되지 않음. 저장된 보고서 읽기에도 동일 로딩 문구를 사용했다. 운영 DB에서 9월 20·22일 v1 보고서가 실제 관리자 조회 뒤 생성된 것을 확인했다. ‘계속 계산 중’이라는 제보를 근거 없이 무한 루프로 단정하지 않았다.
- 최초 목적 재확인: 브라우저 캐시 쓰기만 있던 3d33d060, 서버 순수성/핵심 통계 권위화 d013b0a7, 운영 전역 관리자 기기/회원 연결의 보호 목적을 다시 확인했다. 첫 수정 bdfe2abf는 저장 재사용만 검증해 ‘사용자 조회 전에 과거 통계 준비’ 요구를 놓쳤다. 추가 예외 대신 계산 소유권을 서버로 옮겼다.
- 기존 구현 판정: 일부 있음. getAnalyticsSummaryV2·site_usage_stats·기존 stats cron 스크립트·배포 명령 재사용. cron 스크립트는 0960c01c/7d76a118에서 도입됐으나 빌드 허용 함수 제한 이후 실제 마감 작업에 연결되지 않았다. 새 테이블·큐·엔드포인트 없이 기존 RPC의 report 모드와 snapshot mutation을 연결한다. 새 서버 모듈은 옮긴 상세 계산과 일별 생명주기를 각각 소유하며 브라우저 계산은 제거해 중복 소유자를 남기지 않는다.
- 변경: 종료된 날짜를 서버가 KST 00:35 이후 자동 저장하고 5분 간격으로 누락을 복구한다. 저장된 날은 재계산하지 않는다. 과거 하루 조회는 보고서 읽기만 하며 누락 시 준비 상태를 반환한다. 기간 통계는 저장된 일별 자료로 중복을 제거하고 오늘만 최신 자료를 더한다. 명시 재생성은 기존 create_usage_snapshot 경로를 사용하고 cron과 DB 잠금을 공유한다. partial/error 결과는 확정하지 않으며 coverage는 backfill 완료 후 기록한다. 읽기 시간 제한과 ‘불러오는 중’ 구분을 적용했다.
- 불변조건: KST 경계, 전역 관리자 기기·봇·내부·데이터센터 제외, 회원 ID/기기 연결, 논리 세션과 레거시 session_start fallback, 관리자 전용 개인 상세 보호. 기존 원장과 v1/레거시 snapshot은 보존한다. API 패치는 운영 파일 위에 통계 변경만 적용한다.
- 검증: 회귀 5파일 30개 통과(서버 일별 마감·복구·중복·보호·압축 호환 13, UI 읽기/명시 재생성/실패·시간 제한 7, 기존 순수성·세션·기기 10), 서버 TypeScript 검사와 서버 묶음 빌드 통과, 대상 ESLint 오류 0(기존 이동 코드의 any 등 경고). 앱 프로젝트 타입 검사는 통계 변경 파일의 진단은 없으나 수정하지 않은 다른 파일의 기존 오류가 남아 전체 성공으로 보고하지 않는다. 운영 원본 읽기 전용 대조: 9/22 회원1·게스트20·활동145·세션25, 9/20 회원0·게스트12·활동71·세션14와 상세 목록 수 일치. 실제 운영 의존 모듈/실제 일별 보고서로 Chromium 390px 확인: 날짜 이동 포함 raw query 0, browser write 0, 페이지 오류·가로 넘침 0.
- 최초 서버 전환 시 검증된 실패: 323일 저장 후 MySQL max_allowed_packet=1,048,576바이트 제한으로 큰 보고서 저장 실패. 배포 스크립트가 API·화면을 이전 상태로 자동 복구했고 정상 v2 행은 유지했다. 원인 경로는 report+inputs 직렬화 → generic upsert → MySQL 패킷 제한 → 해당 일 미저장/coverage 미기록이다. 전역 DB 설정을 바꾸거나 특정 날을 건너뛰지 않고 모든 신규 일별 payload를 gzip+base64로 저장하도록 보완했다. 기존 비압축 v2 읽기·손상 보고서 준비 상태/다음 cron 복구 검사까지 30개 통과. 실제 8/11 보고서는 1,703,537바이트에서 262,108바이트로 줄었고 원본량 상위 날짜 4건과 어제 모두 저장 한도 안이다.
- 배포·운영 확인: 817e006f0266c0f0e8812ef21514f99d26286df7을 origin/fix/analytics-closed-reports-20260923에 커밋·푸시한 뒤 통계 전용 배포 성공. 기존 323일은 유지하고 누락 43일을 채워 2025-09-22~2026-09-22 총 366일과 coverage 저장 완료. 운영 함수로 9/22·21·20 조회 시 각각 77·4·3ms, daily_snapshot 응답, site_usage_stats SELECT 각 1회만 실행했고 원장 조회/쓰기 없음. 정기 작업 재실행은 finalized=0, 서비스·crond active. 공개 version의 커밋/버전 2 일치, health ok, 실제 Chromium import map으로 새 모듈 HTTP 200·페이지 오류 0, 읽기/재생성 RPC 비인증 401 확인. HTML 차이는 통계 import map 1줄뿐이며 events-api 해시는 불변이다. 관리패널의 로그인된 실제 세션 조작은 하지 않았고, 화면은 운영 의존 모듈+실제 일별 자료로 앞서 검증했다.
- 제외: 증상을 숨기는 표시 변경만으로 종결하지 않는다. 다른 이벤트/수집/게시판 기능과 방문 원장 수정·삭제는 하지 않는다. 자세한 책임·일별 grace·수동 재생성·롤백은 docs/decisions/2026-09-23-closed-analytics-reports.md에 기록했다.

## 2026-09-23 — 종료된 기간 방문자 통계 저장·재사용

- 상태: 구현·로컬 검증 및 통계 전용 운영 배포 완료.
- 증상 → 판정 로직 → 부작용 실행 → 원장/결과: 날짜 변경/모달 재진입 → 과거 여부·저장 결과 확인 없이 RPC/원시 로그 재집계 → 읽지 않는 메모리 캐시에 쓰기 → 같은 과거 날짜를 계속 계산. 기존 코드의 관리자·봇·내부 접속 제외, KST 범위와 회원/게스트 연결·논리 세션 보호 목적은 유지한다.
- 기존 구현 판정: 일부 있음. 서버의 관리자 전용 site_usage_stats, id 단건 조회/upsert, 기존 집계와 상세 데이터 생성기를 재사용한다. 공개 metrics_cache에는 상세를 저장하지 않는다. 최초 캐시 쓰기만 있던 3d33d060과 RPC/핵심 수치 보강 0960c01c/d013b0a7, 2026-08-27 미수정 기록 및 직접 호출부를 확인했다. 빈틈은 종료된 기간 결과의 영속 저장·조회 연결이었다.
- 수정: KST 기준 지난 기간만 버전·시작/종료일 ID로 전체 summary/users/guests를 함께 저장하고 다시 표시한다. 오늘 포함 기간은 최신 집계를 유지하며 일별 고유 방문자를 더하지 않는다. 기존 새로고침에 ‘다시 계산’을 표시했다. RPC·제외 기준·세션·설치 등 조회 실패나 페이지 상한에 걸린 부분 결과는 저장하지 않는다. 0명 결과는 저장한다. 저장 실패 안내와 재시도, 이전 요청의 늦은 화면 덮기 방지, 드래그 차단을 적용했다. 기존 오늘 스냅샷은 오늘 하루 조회에서만 만들도록 범위를 일치시켰다.
- 중복·데이터 경계: 새 테이블·큐·API 없음. 기존 private snapshot 소유 위치의 JSON만 확장하며 레거시 숫자 행·방문 원장은 유지한다. 규칙 변경 시 보고서 버전 증가, 원장 정정/늦은 유입 시 명시 재계산으로 갱신한다. 해당 기간을 마감 후 처음 조회할 때는 한 번 계산한다.
- 검증: 회귀 4파일 21개 통과(캐시 경계/실패·재시도·회원/게스트 상세 복원 11개와 기존 순수성·세션/기기 보호 10개), TypeScript 통과, 대상 ESLint 오류 0개(기존 any/Hook/console 패턴 포함 경고 101개), bash 문법·git diff --check 통과. 실제 운영 의존 번들을 연결한 Chromium 390px 화면에서 0명 보고서 생성→오늘→과거 재조회 확인: 과거 재조회 집계 RPC 0회, 페이지 오류·가로 넘침 없음. 운영 snapshot 비인증 조회 HTTP 401 확인.
- 배포 결과: bdfe2abf3aebc487940aa2cccea233efd71a7c93을 origin/fix/analytics-closed-reports-20260923에 먼저 푸시한 후 `npm run deploy:cafe24 -- --analytics-only` 성공. 운영 version.json의 analyticsReportCommit 일치, 새 통계 모듈 HTTP 200 및 SHA-256 일치, 실제 운영 Chromium import map 경유 로드·페이지 오류 0, /__health ok=true 확인. 운영 HTML은 import map을 제외하면 배포 전과 바이트 동일하며 generic-data-api.js/events-api.js 해시도 불변이다. 기존 열린 탭은 새로고침하면 적용된다. 운영 원장/방문 보고서를 강제로 재계산하거나 시험 행을 쓰지는 않았다.
- 배포 범위: 사용자가 ‘통계 개선만 배포’를 지정했다. 다른 미커밋 변경 중 일부가 실제 운영 코드와 일치하므로 전체 HEAD 배포는 회귀 위험이 있다. 기존 배포 명령에 --analytics-only 경로를 연결하고 별도 깨끗한 브랜치에서 커밋·푸시 후 실행한다. 운영 모듈 해시 검증과 import map으로 통계 파일만 전환하며 기존 앱/서버/DB/서비스 워커를 보존한다. 배포 기준 및 롤백은 docs/decisions/2026-09-23-closed-analytics-reports.md에 기록했다.
- 제외: 집계 알고리즘을 재작성하지 않았으므로 수집·소셜·게시판 전체 회귀와 운영 방문 원장 수정은 하지 않았다. 오늘 포함 복수 날짜 기간의 증분 집계는 도입하지 않았다.

## 2026-09-22 — 과거 방문자 통계 반복 계산 원인 확인

- 상태: 원인 설명을 위한 코드 조사 완료. 실행 코드·운영 DB·배포 변경 없음.
- 증상 → 판정 로직 → 부작용 실행 → 원장/외부 결과: 운영 통계 열기·날짜/탭 변경·새로고침 → `fetchAnalytics`가 KST 조회 범위만 정하고 과거 확정/캐시 적중은 판정하지 않음 → 매번 `get_analytics_summary_v2`와 브라우저 원시 로그 조회·집계 실행 → 화면 결과와 읽지 않는 메모리 캐시에 저장. 오늘이 범위 끝이면 `site_usage_stats`의 오늘 스냅샷 존재 여부를 확인해 없을 때 일부 숫자를 저장한다.
- 기존 구현 판정: 일부 있음. `SiteAnalyticsModal.tsx:284,350,1678`의 캐시는 쓰기만 있고 읽기가 없으며 `_forceRefresh`도 사용하지 않는다. `generic-data-api.js`의 `getAnalyticsSummaryV2`는 활동·세션 원장을 매번 읽는다. `checkAndAutoSnapshot`/`createUsageSnapshot`은 조회 시점의 회원·비회원·관리자 숫자를 저장할 뿐, 종료된 날짜의 상세 집계를 확정·재사용하는 계약이 아니다.
- 기존 보호 목적·불변조건: KST 날짜 경계, 관리자·내부·봇·데이터센터 제외, 회원/게스트 식별 연결, 30분 논리 세션과 레거시 호환을 유지해야 한다. 과거 날짜라는 이유만으로 기존 스냅샷을 확정 결과로 간주하거나, 일별 고유 방문자 수를 더해 기간 고유 방문자 수로 사용해서는 안 된다. 이번에는 해당 로직을 변경하지 않았다.
- 근거·검증 범위: 모달 직접 호출부, 집계/스냅샷 함수, 관련 테스트 참조 검색, 2026-08-27 미수정 조사 기록, 2026-08-12 신뢰성 결정과 해당 줄의 blame을 대조했다. 캐시 쓰기만 있는 흐름은 최초 `3d33d060`부터 존재하며, 서버 집계 도입 `0960c01c`와 핵심 수치 보강 `d013b0a7` 이후에도 남아 있다. 원인 질문에 대한 정적 조사이므로 운영 재계산·DB 조회/수정·회귀 테스트 실행은 제외했다. 새 구조나 중복 소유자를 추가하지 않았다.


이 문서는 프로젝트의 과거 문제, 조사, 해결 방법, 운영 이슈를 Git에 남기기 위한 중심 기록이다.



## 2026-09-11 살사 강습 원장 연결과 실제 등록

- 상태: 7e303341 선커밋·푸시 후 배포 완료(version 1789061953432, health ok, service active). 신규 강습 6건 모두 HTTP 201 등록. 후보 collected·이벤트 연결·살사/class·포스터 각각 6/6, 중복 0, 기존 살사 이벤트 모두 보존. 운영 PC/모바일에서 6건·포스터 전부 표시 확인. 상세 docs/salsa-class-registration-2026-09-11.md.
- 경로: 살사 강습 공식 게시판 → 기존 수집 레지스트리/서버 허용 출처 미연결 → 자동등록 거부 → 공개 일정에는 앞서 등록한 소셜만 존재. 기준은 source_id, 활동 class, 원문의 개강일·장소·AI 근거와 실제 events 연결이다.
- 기존 구현 판정: 일부 있음. 2d4e23c38/1a6211406의 기존 서버 등록·출처 보호, c0f187f2의 포스터 없는 강습 허용, 기존 expanded-ingestion 및 후보 정규화를 재사용한다. SDA·에버라틴·수원 쿠바 강습 게시판만 기존 양쪽 출처 목록에 연결한다. 새 API·DB·큐·등록 절차 소유자 없음.
- 보호: 게시판을 기존 수집 레지스트리의 고정 위치로 보존, 개별 글을 강습 이벤트의 원문 링크로 보존. class만 허용하며 명시된 장소·원문 근거·AI 검증·완료 재등록 차단을 유지한다. 포스터 유무는 등록 조건이 아니며 실제 원본은 보존한다. 스윙 정기 실행 범위는 불변이다.
- 조사 판정: 에버라틴 고정 글의 일반 동호회 소개와 실제 강습 공지를 분리해 해당 과정의 원문 구간으로 검증했다. 원문 5주/7주 충돌은 설명에 남기고 종료일을 추정하지 않았다. 수원 쿠바의 본문 토요일 오기는 원본 포스터·반복 본문의 화요일·실제 달력을 대조해 기존 day 필드에 기록했다. MT 차단·요일 검사에 특정 행사 예외를 만들지 않았다.
- 검증 범위: 수집 표준 통과, 서버 등록 42건 통과. 기존 검사에 신규 3개 출처의 포스터 없는 강습 성공, AI 누락·장소 근거 불충분·날짜 불일치·잘못된 활동·완료 재시도 거부를 확장. 신규 정기 탐색은 등록 허용 설정과 별개이며 가동 완료로 보고하지 않는다. 후보 원문과 계좌/회원 정보는 Git에 저장하지 않는다.

## 2026-09-11 공개 장르 범위 제한과 살사 카페 심층 조사

- 상태: d8a02eff를 origin/codex/social-retention-recovery-20260910에 먼저 푸시 후 배포 완료. 운영 version 1789061116928, PC·모바일 6경로 검사 통과.
- 경로: 공개 장르 클릭/URL → normalizeVisibleDanceScope(..., true)로 모든 장르 허용 → 해당 scope 조회 → 미준비 장르도 활성 버튼으로 표시. 기준은 공통 장르 목록, URL dance, 실제 버튼 상태다.
- 기존 구현 판정: 일부 있음. 공통 taxonomy 목록/선택 가드는 345d2b961의 공개 노출 보호를 위해 존재한다. 9e713ab8은 사용자가 요구한 기존 화면·데이터 전환 복구 목적이었으나 공개 호출부가 모든 확장 장르를 허용했다. 해당 심볼·호출부·기존 테스트·직접 git 이력만 확인했다.
- 수정: 공통 목록의 공개 활성 여부를 스윙·살사로 제한하고 나머지 3개는 보이는 준비중 버튼으로 유지. 메인/캘린더/행사정보/메인 메뉴 전달/원데이 조회·등록 화면에 같은 기준을 사용. 직접 준비중 장르 URL에서 조회만 스윙으로 돌아가고 등록은 원래 장르로 쓰이는 불일치도 방지한다. 관리자의 명시적 확장 장르 편집과 DB taxonomy는 유지한다. 새 페이지·DB·큐·중복 설정 소유자 없음.
- 검증: taxonomy 기존 검사 확장 12건, Cafe24 빌드 통과, 대상 ESLint 오류 0(기존 경고 56). 실제 운영 DB 연결 로컬 PC·모바일 6경로 확인. 첫 UI 검사에서 모바일의 기존 설명 숨김 규칙이 준비중도 숨기는 것을 발견해 disabled 버튼 설명만 표시했다. 클릭 직후 상태를 읽던 검증은 실제 렌더링 완료를 기다리도록 보완했다. 모든 6경로 재검사 통과. UI-only 변경이므로 수집/등록/알림/DB 쓰기 검사는 제외했다.
- 조사: SDA·라파·보스톤·라틴속으로·에버라틴·라스트댄스의 실제 카페 공지와 현재 회원 활동을 확인. 고정 공지의 본문 수정, 에버라틴 원장 이전, 동일 장소 중복 공지, 장소 미공개·기간 충돌, SA 과거 공지 등 판정 위험을 구체화했다. 카페 부재나 카카오 일변도라는 결론은 근거 없음. 초기 조사만으로 신규 DB 등록·정기수집을 실행하지 않았고, 후속 명시 요청으로 강습 6건을 등록했다(위 항목). 새 정기수집은 미활성 상태다.
- 기록: docs/salsa-cafe-source-research-2026-09-11.md, docs/decisions/2026-09-11-public-dance-availability.md. 추가 12개 카페와 공동 홍보·독립 아카데미 웹 원장을 조사해 지역 오류, 회원 제한, 상시 모집, 날짜·장소 충돌, 다음 일반 글 링크 누락을 기록했다. 조사 원문/익명 브라우저 화면 및 UI 검증 결과는 /tmp/salsa-source-expansion-20260911에 보관. 개인 신청 내역·연락처·계좌·쿠키는 Git 기록에서 제외했다.


## 2026-09-11 살사 서버 자동등록 연결과 Mac 의존 오판 정정

- 상태: 79d7ba0d 커밋을 origin/codex/social-retention-recovery-20260910에 먼저 푸시 후 배포 완료(version 1789058520148·health ok·service active). 서버에서 13건 전부 HTTP 201 공개 등록, 후보 collected 및 실제 이벤트 ID 연결 13/13, 중복/기존 소셜 삭제 0. PC/모바일·9~11월 캘린더·스윙 분리·고정 수집 위치 링크 검증 완료. Mac 불필요.
- 원인 경로: 살사 후보 13건 pending → 새 출처를 manual로만 등록하고 서버 허용 목록에는 빠뜨림 → 서버 API 대신 관리자 UI가 필수라고 오판 → Mac 잠금을 등록의 필수 조건으로 안내. 스윙은 ingestion token으로 후보 저장 후 automatic=true 등록 API를 호출하므로 Mac이 필요 없다.
- 기존 구현 판정: 일부 있음. 2d4e23c38의 서버 자동등록·원장 연결과 1a6211406의 허용 출처/활동 보호를 재사용. 신규 API·테이블·큐·인증 우회 없음. 기존 레지스트리/서버 목록에 보니따·DSN 소셜만 연결. 양쪽 검증 통과가 필요하며 관리자 인증 경로는 유지한다.
- 추가 공통 원인: fe2b77c68의 명시 날짜 검사는 한국어/숫자만 처리해 Meetup의 Sep 17을 놓쳤다. 기존 함수에 영문 월명·일자·명시 연도 일치 검증 추가. 특정 행사/날짜 예외나 시간 숫자의 날짜 확대 없음.
- 검증: 수집 표준 통과, 기존 스윙/강습/신규 살사/원문 불일치/완료 재등록 보호를 포함한 서버 테스트 39건 통과. 운영 13건 사본을 기존 정규화·서버 검증에 입력해 전부 준비 확인. 직접 DB 쓰기나 관리자 세션 사용 없음. 신규 글 정기 탐색의 활성화는 이번 등록과 별도 경계다.
- 관련: docs/decisions/2026-09-11-salsa-server-registration.md, docs/salsa-social-collection-audit-2026-09-11.md.

## 2026-09-11 살사 소셜 추가 수집·자동화 가능성 검토

- 후속 등록 요청: 운영 13건 pending·대상 중복 0·기존 매핑의 살사/소셜/장소 검증 완료. Mac 잠금과 브라우저 연결 종료로 관리자 등록 실행은 대기, 공개 등록 0건. 별도 인증 우회나 직접 이벤트 원장 쓰기 없음.

- 상태: 신규 10건 수집 저장, 이전 3건 포함 13건 pending. 공개등록·배포·예약 실행 활성화 없음. 상세: docs/salsa-social-collection-audit-2026-09-11.md.
- 경로: 공식 출처 2개 미등록 → 기존 포스터 없는 소셜의 출처·날짜·장소·DJ 검증 실패 → 저장 보류. 기존 구현 일부 있음으로 판정하고 출처 설정만 연결했다. 신규 함수/테이블/큐/상태는 없음.
- 보호·이력: fe2b77c68의 무근거 소셜 차단과 1a6211406의 서버 자동등록 허용 출처 경계를 유지. 새 소스 manual, 스윙 예약 범위 불변. 타 채널·타 그룹·목록/캘린더를 실제 행사로 허용하지 않음.
- 자동화 조사: 보니따 주간 원문은 기존 날짜/휴무 파서 모두 빈 결과. 확장 실행기는 두 출처 읽기 미지원. Meetup 초기 로딩이 빈 목록을 보여 누락 위험이 있으며 공개 목록에는 취소·오래된 강습 날짜·장소 충돌이 혼재한다. 날짜 하나 전용 예외나 무조건 자동등록을 추가하지 않았다.
- 검증: 기존 수집 표준 테스트 확장·통과, API 신규 10/중복 0/제외 0, 운영 대상 원장 재조회. UI/알림/예약 실행을 바꾸지 않아 해당 외부 부작용 경로와 프런트 배포는 제외. 인스타그램 접근 제한·과거 채널·Meetup 미확보 후속 목록을 명시했고 전수 완료로 보고하지 않음.

## 2026-09-11 살사 일정 첫 수집 실행

- 상태: 운영 수집센터에 보니따 09-11·12·13 소셜 3건 저장 완료, 모두 pending. 캘린더 등록 0건. 정기 자동수집/자동등록 가동 완료를 뜻하지 않는다.
- 경로: 고정 운영 채널 → 개별 게시물·날짜 문단·원본 포스터 대조 → 기존 후보 정규화/중복 검사 및 수집 API → scraped_events 검수 대기. 자동등록은 출처 등록/검증 조건을 별도로 적용한다.
- 기존 구현 판정: 있음. expanded-ingestion, prepareCandidate/buildCafe24Payload와 POST /api/scraped-events를 그대로 사용했다. 새 수집기·필드·테이블·큐·우회 등록 경로를 추가하지 않았다.
- 보호 목적: 1a6211406의 신뢰 출처 자동등록 제한과 c0f187f2의 검증된 포스터 없는 강습 허용을 유지. 이번 카카오 원문은 기존 자동등록 출처 목록에 없으므로 ready=false/manual을 보존했다. 관리자 UI는 Mac 잠금으로 접근하지 못해 등록을 완료했다고 표시하지 않는다.
- 검증: 수집 표준 통과, 후보 3건 모두 날짜/요일/장소/장르/원본 이미지 검증 통과. API 신규=3·중복=0·제외=0, 운영 원장 재조회 pending=3·로컬 원본 이미지=3 확인. 날짜별 결정론 ID를 사용했고 과거 일정·휴무·기수 시작일을 보존 기준과 구별했다. 코드 변경이 없으므로 앱 빌드/재배포/무관한 UI 회귀는 제외했다.
- 원문 충돌: 09-11 두 번째 메인 DJ가 본문 셀리/포스터 몽구로 달라 해당 슬롯을 확정 DJ 목록에서 제외하고 검수 사유를 저장했다. 09-12 발표회는 같은 소셜 안의 프로그램으로 보존해 별도 중복 행을 만들지 않았다.
- 상세 수집/보류 근거: [실행 기록](./salsa-ingestion-2026-09-11.md).

## 2026-09-11 살사 화면 오해 수정과 회원 활동 근거 재검증

- 상태: 배포·운영 데이터 반영·공개 화면 검증 완료. 앞선 살사 별도 안내 화면은 요구 오해로 철회했다.
- 경로: 장르 선택 → 캘린더의 안내 분기·메인의 강제 스윙/타장르 보충 → 기존 일정 UI 대체 또는 선택과 다른 데이터 표시 → 사용자가 메인과 캘린더에서 같은 데이터 전환을 할 수 없음. 기준은 URL dance, 이벤트 dance_scope와 활동 분류, 원데이 링크 scope, 장소 category다.
- 기존 구현 판정: 일부 있음. 장르 taxonomy/이벤트 DB/조회 API/기존 렌더러와 링크·장소 원장이 이미 있다. a1c6af62의 잘못된 화면 분기를 제거하고 기존 scope 연결만 보완한다. 원데이 JSON 레코드에 scope만 추가하며 NULL/누락은 스윙, 살사 행 추가 후에도 스윙 정적 폴백을 유지한다. 장소는 기존 category에 살사바를 추가한다. 새로운 테이블·큐·별도 장르 UI는 없다.
- 보호 목적·이력: b79d9ba4f의 미검증 확장 장르 가드, efaf276f0의 부족한 장르 카드 보충, c9035490의 지난 광고 제외 목적을 확인했다. 공개 데이터 전환만 열고 미래 날짜·중복 방지·원문 검수·스윙 레거시·포스터·소셜 뱃지·월 전체 일정 표시를 유지한다.
- 조사 보강: 공개 신규 가입 글 6건과 장기 경험담·정모 방문기·역할 경험·국내 거주/방문자 토론을 운영 원문과 구분했다. 보니따의 이번 주 공식 편성표로 기수 발표회·복수 동호회·홀별 음악·소셜 휴무를 교차 확인했다. 가입을 출석으로, 모집 광고를 독립 후기로, 오래된 소개를 현재 활동으로 판정하지 않는다. 18개 단체/운영자/장소의 확인·후보·보류를 근거 문서에 남긴다.
- 데이터 적용 경계: 검증한 원데이/동호회 고정 링크 4건과 공식 주소·로고를 확인한 보니따 1곳. 기존 저장 함수 재사용, 적용 전 중복 대조·백업, 새 ID만 추가. 수집 후보·이벤트·회원 개인정보는 생성하지 않는다.
- 검증: 운영 DB 연결 데스크탑/390px 모바일의 원래 화면·장르별 조회·바로가기·되돌아오기 15개 검사, 데이터가 있는 살사 샘플 표시/분리/비드래그 8개 검사 통과. 선택한 기존·인접 Vitest 8개 파일 47건 통과. 대상 ESLint 오류 0건(기존 경고 87건), Cafe24 로컬 빌드와 diff 공백 검사 통과. 구현 9e713ab8을 origin/codex/social-retention-recovery-20260910에 푸시 후 배포, 공개 version 1789053303882·health ok·서비스 active 확인. 운영 화면 26개 검사(장르 전환 15 + 실데이터 11), 지도 서울 4 핀 데스크탑·모바일 추가 확인, 브라우저 오류 0. 수집 표준 검사 통과. 기존 원장 백업 후 원데이 4행·장소 1행만 추가했고 기존 스윙 링크 보존과 실제 공식 채널 연결을 확인했다. 수집/알림 코드는 변경하지 않아 그 외부 부작용 경로는 재실행하지 않는다.
- 관련: docs/decisions/2026-09-11-shared-genre-data-selection.md, docs/salsa-member-evidence-2026-09-10.md. 초기사료 목록을 현재 활동 검증 명단으로 읽지 않도록 기존 조사와 철회 결정을 연결했다.

## 2026-09-11 단일 지역 모집 지도 뱃지 잘림

- 상태: 공통 원인 수정·추가 배포·운영 확인 완료.
- 경로: 살사 모집의 서울 1개 지역 → OneDayRecruitPage의 단일 지역 지도 분기가 전국 중심·레벨13을 고정 → 서울 지역 표시의 기존 y=-44 이동 → 390px 화면에서 뱃지 bottom=147.95가 지도 top=149보다 위로 나가 잘림. 지도 데이터·링크 수는 정상이고 원장 오류가 아니다.
- 기존 구현 판정: 있음. 지도와 영역별 좌표·뱃지는 기존 OneDayRecruitPage useEffect가 소유한다. d8ab0f9e의 도입부와 직접 표시 코드에서 다중 지역은 bounds로 맞추고 빈/단일 지역은 전국 지도로 유지하던 목적을 확인했다.
- 수정: 기존 단일/빈 지역 분기에서 중심 좌표만 첫 지역 좌표 또는 빈 경우의 기존 전국 중심으로 결정한다. 살사/서울 전용 예외·새 함수·스타일·DB 변경은 없다. 기존 지도 크기·배율·뱃지 오프셋·비드래그·복수 지역 bounds를 유지한다.
- 검증: 최초 DOM 존재 검사만으로 시각적 잘림을 놓쳐 화면 검토에서 발견했다. 실제 DB와 카카오 지도를 연결한 배포 전 빌드로 6개 검사 통과: 1440px·390px의 뱃지 전체 영역이 지도 안에 들어오는지, 서울 링크 수·클릭, 기존 스윙 다중 지역 및 빈 장르를 확인했다. 빌드·대상 ESLint 오류 0도 확인했다. 수집·회원 원장과 무관하여 운영 데이터는 다시 쓰지 않았다. b6d490d5를 기존 upstream에 먼저 푸시한 뒤 배포했고, version 1789053920650·health ok·서비스 active 및 운영 지도 6개 검사 통과를 확인했다.

## 2026-09-10 장르 공통 씬 안내와 살사 조사자료 연결

- 상태: 2026-09-11 사용자 요구 오해로 철회. 기존 화면의 데이터 전환 결정으로 대체. 아래는 당시 구현 이력이다.
- 기존 구현 판정: 일부 있음. 이벤트 DB의 dance_scope/활동 분류·장르 인덱스·조회 API·캘린더/리스트/지도/상세·수집 레지스트리는 이미 존재한다(스키마/API 도입 0960c01ca). 빈틈은 공개 장르 안내와 버튼 연결이었다. DB나 장르별 페이지·큐를 복제하지 않고 공통 장르 선택/주소 판정/안내 렌더러와 편집 콘텐츠를 연결했다.
- 기존 보호 목적: b79d9ba4f의 스윙 외 공개 가드는 미검증 확장 일정 노출을 막았다. 안내 공개와 자동등록 허용은 분리하고 기존 검수·스윙 NULL 장르 호환·포스터/소셜 뱃지·전체 일정 표시를 유지한다.
- 검증 중 발견: 목차 클릭 → HTML base=/가 해시 링크를 홈 기준으로 해석 → /#scene-learning 이동 → 장르 안내가 사라짐. index.html:8의 기존 SPA 기준 경로(3d33d060)는 유지하고 DanceSceneGuide의 모든 목차에 현재 pathname/search를 포함하도록 수정했다. 단일 장르 예외가 아니다.
- 검증: 관련 Vitest 18건 및 실제 DB 연결 Playwright 19건 통과. 데스크탑·390px/320px 모바일, 지역 필터, 고정 원본 링크, 드래그 금지, 직접 URL/뒤로가기, 스윙 리스트 복귀, 포스터/소셜 표시, 목차 이동 확인. 새 회귀검사는 실제 base=/ 환경의 모든 목차 링크를 검사한다. 일정 등록·수집 스케줄·DB는 변경하지 않아 운영 데이터 쓰기나 수집 실행 검사는 제외했다.
- 중복 미발생: 안내 콘텐츠만 danceSceneGuides에 소유하고 수집 실행/저장 허용은 기존 레지스트리에 유지한다. 자세한 결정: docs/decisions/2026-09-10-shared-dance-scene-guide.md.
- 배포 확인 (2026-09-10): a1c6af62를 origin/codex/social-retention-recovery-20260910에 먼저 푸시한 뒤 npm run deploy:cafe24 완료. 서비스 active, 공개 version.json buildTime=1789048747429 일치. 운영 사이트의 살사 고정 링크 12개·모바일 가로 넘침 없음·스윙 복귀 후 포스터 29개 및 소셜 그룹 30개·런타임 오류 0을 확인했다. 자동수집 프로필/DB 구조는 변경하지 않았다.


## 2026-09-10 미수집 소셜 자동 삭제와 예약 수집 누락

- 상태: 코드 수정 및 로컬 실행기 연결 완료, 서버 배포 전.
- 경로: 날짜 경과 → planRegularSocialReconciliation의 과거 생성본 정리 판정 → events 삭제 → 공개 캘린더 누락. 기준은 date/start_date, regular-social ID와 automation, 실제 등록 일정 및 휴무 원장이다.
- 수집 경로: macOS 예약 → 단일 실행 잠금 → 공식 페이지 접근 → 후보 검증·저장 → 공개 일정 대조. 9/5 실행 20260905_203950_96229는 시간 초과했고, 이후 확인한 최신 기록은 9/10이다. 9/10 14:06 동시 시작 4개 중 3개가 PID 96524 잠금으로 종료됐다. 실행 중 로그에는 Instagram login wall 및 페이지 접근 실패가 있다. 기록 공백만으로 PC가 꺼졌다고 확정하지 않는다.
- 기존 구현 판정: 일부 있음. 정규 생성·sourceId 수집 목록·상세 link1·재시도 remainingSources·단일 잠금이 이미 있다. 빈틈은 출처 URL 연결, 과거 보존 정책, 접근 실패의 재시도 연결, 잠금 경합 시 예약 포기다. 기존 acquire_lock은 설치 실행기에만 있어 해당 책임을 Git 관리 run-lock.sh로 이동하고 설치 실행기는 이를 호출하게 했다. 중복 소유자·새 테이블·큐·상태값은 없다.
- 보호 목적·이력: 6a49f44b/4756938c의 90일 생성·정리는 임시 일정 누적/유효하지 않은 규칙을 막았고, 4e52aff1은 근거 있는 과거 휴무만 보존했다. d8ab0f9e/e38beda0f의 접근 실패 기록은 원문 차단 진단과 Instagram 안전 회로를 보호한다. 해당 심볼·직접 호출부·회귀·결정 문서와 blame만 조사했다.
- 수정: 과거 생성본 보존, 기존 수집 목록 URL로 바로가기 보완, 같은 ID 갱신은 선삭제 없이 upsert. 접근 실패를 기존 재시도 상태에 연결하고 실패 소스의 Instagram 완료 기록을 막았다. 예약 잠금은 제한 대기 후 획득하며 살아 있는 소유자를 강제 제거하지 않는다.
- 불변조건: 실제 등록 일정 우선·휴무 원문 우선·미래 규칙 유효기간·DJ/날짜/장소 근거 검증·중복 방지·Cron 인증 유지. 과거 미수집을 실제 개최 확인으로 표시하지 않고 DJ 미정 상태를 유지한다.
- 검증: 기존 수집 표준 통과. 정규 소셜·외부 API 및 저장 실패/건조 실행 회귀, Node 잠금 3건·자동등록 보고 3건, Cafe24 빌드 통과. 대상 ESLint 오류 0(기존 상세 화면 경고 59), git diff --check 통과. 운영 공개 응답 1,000건에서 생성본 89건 중 링크 없음 85건을 읽기 전용 확인. 무관한 키오스크·릴스·통계 경로는 변경 경계 밖이므로 검증을 넓히지 않았다. 외부 로그인 차단 해결이나 실등록 성공은 주장하지 않는다.
- 적용 한계: 로컬 수집 변경은 다음 실행부터 적용, 서버 정책·링크는 배포 전. 삭제된 과거 행의 추정 복원과 운영 쓰기는 하지 않았다.
- 관련 결정: docs/decisions/2026-09-10-retain-uncollected-socials.md

- Deployment follow-up (2026-09-10): user authorized deployment and recovery of Sep 7–10. Commit 10e7a762 was pushed before deployment; health/version 1789018114603 verified. Production reconciliation: creates=0, removes=0, retained=197. Two Sep 9 socials (Chori at Social Club, Yoonseul at Swingtime) restored through existing administrator candidate/registration APIs with official source images. Normal automatic future-only filtering remains unchanged.
- Additional root cause: the legacy document fallback (71723533c) and direct-media URL preference (fe2b77c) admitted recommended post images and alt text. Moved the existing document reader to readInstagramPostDocument and excluded media linked to another post. Original media, hidden carousel slides and legacy article scope are preserved. Two DOM regression tests, ingestion standards and lint passed; the live Kyungsung post retained its original image and rejected six recommendations.

## 기록 원칙

- 버그, 장애, 운영 문제, 데이터 손상 위험, 배포 문제, 장시간 조사, 재발 방지 조치가 있으면 항목을 남긴다.
- 간단한 UI 문구 수정처럼 추적 가치가 낮은 변경은 생략할 수 있다.
- 커밋 전이면 관련 커밋은 `pending`으로 적고, 커밋 후 해시를 보강한다.
- 민감 정보는 적지 않는다. 비밀번호, 토큰, SSH 키, 쿠키, 운영 `.env` 값은 기록 금지.

## 빠른 링크

- 변경 이력: [../CHANGELOG.md](../CHANGELOG.md)
- Cafe24 단일 운영 정책: [cafe24-full-migration.md](./cafe24-full-migration.md)
- Cafe24 전환 점검: [cafe24-only-migration-audit.md](./cafe24-only-migration-audit.md)
- Cafe24 재해 복구: [cafe24-disaster-recovery.md](./cafe24-disaster-recovery.md)
- 수집 자동화 현황/장애 기록: [INGESTION_STATUS.md](./INGESTION_STATUS.md)
- Ingestor V3 설계: [ingestor-v3-design.md](./ingestor-v3-design.md)
- 키오스크 작업 기록: [../ops/kiosk/mini-pc/WORKLOG.md](../ops/kiosk/mini-pc/WORKLOG.md)
- 코드 리뷰 보고서: [../code_review_report.md](../code_review_report.md)
- 사이트 리뷰 보고서: [../site_review_report_v2.md](../site_review_report_v2.md)
- ESLint 유실 조사: [../eslint_final_investigation_report.md](../eslint_final_investigation_report.md)

## 2026-08-31 인제스터 관리자 제외의 DB 종결 상태 미반영

- 상태: 공통 원인 수정·배포 전
- 증상: 관리자 인제스터에서 수집 후보를 선택해 `일괄 제외`하거나 행의 `제외`를 눌렀을 때 화면에서는 즉시 사라졌지만 DB에서 제외 상태로 종결되지 않아 같은 후보가 다시 나타날 수 있었다.
- 판정 경로: 선택 ID → 관리자 확인 → `/api/scraped-events` DELETE → 범용 원장 행 물리 삭제 → 화면 로컬 목록 무조건 제거 순이었다. DELETE 실패 여부와 실제 삭제 건수도 확인하지 않아 서버가 실패해도 성공처럼 보일 수 있었다. 반면 수집 저장 경로에는 `excluded` 상태·제외 사유와 같은 ID 재수집 차단이 이미 있었다.
- 기존 구현 판정: `일부 있음`. `scraped_events.status=excluded`, 신규·혜택 탭 숨김, 수집 재진입 차단과 구조화된 제외 사유가 이미 같은 책임을 소유한다. 정확한 빈틈은 관리자 단건·일괄 버튼이 이 상태 전이를 재사용하지 않고 물리 삭제를 호출한 연결 누락이었다. 새 함수 외에 필드·테이블·큐·상태값은 추가하지 않았다.
- 기존 보호 목적: 완료·중복·제외 같은 종결 후보를 일반 재수집이 다시 신규로 열지 않게 하고, 제외 근거를 원장에 보존한다. 운영 이벤트 DB와 이미 등록된 일정 연결은 인제스터 후보 제외와 별개로 유지한다.
- 수정: 단건·일괄 제외를 기존 후보 저장 API의 `excluded` 상태 전이로 연결하고 `관리자 수동 제외/admin_review` 근거를 보존한다. 서버 성공 응답과 반환 상태를 확인한 건만 화면에서 제거하며, 일괄 처리 중 일부 실패 시 성공한 건만 제거하고 나머지 선택은 유지해 재시도할 수 있게 했다. 같은 ID뿐 아니라 다른 ID로 다시 발견된 동일 콘텐츠도 기존 날짜·출처·장소·DJ/제목 고신뢰 중복 기준으로 제외 원장과 비교해 차단한다. `duplicate` 보조행은 권위 원장으로 승격하지 않고 기존처럼 비교에서 제외한다.
- 검증 범위: TypeScript 검사, 대상 ESLint, 서버 후보 원장 회귀, 프로덕션 빌드와 변경 공백 검사를 수행한다. 정책 제외 행 생성, 같은 ID와 다른 ID 재수집 차단, 일반 대기·완료 후보 중복, `duplicate` 보조행 제외를 검증한다. 운영 DB의 기존 행 수정은 하지 않는다.
- 관련 파일: `src/pages/admin/v2/EventIngestorV2.tsx`, `server/cafe24/function-api.js`

## 2026-08-27 공식 소셜 자동등록 누락을 단건 성공으로 오판

- 상태: 공통 원인 수정·운영 배포·재수집·공식 소스 전체 공개 검증 완료
- 증상: 2026-08-29 봉천살롱 졸공 한 건의 자동등록 성공만 확인하고 전체 자동등록이 정상화됐다고 잘못 보고했다. 실제로는 스윙프렌즈 공식 해피홀 공지의 8월 28일 금햎 `DJ 쓴귤`이 후보로 생성되지 않았고, 스윙스캔들 8월 27일 `DJ 호두`도 2차 근거 비교에서 차단됐다.
- 판정 경로: 공식 원문 발견 → 날짜별 분해 → AI 추출·2차 판정 → 후보 저장 → 등록 API → 공개 이벤트 API를 다시 대조했다. 해피홀 제목 `8월 28일 금햎 & 29일토 정모 쉽니다`에서 축약된 29일을 휴무 날짜로 인식하지 못해 28일을 휴무로 만들고 포스터 추출 전에 반환했다. 이후 AI도 `금햅/금햎`을 소셜 명칭으로 인정하지 않았고 포스터 DJ `쓴귤`을 `쏜꽃`으로 오독했다. 스캔들은 `호두님입니다`의 존칭을 이름 경계로 처리하지 못했다. 기존 실행 요약은 등록 성공 건수만 보여 공식 원문 기대 건수와 공개 결과의 누락을 실패로 만들지 않았다.
- 기존 구현 판정: `일부 있음`. 공식 소스 목록, 날짜별 후보·휴무 분해, 2단계 AI, 단계 차단 로그, 공개 이벤트 API와 과거 DJ 원장이 이미 있었다. 정확한 빈틈은 축약 휴무 날짜의 범위, 해피홀 소셜 별칭, OCR 철자 교차검증, 존칭 근거 비교, 그리고 기대 후보와 공개 결과의 완결성 연결이었다. 새 DB·필드·테이블·큐·상태값은 만들지 않았다.
- 기존 보호 목적: 휴무를 정상 소셜로 등록하지 않고, 포스터 글자가 불명확하면 DJ를 발명하지 않으며, 공식 장소·날짜·활동·전체 DJ가 모두 근거와 일치한 경우에만 자동등록한다. 부분 실행·시간 초과는 성공으로 보지 않는다.
- 수정: 같은 달의 축약 일자를 별도 날짜 앵커로 분해하고 혼합 공지에서 휴무 후보와 활성 소셜 AI 추출을 함께 보존한다. `금햅/금햎/금해피/일햅/일햎/일해피`를 해피홀 소셜 근거로 연결하고, 과거 공식 일정의 DJ 이름은 포스터 OCR 철자 힌트로만 사용하며 현재 이미지 근거 없이는 승인하지 않는다. 한국어 존칭은 DJ 이름 밖의 문법 요소로 비교한다. 자동등록 대상 공식 소셜은 제목 기반 기대 일정과 실제 추출 후보를 합쳐 원장화하고, 실운영 실행이 후보 준비 및 공개 API 일치를 확인하지 못하면 누락을 기록하고 실패 종료한다. 건조 실행도 저장 직전 2차 AI까지 수행한다.
- 유지한 불변조건: 휴무 날짜는 소셜로 등록하지 않고, 힌트는 현재 원문의 증거를 대신하지 않는다. 날짜·장소·DJ·이미지·공식 출처 검증, 일반 행사·대회·수동 출처 차단, 결정론 후보 ID와 중복 방지는 유지한다. 정규 소셜 원장과 운영 DB를 직접 수정하지 않는다.
- 검증: 수집 표준, AI 판정 28건, 자동등록 보고 3건, 서버 등록 연결 36건과 프로덕션 빌드가 통과했다. 실제 공식 게시판 건조 실행에서 해피홀은 `8/28 DJ 쓴귤 소셜 + 8/29 휴무`, 스윙스캔들은 `8/27 DJ 호두 + 8/29 졸공`으로 분리됐고 두 실행 모두 기대 건수와 준비 건수가 일치하며 누락 0이었다. 6개 네이버 자동등록 소스 통합 실행은 시간 예산 초과 시 남은 소스 2곳과 종료 코드 75를 남겨 성공으로 오인되지 않음을 확인했다.
- 추가 운영 근거: 첫 실운영 완결성 실행 `20260827_033929_9213`은 8월 27일 `DJ 호두` (`16b9c3de-5980-4134-a94c-fba2650b751b`)와 8월 29일 스윙스캔들 졸공 (`7fd7e588-6c9a-48c8-8be5-a0bde90b5db6`)을 등록했지만, 8월 28일 해피를 후보 중복으로 차단해 기대 3·공개 확인 2·누락 1, 종료 코드 1을 정확히 남겼다. 운영 후보 원장에서 공식 후보 `쓴귤`은 수동 대기 중인 Instagram OCR 후보 `쓴굴`과 제목·날짜·장소가 같다는 이유로 중복 처리됐다.
- 추가 근본 원인·수정: 날짜·장소·활동·DJ가 같은 소셜만 합친다는 기존 보호 로직 뒤에 2026-06-12의 일반 유사 제목 규칙이 계속 실행돼 DJ 불일치를 다시 중복으로 뒤집었다. 소셜 양쪽의 DJ가 모두 확정됐고 다르면 일반 제목·출처 유사도 규칙을 적용하지 않는다. 이전 오판 `duplicate`는 같은 원문·날짜의 완전한 자동등록 후보이고 참조 후보가 미완료이며 현재 규칙으로 중복이 아닐 때만 기존 `pending`으로 재판정한다. 새 DB·필드·테이블·큐·상태값은 추가하지 않았다.
- 최종 운영 검증: 구현 커밋 `ec0a7be1`을 배포한 뒤 해피홀 단독 실행 `20260827_040036_10898`은 8월 28일 `DJ 쓴귤 | 해피홀 금요 소셜` (`6627c20e-1b4b-474d-9420-c125432b7905`)을 등록하고 기대 1·준비 1·공개 확인 1·누락 0, 종료 코드 0으로 완료했다. 이어 공식 네이버 6개 소스 전체 실행 `20260827_040256_11323`은 기대 4·준비 4·공개 확인 4·누락 0, 남은 소스 0, 접근 실패 0, 종료 코드 0이었다. 공개 API에서 8월 27일 사보이 `DJ 호두`, 8월 28일 해피홀 `DJ 쓴귤`, 8월 29일 봉천살롱·사보이 졸공 네 건의 날짜·장소·분류·포스터·원문 링크를 모두 확인했고 해피 포스터는 HTTP 200이다. 수집 표준, 자동등록 보고 3건, AI·서버·정기 소셜 Vitest 78건, 대상 ESLint, `git diff --check`, 프로덕션 빌드가 통과했다. 부산 최근 게시물의 미래 날짜·포스터 부재는 자동등록 대상 일정 근거가 아니어서 기존 정책대로 저장에서 제외했다.
- 관련 커밋: `8524db2b`, `ec0a7be1`
- 관련 결정: `docs/decisions/2026-08-27-ingestion-registration-completeness.md`, `docs/decisions/2026-08-27-social-dj-conflict-deduplication.md`
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/ai-candidate-adjudicator.mjs`, `scripts/ingestion/auto-registration-report.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/swing-social-map.mjs`, `server/cafe24/function-api.js`, 관련 테스트

## 2026-08-26 알림함 당일 요약 썸네일·내용 불일치

- 상태: 근본 원인 수정·운영 배포 및 공개 검증 완료
- 증상: 알림함에서 `DJ 미우 | 스윙타운 월간 일정 화요 소셜` 카드가 다른 항목인 `엘라스틴 _ LINDINIT SEASON TWO`의 포스터를 표시해 제목과 썸네일이 일치하지 않았다.
- 판정 경로: 당일 요약 생성기가 행사별 `items[]`와 요약 대표 이미지를 서버 `user_notifications.data_json`에 저장하고, 알림함이 이를 행사별 카드로 펼친 뒤 행사 ID로 최신 미리보기를 조회한다. 운영 원장을 읽기 전용으로 대조한 결과 2026-08-25 요약의 `엘라스틴` 항목에는 자체 이미지가 있었고 `DJ 미우` 항목은 `image=null`이었지만, 요약 대표 이미지는 첫 항목인 `엘라스틴` 이미지였다. 화면은 이미지가 없는 항목에 이 대표 이미지를 대입하고 최신 미리보기보다 먼저 사용했다. 읽음 처리·Push 전송·일정 원장은 정상이며 화면 조합 단계만 잘못됐다.
- 기존 구현 판정: `일부 있음`. 행사별 이미지·행사 ID, 최신 행사 미리보기 조회, 이미지 없는 카드의 기본 아이콘과 단일 알림 자체 이미지 처리가 이미 있었다. 정확한 빈틈은 다항목 요약의 대표 이미지를 개별 항목 이미지처럼 재사용한 폴백이며 새 함수·필드·테이블·큐·상태값은 필요하지 않다.
- 기존 보호 목적: 한 개의 요약 알림을 여러 행사 카드로 펼치고, 단일·레거시 알림의 자체 이미지와 최신 행사 이미지 대체를 제공한다. 각 카드의 제목·행사 ID·이미지는 같은 행사를 가리켜야 하며 이미지가 없는 일정은 다른 일정의 포스터를 빌리지 않는다.
- 수정: 펼친 `items[]` 카드는 해당 항목의 이미지 필드와 같은 행사 ID로 조회한 최신 미리보기만 사용한다. 요약 대표 이미지와 알림 전역 이미지 폴백은 펼친 카드에서 제거하고, 단일 알림은 기존 매핑 단계에서 자체 이미지를 계속 보존한다.
- 유지한 불변조건: 알림 종류·날짜 분류, 행사 상세 열기, 읽음 원장, Push 대표 이미지, 당일 요약 저장·전송·재시도, 단일 신규 알림 이미지와 전역 이미지 비드래그 정책은 변경하지 않는다. 운영 DB와 기존 알림 행은 수정하지 않는다.
- 검증: 실제 운영 형태처럼 첫 항목 자체 이미지·둘째 항목 이미지 없음·요약 대표 이미지가 함께 있는 회귀 사례를 기존 알림함 테스트에 확장했다. 이미지 없는 둘째 카드가 대표 이미지를 공유하지 않고 기본 아이콘을 쓰며, 단일 알림 자체 이미지는 유지됨을 확인했다. 전체 Vitest 제품 스위트 74개 파일·469개 테스트와 Node 전용 35개 테스트, 대상 ESLint·TypeScript 무출력 검사, Cafe24 프로덕션 빌드와 `git diff --check`가 통과했다. 서버 생성 데이터는 운영 원장에서 항목별 값이 정상임을 확인했고 생성·전송 코드는 변경하지 않았으므로 중복 서버 테스트를 새로 추가하지 않았다. 배포 후 공개 빌드 `1787754258056` (`2026-08-26 23:24 KST`)과 외부·내부 헬스가 정상이고 서비스는 `active`였다. 알림 모달 JS의 SHA-256은 로컬·운영 서버·공개 응답에서 `a1d58c1f61f1d116ee61263ae03e5d6231b0b5b143e2b7019f489bc40f5dbb79`로 일치했다.
- 관련 커밋: `1b2f84b7`
- 관련 파일: `src/components/NotificationHistoryModal.tsx`, `src/components/NotificationHistoryModal.test.tsx`, `CHANGELOG.md`

## 2026-08-25 메인 광고의 지난 일정 보충 노출

- 상태: 근본 원인 수정·운영 배포 및 공개 검증 완료
- 증상: 2026-08-25 홈 메인 광고의 `10 / 10` 위치에 2026-08-23 `스윙프렌즈 3학기 특별 워크샵 2탄`이 전면 노출됐다.
- 판정 경로: 운영 이벤트의 시작일·대표일·종료일은 모두 `2026-08-23`이라 날짜 판정에서는 정상적으로 지난 일정이었다. 현재·미래 후보가 작성자·장소 중복 제거 후 8개뿐이자 표시 단계가 최소 10개를 맞추기 위해 8월 24일과 23일 지난 후보를 추가했고, 자동 순환이 지난 후보도 전면에 올렸다. 원장 데이터·시간대·캐시는 원인이 아니었다.
- 기존 보호 목적: 광고 카드 스택을 최소 10개로 유지하면서 현재·미래 일정을 우선하고, 부족분만 지난 일정으로 낮은 빈도 보충하려는 정책이었다.
- 수정: 기존 최초 시작일 판정과 현재·미래 우선순위 함수를 유지하고, 지난 후보 생성·표시 전달·저빈도 자동 순환 경로를 제거했다. 현재·미래 후보가 10개 미만이면 있는 개수만 표시하며, 장르 필터·작성자/장소 중복 제거·최대 15개·오래 등록된 미래 일정 보충 설정은 유지한다. 새 함수·필드·테이블·큐·상태값은 추가하지 않았다.
- 검증: 메인 광고 날짜·정렬·중복 제거·최대 개수·자동 순환 회귀와 배너 번역 회귀 21건이 통과했다. 대상 ESLint 오류 0건(기존 경고 17건)과 Cafe24 프로덕션 번들 빌드가 성공했다. 운영 공개 API 데이터를 2026-08-25 기준으로 같은 선택 함수에 넣은 결과 현재·미래 후보는 중복 제거 후 8개, 표시 8개, 지난 일정 0개였고 문제의 8월 23일 이벤트가 제외됐다.
- 배포: 구현 커밋 `c9035490`을 `origin/fix/sitewide-image-drag-integration-20260811`에 푸시한 뒤 Cafe24 공개 빌드 `1787614754823` (`2026-08-25 08:39:19 KST`)을 배포했다. 공개·로컬 `version.json`이 일치하고 외부 헬스 `ok`, 서버 서비스 `active`였다. 공개 홈 DOM은 광고 `1 / 8`, 지난 일정 0건으로 렌더링됐고 문제의 8월 23일 광고는 목록에 없었으며 브라우저 오류는 0건이었다.
- 관련 파일: `src/pages/v2/components/EventList.tsx`, `src/pages/v2/components/EventList/components/EventPreviewSection.tsx`, `src/pages/v2/components/EventList/utils/homeAdPriority.ts`, `src/pages/v2/components/EventList/utils/homeAdPriority.test.ts`, `src/pages/v2/components/NewEventsBanner.tsx`, `CHANGELOG.md`

## 2026-08-22 하단 메뉴 혜택 아이콘 설정 예외·메인 독립 아이콘 오삭제

- 상태: 공통 원인 수정·독립 아이콘 교정·운영 배포 검증 완료
- 현상: 하단 컴팩트 메뉴의 `무료, 할인 이벤트`가 사용자/관리자 고정 설정을 따르지 않고 노출됐다. 이 예외와 별개인 `오늘 일정` 왼쪽의 분홍색 혜택 바로가기까지 같은 문제로 잘못 판정해 1차 수정에서 필수 아이콘이 삭제됐다.
- 실행 경로: 하단은 `HomeV2MenuPanel.quickMenuItems` 렌더 전 `benefits`를 `pinned_menu_ids` 뒤에 강제 추가해 설정 원장을 무시했다. 반면 분홍색 아이콘은 `NewEventsBanner.NEB-quickActions`가 원데이·연습실과 함께 소유하는 항상 노출 독립 진입점이며 하단 고정 설정의 소비자가 아니다. 1차 수정은 두 소유 경계를 합쳐 독립 버튼·배지·반응형 스타일까지 제거했다. DB 원장 쓰기는 없었다.
- 기존 구현 판정: `일부 있음`. 하단은 개인 `user_home_menu_settings` 우선·행 부재 시 `home_menu_layout_defaults` 공유·최대 5개 고정 구조가 이미 있었고, 독립 버튼은 기존 미확인 혜택 수·진입 확인 처리·모바일·키오스크 스타일을 이미 가졌다. 정확한 빈틈은 하단 강제 추가 하나였으며 새 필드·테이블·상태값은 필요하지 않다.
- 기존 보호 목적: `오늘 일정` 왼쪽 혜택 아이콘은 하단 메뉴 배치와 관계없이 항상 진입 가능해야 하고, 새 혜택 수 배지와 목록 진입 시 확인 처리를 유지해야 한다. 하단은 개인 설정 행이 있으면 그 값, 행이 없으면 관리자 기본값을 공유해야 한다.
- 수정: 하단 컴팩트 메뉴는 `pinnedMenuIds`의 최대 5개만 렌더하고 `benefits` 강제 추가는 제거한 상태를 유지했다. `NewEventsBanner` 독립 혜택 버튼과 기존 미확인 계산·확인 콜백·반응형·키오스크 스타일만 원상 복구했다. 레이아웃 선택은 값 비교 추정 없이 개인 행 존재 시 개인값, 행 부재 시 관리자 기본값을 적용한다.
- 유지한 불변조건: 독립 혜택 버튼은 하단 고정 여부와 무관하게 존재한다. 하단은 고정하지 않은 항목을 자동 승격하지 않고 최대 5개, 개인값 우선·미설정 사용자 관리자 기본값 공유를 유지한다. 혜택 분류·목록·읽음 원장, 원데이·연습실·자유게시판 동작, 직접 URL 진입, 기본값과 같게 저장할 때 개인 행을 지우는 상속 전환은 변경하지 않았다. 스키마·운영 데이터 쓰기·새 드래그 동작은 없다.
- 검증: `NewEventsBanner` 독립 아이콘·배지·클릭 확인 콜백과 `HomeV2MenuPanel` 하단 미고정 제외·고정 포함·개인/관리자 우선순위, 혜택 미확인 ID 안정성을 함께 검증한 대상 Vitest 3개 파일 8건이 통과했다. TypeScript 무출력 검사, 대상 ESLint 오류 0건(기존 경고 17건), Cafe24 프로덕션 빌드와 `git diff --check`도 통과했다. 운영 DB 연결 로컬과 배포 후 운영의 기본 뷰·390×844에서 독립 분홍 아이콘 1개가 보이고, 하단은 설정된 5개만 보이며 미고정 `benefits` 0개, page error 0건임을 확인했다. 기본 뷰에서 독립 아이콘은 `오늘 일정` 왼쪽에 위치했다.
- 배포·데이터 영향: 교정 커밋 `6d42b262`를 활성 브랜치와 배포 브랜치에 푸시한 뒤 Cafe24 공개 빌드 `1787392580831`로 배포했다. 서비스 `active`, 외부 헬스 `ok`, 공개 `version.json`의 버전 일치를 확인했다. DB·스키마·메뉴 설정·일정 원장은 수정하지 않았다.
- 관련 결정: `docs/decisions/2026-08-21-benefit-event-main-badge.md`
- 관련 파일: `src/pages/v2/components/HomeV2MenuPanel.tsx`, `src/pages/v2/components/NewEventsBanner.tsx`, `src/pages/v2/components/EventList.tsx`, `src/pages/v2/components/EventList/components/EventPreviewSection.tsx`, `src/pages/v2/components/HomeNavButtonsSection.tsx`

## 2026-08-22 무료·할인 상세 화면 중단 및 해피홀 일반 소셜 혜택 오분류

- 상태: 근본 원인 수정·운영 데이터 교정·배포 검증 완료
- 현상: `/benefit-events`에서 항목을 누르면 `ReferenceError: getDisplayDate is not defined`로 페이지 전체가 오류 화면으로 바뀌었다. `DJ 테일 | 스윙프렌즈 해피홀 게시판 금요 소셜`은 무료·할인과 무관한 일반 소셜인데도 무료 이벤트로 표시됐다.
- 실행 경로: 목록 항목 선택이 상세 모달을 렌더하면서 삭제된 날짜 함수명을 호출해 React 트리가 중단됐다. 해피홀 원문의 `무료라인강습이 없는 날` 문구는 수집 혜택 판정에서 `free_event`가 됐고, 후보 `d87841b70fcf84a0`의 혜택 필드가 자동등록 이벤트 `73a4c5d8-31a5-435e-b933-2741fa971fba`까지 전파돼 혜택 목록에 노출됐다.
- 기존 구현 판정: `일부 있음`. 공통 `getBenefitEventDisplayDate`, 혜택 목록 필터, 무료 부정문 제거 규칙, 후보와 공개 이벤트의 혜택 필드 연결, 관리자 일반 분류와 운영 교정 패턴은 이미 있었다. 정확한 빈틈은 상세 모달 한 곳의 옛 함수명과 한국어 부정 관형형 `없는`이었다. 새 필드·테이블·큐·상태값은 추가하지 않았다.
- 기존 보호 목적: 다회차 혜택은 오늘 이후의 다음 실제 날짜를 상세과 목록에서 동일하게 표시한다. 무료 분류는 실제 무료 참가·입장·관람·수강 근거가 있을 때만 허용하고, 부정된 무료 문구나 무료 주차·음료 같은 부수 혜택은 일반 일정으로 유지한다. 후보와 공개 이벤트의 혜택 상태는 함께 교정해야 다음 재수집에서 잘못된 값이 복원되지 않는다.
- 근본 원인: 2026-08-21 날짜·읽음 상태 공통화에서 지역 함수 `getDisplayDate`를 제거하고 목록 정렬은 새 공통 함수로 연결했지만 상세 모달 호출 한 곳과 상세 클릭 회귀 테스트가 누락됐다. 무료 부정문 정규식은 `없음/없습니다/없다`만 제거하고 `없는`을 남겨, 뒤의 긍정 규칙이 같은 문장의 `무료라인강습`만 근거로 오인했다.
- 수정: 상세 모달도 기존 공통 날짜 함수를 사용하게 하고 실제 항목 클릭·일정·장소 렌더 회귀 테스트를 추가했다. 무료 부정문 규칙에 관형형 `없는`을 연결하고 실제 해피홀 문장을 수집 표준 회귀 사례로 확장했다. 운영 교정은 정확한 이벤트·후보·원문 링크와 기존 상태를 검증한 뒤 이벤트를 `false/null`, 후보의 혜택 필드는 제거하며 적용 전 두 원본 레코드를 백업하고 한 트랜잭션에서 갱신한다.
- 유지한 불변조건: 별도의 명시적 무료 입장·참가 근거가 같은 원문에 있으면 계속 무료 이벤트로 판정한다. 정기권·할인·원데이 혜택, 과거 혜택 표시, 상세 포스터·원본 링크, 일반 소셜 일정 자체와 후보의 등록 연결은 변경하지 않는다. 운영 교정은 이벤트를 삭제하거나 알림을 재발송하지 않는다.
- 검증: 혜택 페이지 6건과 인접 혜택 UI 포함 Vitest 4개 파일 11건, 전체 Vitest 제품 단언 445건, Vitest가 스위트로 인식하지 않는 기존 Node 전용 파일 4건과 Node 스크립트 27건을 올바른 러너로 분리해 모두 통과했다. TypeScript 무출력 검사, 전체 수집 표준 검사, 교정·배포 스크립트 구문, 대상 ESLint 오류 0건, Cafe24 프로덕션 빌드와 `git diff --check`도 통과했다. 운영 DB 읽기 전용 미리보기에서 대상 이벤트와 후보가 모두 `free_event → 일반`으로만 바뀌며 대상 외 레코드 쓰기가 없음을 확인했다. 배포 후 새 브라우저에서 혜택 카드 4개를 불러와 첫 항목 상세를 실제 클릭했으며 일정·장소와 같은 제목의 상세창이 열리고 page error 및 예상 밖 HTTP 오류는 0건이었다.
- 배포·데이터 영향: 구현 커밋 `43965c42`를 푸시하고 Cafe24 공개 빌드 `1787381114604`를 배포했다. 서비스 `active`, 외부 헬스 `ok`, 공개 버전을 확인했다. 운영 교정은 적용 전 이벤트·후보 원본을 권한 `600` 백업으로 보존하고 두 행만 한 트랜잭션에서 일반 상태로 갱신했으며 재실행은 `alreadyApplied=true`였다. 공개 API에서 이벤트는 `benefit_eligible=false`, `benefit_kind=null`, 전체 혜택 4건 중 `DJ 테일` 혜택 0건임을 확인했다.
- 관련 파일: `src/pages/benefit-events/BenefitEventsPage.tsx`, `src/pages/benefit-events/BenefitEventsPage.test.tsx`, `scripts/ingestion/candidate-utils.mjs`, `scripts/test-ingestion-standards.mjs`, `scripts/repair-2026-08-22-happyhall-benefit.mjs`, `scripts/deploy-cafe24.sh`

## 2026-08-21 자유게시판 게시 방식 중복 선택과 모바일 겹침

- 상태: 근본 원인 수정, 운영 배포 및 검증 완료
- 현상: 관리자 자유게시판 글쓰기에서 익명·비공개·공지를 동시에 선택할 수 있었고 세 카드가 모두 선택 상태로 표시됐다. 모바일에서는 게시 방식 카드가 내용 편집기 안으로 올라와 입력 영역을 침범했다.
- 실행 경로: 각 카드의 독립 `checkbox` 변경이 `formData.is_anonymous`·`is_hidden`·`is_notice`를 따로 갱신하고, 등록·수정 시 세 값을 그대로 `board_posts`에 저장했다. 화면에서는 높이가 제한된 `.pem-form-content`의 flex 자식들이 축소되고 넘친 카드 내용이 편집기 위에 그려졌다. 게시 모달과 내부 `UniversalEditor`가 `.universal-editor-container` 이름까지 공유해 양쪽 CSS가 교차 적용됐다. 게시글을 실제로 저장했을 때만 DB 결과가 생기며, 이번 재현·검증에서는 등록·수정 버튼을 실행하지 않았다.
- 기존 구현 판정: `일부 있음`. 게시글의 세 불리언 저장, 비공개 접근 제한, 익명 작성자 마스킹, 공지 정렬과 레거시 복합값 처리 경로는 이미 있었다. 정확한 빈틈은 글쓰기 UI의 단일 선택 판정과 바깥 모달 전용 CSS 범위였다. 새 필드·테이블·API·상태값 없이 기존 값을 재사용했다.
- 기존 보호 목적: `is_hidden`은 제목·본문·작성자까지 작성자와 관리자에게만 공개하고, `is_anonymous`는 공개 글의 작성자 표시만 가린다. 서버가 과거 복합값을 읽을 때 비공개 보호를 먼저 적용하는 방어는 그대로 유지해야 한다.
- 해결: 게시 방식을 `일반 공개`·`익명`·`비공개`·관리자용 `공지`의 단일 라디오 그룹으로 바꾸고, 선택 시 기존 세 불리언 중 대응 값만 true가 되도록 연결했다. 과거 복합값을 수정할 때는 `비공개 → 공지 → 익명 → 일반 공개` 우선순위로 한 방식만 표시·저장해 공개 범위를 실수로 넓히지 않는다. 게시 모달 클래스를 내부 편집기와 분리하고 스크롤 내용의 직접 자식이 축소되지 않게 해 카드가 편집기 아래 정상 문서 흐름을 유지하도록 했다.
- 유지한 불변조건: 비공개 서버 비식별화, 익명 작성자 원본 보존과 관리자 확인, 공지 머릿말·상단 표시, 건의 작성의 비공개 기본값, 일반 글의 공개 기본값은 유지했다. 서버는 배포 전부터 존재할 수 있는 복합값을 계속 안전하게 읽으며 DB 마이그레이션이나 운영 게시글 변환은 하지 않았다. 새 드래그 동작은 추가하지 않았다.
- 검증: 편집기의 기본·전환·저장·관리자 공지·레거시 복합값 정규화·CSS 클래스 분리와 서버의 게스트·작성자·관리자·익명+비공개 보호, 목록·상세 갱신을 포함한 대상·인접 Vitest 4개 파일 22개 테스트가 통과했다. 대상·인접 ESLint 오류 0건(기존 경고 38건, 변경 편집기 13건), Cafe24 프로덕션 빌드와 `git diff --check`가 통과했다. 운영 DB 연결 로컬 433×938 CSS 뷰포트(요청 390×844)에서 편집기와 게시 방식 사이 17.99px 간격, 스크롤 가능 범위, 네 방식 전환마다 체크값 1개와 브라우저 오류 0건을 확인했다. 비공개·익명 서버 정책은 기존 테스트가 같은 안전장치를 이미 검증하므로 중복 API 테스트를 새로 만들지 않고 해당 테스트를 함께 실행했다.
- 배포·데이터 영향: 구현 커밋 `62aed9ae`를 운영 버전 `1787283610089`로 배포했다. 공개 홈 200, 서버 헬스 `ok: true`, 서비스 `active`를 확인했고 운영 `UniversalPostEditor` JS·CSS의 SHA-256이 배포 빌드와 각각 일치했다. 운영 자산에서 단일 선택 문구와 분리된 CSS 클래스를 확인했으며 스키마·마이그레이션·운영 게시글 쓰기는 없었다.
- 관련 결정: `docs/decisions/2026-08-21-free-board-post-mode-single-selection.md`
- 관련 파일: `src/pages/board/components/UniversalPostEditor.tsx`, `src/pages/board/components/UniversalPostEditor.css`, `src/pages/board/components/UniversalPostEditor.test.tsx`, `server/cafe24/board-post-security.test.js`

## 2026-08-21 캘린더 리스트·지도 하위 분류 시 보기 초기화

- 상태: 근본 원인 수정, 운영 배포 및 검증 완료
- 현상: 캘린더에서 `리스트` 또는 `지도`를 선택한 뒤 `전체`·`소셜&행사`·`강습` 분류를 누르면 상위 보기 모드가 `캘린더`로 돌아갔다. 지도는 보기 모드를 유지하더라도 하위 분류가 지도 내용에 연결되지 않은 상태였다.
- 실행 경로: 하위 분류 클릭이 `tabFilter`와 캘린더 레이아웃 계산을 갱신하고, 이 계산을 의존하는 오늘 이동 콜백이 다시 생성되면서 URL 보기 동기화 효과가 재실행됐다. 보기 전환은 실제 주소를 native `replaceState`로 `view=list/map`으로 바꿨지만 React Router의 `location.search`는 이전 `view=calendar&scrollToToday=true`를 유지했고, 효과가 이 오래된 값을 읽어 `displayMode=calendar`를 실행했다. 결과는 화면 상태 초기화뿐이며 DB·외부 데이터 쓰기는 없었다.
- 기존 구현 판정: `일부 있음`. 리스트에는 분류 클릭 시 모드를 유지하는 분기가 있었고 보기 전환은 `view` URL, 오늘 위치 이동, 라우트 재진입 방어를 이미 관리했다. 공통 강습류 판정도 있었지만, 실제 주소와 Router 주소의 경계가 연결되지 않았고 지도에는 `tabFilter` 전달이 빠져 있었다.
- 기존 보호 목적: native `replaceState`는 보기 전환을 새 방문으로 만들지 않고 `scrollToToday`·`nav`를 제거해 캘린더 오늘 위치 워프가 리스트·지도에 개입하지 않게 한다. 알림 링크의 명시적 `category=all`이 이전 필터를 초기화하는 기존 정책도 유지해야 한다.
- 해결: URL 보기 동기화 효과가 native 교체 직후에는 실제 `window.location.search`를 읽도록 맞춰 기존 스크롤·재진입 보호를 유지했다. 비캘린더 모드의 하위 분류는 공통적으로 현재 모드를 보존한다. 기존 강습류 판정을 `matchesCalendarTabFilter`로 한곳에 연결해 리스트·지도·요약·레이아웃이 같은 기준으로 내용만 갱신하도록 했고, 지도에 `tabFilter`를 전달했다.
- 유지한 불변조건: `class`·`regular`·`club`·레거시 동호회 강습류는 `강습`, 그 외 소셜·행사는 `소셜&행사`로 처리한다. 캘린더 모드에서 같은 분류를 다시 누르는 오늘 위치 이동, 보기 URL의 replace 방식, 직접 URL·뒤로가기 동기화, 알림의 명시적 전체 필터는 변경하지 않았다. 새 드래그 동작은 추가하지 않았다.
- 검증: 캘린더 분류·이벤트 종류·레이아웃 대상 Vitest 3개 파일·19개 테스트, 대상 ESLint 오류 0건, Cafe24 프로덕션 빌드와 `git diff --check`가 통과했다. 운영 DB 연결 390×844 화면에서 `리스트 → 강습` 후 리스트 유지 및 강습·동호회 항목만 표시, `리스트 → 소셜&행사` 후 소셜·행사 항목만 표시를 확인했다. `지도 → 소셜&행사`는 8월 21일 2개, 이어 `강습`은 0개로 내용만 바뀌면서 지도 모드가 유지됐다. 검증 중 정규 소셜 조정은 두 실행 모두 생성·삭제 0건이었다.
- 배포·데이터 영향: 커밋 `e3a069f3`을 운영 버전 `1787255673746`으로 배포했다. 스키마·마이그레이션·새 상태값과 운영 일정 데이터 수정은 없다. 공개·서버 버전, 서비스 헬스, 리스트·지도 분류 유지 흐름을 운영 화면에서 확인했다. 배포 뒤 키오스크의 Chrome·세로 화면·page watchdog도 모두 정상 상태를 유지했다.
- 관련 파일: `src/pages/calendar/page.tsx`, `src/pages/calendar/components/CalendarListView.tsx`, `src/pages/calendar/components/CalendarMapView.tsx`, `src/pages/calendar/utils/calendarTabFilter.ts`

## 2026-08-21 메인 무료·할인 이벤트 신규 배지

- 상태: 구현, 운영 배포 및 검증 완료
- 요청: 메인 화면의 `무료, 할인 이벤트` 바로가기 오른쪽 위에 새 혜택 수를 숫자 배지로 표시하고, 사용자가 혜택 목록에 들어가면 배지를 없앤다.
- 기존 구현 판정: `일부 있음`. 메인 바로가기와 혜택 목록, 일정의 `benefit_eligible` 분류, 자유게시판의 미확인 숫자 배지·읽음 동기화 방식은 이미 있었다. 그러나 무료·할인 목록의 현재 항목 ID를 사용자별로 확인 처리하는 상태와 메인 버튼 연결은 없었다.
- 해결: 기존 일정 조회 결과와 `swing_oneday_recruit_links` 혜택을 같은 변환 함수로 합치고, `benefit_eligible=true`이면서 종료되지 않은 항목의 고유 ID만 미확인 수로 계산한다. 메인 버튼 클릭과 `/benefit-events` 직접 진입 모두 현재 ID를 확인 처리하며 같은 탭·다른 탭에도 즉시 반영한다. 숫자는 버튼 우상단에 표시하고 99개 초과는 `99+`로 제한했다. 초기 로딩처럼 입력 배열 참조가 반복해서 바뀌어도 ID 내용이 같으면 파생 목록 참조를 유지해 상태 갱신 루프가 생기지 않게 했다.
- 상태 소유: 새 DB·테이블·알림 레코드는 만들지 않았다. 확인 ID는 로그인 계정 ID 또는 게스트 범위로 현재 브라우저 `localStorage`에 분리 저장하며 최대 5,000개로 제한한다. 따라서 알림함 읽음 상태와 일정 원본은 바뀌지 않고, 다른 기기 간 확인 상태는 동기화하지 않는다.
- 유지한 불변조건: 일반 일정과 종료된 혜택은 배지에서 제외하고, 날짜가 없는 상시 원데이 혜택은 현재 항목으로 유지한다. 혜택 목록의 기존 과거 항목 표시, 상세 열기, 분류·정렬과 알림 동작은 유지했다. 새 드래그 동작은 추가하지 않았다.
- 검증: 대상 Vitest 4개 파일·11개 테스트에는 내용이 같은 새 배열 입력의 반복 렌더 방지까지 포함했고, 새 훅·공통 유틸 대상 ESLint, Cafe24 프로덕션 빌드와 `git diff --check`가 통과했다. 운영 DB 연결 로컬 서버의 390×844 화면에서 실제 배지 `2`가 버튼 우상단에 표시되고, 목록 진입 후 메인으로 돌아왔을 때 배지와 접근성 문구가 사라지는 것을 확인했다. 검증 중 정규 소셜 조정은 생성·삭제 모두 0건이었다.
- 배포·데이터 영향: 커밋 `e3a069f3`을 운영 버전 `1787255673746`으로 배포했다. 운영 일정 등록·수정과 DB 마이그레이션은 없었다. 공개·서버 버전과 서비스 헬스를 대조했고, 운영 화면에서 배지 `2` 표시와 목록 진입 후 소멸을 확인했다. 롤백 시 UI 연결과 로컬 저장 키만 제거하면 된다.
- 관련 결정: `docs/decisions/2026-08-21-benefit-event-main-badge.md`
- 관련 파일: `src/hooks/useBenefitEventsUnreadCount.ts`, `src/lib/benefitEventsData.ts`, `src/utils/benefitEventVisibility.ts`, `src/pages/v2/components/NewEventsBanner.tsx`, `src/pages/benefit-events/BenefitEventsPage.tsx`

## 2026-08-21 자유게시판 글 단위 익명 작성

- 상태: 구현, 운영 배포 및 검증 완료
- 요청: 자유게시판 공개 글에서 작성자 표시만 가리는 익명 옵션을 추가하고, 글 내용까지 제한하는 기존 비공개 기능과 명확히 구분한다. 글쓰기 화면은 기존 제목·머릿말·편집기·공지·비공개 기능을 유지하면서 더 직관적으로 정리한다.
- 기존 구현 판정: `일부 있음`. `board_posts`는 `generic_records.data_json`에 게시글 속성과 실제 `user_id`·작성자 이름을 보관하고, 서버 조회 경로는 기존 `is_hidden` 글을 작성자·관리자 외 사용자에게 placeholder로 축소한다. 글쓰기 공통 컴포넌트와 목록·상세 작성자 표시도 이미 있었지만, 공개 익명 상태·입력 옵션·작성자 마스킹 연결만 없었다.
- 해결: 기존 게시글 JSON에 `is_anonymous`를 저장하며 스키마나 별도 익명 테이블은 추가하지 않았다. 서버는 관리자에게만 저장된 작성자 정보를 반환하고, 일반 사용자에게는 작성자 이름을 `익명`으로 바꾸며 프로필 관계를 제거한다. 다른 사용자의 응답에서는 `user_id`도 제거하고, 작성자 본인 응답에서만 수정·삭제 소유권 판정을 위해 본인 ID를 유지한다. 목록과 두 상세 경로에도 같은 표시 유틸을 적용해 클라이언트 방어를 겹쳤다.
- 유지한 불변조건: `is_hidden`은 본문·제목·작성자까지 작성자와 관리자에게만 공개하는 기존 보호를 먼저 적용한다. `is_anonymous`는 공개 범위를 바꾸지 않고 작성자 표시만 가리며 두 옵션은 독립적으로 저장된다. 관리자 편집이 저장된 익명 작성자를 `관리자`로 덮어쓰지 않으며, 기존 비공개·공지·머릿말·이미지·금지어·수정/삭제 흐름은 유지한다.
- 검증: 서버 마스킹의 게스트·타 사용자·작성자·관리자 및 익명+비공개 조합, 글쓰기 옵션 독립 저장, 관리자 편집, 게시판 읽기·목록·상세·본문 표시 회귀를 포함한 대상 Vitest 25개가 통과했다. 로그인된 로컬 자유게시판에서 데스크톱·390px 모바일 UI와 익명 선택 시 비공개 미선택을 확인했고 콘솔 오류가 없었다. 대상 ESLint는 오류 0건이며 기존 경고만 남았고, Cafe24 프로덕션 프런트와 서버 함수 패키징 및 `git diff --check`가 통과했다.
- 배포·데이터 영향: 커밋 `e3a069f3`을 운영 버전 `1787255673746`으로 배포했다. 운영 게시글 등록·수정과 DB 마이그레이션은 없었다. 운영 서버의 익명 마스킹 모듈 SHA-256이 배포 커밋과 일치하고 자유게시판 목록·글쓰기 진입이 정상임을 확인했다. 필드가 없는 기존 글은 일반 공개 글로 처리한다.
- 관련 결정: `docs/decisions/2026-08-21-free-board-anonymous-authorship.md`
- 관련 파일: `server/cafe24/board-post-security.js`, `src/pages/board/components/UniversalPostEditor.tsx`, `src/utils/boardPostPrivacy.ts`, `src/pages/board/hooks/useBoardPosts.ts`, `src/pages/board/hooks/useBoardDetail.ts`

## 2026-08-18 오후 게시된 익일 스윙타임 소셜 미수집

- 상태: 대상 일정 복구 및 저녁 재수집 스케줄 운영 반영 완료
- 현상: 2026-08-19 스윙타임 수요 소셜의 공식 공지가 있었지만 공개 캘린더에는 `DJ 미정` 정규 반복 일정만 표시됐고 운영 수집 후보는 없었다.
- 원인:
  - priority1·2 `swing-daily` LaunchAgent는 각각 08:00·09:00에만 실행됐다. 오전 실행 당시 스윙타임 Instagram은 새 게시물이 없었고 스윙프렌즈 Naver 카페 최신 글도 8월 15·16일 이전 일정뿐이었다.
  - 20시대 재확인에서는 스윙타임 공식 Instagram과 스윙프렌즈 Naver 카페 양쪽에 8월 19일 `DJ 유광` 공지가 노출됐다. 수집·자동등록 판정은 모두 통과했으므로 파싱이나 서버 등록 오류가 아니라 오전 실행 뒤 게시된 공지를 다시 확인할 예약 실행이 없었던 운영 스케줄 공백이다.
- 해결:
  - 실제 공식 Instagram 원문을 `swing-daily` 수집 API 경로로 재처리해 후보를 저장하고 `DJ 유광 | 스윙타임 수요 소셜`을 자동등록했다. 운영 DB를 직접 수정하지 않았다.
  - priority1 LaunchAgent에 20:30, priority2 LaunchAgent에 21:00 재실행을 추가한다. 오전 체크포인트와 동일한 `getAutomationSourceList('swing-daily')`·전역 실행 잠금·결정론 후보 ID를 그대로 사용해 새 게시물만 보강하고 중복 이벤트는 만들지 않는다.
  - 혜택 검색 중심 priority3·4는 이번 익일 일정 누락과 무관하므로 기존 10:00·11:00 실행을 유지한다.
- 운영 복구:
  - 실행 `20260818_202809_87701`은 신규 1건, 자동등록 1건, 접근 실패 0건, 남은 소스 0건으로 종료됐다.
  - 생성 일정은 `50bf473c-b621-4efe-9e2b-ca101db66c45`, 공식 원문은 `https://www.instagram.com/swingtimebar/p/DcLRUYAGvSI`다.
- 검증:
  - 수집 표준 검사와 저장소·설치본 LaunchAgent plist 문법 검사가 통과했다.
  - `launchctl print`에서 priority1의 `08:00·20:30`, priority2의 `09:00·21:00` 캘린더 트리거와 두 작업의 활성 상태를 확인했다.
  - 운영 후보 `cc66005dc6ccbdd8`은 `status=collected`, `is_collected=true`, AI 검증 0.99, 등록 일정 `50bf473c-b621-4efe-9e2b-ca101db66c45` 연결 상태다.
  - 공개 일정 API에는 실제 일정만 노출되고 `regular-social:swingtime-wed:2026-08-19`는 제거됐다. 저장 이미지 응답은 HTTP 200이다.
- 관련 결정: `docs/decisions/2026-08-18-evening-ingestion-catchup.md`
- 관련 커밋: `bd0aebe8`
- 관련 파일: `scripts/com.rhythmjoy.codex-ingestion.plist`, `scripts/com.rhythmjoy.codex-ingestion-priority2.plist`

## 2026-08-18 수집 후보 자동등록 0건 개선

- 상태: 수집기·서버 수정, 운영 배포, 자동등록 대상 누락 2건 복구 완료
- 현상: 08시 실행은 신규 후보 1건, 09시 실행은 신규 후보 2건을 저장했지만 두 실행 모두 자동등록은 0건이었다. 작업 자체와 후보 저장 API는 오류 없이 종료됐다.
- 원인:
  - 네오 141기 공지는 8월 18일을 신청 시작일로, 8월 30일 이후를 실제 강습일로 안내했지만 수집기가 신청일을 첫 행사일로 선택했다. 기존 마감일 방어는 날짜 주변에 `강습` 문구가 있다는 이유로 이 날짜를 통과시켰다.
  - 스윙타운 8월 18일 공지의 `DJ 루나`를 장소 별칭 `루나`로 잘못 인식해, 공식 수집원의 고정 장소 `봉천살롱`과 충돌했다.
  - 스윙타운 8월 22일 공지는 원문에 날짜·장소·DJ가 정확했지만 포스터에도 같은 날짜가 다시 보여야 한다는 2차 AI 규칙 때문에 보류됐다.
  - 건조 실행 중 `DJ` 접두사가 붙은 이름과 `조춘식이`를 `조춘식`으로 줄이는 모델 응답도 자동등록 불일치의 잠재 원인으로 확인했다.
  - 수정 수집기로 8월 22일 후보를 갱신한 뒤에는 후보가 `ready=true`, AI 검증 0.99까지 통과했지만 서버가 HTTP 422를 반환했다. 포스터에서 장소를 직접 확인해 `venue_provenance=poster_text`가 된 후보에 대해, 수집기는 동일한 공식 고정 장소 문구를 AI 근거로 허용하지만 서버는 `source_registry`일 때만 그 문구를 저장 원문 근거로 인정해 양쪽 안전 규칙이 불일치했다.
- 해결:
  - 행사일 후보를 고르기 전에 해당 날짜가 신청 시작·접수·입금·마감에만 쓰였는지 날짜별 문맥으로 판정해 제거한다. 실제 회차 날짜 문맥이 함께 있으면 보존한다.
  - 장소 별칭이 DJ 표기 안에만 있으면 공식 고정 장소를 덮어쓰지 않는다. `장소:`로 명시한 별칭은 우선하며, `강습별 상이`처럼 장소가 가변이라고 명시되면 고정 장소도 상속하지 않는다.
  - 포스터는 원문과의 모순 및 포스터에서 추출한 필드를 검증하는 증거로 사용한다. 신뢰 가능한 원문에 정확한 날짜가 있고 포스터가 모순하지 않으면, 포스터의 날짜 미표시는 중립으로 처리한다.
  - DJ는 `DJ`·`디제이` 접두사를 제거한 완전한 활동명으로 정규화하고, 원문 부분문자열만 맞는 축약 이름은 경계 검사로 거절한다.
  - 서버는 후보 장소가 출처의 공식 고정 장소와 정확히 일치하면 `source_registry`뿐 아니라 `poster_text`로 확인된 경우에도 같은 고정 장소 문구를 AI 근거 문맥으로 인정한다. 후보 장소가 다른 경우에는 기존처럼 인정하지 않는다.
- 검증:
  - 네오 실제 공지를 운영 저장 없이 다시 처리해 후보 날짜가 `2026-08-18`에서 첫 강습일 `2026-08-30`으로 교정되고, 강습별 장소가 달라 수동 검수로 안전하게 남는 것을 확인했다.
  - 스윙타운 실제 원문을 운영 저장 없이 다시 처리해 8월 22일 `조춘식이`, 8월 18일 `루나` 후보가 모두 장소 `봉천살롱`, 자동등록 준비 상태로 생성되는 것을 확인했다.
  - 두 스윙타운 후보를 실제 2차 모델로 검증해 각각 `register`, 신뢰도 `0.99`를 받았고 서버 자동등록 안전 검사도 둘 다 통과했다.
  - 수집 표준 검사, 수정 파일 문법 검사와 대상 ESLint, 전체 Vitest 70개 파일·410개 테스트, Node 회귀 31개, Cafe24 운영 정책 감사와 `git diff --check`가 통과했다.
  - 실제 8월 22일 후보 형태를 재현한 서버 회귀를 추가했다. 수집기·서버 대상 Vitest 49개, 전체 Vitest 실제 테스트 411개, 별도 Node 러너 4개, 수집 표준 검사, Cafe24 운영 정책 감사, 프로덕션 빌드와 `git diff --check`가 통과했다. 전체 Vitest 명령은 `node:test` 형식 파일 2개를 스위트로 인식하지 못해 종료코드 1을 반환했으며 두 파일은 전용 Node 러너에서 통과했다.
- 운영 복구:
  - `swingtown-cafe` 공식 수집원만 대상으로 실행 `20260818_194707_84079`를 수행했다. 8월 18일 후보 `bbedcf3555ee2fc5`를 같은 결정론 ID로 갱신한 뒤 자동등록했고, 실제 일정 `9016ff11-3b09-4902-877a-6dad4638b9d8`이 생성됐다.
  - 공개 일정 API에서 `DJ 루나 | 봉천살롱 화요 소셜`, 날짜 `2026-08-18`, 장소 `봉천살롱`, 저장 이미지 `/uploads/images/ingestor-events/bbedcf3555ee2fc5/full.webp`를 확인했다. 이미지 응답은 HTTP 200이다.
  - 실제 일정 등록 뒤 기본 반복 일정 `regular-social:swingtown-tue:2026-08-18`은 공개 응답에서 제거돼 중복 노출되지 않는다.
  - 같은 실행에서 8월 22일 후보는 운영 서버의 저장 원문 근거 검사에서 HTTP 422로 보류됐다. 당일 봉천살롱 복구에는 영향이 없으며 별도 후속 검토 대상으로 남긴다.
  - 미래 대기 소셜 후보 전수 점검 결과 자동등록 준비·AI 검증을 모두 통과하고도 일정 연결이 없는 후보는 이 8월 22일 1건뿐이었다. 8월 25일 `미우`와 8월 29일 `후안` 후보는 교차검증 전용 월간표 출처라 현재 정책상 수동 보류이며, 날짜별 공식 소셜 공지가 올라오면 자동등록 대상이 된다.
  - 서버 수정 배포 뒤 8월 22일 후보의 운영 건조 검증이 `wouldInsert=true`로 통과했고, 실제 일정 `DJ 조춘식이 | 봉천살롱 토요 소셜`(`86672fa8-dfce-4e90-bd43-bc4062110578`)을 자동등록했다. 공식 원문 링크와 저장 이미지가 연결됐고 이미지는 HTTP 200이다.
  - 등록 후 기본 반복 일정 `regular-social:swingtown-sat:2026-08-22`은 공개 응답에서 제거됐다. 미래 소셜 후보를 다시 전수 확인해 자동등록 준비·AI 검증 완료 상태인데 일정 연결이 없는 후보가 0건임을 확인했다.
- 배포 영향: 커밋 `a824013c`를 `origin/fix/sitewide-image-drag-integration-20260811`에 푸시하고 Cafe24 운영 버전 `1787051094438`을 배포했다. 공개 헬스 응답 `ok`, 서비스 `active`, 공개 버전 일치를 확인했다.
- 관련 커밋: `a824013c`
- 관련 결정: `docs/decisions/2026-07-28-ingestion-automatic-registration.md`
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/ai-candidate-adjudicator.mjs`, `scripts/ingestion/ai-candidate-adjudicator.test.js`, `scripts/test-ingestion-standards.mjs`, `server/cafe24/function-api.js`, `server/cafe24/ingestor-registration-link.test.js`

## 2026-08-15 8월 16일 해피홀·인더무드신림 실제 소셜 미수집

- 상태: 수집기·서버 수정, 운영 배포, 대상 원문 재수집 및 일정 복구 완료
- 현상: 공개 캘린더에는 2026-08-16 해피홀 `네오스윙 일요 소셜`과 인더무드신림 `드림발 일요 소셜`이 이미지와 DJ가 없는 정규 반복 일정으로만 남고, 공식 공지의 실제 DJ·포스터가 반영되지 않았다.
- 해피홀 원인: 네오스윙 공식 8월 2주 공지에는 8월 16일 `일햅 DJ 익두`가 있었고 원문 게시물의 직접 이미지는 1440×1800이었다. 수집기는 해당 날짜 후보를 두 번 만들었지만 게시물 직접 이미지가 아닌 프로필 격자 크롭 URL을 `poster_url`로 선택했고, 이미지 품질 검사가 두 후보를 모두 `poster_url looks cropped or thumbnail-sized`로 거절했다. 따라서 후보 저장·자동등록 단계까지 도달하지 않았다.
- 인더무드신림 원인: 공식 계정은 `inthemoodsillim`이지만 수집 레지스트리에는 존재하지 않는 `inthemood_sillim`이 등록돼 있다. 8월 15일 실행은 잘못된 경로를 Instagram 접근 실패로 기록했고, 앞선 실행의 대체 경로 결과도 예상 작성자 `inthemood_sillim`과 일치하지 않는다는 이유로 거절했다. 실제 공동 게시물에는 8월 16일 장소 `인더무드신림`, DJ `제이`, 소셜 시작이 명시돼 있다.
- 해결:
  - 인더무드신림 공식 계정을 `inthemoodsillim`으로 바로잡고 공식 공동 작성자 `dreambal_balboa`를 허용했다. 기존 수집원 ID는 반복 규칙과 진행 상태의 연속성을 위해 유지했다.
  - 이름이 있는 DJ 소셜은 공식 상세 원문에서 날짜·장소·DJ가 확정되면 이미지가 없어도 등록할 수 있게 했다. 잘못 선택된 격자 썸네일은 후보 전체를 거절하지 않고 제거하며, 명시 일정이 정기 일정을 대체할 때 주소·장소 ID·지도 링크를 상속한다.
  - Instagram 상세 원문의 작성자 검증은 등록된 공식 계정과 공동 작성자만 허용한다. 네오스윙 주간 공지의 대괄호 OCR 날짜와 DJ 문맥 잔여 문구도 정규화했다.
  - 점검·복구용 실행은 레지스트리에 있는 수집원과 해당 수집원의 명시적 게시물 URL만 받을 수 있도록 제한했다. 운영 DB를 직접 수정하지 않고 공식 수집 API로 두 게시물을 재수집했다.
- 운영 검증:
  - 실행 `20260815_162822_78693`은 신규 2건, 자동등록 2건, 스킵 0건, 접근 실패 0건으로 종료됐다.
  - 공개 일정 API에서 `DJ 익두 | 네오스윙 일요 소셜`(`65bd114e-e852-47ff-83fe-11010b539528`)과 `DJ 제이 | 인더무드신림 일요 소셜`(`6b2cf698-c749-4474-a10e-2ce5a0e8051b`)을 2026-08-16 일정으로 확인했다. 두 일정 모두 공식 원문 링크, 주소, 장소 ID와 저장 이미지를 갖고 있다.
  - 기존 `regular-social:neo-sun:2026-08-16`과 `regular-social:dreambal-sun:2026-08-16`은 명시 일정으로 대체돼 공개 응답에서 제거됐다.
  - 수집 표준 검사, 대상 ESLint, Node 테스트 4개, 대상 Vitest 45개, 전체 Vitest 실제 테스트 407개, 프로덕션 빌드와 `git diff --check`가 통과했다. 전체 Vitest에서 `node:test` 형식 파일 2개는 발견 방식 차이로 별도 Node 러너에서 통과했다.
  - 운영 배포 버전은 `1786778803122`이며 공개 `version.json`과 서비스 재시작 후 `active` 상태를 확인했다.
- 관련 커밋: `fe2b77c6`
- 관련 파일: `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/benefit-search-utils.mjs`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `server/cafe24/function-api.js`, `scripts/test-ingestion-standards.mjs`

## 2026-08-15 일일 자동수집 2건의 캘린더 미반영

- 상태: 수집기 수정·대상 원문 재수집·운영 일정 복구·재발 검증 완료, Instagram 릴스 예약 게시 재개 및 당일분 게시 완료
- 현상: 2026-08-15 캘린더에서 스윙타임은 DJ가 없는 기본 정규 소셜로 남고, 휴무 공지가 나온 봉천살롱 정규 소셜도 계속 표시됐다.
- 실행 확인: 08·09·10·11시 `swing-daily` 우선순위 작업은 모두 종료 코드 0으로 완료됐고 남은 소스도 0건이었다. 08시와 09시 작업이 신규 후보를 각각 1건씩 저장해 당일 신규는 총 2건이었지만 네 작업의 자동등록 합계는 0건이었다.
- 원인:
  - 봉천살롱 8월 15일 휴무 후보는 `source_id=bongcheonsalon`, `activity_type=social_exception`으로 저장됐다. 정규 소셜 조정기는 휴무 후보와 반복 규칙의 `sourceId`가 정확히 같을 때만 일정을 억제하는데 봉천살롱 토요 규칙은 `sourceId=swingtown-cafe`다. 같은 장소를 가리키는 두 소스의 연결 정보가 없어 `regular-social:swingtown-sat:2026-08-15`가 제거되지 않았다.
  - 스윙프렌즈 카페 글은 제목에 `8월 15,16일`, 본문에 토요일 DJ 이정과 일요일 DJ 캐롤을 각각 안내하고 첨부도 같은 순서였다. 수집기가 첨부 이미지를 원문 순서가 아니라 면적순으로 다시 정렬해 15일 후보에는 캐롤 사진, 16일 후보에는 이정 포스터를 연결했고 독립 AI 검증이 두 후보를 모두 보류했다.
  - 이미지 순서를 바로잡은 뒤에도 운영 서버의 구 날짜 검사는 축약 목록의 마지막 날짜인 16일만 인식하고 첫 날짜인 15일은 인식하지 못했다.
- 해결:
  - Naver 다중 첨부는 작성자가 올린 DOM 순서를 보존한다. 축약 날짜 공지는 제목·요일·해당 본문 구간을 회차별 근거로 보존하고, 확정한 각 날짜를 `YYYY년 M월 D일` 형태로 함께 기록해 안전 검사가 목록의 첫 날짜도 확인할 수 있게 했다.
  - 봉천살롱 공식 Instagram의 휴무 후보는 원본 출처를 별도로 보존하면서 정규 소셜 예외용 `source_id`를 `swingtown-cafe`로 저장하도록 연결했다.
  - 스윙프렌즈 카페와 봉천살롱만 공식 수집 API로 재수집했다. 운영 DB를 직접 수정하지 않았다.
  - 사용자 지시에 따라 별도 기능인 Instagram 릴스 예약 게시 LaunchAgent를 일시적으로 `disabled`·`unloaded`로 전환했다. 일정 수집 LaunchAgent는 유지했으며, 휴무 반영을 확인한 뒤 후속 지시에 따라 릴스 예약 게시를 다시 활성화했다.
- 운영 검증:
  - 공개 일정 API에서 8월 15일 `DJ 이정 | 스윙프렌즈 카페 토요 소셜`과 8월 16일 `DJ 캐롤 | 스윙프렌즈 카페 일요 소셜`이 각 날짜의 올바른 저장 이미지로 등록된 것을 확인했다.
  - 봉천살롱 휴무 후보 `exception-backtest:bongcheonsalon:closure:2026-08-15:*`의 `source_id=swingtown-cafe`와 원본 출처 `exception_origin_source_id=bongcheonsalon`을 확인했다. 정규 소셜 조정 실행은 잘못 재생성된 15일 항목 1건을 제거했다.
  - 제거 뒤 운영 DB와 공개 API의 `regular-social:swingtown-sat:2026-08-15`가 모두 0건이다. 후속 건조 조정도 `creates=0`, `removes=0`이므로 새로고침이나 다음 조정에서 재생성할 대상이 아니다.
  - 예약 시각이 지난 8월 15일 릴스를 수동 재실행해 `published` 상태를 기록했고 Instagram 프로필 게시물 수가 `20 → 21`로 증가한 것을 확인했다. 선택 음악은 `Like It Is — Erroll Garner`이며 LaunchAgent는 이후 예약을 위해 `enabled` 상태다.
  - 수집 표준 검사, 수정 파일 문법 검사, AI 판정·서버 등록 링크 회귀 테스트 43개와 `git diff --check`가 통과했다.
- 배포 영향: 애플리케이션 배포·커밋·푸시는 수행하지 않았다. 로컬 수집기 수정과 공식 운영 API 재수집·조정만 수행했다.
- 관련 커밋: `pending`
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `server/cafe24/regular-social-reconciler.js`

## 2026-08-14 수집 행사 메인 광고 누락

- 상태: 운영 배포·로그인 관리자 화면 검증 완료 (2026-08-14 20:45 KST)
- 현상: 2026-08-17 `챔피언스컵`이 행사 `대회`로 정상 분류됐지만 홈 메인 광고에 표시되지 않았다.
- 운영 근거: 운영 DB에서 해당 일정은 `category=event`, `activity_type=event`, `genre=대회`, `dance_scope=swing`과 유효한 미래 시작일·이미지를 가지고 있었다. 비로그인 계산에서는 15개 중 8번째였지만, 실제 로그인 관리자 화면의 15개에는 없었다.
- 원인:
  - 자동수집 행사는 원문 출처 외에 공용 수집 `user_id`, `organizer=Swing Enjoy`, `organizer_name=관리자`를 함께 가진다. 관리자 응답에만 보이는 `user_id`와 `organizer_name`을 실제 작성자로 먼저 사용하면서 같은 경성홀의 앞 일정과 중복 처리됐다. 비로그인 응답에서는 이 관리 필드가 숨겨져 결과가 달라졌다.
  - 기존 광고 정렬은 가까운 강습·동호회 일정이 이번 달 행사보다 앞설 수 있었고, 자동 순환도 뒤쪽 순번부터 진행되는 문제가 있었다.
- 해결:
  - `Swing Enjoy`, `익명`, `관리자`, `admin` 등 수집 대체 작성자는 공용 사용자 ID보다 원문 출처·장소 조합을 우선해 로그인 여부와 무관한 중복 결과를 만든다. 경성홀 자체 게시물은 한 건으로 제한하면서 다른 출처인 챔피언스컵은 보존한다.
  - 최대 노출 수를 15개로 유지하고, 이번 달 `행사/대회`는 장르 설정과 무관하게 먼저 보장한다. 다음 주 일요일까지의 가까운 일정에 이어 일반 일정을 배치하고 동호회·팀원모집 일정은 뒤로 보낸다. 자동 순환은 1→2→3 순서로 진행한다.
  - 수집 분류에서는 명시적인 행사·대회·챔피언십 신호가 DJ 문구보다 우선한다. 서버도 행사 구조화 필드와 `social` 자동등록이 충돌하면 거절한다. 일일 자동수집 Telegram 요약에는 서버가 최종 저장한 날짜·제목·활동/분류 목록을 최대 15건까지 포함한다.
- 검증:
  - 메인 광고·NEB 회귀 테스트 17개, 자동등록 행사 분류·Telegram 보고 테스트 5개, 수집 표준 검사, 대상 ESLint(오류 0), Cafe24 프로덕션 빌드와 `git diff --check`가 통과했다.
  - 배포 버전 `1786707906677`의 로그인 관리자 Chrome 화면에서 15개를 직접 읽어 `광복의 리듬`, `경성홀 BEER PARTY`, `챔피언스컵`, `대전 피버 토 졸파`가 1~4번에 배치된 것을 확인했다. 3번 인디케이터 선택 시 `챔피언스컵 / 3 / 15`가 활성화됐고 자동 순환은 1번에서 2번으로 진행됐으며 브라우저 오류는 0건이었다.
  - 공개 버전·루트 HTTP 200, 서비스 `active`, 내부 헬스 `ok`를 확인했다. 운영 일정 데이터 자체는 수정하지 않았다.
- 관련 커밋: `71ee3bdb`, `fdc555bd`, `d1a150b2`, `ba5fa490`, `fc7bb9be`
- 관련 파일: `src/pages/v2/components/EventList.tsx`, `src/pages/v2/components/EventList/components/EventPreviewSection.tsx`, `src/pages/v2/components/EventList/utils/homeAdPriority.ts`, `src/pages/v2/components/EventList/utils/homeAdPriority.test.ts`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/auto-registration-report.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `server/cafe24/function-api.js`

## 2026-08-14 키오스크 503 고정 화면 재발

- 상태: 현장 복구·감시 적용·배포 경합 제거 및 운영 검증 완료 (2026-08-14 22:31 KST)
- 현상: 키오스크가 다시 다운된 것처럼 보였지만 Mini PC와 Chrome·디스플레이 서비스는 모두 실행 중이었다. 실제 Chrome 탭은 `https://swingenjoy.com/`에서 제목 `503 Service Unavailable`인 오류 화면에 고정돼 있었다.
- 원인:
  - 기존 배포는 새 `version.json`을 서비스 재시작보다 먼저 공개했다. 1분 간격으로 버전을 확인하는 실행 중 앱이 변경을 감지해 1.5초 뒤 새로고침했고, 단일 Node 프로세스의 약 3~4초 재시작 구간과 겹치면 Apache 503 문서로 이동했다. 앱 JavaScript가 사라진 뒤에는 서버가 정상화돼도 스스로 복구할 수 없었다.
  - 새 작업 트리의 체크아웃 시각 차이를 rsync가 서버 변경으로 계산해, 서버 내용이 같아도 Node를 불필요하게 재시작했다.
  - Chrome 프로세스는 계속 살아 있어 기존 systemd `Restart=always`가 페이지 수준 오류를 감지하지 못했다.
- 즉시 복구: 장애 로그와 탭 상태를 보존한 뒤 `kiosk-chrome.service`를 재시작했다. inactive이지만 enabled였던 legacy URL guard도 기본 운영 기준대로 disable했다.
- 재발 방지:
  - 1분 간격 watchdog이 Chrome DevTools의 실제 page target을 검사한다. 502·503·504·Chrome 네트워크 오류나 도메인 이탈을 감지하고 운영 `/kiosk`가 이미 HTTP 정상 상태일 때만 Chrome 서비스를 재시작한다. 운영 사이트가 장애 중이면 재시작하지 않는다.
  - 배포는 해시 자산을 먼저 전송하고 서버 재시작·헬스 통과 뒤 `index.html`, 서비스 워커, `version.json` 순서로 원자 교체한다. 클라이언트가 준비 완료 전에 새 버전을 감지하지 않도록 버전 표식을 마지막에 공개한다.
  - 서버 파일과 패키지 manifest는 체크섬으로 비교해 내용 변경이 있을 때만 Node를 재시작한다.
- 자원 측정: watchdog 1회 실행은 약 0.22초, 최대 RSS 약 22MB였고 60초 주기 평균 CPU는 한 코어의 약 0.3%다. 정상 상태에서는 Mini PC 내부 Chrome CDP만 조회하며 오류를 감지했을 때만 운영 HTTP 상태를 확인한다.
- 검증:
  - 실제 Node 재시작이 포함된 첫 배포에서는 약 4초간 서버 503이 있었지만 키오스크는 오류 문서로 이동하지 않았고, Chrome 시작 시각도 유지한 채 헬스 통과 뒤 새 자산을 로드했다.
  - 체크섬 보강 뒤 두 번째 배포는 서버 재시작을 건너뛰었으며 0.25초 간격 연속 요청 246회가 모두 HTTP 200이었다. 공개 버전은 `1786714076368`, 루트는 HTTP 200, 서비스는 active다.
  - 키오스크 탭은 `https://swingenjoy.com/`, 제목은 `댄스빌보드`이고 새 자산 `index-D6q_ZsJ7.js`를 로드했다. Chrome은 21:21:56 시작 상태를 유지했으며 watchdog timer는 enabled·active이고 배포 중과 이후 모두 healthy를 기록했다.
- 관련 커밋: `a933beba`, `83049c4d`, `8bd57565`
- 관련 결정: `docs/decisions/2026-08-14-kiosk-page-watchdog.md`, `docs/decisions/2026-08-14-version-last-deployment.md`
- 관련 파일: `scripts/deploy-cafe24.sh`, `src/lib/pwaReleaseContract.test.ts`, `ops/kiosk/mini-pc/restore-mini-pc-kiosk.sh`, `ops/kiosk/mini-pc/snapshot/home/kiosk-j/dot-local/bin/kiosk-page-watchdog.py`, `ops/kiosk/mini-pc/snapshot/home/kiosk-j/dot-config/systemd/user/kiosk-page-watchdog.service`, `ops/kiosk/mini-pc/snapshot/home/kiosk-j/dot-config/systemd/user/kiosk-page-watchdog.timer`

## 2026-08-14 주간 소셜 원문 존재 여부와 자동수집 누락 조사

- 상태: 로컬 수정·원문 건조 실행 검증 완료, 운영 미배포
- 현상: 2026-08-12~16 외부 소셜 목록에는 여러 일정이 있었지만 자동수집 결과와 차이가 났다. 외부 목록에 있다는 사실만으로는 공식 수집 경로에 원문이 있었는지, 원문은 있었지만 수집기가 놓쳤는지 구분할 수 없었다.
- 원문이 있었는데 놓친 대표 사례:
  - 스윙타임 8월 15일 `이정`, 16일 `캐롤`은 공식 원문에 있었지만 고해상도 크롭 이미지 제외, 여러 날짜의 DJ 병합, 축약 날짜 해석 실패가 겹쳤다.
  - 봉천살롱 8월 15일 TC 워크숍·파티는 공식 Instagram 게시물에 있었지만 현재 게시물 대신 추천 게시물 이미지를 섞어 읽어 정기권과 8월 29일 일정으로 오인했다.
  - 해피홀 8월 14일 `메이저`와 15일 `유광`은 스윙프렌즈 공식 카페 해피홀 게시판 원문에 함께 있었지만 메뉴 305가 수집 목록에 없었고, 수집기가 다중 첨부 중 한 장만 AI에 전달했다.
  - 스윙243 8월 15일 부산프렌즈 `시니컬`은 공식 카페 메뉴 284 원문에 있었지만 해당 메뉴가 수집 목록에 없었다.
  - Swing Kids 공식 Instagram 계정은 `swingkids_`인데 레지스트리에는 존재하지 않는 `swingkids_kr` 경로가 들어 있었다.
  - 스윙타운 7·8월 일정은 카페 메뉴 233의 일정 공지에 있었지만 기존 메뉴 264만 확인했다.
- 공식 경로에서 같은 내용을 확인하지 못했거나 충돌한 대표 사례:
  - 봉천살롱 서울발보아클럽 공식 공지는 7·8월 휴식 후 9월 재개라고 안내해 외부 목록의 8월 13일 항목과 충돌했다.
  - Swingpop 공식 8월 공지는 토요일 운영은 확인되지만 `감자` DJ 근거는 확인되지 않았다.
  - Swing Fever의 8월 14일 `에릭`·15일 휴무와 스윙243의 8월 16일 어반 일정은 현재 등록된 공식 경로에서 같은 원문을 찾지 못했다. 외부 목록만 보고 자동등록하지 않는다.
- 원인:
  - 같은 Naver 카페의 게시판별 메뉴를 별도 수집원으로 보존하지 않았고, 정규화된 글 URL만으로 수집원을 다시 찾아 먼저 등록된 다른 메뉴 ID로 덮어썼다.
  - Naver 글의 첨부 이미지를 한 장만 전달했고 Instagram은 현재 게시물과 추천 게시물의 대형 이미지를 구분하지 못했다.
  - `8/14 … /15일`, `8월 15,16일` 같은 축약 날짜, `DJ는 ♥메이저♥`의 조사, 영문 `HAPPY HALL` 장소 표기를 결정 규칙만으로 안정적으로 해석하지 못했다.
  - 일부 무해한 파서 이슈만 있어도 Instagram 체크포인트를 고정해 다음 게시물 탐색이 정체됐다.
  - 포스터 의미 해석을 고정 정규식에만 맡기면 레이아웃·표현이 바뀔 때 계속 예외 규칙이 늘어나는 구조였다.
- 해결:
  - 해피홀 메뉴 305, 부산프렌즈 메뉴 284, 스윙타운 일정 메뉴 233을 독립 수집원으로 추가하고 Swing Kids 공식 계정 경로를 바로잡았다. 후보가 명시한 수집원 ID를 같은 카페의 일반 URL 매칭보다 우선한다.
  - Naver 원문의 유효 첨부 이미지를 최대 3장 모두 보존하고 Instagram은 열린 게시물의 직접 미디어만 선택한다. 고해상도 크롭은 허용하며 저장·자동등록 실패가 아닌 파서 경고만으로 체크포인트를 막지 않는다.
  - 결정 규칙은 날짜 후보·수집원·중복·금지 조건을 담당하고, 첫 이미지 AI가 원문과 포스터에서 세션을 구조화한다. 날짜 후보를 일부만 반환하면 누락 날짜만 상위 모델로 다시 검사한다.
  - AI는 각 세션을 뒷받침하는 포스터 번호와 정확한 원문 인용을 반환한다. `HAPPY HALL` 같은 검증된 장소 별칭과 축약 날짜를 표준값으로 정규화하고, 세션별 포스터를 저장한다.
  - 별도 2차 AI가 같은 원문·원본 포스터와 수집 후보를 독립 대조한다. 결정 규칙, 1차 추출, 2차 검증, 서버의 0.98 신뢰도·정확 인용 검사를 모두 통과해야 자동등록한다. 일부 날짜 누락, AI 불가, 근거 불일치는 자동등록하지 않는다.
- 검증:
  - 수집 표준 검사, AI 추출·자동등록 서버 회귀 테스트 43개, 수정 JavaScript 문법 검사와 `git diff --check`가 통과했다.
  - 해피홀 공식 글 56130을 운영 저장 없이 건조 실행해 8월 14일 `메이저`, 15일 `유광` 두 후보와 각 날짜의 서로 다른 실제 포스터 URL, 표준 장소 `해피홀`, 접근 실패 0건을 확인했다.
  - 같은 8월 15일 후보와 AI가 선택한 유광 포스터를 독립 2차 검증기에 직접 넣어 `register`, 신뢰도 `0.99`, 검증 오류 0건을 확인했다.
  - 봉천살롱 건조 실행에서는 8월 15일 후보만 남고 추천 게시물의 정기권·8월 29일 오염이 사라졌으며, 새 해피홀·부산프렌즈 게시판과 스윙타운 일정 게시판의 원문 탐색도 확인했다.
- 운영 영향: 이 작업에서는 운영 API에 후보를 저장하거나 일정을 등록하지 않았고 배포·커밋·푸시도 하지 않았다.
- 관련 결정: `docs/decisions/2026-08-14-grounded-ai-social-ingestion.md`
- 관련 파일: `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/ai-candidate-adjudicator.mjs`, `scripts/ingestion/ai-social-extraction.schema.json`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/benefit-search-utils.mjs`, `scripts/ingestion/ingestion-progress.mjs`, `server/cafe24/function-api.js`

## 2026-08-13 자유게시판 일반 글의 비공개 선택 누락

- 상태: 운영 배포·보안 검증 완료 (2026-08-13 14:50 KST)
- 현상: 일반 자유게시판 글쓰기에는 비공개 선택란이 없었다. 건의 작성에서는 비공개가 기본 적용됐고, 수정 화면에서는 이미 숨겨진 글에만 선택란이 나타났으며 관리자는 상세 화면의 숨기기 기능만 사용할 수 있었다.
- 원인: 공용 편집기의 비공개 UI가 `건의` 작성 프리셋 또는 기존 `is_hidden=true` 글에만 렌더링되도록 제한돼 있었다. 또한 클라이언트는 비공개 행을 가렸지만 범용 조회 API가 비공개 글을 서버에서 강제 비식별화하지 않아 직접 조회에 대한 방어가 부족했다.
- 해결:
  - 자유게시판의 일반 신규 글과 수정 화면에 `비공개로 등록` 선택란을 표시하고 기본값은 공개로 유지한다. 건의 작성은 기존처럼 비공개가 기본값이다.
  - 선택란에 `작성자와 관리자만 볼 수 있습니다`를 명시한다. 관리자의 상세 화면 숨기기·해제 기능은 그대로 유지한다.
  - 비공개 글은 서버에서 작성자 또는 관리자에게만 전체 데이터를 반환한다. 그 외 사용자에게는 목록 순서 유지를 위한 식별자·날짜·머릿말·통계만 남기고 제목·본문·작성자·사용자 ID·이미지를 제거한다.
  - 요청 필터를 적용하기 전에 비공개 데이터를 비식별화해 제목·본문·작성자 값의 일치 여부도 외부에서 추측할 수 없게 한다.
- 검증:
  - 편집기·목록·상세·서버 비공개 정책 회귀 테스트 10개, 전체 Vitest 382개, 대상 ESLint, `git diff --check`, Cafe24 프로덕션 빌드가 통과했다.
  - 커밋 `57f6a439`를 원격 기능 브랜치에 푸시한 뒤 배포했다. 운영 빌드 시각은 `2026-08-13T05:48:38.823Z`이며 사이트·내부 헬스 체크·서비스 상태가 정상이다.
  - 운영 비로그인 API에서 비공개 글 3건을 조회해 제목·본문·작성자·사용자 ID·이미지가 모두 제거되는 것과 공개 글 제목은 유지되는 것을 확인했다. 운영 편집기 자산의 문구와 SHA-256도 로컬 배포 산출물과 일치했다.
- 관련 파일: `src/pages/board/components/UniversalPostEditor.tsx`, `src/pages/board/components/UniversalPostEditor.test.tsx`, `server/cafe24/board-post-security.js`, `server/cafe24/generic-data-api.js`

## 2026-08-13 키오스크 자유게시판 상세 일시 표시 실패

- 상태: 브라우저 새로고침으로 복구·운영 상태 확인 완료 (2026-08-13 13:38 KST)
- 현상: 미니 PC 키오스크에서 자유게시판 글 일부가 열리지 않고 한 글만 정상 표시됐으며, 브라우저 새로고침 뒤에는 같은 글들이 정상 표시됐다.
- 조사: 해당 키오스크의 13:36~13:38 KST 접근 기록에서 자유게시판 목록·상세 API, 댓글 모듈, 현재 배포 자산은 모두 HTTP 200으로 응답했다. 상세 조회 대상 글도 공개 상태였고 서버 조회 결과가 존재했으며, 같은 경로를 키오스크 해상도로 재검증했을 때 목록 10건과 상세 화면이 정상 표시되고 브라우저 오류가 발생하지 않았다.
- 판단: 서버·DB·배포 자산 장애는 확인되지 않았고 새로고침으로 즉시 복구돼, 키오스크 브라우저의 일시적인 화면 상태 문제로 판단했다. 재현되지 않은 상태에서 별도 재시도 로직을 중복 추가하지 않았으며 재발 시 화면 문구와 시각을 기준으로 클라이언트 진단 범위를 좁힌다.

## 2026-08-13 Instagram Reel 게시 성공의 검증 실패 오판

- 상태: 자동화 수정·당일 상태 복구·검증 완료 (2026-08-13 12:55 KST)
- 현상: 12:30 예약 실행이 Instagram Reel의 `Share`를 완료했고 실제 계정에도 새 릴스가 게시됐지만, 약 10분 뒤 Telegram에는 `Instagram 릴스 자동화 실패`와 게시 확인 실패가 표시됐다.
- 원인: 공유 후 검증기가 같은 프로필 딥링크를 반복해서 열었지만 이미 열린 Instagram 프로필 화면의 캐시된 `19posts` 값을 계속 읽었다. 프로필 화면을 직접 아래로 당겨 새로고침하자 즉시 `20posts`로 바뀌었고, 최신 릴스의 게시 시각과 `Take Five — Dave Brubeck` 오디오로 당일 게시 성공을 확인했다. `verification-required`는 중복 재게시를 차단하는 안전 상태인데 운영 알림이 모든 예외를 자동화 실패로 표시해 판정 의미도 부정확했다.
- 해결:
  - 프로필 게시물 수가 공유 전 기준값보다 늘지 않았으면 현재 에뮬레이터 화면 크기에 맞춘 pull-to-refresh를 수행하고 갱신된 접근성 값을 다시 읽는다.
  - 검증 재조회 간격을 늘리고 각 관측값을 로그에 남겨 캐시·업로드 지연을 구분할 수 있게 한다.
  - `sharing`·`verification-required` 알림은 `공유 완료 · 게시 확인 대기`로 표시하고, 공유 전 실제 오류만 `자동화 실패`로 표시한다.
  - 같은 날짜 복구에서는 게시물 수 증가가 확인될 때만 `published`로 전환하며 `Share`를 다시 누르지 않는 기존 중복 방지 원칙을 유지한다.
- 검증:
  - 소셜 릴스 Node 회귀 테스트 22개와 대상 ESLint, `git diff --check`가 통과했다. Android `wm size`의 실제 `1080x2400`을 기준으로 pull-to-refresh 좌표 `540,504 → 540,1500`을 생성하는 계약도 고정했다.
  - 같은 날짜 안전 복구에서 프로필 게시물 수 `19 → 20`을 확인해 `published / profile-post-count`로 전환했다. 최초 `shareCommittedAt`은 `12:33:33 KST`로 유지되고 복구 시각만 `12:55:45 KST`로 추가되어 Share 재실행이 없음을 확인했다.
  - 당일 음악 이력은 `Take Five — Dave Brubeck` 1건만 기록됐고, 게시기를 다시 실행하면 `already-published`로 즉시 종료돼 중복 게시와 이력 중복이 모두 차단된다.
- 관련 파일: `scripts/social-reels/instagram-reel-adb.mjs`, `scripts/social-reels/run-scheduled-social-reel.mjs`, `scripts/social-reels/instagram-reel-adb.test.mjs`, `docs/social-reel-automation.md`

## 2026-08-12 스윙타임 수요 소셜의 교차 출처 중복 자동등록

- 상태: 운영 배포·데이터 정리·검증 완료 (2026-08-12 12:09 KST)
- 현상: 2026-08-12(수) 캘린더의 스윙타임 소셜에 `DJ 뉴야 | 스윙타임 수요 소셜`과 `DJ 뉴야 | 스윙프렌즈 카페 수요 소셜` 두 건이 함께 노출됐다.
- 운영 근거:
  - 두 행은 날짜 `2026-08-12`, 장소 `스윙타임`, 활동 `social`, DJ `뉴야`가 같고 저장 포스터도 같은 사진의 서로 다른 크롭이다.
  - 첫 행 `88469596-4057-44f2-af77-f3885fd2109e`는 스윙타임 공식 Instagram 원문을 08:06 KST 자동등록했고, 두 번째 행 `aeb67f2f-7b87-47a2-84f4-b980427beb16`은 스윙프렌즈 Naver 카페 원문을 09:00 KST 자동등록했다. 두 실행 로그 모두 `auto-registered`로 기록됐으며 각각의 수집 후보에 별도 `registered_event_id`가 연결됐다.
  - 이벤트의 소유 사용자 ID는 자동등록 엔드포인트가 운영 관리자 소유자로 지정한 값이며 수동 입력 근거가 아니다.
- 원인: 운영 중복 검사는 같은 원본 URL 또는 같은 출처 식별자를 우선 사용한다. 출처가 Instagram과 Naver 카페로 달랐고, 출처명이 들어간 두 제목의 현재 유사도는 `0.40`으로 자동 중복 기준 `0.88`에 미달했다. 날짜·장소·활동·DJ의 일치 조합과 포스터 유사성은 현재 교차 출처 중복 키에 포함되지 않아 두 번째 후보가 통과했다.
- 해결:
  - 후보 원본 URL·출처·제목이 달라도 정확한 개최일·정규화 장소·소셜 활동·알려진 DJ 전체 명단이 모두 같으면 운영 일정 중복으로 판정한다. 반복 일정은 시작일~종료일 범위가 아니라 명시 개최일을 비교하며, DJ 미정·다른 DJ·다른 날짜·다른 활동 유형은 이 규칙으로 합치지 않는다.
  - 후보 저장 단계와 자동등록 직전 단계에서 같은 서버 판정을 적용해 먼저 저장된 후보나 요청 순서 차이로 최종 방어선을 우회하지 못하게 했다.
  - 중복 후보는 `duplicate` 상태와 권위 일정 근거를 보존하되 삭제될 일정의 `registered_event_id`와 등록 시각은 제거한다. 수집기는 저장 API가 이미 중복으로 건너뛴 후보를 자동등록 요청하지 않고, 등록 직전 충돌도 정상 스킵으로 기록한다.
  - 생성 정규 소셜의 `DJ 미정`은 확정 DJ 일치로 취급하지 않아 실제 원문 소셜이 기존처럼 생성 일정을 교체할 수 있다.
- 검증:
  - 서버 중복·정규 소셜 대상 테스트 28개, 전체 Vitest 361개, Node 테스트 24개, 수집 표준 검사, ESLint, Cafe24 정책 감사와 프로덕션 빌드가 통과했다.
  - 새 규칙을 운영 미래 소셜 199행에 읽기 전용 적용한 결과 중복 후보는 신고된 스윙타임 2행으로 구성된 1그룹뿐이었고, DJ 미정 정규 일정과 다른 일정은 포함되지 않았다.
  - 커밋 `e3f76dc3`을 배포했고 운영 빌드 시각은 `2026-08-12T03:08:41.018Z`다. 서비스 상태와 내부 헬스 체크가 정상이고 공개 홈·캘린더가 HTTP 200을 반환했으며, 핵심 서버 파일과 정리 스크립트의 로컬/운영 SHA-256이 일치했다.
  - 권위 Instagram 일정 1건은 유지하고 Naver 카페 중복 일정 1건과 그 알림함 카드 1건을 제거했다. 후보는 권위 일정 근거가 있는 `duplicate`로 전환해 잘못된 등록 ID를 제거했고, 이미 발송된 알림 큐 1건은 감사 이력으로 보존했다. 정리 전 데이터는 운영 서버의 전용 데이터 수정 백업 디렉터리에 권한 `600`으로 보관했다.
  - 정리 후 DB와 공개 API에서 권위 일정 `1`, 중복 일정 `0`, 중복 알림함 `0`, 보존 발송 큐 `1`을 재확인했고 정리 스크립트 재실행도 `alreadyApplied`로 안전하게 종료됐다.
- 관련 결정: `docs/decisions/2026-08-12-cross-source-social-deduplication.md`
- 관련 파일: `server/cafe24/function-api.js`, `server/cafe24/ingestor-registration-link.test.js`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/reconcile-2026-08-12-swingtime-duplicate.mjs`

## 2026-08-12 오늘 일정 알림의 캘린더 필터 오선택

- 상태: 운영 배포·검증 완료 (2026-08-12 08:45 KST)
- 현상: 운영체제의 오늘 일정 알림을 눌러 캘린더에 진입했을 때 하위 필터가 `전체`가 아니라 이전의 `소셜&행사` 상태로 표시될 수 있었다.
- 원인:
  - 오늘 일정 알림 URL은 날짜와 오늘 위치 이동만 지정하고 `category=all`을 명시하지 않았다.
  - 캘린더는 URL의 `category`를 최초 마운트에서만 읽어 같은 컴포넌트가 유지되는 재진입에서는 이전 탭 상태가 남을 수 있었다.
  - 이벤트 ID가 함께 있는 링크는 이벤트 유형에 따라 소셜·강습 탭을 다시 선택해 명시적인 전체 필터보다 우선할 수 있었다.
- 해결:
  - 운영 서버와 로컬 미리보기에서 만드는 오늘 일정 요약·개별 항목 URL에 `category=all`을 명시했다.
  - Push fragment, 과거 query 방식, 인앱 알림함의 오늘 일정 링크를 클릭할 때 같은 출처별 보정기를 거쳐 기존 `category=social`도 `all`로 덮어쓴다. 신규 등록 등 다른 알림의 명시 필터는 유지한다.
  - 캘린더가 이미 열린 상태에서도 새 URL 탐색의 명시 필터를 상태에 반영하고, `category=all`이 있으면 이벤트 상세 자동 탭 선택보다 우선하게 했다.
- 검증: 알림 출처·과거 URL·타 알림 보존·캘린더 재진입·알림 payload·서버 큐·기존 캘린더 레이아웃 회귀 테스트 103개가 통과했다. 날짜 고정값 때문에 자정 이후 실패하던 알림함 테스트는 실행 시점의 KST 날짜를 사용하도록 안정화했다. 최종 전체 Vitest 60개 파일·356개 테스트와 Node 24개 테스트, 대상 ESLint 오류 0건, Cafe24 단일 운영 감사 및 프런트·서버 함수 프로덕션 빌드가 통과했다. 운영 버전 `1786491959304`를 배포한 뒤 공개 루트·캘린더가 HTTP 200이고 앱·cron·httpd가 active이며 내부 헬스가 `ok: true`임을 확인했다. 공개 `index.html`과 service worker의 SHA-256도 로컬 산출물과 일치했다.
- 관련 파일: `src/lib/notificationLaunch.ts`, `src/pages/calendar/page.tsx`, `src/pages/calendar/utils/calendarTabFilter.ts`, `src/components/NotificationHistoryModal.tsx`, `server/cafe24/push-api.js`, `src/utils/dailyScheduleNotification.ts`
- 관련 커밋: `d013b0a7`

## 2026-08-12 방문자 급증 검증과 스윙씬 통계 신뢰성 재설계

- 상태: 운영 배포·검증 완료 (2026-08-12 08:45 KST)
- 현상: 2026-08-11 운영 통계가 고유 방문자 102명, 보정 세션 129개, 활동 로그 1,039개로 갑자기 증가했다. 같은 화면의 날짜 범위·세션 품질 수치가 실제 KST 일자 재계산 결과와도 일부 달랐다. 게시물 통계의 스윙씬 화면은 등록 이벤트를 사용했지만 월별 소셜을 중복 합산하고, 실제 조회 데이터가 없는 홍보 도달율을 0% 막대로 표시했으며, 최대 개최일의 상세 목록을 별도 조회로 다시 추정해 요약과 상세가 어긋날 수 있었다.
- 방문자 조사:
  - 8월 11일 KST 기준 순수 고유 방문자는 102명(로그인 3, 게스트 99)으로 직전 13일 평균 7.6명·중앙값 7명 대비 약 13.4배였다. 분석 요청은 모두 HTTP 200이었고, 107개 고유 IP와 104개 신규 IP가 확인돼 서버 오류나 단순 재전송만으로 만든 수치는 아니었다.
  - 75개 방문 UA가 Kakao 인앱 브라우저였고 트래픽은 14:35 KST 전후에 집중됐다. Instagram 직접 유입으로 보존된 식별자는 2개뿐이어서 게시물의 직접 효과라고 단정할 수 없으며, Kakao 공유·재분배 유입 가능성이 가장 큰 정황으로 남는다.
  - Android 인앱 브라우저가 앱을 먼저 실행한 뒤 Chrome으로 넘겨 인앱과 Chrome 저장소에 서로 다른 fingerprint·세션이 생겼다. 게스트 54명은 fingerprint가 2개였고 52명은 원시 세션이 2개였다. 기존 서버의 IP+기기+KST 브리지가 고유 방문자는 합쳤지만 원시 세션·활동 로그는 부풀 수 있었다.
  - 화면은 `+09:00` 경계값을 보냈지만 범용 데이터 API가 UTC ISO 문자열과 사전식으로 비교해 KST 하루가 아닌 UTC 날짜를 읽었다. KST로 다시 계산한 논리 세션은 117개, 포함 활동은 885개, 평균 활성 체류는 318초, 참여율은 93.2%였다.
- 방문자 조치:
  - 범용 필터는 양쪽이 절대 ISO 시각일 때 epoch로 비교하고 날짜 전용 값은 달력 문자열로 유지한다. 화면의 종료 경계도 KST `23:59:59.999`로 명시했다.
  - 순수성 필터와 동일한 서버 원본에서 30분 논리 세션, 체류시간 상한, 참여율, PWA·브라우저 세션을 계산해 핵심 수치의 권위 원본으로 반환한다.
  - Android 인앱→Chrome 인계 화면은 분석을 시작하지 않고, 기존 캠페인 값을 덮지 않는 `utm_source`와 `in_app_handoff`만 Chrome URL에 보존한다. 세션 sequence와 최초 referrer·UTM·landing은 `sessionStorage`에 저장해 새로고침 때 중복 sequence와 마지막 터치 덮어쓰기를 막는다.
- 스윙씬 조치:
  - 운영 `events`의 구조화된 지원 분류만 사용하고 숨김·삭제·초안·취소·비공개·판매·타 장르 범위를 제외한다. 비어 있지 않은 `event_dates`를 개최일의 권위 원본으로 사용하며 없을 때만 단일 시작일로 대체한다. 기간을 임의의 일별 이벤트로 확장하지 않는다.
  - 현재 월을 포함한 최근 12개 달력월의 실제 개최 회차와 고유 이벤트를 분리하고, 완전히 같은 제목·일자·시각·장소·유형만 중복 제거한다. 월별 소셜 이중 합산을 제거하고 최대 개최일의 요약·상세가 같은 서버 결과를 사용하게 했다.
  - 장르는 구조화 값과 인정된 레거시 토큰만 분류하며 제목으로 추측하지 않는다. 분류 가능 비율을 공개한다. 근거가 없던 조회 도달율은 삭제하고 등록일→첫 개최일의 실제 리드타임 분포·표본 수·중앙값으로 교체했다.
  - 화면에 운영 DB, 집계 버전, KST 기준일·기간, 고유 이벤트·개최 회차, 명시 개최일 사용 건수, 중복 제거 건수, 장르 분류율과 현재 월의 예정 회차 포함 정책을 표시한다.
- 운영 데이터 대조: 읽기 전용으로 운영 이벤트 1,034행을 같은 순수 집계기에 적용했다. 최근 12개월 신뢰 대상은 고유 이벤트 850개·개최 회차 1,148회이며 정확 중복 3회를 제거했다. 2026년 8월은 예정 포함 120회(103개 이벤트), 이후 예정은 73회였다. 유형은 강습 432회·행사 132회·소셜 584회이고 장르 분류 가능 회차는 462/1,148(40.2%)였다. 제외 181행은 기간 밖 159, 유효일 없음 8, 미지원 `swing-bar` 13, 판매 1이었다.
- 검증: 시간대 비교, 서버 세션 집계, 분석 순수성, Android 인계, 스윙씬 개최일·제외·중복·장르·리드타임 계약의 대상 테스트 18개가 통과했다. Cafe24 프런트·서버 함수 빌드, 대상 ESLint 오류 0건, 변경 공백 검사가 통과했다. 알림 진입 후속 작업까지 포함한 최종 전체 Vitest는 60개 파일·356개 테스트가 모두 통과했고 Node 테스트 24개와 Cafe24 단일 운영 감사도 통과했다. 첫 전체 실행에서 기존 키오스크 테스트의 teardown 후 timer 오류가 한 번 관찰됐지만 단독 재현과 전체 재실행은 모두 통과했다. 사전 운영 DB 대조는 읽기 전용으로 수행했다. 운영 버전 `1786491959304` 배포 후 로컬·운영의 `stats-api.js`, `scene-stats.js`, `index.html`, service worker SHA-256이 모두 일치했고, 공개 `version.json`과 운영 서버 버전도 일치했다.
- 관련 결정: `docs/decisions/2026-08-12-trusted-analytics-statistics.md`
- 관련 파일: `server/cafe24/generic-filter-comparison.js`, `server/cafe24/analytics-session-summary.js`, `server/cafe24/scene-stats.js`, `server/cafe24/generic-data-api.js`, `server/cafe24/stats-api.js`, `src/utils/analyticsEngine.ts`, `src/utils/analyticsGuards.ts`, `src/components/SiteAnalyticsModal.tsx`, `src/components/SideDrawer.tsx`, `src/pages/v2/components/SwingSceneStats.tsx`, `src/pages/v2/hooks/useSwingSceneStats.ts`
- 관련 커밋: `d013b0a7`

## 2026-08-11 사이트 전역 이미지 드래그 수정 회귀

- 상태: 구조 개선 및 Cafe24 운영 배포 완료
- 범위: 사이트 전체 정적·동적 이미지, 자유게시판 저장 HTML, 모바일 드래그 폴리필, 게시물·웹진 편집기, Cafe24 배포 기준선
- 증상: 자유게시판 본문 이미지에서 세로 스크롤 대신 반투명 이미지 드래그가 다시 발생했다.
- 원인:
  - 자유게시판 반응형 이미지 수정 `74925660`과 전역 이미지 드래그 정책 `c003b875`은 `origin/main`에 있었지만, 이후 전체 배포한 병렬 브랜치가 두 커밋을 포함하지 않아 운영 산출물에서 수정이 사라졌다.
  - 모바일 `mobile-drag-drop` 폴리필은 기본 `draggable=true`인 이미지뿐 아니라 이미지의 상위 링크도 드래그 대상으로 승격했다.
  - 현재 JSX 이미지 180개 중 129개가 개별 `draggable` 속성을 두지 않았고 저장 HTML·지도 오버레이 같은 동적 DOM도 있어 화면별 속성 추가로는 전역 정책을 보장할 수 없었다.
  - 기존 전역 정책에는 허용 표식은 있었지만 실제 이미지 이동 기능인 UniversalEditor와 WebzineEditor가 명시적으로 등록되지 않았다.
- 해결:
  - 최신 `origin/main`에서 통합 배포 브랜치를 만들고 병렬 브랜치의 고유 수집·메인 광고·Groove·릴스 변경을 옮겨 하나의 배포 기준선으로 합쳤다.
  - 앱 시작 시 기존 이미지와 MutationObserver로 추가·변경되는 이미지에 `draggable=false`를 강제하고 캡처 단계 `dragstart`를 차단한다.
  - 모바일 폴리필의 대상 탐색기를 교체해 비허용 이미지에서 시작한 터치가 상위 링크·카드 드래그로 승격되지 않게 했다.
  - 저장 HTML의 `data-image-drag`를 제거하고 모든 `draggable`을 `false`로 정규화한다.
  - 게시물 UniversalEditor와 관리자 WebzineEditor만 `data-image-drag="allow"`로 명시하고 실제 편집 이미지에 `draggable=true`를 설정했다.
  - 배포 스크립트가 최신 `origin/main` 포함, 깨끗한 작업 트리, upstream 푸시 완료를 확인하지 못하면 배포를 시작하지 않도록 했다.
- 검증:
  - 이미지 정책·저장 HTML·자유게시판 본문 회귀 테스트 12개 통과.
  - 전체 Vitest 54개 파일, 330개 테스트 통과.
  - 수집 기준 테스트 및 소셜 릴스 테스트 20개 통과. 외부 API 예제는 cURL·Node·Python·Java가 통과했고 로컬 Docker 비실행으로 PHP 컨테이너 항목만 실행하지 못했다.
  - 대상 ESLint 오류 0개(기존 경고만 유지), `git diff --check`, 배포 스크립트 문법 및 미커밋 변경 차단 검증, Cafe24 프로덕션 빌드 통과.
  - 운영 DB 연결 모바일 폭 로컬 화면에서 신고된 게시물의 본문 이미지가 `draggable=false`, 허용 표식 없음, 본문 폭 356px 안에 수납되고 가로 넘침이 없음을 확인했다. 같은 화면의 이미지 9개가 모두 전역 차단됐고 브라우저 오류 로그는 0건이었다.
  - Cafe24 배포 후 서비스와 cron은 `active`, 내부 헬스는 `ok: true`이며 로컬·서버·외부 버전이 `2026-08-11T05:06:38.837Z`로 일치했다.
  - 공개 `index.html`과 전역 정책·자유게시판 본문·UniversalEditor·WebzineEditor 번들의 SHA-256이 로컬 배포 산출물과 모두 일치했다.
- 관련 결정: `docs/decisions/2026-08-11-global-image-drag-policy.md`, `docs/decisions/2026-08-11-deployment-branch-integration-guard.md`
- 관련 파일: `src/utils/imageDragPolicy.ts`, `src/utils/sanitizeHtml.ts`, `src/main.tsx`, `src/components/UniversalEditor/Core/UniversalEditor.tsx`, `src/pages/admin/webzine/WebzineEditor.tsx`, `scripts/deploy-cafe24.sh`
- 관련 커밋: `c02cecdb`

## 2026-08-11 오늘 일정·신규 등록 혼합 및 알림 클릭 403

- 상태: 운영 배포·검증 완료 (2026-08-11 09:34 KST)
- 현상:
  - 08:30 오늘 일정 알림이 기존에 등록된 오늘 일정 전체를 포함했지만, 인앱 알림함에서 신규 등록과 구분되지 않아 신규 알림이 과거 일정까지 소급한 것처럼 보였다.
  - 신규 등록 경로를 켜기 전에 만들어진 큐도 처리 시점에 설정이 켜져 있으면 전달될 수 있었고, 설정을 끈 계정에도 인앱 신규 카드가 생성될 수 있었다.
  - Android에서 오늘 일정 Push를 누르면 Apache `Forbidden /calendar` 화면이 표시됐다.
- 운영 근거:
  - 문제 Push는 `daily_schedule_morning` 종류의 당일 요약 11건이었고 신규 등록 큐가 아니었다.
  - 08:42:53 KST 요청의 `notification_source_id`에 포함된 큐 UUID가 ModSecurity 규칙 981173에 걸려 HTTP 403이 발생했다. 서비스 워커는 일일 요약 데이터에 큐 ID도 있다는 이유로 신규 등록으로 오분류했다.
  - 인앱 신규 카드 대상은 정규 알림 설정 대상이 아니라 전체 사용자 조회 결과에서 만들어졌다.
- 해결:
  - `new_event_enabled_at` 경계를 계정 설정에 저장한다. 신규 등록은 큐 생성 시각이 경계 이후이고 경로·분류 설정까지 일치하는 계정에만 Push와 인앱 카드를 함께 만든다. 경계가 없는 상태는 전달하지 않는다.
  - 기존 미확인 신규 카드 중 설정 미동의·경계 이전·분류 불일치 항목은 삭제하지 않고 1회 읽음 정리한다.
  - 알림함을 `오늘 일정`, `신규 등록`, `기타 알림` 구역으로 나누고 종류별 카드 수와 의미를 표시한다. 읽음 처리가 다음 날 발송 설정을 바꾸지 않는다는 안내도 추가한다.
  - 서비스 워커가 일일 요약을 큐 ID보다 먼저 분류하고, 서버에는 `/`만 요청한 뒤 fragment에 목적지를 담아 앱 내부에서 이동한다. UUID가 Apache 쿼리에 전달되지 않는다.
- 검증:
  - 신규 등록 활성화 경계, 설정별 Push·인앱 동일 대상, 과거 카드 정리, 일일 요약 우선 분류, 안전한 클릭 URL, 모바일·데스크톱 공통 구역 렌더링 회귀 테스트를 추가했다.
  - 411×803 실측에서 가로 넘침 0, 오늘 일정 11건·신규 등록 1건의 구역과 숫자 분리, 목록 스크롤을 확인했다.
  - 공개·서버·로컬 버전은 `1786408352926`으로 일치하고 서비스워커 SHA-256은 `48adab7f4900b02f21425a143d11ccd2de5e4e2af0cf0f629f35d9c16b2af1e3`으로 일치한다. 루트·캘린더·내부 헬스는 HTTP 200이며 앱·cron·httpd는 active다.
  - 운영 DB에서 활성 신규 알림 경계 누락, 미확인 신규 알림, 비활성 설정 대상 및 경계 이전 미확인 항목은 모두 0건이다. 기존 잘못된 미확인 신규 카드 306건은 삭제하지 않고 읽음 처리했으며 당일 요약 8행은 보존했다.
  - 배포 후 09:33·09:34 KST 정규 일정·아침 요약·알림 큐 cron 호출이 모두 HTTP 200으로 완료됐다.
- 관련 결정: `docs/decisions/2026-08-11-notification-delivery-lifecycle.md`
- 관련 커밋: `c003b875`

## 2026-08-11 사이트 전역 이미지 드래그 기본 차단

- 상태: 운영 배포·검증 완료 (2026-08-11 09:34 KST)
- 현상: 자유게시판 상세에 이어 알림함 등 다른 스크롤 화면에서도 이미지 위로 손가락을 움직이면 반투명 이미지 드래그가 시작됐다.
- 원인: 모바일 네이티브 드래그 폴리필이 실행되는 반면 드래그 차단은 일부 컴포넌트와 CSS 클래스에만 적용됐다. 속성이 없는 일반 `<img>`는 브라우저 기본 드래그 대상이었다.
- 해결:
  - 모든 기존·동적 이미지의 `draggable`을 기본 `false`로 강제하고 캡처 단계 `dragstart`도 차단하는 전역 정책을 앱 진입점에 설치했다.
  - CSS도 클릭 이벤트는 유지한 채 이미지 선택·네이티브 드래그·길게 누르기 호출만 전역 차단한다.
  - 실제 이미지 드래그 기능이 생길 경우 `data-image-drag="allow"`와 `draggable="true"`를 함께 지정해야만 허용한다.
- 검증:
  - 기존 이미지, 동적 삽입 이미지, 드래그 이벤트 취소, 명시적 허용 예외를 검증하는 테스트를 추가했다.
  - 실제 411×803 알림함에서 이미지 위 스크롤로 목록 위치가 0에서 506px로 이동했고 이미지 12개 모두 `draggable=false`, 가로 넘침 0이었다.
  - 공개 메인 CSS·JS와 알림함 번들의 SHA-256이 로컬 배포 산출물과 일치하며, 전역 opt-in 정책과 오늘 일정·신규 등록 구분 문구가 공개 자산에 포함된 것을 확인했다.
- 관련 결정: `docs/decisions/2026-08-11-global-image-drag-policy.md`
- 관련 커밋: `c003b875`

## 2026-08-11 자유게시판 상세 첨부 이미지 드래그·모바일 넘침

- 상태: 운영 배포 완료 (2026-08-11 03:48 KST)
- 현상:
  - 자유게시판 글에 첨부한 이미지를 상세 화면에서 위아래로 스크롤하려 하면 이미지가 브라우저 드래그 대상으로 잡혔다.
  - 원본 크기가 큰 이미지는 모바일 본문 너비를 넘거나 정렬이 어색해질 수 있었다.
- 원인:
  - 저장된 본문 HTML을 세 상세 화면에서 각각 직접 렌더링했고, 대표 이미지와 달리 본문 이미지에는 읽기 전용 화면의 드래그 방지 계약이 없었다.
  - 공용 편집기 스타일은 이미지 크기만 일부 제한했으며, 과거 저장 HTML의 `draggable` 속성·인라인 너비와 모바일 터치 스크롤까지 정규화하지 않았다.
- 해결:
  - 세 상세 화면을 공용 `ReadOnlyBoardContent` 렌더러로 통합하고, 정제 단계에서 모든 본문 이미지와 감싸는 링크를 드래그 불가로 강제했다.
  - 이미지는 본문 너비 100% 이내에서 원본 비율을 유지하고 가운데 정렬하며, `contain` 배치와 세로 스크롤·핀치 줌을 허용한다.
  - 이미지·그림 영역에서만 기본 드래그를 차단해 일반 본문 텍스트 선택과 편집기의 의도된 이미지 이동은 유지했다.
- 검증:
  - 과거 인라인 1600px 이미지, 링크로 감싼 이미지, 이미지 드래그 차단, 일반 텍스트 드래그 허용을 포함한 컴포넌트 테스트 4개와 앱 테스트 1개가 통과했다.
  - 프로덕션 빌드와 대상 ESLint(오류 0건)가 통과했다.
  - 실제 브라우저 411×803에서 본문·이미지 너비가 모두 379px, 좌우 넘침과 중심 오차가 0px였고 이미지 위 스와이프로 스크롤 위치가 0에서 380px로 이동했다. 1280px 화면에서도 800px 본문 안에 비율 유지·가운데 정렬됐다.
  - 운영 버전은 `1786387579328`이다. 공개·서버 `version.json`, 새 읽기 전용 CSS·JS, service worker의 SHA-256이 로컬 배포 산출물과 일치했고 루트와 헬스체크는 HTTP 200이었다.
  - 운영 앱과 cron은 active이며 배포 직후 03:46 정규 일정·아침 digest·알림 전달 작업이 모두 HTTP 200으로 완료됐다.
- 관련 파일:
  - `src/pages/board/components/ReadOnlyBoardContent.tsx`
  - `src/pages/board/components/ReadOnlyBoardContent.css`
  - `src/pages/board/components/BoardDetailModal.tsx`
  - `src/pages/board/components/PostDetailModal.tsx`
  - `src/pages/board/detail/page.tsx`
  - `src/utils/sanitizeHtml.ts`
- 관련 커밋: `74925660`

## 2026-08-11 관리자 알림 설정 표시·아침 Push·알림함 카드 불일치

- 상태: 운영 배포 완료 (2026-08-11 03:20 KST)
- 현상:
  - 관리자가 저장한 알림 설정이 화면에서 풀린 것처럼 보였고 2026-08-04~10 매일 08:30 아침 Push가 한 번도 성공하지 않았다.
  - 오른쪽 위 종의 모바일 알림함은 여러 일정을 한 카드와 잘못된 숫자로 표시했고, 종을 여는 즉시 모든 신규등록 알림이 읽음 처리됐다.
- 운영 근거:
  - 계정의 정규 알림 설정은 활성·08:30·매일·전체 대상으로 저장돼 있었다. 같은 기간 아침 cron은 매번 정상 실행됐지만 대상 1명, Push 성공 0건이었다.
  - 최근 전달 실패는 HTTP 410 만료 응답이었다. `generic_records.record_id`는 160자인데 FCM endpoint는 188자여서 저장 키가 잘렸고, 삭제는 원래 188자 값으로 시도해 실제 0건을 지우면서 요청 행 수를 삭제 성공으로 기록했다.
  - 아침 Push payload에는 일정 `items`가 있었지만 서버 `user_notifications.data_json`에는 종류·날짜·합계만 저장돼 인앱 알림함이 한 개의 집계 카드로 렌더링했다.
- 원인:
  - 계정 설정의 ON/OFF를 현재 브라우저 endpoint 소유 여부로 대체했고, 브라우저에 남은 만료 endpoint를 그대로 재사용했다. 같은 OS를 같은 기기로 간주하는 정리도 여러 Android 기기를 오삭제할 위험이 있었다.
  - 아침 알림함 INSERT를 Push 발송의 중복 방지 값으로도 사용해, 인앱 저장 뒤 Push가 실패하면 같은 날 재시도할 수 없었다. 큐 상태도 인앱 저장만 성공하면 `sent`로 기록했다.
  - 신규등록 인앱 생성은 마지막 사이트 방문 시각으로 제외됐고, 종 클릭은 사용자의 카드 확인 전 전체 읽음을 수행했다.
- 해결:
  - endpoint SHA-256 기반 고정 길이 키와 기존 행 이관을 추가하고, 만료 삭제는 실제 DB 영향 행 수를 사용한다. UA/OS 기반 기기 삭제는 제거했다.
  - 계정 설정을 화면의 원본으로 사용하고 기기 연결을 별도 상태로 표시한다. 계정 설정이 켜져 있고 이 기기에서 명시적으로 끄지 않았다면 서버가 소유하지 않는 브라우저 구독을 자동 갱신한다.
  - 아침 알림을 사용자·날짜별 결정적 큐 항목으로 만들고 실제 Push 성공이 없으면 제한적으로 재시도한다. Push 성공만 `sent`, 인앱만 저장되면 `inbox_only`로 구분한다.
  - 인앱에는 요약의 전체 일정 항목을 저장하고 뱃지는 실제 표시 카드 수를 계산한다. 모바일과 데스크톱은 같은 서버 원본을 사용한다.
  - 종 클릭의 자동 전체 읽음을 제거하고 최근 신규등록의 과거 자동 읽음 상태를 1회 데이터 마이그레이션으로 복구한다.
- 검증:
  - endpoint 키·실삭제, 410 재시도, 아침 큐·전체 카드, 신규등록 명시적 읽음, 계정 ON·기기 만료 UI를 포함한 관련 테스트 98개가 통과했다. 대상 ESLint는 오류 0건이고 프로덕션 빌드가 성공했다.
  - 첫 배포 검증에서 운영 MariaDB 5.5가 `JSON_UNQUOTE`를 지원하지 않아 SQL 이관이 중단된 것을 확인했다. cron 파일을 즉시 복구한 뒤, JSON을 Node.js에서 파싱하고 한 트랜잭션으로 이관하는 호환 마이그레이션으로 교체했다.
  - 최종 운영 버전은 `1786386034008`이다. 공개·서버 `version.json`과 service worker SHA-256이 일치하고 서비스 및 cron이 active이다.
  - 운영 구독 3건은 모두 69자 해시 키로 이관됐고 레거시 키는 0건이다. 읽음 복구 마커는 1건이며 2026-08-03 이후 신규등록 알림 306건이 명시적 미확인 상태로 복구됐다. 08:30 활성 설정 1건도 유지됐다.
  - 배포 후 03:21 cron에서 정규 일정, 아침 digest 큐 등록, 알림 전달 큐가 모두 HTTP 200으로 완료됐다.
- 관련 결정: `docs/decisions/2026-08-11-notification-delivery-lifecycle.md`
- 관련 커밋: `e0fcf669`, `583942d3`

## 2026-08-11 메인 신규 이벤트 광고의 지난 일정 우선 노출

- 상태: 수정 완료, 배포 전
- 범위: 홈 `신규 이벤트` 광고 후보 선정, 첫 진입 순서, 8초 자동 순환
- 증상: 시작일이 지난 일정이 유효 일정과 같은 우선순위로 광고 전면에 노출됐고, 관리자 최대 개수 설정과 부족분 보충 기준도 최종 표시 목록에 일관되게 적용되지 않았다.
- 원인:
  - 유효 여부를 시작일이 아닌 `end_date`로 판단해 이미 시작한 장기 일정이 계속 주 후보에 남았다.
  - 최근 등록 후보가 부족하면 같은 종료일 기준 후보를 다시 합쳐 지난 일정이 전면 순서에 재진입했다.
  - 광고 목록 뒤쪽 카드를 8초마다 한 칸씩 동일 비율로 순환해, 뒤에 둔 보충 일정도 첫 자동 전환부터 전면으로 이동했다.
- 해결:
  - 단일 일정은 `start_date` 우선, 여러 날짜 일정은 `event_dates`의 오늘 이후 다음 회차를 기준으로 현재·미래 후보를 판정한다.
  - 오늘 일정 중 한 건을 페이지 진입 시드로 선두에 고정하고, 나머지는 최근 등록 시각과 가까운 미래 시작일 순으로 정렬한다.
  - 시작일이 지난 일정은 현재·미래 일정만으로 최종 광고 개수를 채우지 못할 때만 뒤쪽 보충 후보로 사용한다.
  - 지난 보충 일정은 배경 카드 스택에는 남기되 자동 전면 노출을 8회 중 1회로 제한하고, 수동 선택은 계속 허용한다.
  - 최종 작성자·장소 중복 제거 뒤에도 관리자 최대 노출 개수가 적용되도록 표시 단계에 설정값을 전달한다.
- 검증: 메인 광고 우선순위 및 저빈도 순환 테스트 6개, 기존 배너 번역 회귀 테스트 1개 통과. 대상 ESLint 오류 0개(기존 경고만 유지), Cafe24 프로덕션 번들 빌드 성공.
- 관련 파일: `src/pages/v2/components/EventList.tsx`, `src/pages/v2/components/EventList/components/EventPreviewSection.tsx`, `src/pages/v2/components/NewEventsBanner.tsx`, `src/pages/v2/components/EventList/utils/homeAdPriority.ts`
- 관련 커밋: pending

## 2026-08-05 알림함 시간 정보 노출

- 상태: 수정 완료, 배포 전
- 범위: 인앱 알림함, 오늘 일정 Web Push, 관리자 알림 미리보기
- 증상: 알림 카드 하단에 수신 시각이 표시되고, 오늘 일정 요약 본문과 미리보기 항목 데이터에 첫 일정의 시작 시각이 포함됐다.
- 원인: 알림 카드가 저장된 `received_at`을 무조건 시각으로 렌더링했고, 클라이언트와 서버의 일정 요약 생성기가 이벤트 `time`을 사용자용 본문과 데이터에 복사했다.
- 해결: 알림 카드의 수신 시각 렌더링을 제거하고 일정 요약 본문·항목 데이터·관리자 미리보기에서 시작 시각을 제거했다. 시간 값은 정렬·만료·전달·읽음 처리 같은 내부 처리에만 사용할 수 있다는 장기 운영 원칙을 `AGENTS.md`와 결정 기록에 남겼다.
- 검증: 일정 요약 클라이언트·서버 회귀 테스트를 포함한 알림 테스트 81개가 통과했고, 본문에 `19:30`이 없으며 미리보기 항목에 `time` 필드가 없는 것을 확인했다. 대상 ESLint는 오류 없이 통과했고 기존 `any` 경고 2건만 남았다. 프로덕션 빌드가 성공했다.
- 관련 파일: `AGENTS.md`, `src/components/NotificationHistoryModal.tsx`, `src/utils/dailyScheduleNotification.ts`, `server/cafe24/push-api.js`, `src/pages/admin/NotificationPreviewPage.tsx`, `docs/decisions/2026-08-05-notification-time-information-policy.md`
- 관련 커밋: pending

## 2026-08-05 모바일 월간 캘린더 일정 숨김

- 상태: 수정 완료, 배포 전
- 범위: 월간 캘린더 모바일·데스크톱 날짜 칸
- 증상: 모바일 날짜 칸은 일정을 최대 5개까지만 렌더링하고 나머지를 `+N 더보기` 버튼으로 접어 실제 일정 전체가 캘린더에 바로 보이지 않았다.
- 원인: 모바일 레이아웃이 `maxVisibleEvents=5`를 전달하고 날짜 셀이 소셜·일반 일정을 그 한도에 맞춰 잘랐다.
- 해결: 날짜 칸의 일정 개수 제한과 더보기 버튼·스타일을 제거했다. 기간 막대로 별도 표현되는 중복 항목을 제외한 날짜별 일정은 모두 셀 안에 직접 렌더링한다. 이 원칙을 `AGENTS.md`에 기록했다.
- 검증: `FullEventCalendar`에서 이벤트 개수 제한, 이벤트 `slice`, `calendar-day-more-button`, `더보기` 문구가 모두 제거된 것을 확인했다. 대상 ESLint는 오류 없이 통과했고 기존 경고만 남았으며 프로덕션 빌드가 성공했다.
- 관련 파일: `AGENTS.md`, `src/pages/calendar/components/FullEventCalendar.tsx`, `src/pages/calendar/styles/FullEventCalendar.css`
- 관련 커밋: pending

## 2026-08-03 연습실 탐색 화면 및 지도 로딩 개선

- 상태: 완료
- 범위: 홈 연습실 바로가기, 연습실 목록·검색·지역 필터·지도 전환, 외부 링크 이동
- 증상:
  - 연습실 검색 진입점이 화면에 상시 노출되지 않았고 지도 컴포넌트 호출 규격이 실제 구현과 맞지 않았다.
  - 연습실 카드를 누르면 예약 또는 지도 링크 대신 소개 상세 모달을 먼저 거쳤다.
  - 지도 SDK가 늦게 로드되거나 차단되면 빈 지도 영역만 남았다.
- 해결:
  - 홈의 기존 바로가기 두 개 아래에 연습실 버튼을 추가했다.
  - 운영 `venues` 데이터를 한 번 불러와 기본 리스트, 즉시 검색, 서울/기타 필터, 지도 보기에 함께 사용하도록 정리했다.
  - 카드와 지도 마커는 홈페이지·추가 링크·등록 지도 링크·주소 검색 순으로 목적지를 결정해 바로 이동한다.
  - 지도 SDK를 일정 시간 재확인하고 끝내 사용할 수 없으면 빈 화면 대신 재시도 안내를 표시한다.
  - 일반 카드 이미지와 지도 마커 이미지는 브라우저 기본 드래그 대상에서 제외했다.
- 검증:
  - Cafe24 운영 DB 연결 로컬 화면에서 활성 연습실 16개 노출 확인
  - `사당` 검색 결과 1개, 리스트 기본 선택, 지도 전환, 모바일 가로 넘침 없음 확인
  - 링크 해석 단위 테스트 4개 통과, 대상 파일 ESLint 통과, 프로덕션 빌드 통과
- 관련 파일:
  - `src/pages/practice/page.tsx`
  - `src/pages/practice/components/PracticeRoomList.tsx`
  - `src/pages/practice/components/VenueMapView.tsx`
  - `src/pages/practice/utils/venueLinks.ts`
  - `src/pages/v2/components/NewEventsBanner.tsx`
- 관련 커밋: pending

## 2026-07-26 소셜 캘린더 릴스 생성 자동화

- 상태: 해결
- 범위: swingenjoy 캘린더 캡처, 릴스용 15초 영상 생성, 동적 오버레이 배치
- 증상:
  - 모바일 편집 앱에서는 화살표 회전과 정밀 배치가 불안정했다.
  - 화면 잠금 또는 잠자기 상태에서 에뮬레이터 기반 자동화가 중단됐다.
  - 오늘 날짜의 요일 열이 매일 바뀌어 고정 좌표로는 글자와 화살표가 다른 요소를 가릴 수 있었다.
- 해결:
  - 캘린더의 `오늘` 버튼을 누른 뒤 파란 오늘 표시의 실제 DOM 좌표를 읽는다.
  - 오늘 위치에 따라 글자 상자를 항상 위쪽 좌·우 중 한쪽으로 자동 배치한다.
  - 화살표 방향과 각도를 오늘 좌표에 맞춰 다시 계산하고, 글자와 오늘 표시의 안전거리를 유지한다.
  - 고해상도 PNG 캡처에서 2160×3840, 15초, BT.709 H.264 영상을 직접 생성한다.
  - 좌·가운데·우 배치와 안전거리 테스트를 추가했다.
  - 운영 실행기에 중복 실행 잠금, 최대 3회 재시도, 4K 결과 검증과 상태 기록을 추가했다.
- 검증:
  - `npm run test:social-reel`: 7개 테스트 통과
  - 실제 사이트 DOM 좌표로 2026-07-26 영상 생성
  - 2160×3840, 30fps, 15초, yuv420p, BT.709 자동 검증 통과
  - 움직임 양끝 프레임에서 흰 상자·글자·요일·오늘 날짜 비가림 확인
- 관련 파일:
  - `scripts/social-reels/generate-social-reel.mjs`
  - `scripts/social-reels/run-social-reel.mjs`
  - `scripts/social-reels/layout.mjs`
  - `scripts/social-reels/layout.test.mjs`
  - `docs/social-reel-automation.md`
  - `docs/decisions/2026-07-26-social-reel-dynamic-layout.md`
- 관련 커밋: `7badf4d0`

## 2026-07-26 자유게시판 댓글 알림 누락 개선

- 상태: 완료
- 범위: 자유게시판 댓글 등록, 웹 푸시, 오른쪽 위 알림함
- 배경: 자유게시판 글에 댓글이 등록되어도 글 작성자가 알 수 없었고, 사이트 알림함에도 댓글 활동이 남지 않았다.
- 원인: 댓글은 `board_comments`에 직접 등록되지만 글 작성자를 대상으로 푸시를 만드는 후속 처리가 없었다.
- 해결:
  - 새 댓글 등록 성공 후 인증된 댓글 ID로 서버 알림 API를 호출한다.
  - 서버가 현재 로그인 사용자, 댓글 작성자, 원글 작성자를 다시 검증한다.
  - 본인 댓글은 제외하고 모든 로그인 원글 작성자의 서버 알림함에 댓글 알림을 기록한다.
  - 기존 웹 푸시 구독이 있는 작성자에게만 기기 푸시를 추가 발송한다.
  - 오른쪽 위 알림함은 서버 알림과 기기 푸시 기록을 합치고, 같은 댓글은 중복 표시하지 않는다.
- 검증: 댓글 작성자 대상 제한, 구독 없는 작성자의 알림함 저장, 푸시 구독 대상 제한, 본인 댓글 제외 테스트 통과 및 프로덕션 빌드 완료.
- 관련 파일:
  - `src/pages/board/components/CommentForm.tsx`
  - `server/cafe24/app.js`
  - `server/cafe24/push-api.js`
  - `server/cafe24/push-api.test.js`
  - `server/cafe24/migrations/2026-07-26-user-notifications.sql`
  - `src/lib/notificationStore.ts`
- 관련 커밋: pending

## 2026-07-14 기록 체계 도입

- 상태: 완료
- 범위: 프로젝트 기록 방식 정리
- 배경: 과거 이슈, 변천사, 문제점, 해결 방법이 남아 있는지 확인한 결과 Git 커밋, `CHANGELOG.md`, `docs/`, `ops/`에 기록이 흩어져 있었다. GitHub Issues와 PR 목록은 현재 비어 있다.
- 확인된 기존 기록:
  - Git 커밋 히스토리 466개가 기능 변화와 버그 수정 흐름을 보유한다.
  - `CHANGELOG.md`는 버전별 사용자 영향 변경을 보유한다.
  - `docs/INGESTION_STATUS.md`는 수집 자동화 장애, 원인, 조치, 검증까지 상세히 보유한다.
  - Cafe24 관련 문서는 운영 전환, 감사, 복구 절차를 보유한다.
  - `ops/kiosk/mini-pc/WORKLOG.md`는 키오스크 구축과 운영 조치 기록을 보유한다.
  - 코드/사이트/ESLint 조사 보고서는 특정 문제의 원인 분석과 해결책을 보유한다.
- 조치:
  - 이 통합 이슈 로그를 추가했다.
  - 장기 의사결정 기록을 위한 `docs/decisions/` 구조를 추가했다.
  - 향후 작업 기록 기준을 `AGENTS.md`에 추가했다.
- 해결 방법: 앞으로는 문제성 작업은 이 파일에 요약하고, 장기 정책과 아키텍처 결정은 `docs/decisions/`에 별도 기록한다.
- 검증: 문서 파일 생성 및 링크 경로를 로컬 파일 구조 기준으로 확인한다.
- 관련 파일:
  - `AGENTS.md`
  - `docs/ISSUE_LOG.md`
  - `docs/decisions/README.md`
  - `docs/decisions/2026-07-14-project-record-keeping.md`
- 관련 커밋: pending

## 2026-07-14 Cafe24 DB 백업 복구 가능성 점검

- 상태: 해결
- 범위: Cafe24 운영 DB와 업로드 파일 로컬 백업
- 증상: DB가 날아갔을 때 복구 가능한 백업 방법이 있는지 확인이 필요했다.
- 확인:
  - 백업 문서가 이미 존재한다: `docs/cafe24-local-backup.md`
  - 복구 문서가 이미 존재한다: `docs/cafe24-disaster-recovery.md`
  - 백업 스크립트가 존재한다: `scripts/backup-cafe24-to-local.sh`
  - 복구 스크립트가 존재한다: `scripts/restore-cafe24-from-local.sh`
  - macOS LaunchAgent가 설치되어 있고 매일 04:30 실행 설정이다: `com.rhythmjoy.cafe24-local-backup`
  - 로컬 백업 위치는 `~/RhythmjoyBackups/swingenjoy-cafe24/YYYYMMDD-HHMMSS/`다.
  - 2026-07-13 04:30 백업에는 `swingenjoy_app.sql.gz` DB 덤프가 있고 `gzip -t` 검증을 통과했다.
- 발견한 문제:
  - 백업 스크립트가 원격에서 `SHA256SUMS`를 만든 뒤 로컬에서 `manifest.txt`에 내용을 추가했다. 그 결과 최신 백업의 DB 덤프는 정상이지만 `manifest.txt` 체크섬 검증이 실패했다.
- 원인:
  - 체크섬 생성 시점이 최종 로컬 파일 작성보다 빨랐다.
- 조치:
  - 백업 스크립트 마지막 단계에서 로컬 최종 파일 상태 기준으로 `SHA256SUMS`를 다시 생성하도록 수정했다.
  - `sha256sum`이 없을 경우 macOS 기본 `shasum -a 256`으로 fallback하도록 했다.
  - 백업 정상성 점검 명령을 `docs/cafe24-local-backup.md`에 추가했다.
- 검증:
  - `bash -n scripts/backup-cafe24-to-local.sh`
  - `bash -n scripts/restore-cafe24-from-local.sh`
  - 최신 DB 덤프 `gzip -t` 통과
  - 최신 백업의 `SHA256SUMS`를 최종 파일 상태 기준으로 재생성한 뒤 `shasum -a 256 -c SHA256SUMS` 통과
  - LaunchAgent 설치 및 마지막 실행 exit code 0 확인
- 재발 방지:
  - 앞으로 백업 후 `gzip -t`와 `SHA256SUMS` 검증을 같이 본다.
  - 복구 테스트는 실제 운영 서버가 아니라 새 VPS 또는 임시 DB에서 먼저 수행한다.
- 관련 파일:
  - `docs/cafe24-local-backup.md`
  - `docs/cafe24-disaster-recovery.md`
  - `scripts/backup-cafe24-to-local.sh`
  - `scripts/restore-cafe24-from-local.sh`
- 관련 커밋: pending

## 2026-07-16 수집 후보 중복 재등록 방지 보강

- 상태: 해결
- 범위: Cafe24 `scraped_events` 수집 저장 API
- 증상: 같은 수집 후보가 반복 실행 때마다 다시 신규처럼 저장되거나, 제목이 `강습 안내`처럼 일반적인 후보가 운영 이벤트와 중복 매칭되지 않을 수 있었다.
- 원인:
  - 같은 후보 ID가 이미 존재해도 `collected`, `duplicate`, `excluded` 같은 terminal 상태가 아니면 다시 upsert했다.
  - 운영 이벤트 중복 비교가 같은 원본 URL/날짜 또는 높은 제목 유사도에 치우쳐 있어, 같은 출처/날짜/장소의 일반 제목 후보를 놓칠 수 있었다.
- 조치:
  - 같은 ID의 검토 대기 후보가 이미 있으면 다시 저장하지 않고 스킵하도록 했다.
  - 일반 제목 후보는 같은 출처 정체성, 날짜, 장소, 활동 유형이 같으면 운영 이벤트 중복으로 판단하도록 보강했다.
- 검증:
  - `node scripts/test-ingestion-standards.mjs`
  - `node --check server/cafe24/function-api.js`
  - `bash -n /Users/inteyeo/scripts/run-ingestion.sh`
  - `bash -n scripts/run-ingestion.sh`
  - `npx vitest run server/cafe24/generic-data-api.event-metadata.test.js server/cafe24/push-api.test.js`
- 재발 방지:
  - 제목이 일반적인 Instagram/카페 후보는 원본 URL만 보지 말고 출처 계정/카페, 날짜, 장소, 활동 유형을 함께 본다.
- 관련 파일:
  - `server/cafe24/function-api.js`
  - `docs/ISSUE_LOG.md`
- 관련 커밋: pending

## 2026-07-18 09시 priority2 수집 예산 초과

- 상태: 해결
- 범위: `swing-daily` 09:00 priority2 LaunchAgent 및 native 수집 wrapper
- 증상: 2026-07-18 09:00 실행이 native collector와 Telegram summary는 정상 출력했지만 20분 예산에 도달해 priority2 소스 14개를 확인하지 못했다.
- 원인:
  - priority2 자동수집 소스가 20개까지 늘어난 상태에서 Instagram 안전 대기와 소스당 2개 포스트 스캔이 같이 적용되어 09:00 실행 창을 초과했다.
- 조치:
  - 09:00 priority2 LaunchAgent에 `INGESTION_NATIVE_INSTAGRAM_POST_LIMIT=1`을 설정해 모든 priority2 계정을 얕게 순회하도록 조정했다.
  - 설치된 LaunchAgent를 같은 값으로 갱신하고 reload했다.
  - wrapper meta에 `native_instagram_post_limit`을 기록하도록 했다.
- 검증:
  - `bash -n /Users/inteyeo/scripts/run-ingestion.sh`
  - `bash -n scripts/run-ingestion.sh`
  - `plutil -lint scripts/com.rhythmjoy.codex-ingestion-priority2.plist /Users/inteyeo/Library/LaunchAgents/com.rhythmjoy.codex-ingestion-priority2.plist`
  - `node scripts/test-ingestion-standards.mjs`
  - `TELEGRAM_DRY_RUN=1 INGESTION_NATIVE_DRY_RUN=1 INGESTION_SKIP_CLEANUP=1 INGESTION_NATIVE_SOURCE_PRIORITY=2 INGESTION_NATIVE_SOURCE_IDS=gangnam_westies INGESTION_NATIVE_INSTAGRAM_POST_LIMIT=1 INGESTION_NATIVE_RUN_BUDGET_MS=300000 TIMEOUT_SECONDS=360 /bin/bash /Users/inteyeo/scripts/run-ingestion.sh`
- 재발 방지:
  - priority2 소스 수가 더 늘면 LaunchAgent 분할 또는 별도 source priority 재배치를 검토한다.
- 관련 파일:
  - `scripts/com.rhythmjoy.codex-ingestion-priority2.plist`
  - `/Users/inteyeo/Library/LaunchAgents/com.rhythmjoy.codex-ingestion-priority2.plist`
  - `/Users/inteyeo/scripts/run-ingestion.sh`
  - `docs/ISSUE_LOG.md`
- 관련 커밋: pending

## 2026-07-21 09시 priority2 네트워크 단절 조기 중단

- 상태: 해결
- 범위: `swing-daily` 09:00 priority2 native 수집
- 증상: 2026-07-21 09:03 실행이 native collector와 Telegram summary는 출력했지만 `ERR_INTERNET_DISCONNECTED`가 반복된 뒤 20분 예산에 도달해 14개 소스를 남겼다.
- 원인:
  - Playwright의 전체 네트워크 단절 오류를 일반 소스 접근불가와 동일하게 처리해, 네트워크가 복구되지 않는 상태에서도 다음 소스로 계속 이동했다.
- 조치:
  - native 수집기가 `ERR_INTERNET_DISCONNECTED`, 네트워크 변경, 프록시/터널 연결 실패를 네트워크 장애로 분류하도록 했다.
  - 해당 장애가 발생하면 현재 소스를 포함한 남은 소스를 summary에 기록하고 조기 종료하도록 했다.
- 검증:
  - `bash -n /Users/inteyeo/scripts/run-ingestion.sh`
  - `bash -n scripts/run-ingestion.sh`
  - `node scripts/test-ingestion-standards.mjs`
  - `TELEGRAM_DRY_RUN=1 INGESTION_NATIVE_DRY_RUN=1 INGESTION_SKIP_CLEANUP=1 INGESTION_NATIVE_SOURCE_IDS=swingfamily-lessons INGESTION_NATIVE_RUN_BUDGET_MS=120000 TIMEOUT_SECONDS=180 /bin/bash /Users/inteyeo/scripts/run-ingestion.sh`
- 재발 방지:
  - 전체 네트워크 장애는 소스별 실패가 아니라 실행 환경 장애로 보고 예산 소모 전에 다음 자동 실행으로 넘긴다.
- 관련 파일:
  - `scripts/ingestion/swing-daily-native.mjs`
  - `docs/ISSUE_LOG.md`
- 관련 커밋: pending

## 2026-07-24 아침 수집 CDP 상태 확인 장기 대기

- 상태: 해결
- 범위: `swing-daily` 08:00/09:00 native 수집 wrapper
- 증상: 두 실행 모두 native collector와 summary는 출력했지만, collector 시작 전에 각각 약 14분과 16분을 소진해 20분 수집 예산에 도달했고 대부분의 소스를 확인하지 못했다.
- 원인: 기존 Chrome의 CDP 포트가 연결은 수락하지만 `/json/version`에 응답하지 않는 상태에서 wrapper의 `curl` 상태 확인에 연결/전체 시간 제한이 없었다.
- 조치: CDP 상태 확인에 2초 연결 제한과 3초 전체 제한을 추가해 응답 없는 Chrome 때문에 collector 시작이 장시간 지연되지 않도록 했다.
- 검증:
  - `bash -n /Users/inteyeo/scripts/run-ingestion.sh`
  - `bash -n scripts/run-ingestion.sh`
  - `node scripts/test-ingestion-standards.mjs`
  - `TELEGRAM_DRY_RUN=1 INGESTION_NATIVE_DRY_RUN=1` 단일 소스 단축 실행
- 재발 방지: 외부 프로세스 상태를 확인하는 wrapper 네트워크 호출은 항상 명시적인 짧은 시간 제한을 둔다.
- 관련 파일:
  - `/Users/inteyeo/scripts/run-ingestion.sh`
  - `docs/ISSUE_LOG.md`
- 관련 커밋: pending

## 2026-07-25 native 소스 timeout 후 페이지 정리 장기 대기

- 상태: 해결
- 범위: `swing-daily` 08:00/09:00 native collector
- 증상: 두 실행 모두 native collector와 summary는 출력했지만, timeout 처리된 소스의 페이지 정리에서 장시간 대기해 20분 예산에 도달했고 각각 18개와 17개 소스를 확인하지 못했다.
- 원인: `withBoundedStep()`의 `Promise.race`가 timeout을 반환한 뒤에도 Playwright 작업은 남아 있었고, 소스별 `page.close()`와 최종 브라우저 연결 종료가 그 미완료 작업을 제한 없이 기다렸다.
- 조치: timeout 뒤의 페이지 및 브라우저 연결 정리를 각각 최대 2초만 기다리도록 제한했다.
- 검증:
  - `bash -n /Users/inteyeo/scripts/run-ingestion.sh`
  - `bash -n scripts/run-ingestion.sh`
  - `node scripts/test-ingestion-standards.mjs`
  - `TELEGRAM_DRY_RUN=1 INGESTION_NATIVE_DRY_RUN=1` 단일 소스 단축 실행
- 재발 방지: 수집 작업의 timeout뿐 아니라 timeout 이후 Playwright 자원 정리에도 별도 상한을 둔다.
- 관련 파일:
  - `scripts/ingestion/swing-daily-native.mjs`
  - `docs/ISSUE_LOG.md`
- 관련 커밋: pending

## 2026-07-26 외부 일정 API 배포 후 이미지 런타임 의존성 누락

- 상태: 해결
- 범위: Cafe24 Swing Enjoy 서버와 외부 일정 이미지 업로드 API
- 증상: 서버 파일 배포 후 `sharp` 모듈을 찾지 못해 `swingenjoy.service`가 기동하지 못했다. 최신 버전을 운영 설치한 뒤에는 구형 CentOS `libstdc++`와 바이너리 호환 오류가 발생했다.
- 원인:
  - 런타임에서 사용하는 `sharp`가 `devDependencies`에 들어 있었다.
  - 배포 스크립트가 `package.json`과 잠금 파일을 전송한 뒤 운영 의존성을 설치하지 않았다.
  - 최신 `sharp` 바이너리가 운영 서버에서 제공하는 `GLIBCXX`보다 새 버전을 요구했다.
- 조치:
  - `sharp`를 운영 의존성으로 이동하고 최초 복구 시 Cafe24 OS 호환 버전 `0.32.6`을 적용했다.
  - 후속 보안 감사에서 구버전 libvips 취약점이 확인됐다. 운영 CentOS 7/glibc 2.17에서 수정 바이너리 `0.35.3`이 기동되지 않아, 공급자 공식 우회책대로 취약 GIF·TIFF·VIPS 디코더를 프로세스 시작 시 차단했다. API 허용 형식에도 이 세 형식은 포함하지 않는다.
  - 패키지 파일이 변경된 배포에서는 `npm install --omit=dev`를 자동 실행하도록 배포 스크립트를 보강했다.
  - CRS가 포함되기 전에 로드되는 설정에서 이미지 업로드 경로의 Content-Type 정책 규칙만 제외하고, 나머지 ModSecurity 검사와 애플리케이션의 키 인증·용량·실제 디코딩 검사를 유지했다.
  - 운영 DB 백업 후 외부 API용 새 테이블만 생성했으며 기존 `events` 레코드는 변경하지 않았다.
- 검증:
  - 운영 서버에서 `sharp` 런타임 로딩 확인
  - `swingenjoy.service` active 및 `/__health` 응답 확인
  - 외부 일정 API 테스트 통과
- 관련 파일:
  - `package.json`
  - `package-lock.json`
  - `scripts/deploy-cafe24.sh`
  - `deploy/cafe24/apache/swingenjoy-modsecurity-exceptions.conf`
  - `server/cafe24/external-events-api.js`
- 관련 커밋: pending

## 2026-07-26 외부 일정 API 양방향 동기화·이미지 보안 보강

- 상태: 해결
- 범위: 파트너 일정 API, 이미지 저장, 혜택 이벤트 수집 판별
- 증상: 외부 URL 이미지는 원격 서버에 의존했고 4종 이미지가 생성되지 않았으며, 파트너 원본 수정·삭제 동기화와 엄격한 혜택 판별이 없었다.
- 조치:
  - 모든 이미지 방식을 실제 디코딩 후 WebP 4종으로 자체 저장하도록 통일했다.
  - DNS·사설 IP·리디렉션·응답 크기·Content-Type 검증과 공인 IP 고정 연결을 적용했다.
  - `partner_id + external_id` 소유권에 한정한 수정·삭제 API를 추가했다.
  - 관리자 세션 전용 파트너 목록·활성 상태·요청 로그 API를 추가했다.
  - 명시적인 무료 금액/무료 행사/정기권 판매만 혜택 대상으로 표시하고 부정 표현은 제외했다.
  - 운영 OS 호환 이미지 디코더에서 공급자 권고 취약 디코더 3종을 명시적으로 차단하고, SSRF 차단에 IPv4-mapped IPv6, 예약·문서·멀티캐스트 대역을 포함했다.
  - 운영 E2E에서 발견한 Undici의 `lookup({ all: true })` 계약 불일치를 수정해, DNS 검증을 통과한 공인 IP 목록만 배열 형식으로 연결 계층에 전달한다.
  - ModSecurity의 PUT/DELETE 정책 예외를 전체 서버가 아닌 `/api/external/v1/events/{external_id}` 경로에만 적용하고 애플리케이션의 키·소유권 검사를 유지한다.
- 검증:
  - 전체 단위 테스트 60개 및 외부 API 테스트 18개 통과
  - 수집 기준 테스트, TypeScript, ESLint, OpenAPI YAML 파싱 및 프로덕션 빌드 통과
- 관련 파일:
  - `server/cafe24/external-events-api.js`
  - `server/cafe24/app.js`
  - `scripts/ingestion/candidate-utils.mjs`
  - `docs/external-event-api.md`
  - `docs/external-event-api.openapi.yaml`
- 관련 커밋: pending

## 2026-07-26 외부 일정 API 파트너 문서와 주소 확인 보강

- 상태: 해결
- 범위: 파트너 전달 문서, API 인증 설명, 이미지 없는 소셜 주소 입력
- 증상: 파트너용 문서에 관리자 명령과 내부 광고 설명이 섞여 있었고, 주소 형식 설명만으로는 카카오맵에서 실제 검색되는 주소를 보장할 수 없었다.
- 조치:
  - 파트너 문서를 존댓말 안내서로 다시 작성하고 관리자 운영 절차를 별도 문서로 분리했다.
  - 최상위 분류 `category`와 하위 분류 `genre`를 조합별 표와 사용 사례로 설명했다.
  - 서버용 API Key 인증과 웹 로그인 세션을 사용하지 않는 이유·요청 흐름·소유권 범위를 명시했다.
  - 파트너 인증이 필요한 카카오 주소 확인 API를 추가하고, 이미지 없는 소셜 등록·수정 시 서버가 주소를 다시 확인하도록 했다.
- 검증:
  - 외부 API 단위 테스트, OpenAPI 파싱, TypeScript, ESLint, 프로덕션 빌드
  - 운영 배포 후 실제 API Key를 사용한 주소 검색·등록 거절 및 인증 차단 E2E
- 관련 파일:
  - `docs/external-event-api.md`
  - `docs/external-event-api-operations.md`
  - `docs/external-event-api.openapi.yaml`
  - `server/cafe24/external-events-api.js`
  - `server/cafe24/app.js`
- 관련 커밋: pending

## 2026-07-26 외부 API Key 회원 귀속과 관리자 통제 화면

- 상태: 해결
- 범위: 외부 일정 API 인증 운영, 관리자 메뉴, 감사 추적
- 증상: 파트너 키를 어느 계정에 귀속하고 두 관리자 계정이 어떻게 발급·통제할지 관리 화면과 기록 체계가 없었다.
- 조치:
  - 신규 파트너 키를 기존 회원 계정에 필수 연결하고 API 일정의 내부 소유자로 사용했다.
  - 햄버거 메뉴의 `관리자 콘솔 → 회원 & 보안`에 외부 API 파트너 관리 화면을 추가했다.
  - 발급·중지·재활성화·재발급·회원 연결·분류·호출 한도 설정과 요청 로그 조회를 제공했다.
  - 관리자 세션과 동일 출처를 함께 검사하고 모든 관리자 변경을 별도 감사 로그에 기록했다.
  - 키 원문은 발급·재발급 때 한 번만 반환하고 DB에는 해시만 저장하며, 재발급 시 이전 키를 즉시 무효화했다.
  - 운영 MariaDB 호환성을 위해 감사 세부정보는 JSON 문자열을 담는 `LONGTEXT`로 저장하도록 수정했다.
- 검증:
  - 두 운영 관리자 계정의 권한 확인
  - 비로그인·비관리자·교차 출처 차단, 키 발급·재발급·중지·요청 로그·감사 로그 API 테스트
  - 외부 API 단위 테스트, TypeScript, ESLint, 프로덕션 빌드 및 운영 배포 후 E2E
- 관련 파일:
  - `src/components/ExternalApiPartnerManagementModal.tsx`
  - `server/cafe24/external-events-api.js`
  - `server/cafe24/migrations/2026-07-26-external-api-admin-audit.sql`
  - `docs/external-event-api-operations.md`
- 관련 커밋: pending

## 2026-07-26 외부 API 승인·테스트 운영 절차 및 공개 안내 페이지

- 상태: 해결
- 범위: 파트너 신청, 관리자 승인, 장르 권한, 테스트 격리, 공개 매뉴얼
- 증상: 관리자가 키를 바로 발급하는 구조라 신청 승인 절차가 없었고, 단일 기본 장르와 실운영 직접 등록 방식은 파트너 개발 테스트와 복수 장르 권한을 안전하게 처리하기 어려웠다.
- 조치:
  - 로그인한 파트너가 공개 안내 페이지에서 연동을 신청하고 관리자가 승인 설정을 불러오는 흐름을 추가했다.
  - 최상위 분류는 반드시 1개만 지정하고, 그 안의 하위 장르만 복수 선택하도록 사이트 분류 구조와 통일했다. 하위 장르 미선택 시에는 지정한 최상위 분류의 하위 장르 전체만 허용한다.
  - 신규 키를 테스트 모드로 발급해 실제 일정은 변경하지 않고 검증 결과와 로그만 남기며, 검토 후 운영 모드로 전환하도록 했다.
  - 관리자 화면 탭을 `승인·파트너`, `기록`으로 단순화하고 공개 페이지 공유 버튼과 공유 메타데이터를 추가했다.
  - 문서의 기준 경로(`/`) 때문에 단순 해시 링크가 메인으로 이동하던 문제를 문서 절대 경로 해시 링크로 수정했다.
  - 파트너 Key가 하나의 최상위 분류만 사용하고, 일정 요청 한 건은 그 안의 하위 장르 하나를 보내는 규칙을 문서에 명시했다.
  - 이미지 없는 소셜은 등록 시 카카오맵 표준 주소로 자동 변환하고, 안내 페이지에서 저장 결과를 미리 확인하는 로그인 사용자용 주소 변환기를 제공했다.
  - 이미지 전달 방식을 선택하는 이유와 `upload` 2단계 요청, `url` 자동 다운로드, 실제 파일 검사, WebP 4종 생성, 내부 주소 연결 및 실패 시 일정 미등록 절차를 공개 문서에 명시했다.
  - 파트너 계정과 `external_id`를 일정·이미지 CRUD의 연결 고리로 명시하고, 수정 시 이전 이미지 교체·정리와 삭제 시 연결 이미지 정리를 구현했다.
  - 주소 확인 API와 일정 전체 수정·삭제를 파트너 서버에서 바로 구현할 수 있도록 JavaScript 예제를 공개 안내 페이지와 문서에 추가했다.
  - 정상적인 개발 테스트에 한해 연락처로 파트너명과 Key 앞부분을 보내 한도 증액 또는 사용량 초기화를 요청하는 절차를 명시했다.
  - 공유 API 안내 링크에 비로그인으로 진입하면 로그인 모달을 자동으로 열고, 로그인 후 신청 폼에는 연결될 본인 계정을 수정 불가 값으로 표시했다.
  - 외부 API 이미지 원본 수신 한도를 32MB로 확대하되 4천만 픽셀 제한과 실제 이미지 검사를 유지하고, 변환 후 원본은 저장하지 않도록 명시했다.
  - 큰 원본을 4종으로 동시에 변환할 때의 순간 메모리 부하를 줄이기 위해 사전 디코딩 검사 후 순차 변환하고, 각 WebP 결과를 재검사하도록 보강했다.
  - 카카오 주소 검색 첫 결과를 자동 채택하면 다른 장소가 저장될 수 있어, 파트너용 주소 API가 후보 목록을 반환하고 사용자가 선택한 표준 주소와 등록값이 정확히 일치할 때만 저장하도록 변경했다.
  - 테스트 한도 증액의 관리자 병목을 없애되 도배 우회를 막기 위해, 로그인 계정 소유의 활성 테스트 파트너에 한해 분당 60회·일 3,000회까지 24시간 1회 자동 상향하도록 구현했다. 운영 키는 자동 상향 대상에서 제외했다.
  - 운영 Apache 보안 모듈이 한글 주소 URL 쿼리를 애플리케이션 도달 전에 403으로 차단함을 운영 요청으로 재현했다. 주소 검색 입력을 URL이 아닌 인증된 POST JSON 본문으로 전달하도록 파트너 API와 체험 도구를 함께 변경했다.
  - 운영 브라우저 클릭 검증에서 문서 하단의 상대 해시 링크가 전역 `base href="/"` 때문에 홈페이지로 이동하는 문제를 발견했다. 안내 페이지 절대 경로가 포함된 해시 링크로 수정했다.
  - 운영 DB를 연결한 로컬 Playwright 검증에서 Vite 화면 포트와 API 프록시 포트 차이를 동일 출처 위반으로 오인했다. production의 정확한 host 일치 규칙은 유지하고, 비운영 loopback 호스트끼리만 포트 차이를 허용했다.
  - 로컬 운영 DB 테스트 서버가 카카오 REST 설정을 전달하지 않아 주소 UI를 운영 조건으로 검증할 수 없었다. 테스트 실행 중에만 운영 서버 환경에서 해당 키를 전달하도록 시작 스크립트를 보완했으며 값은 출력하거나 파일에 저장하지 않는다.
- 검증:
  - TypeScript, ESLint, 서버 문법, 외부 API 단위 테스트 23개, 프로덕션 빌드
  - 데스크톱·모바일 헤드리스 렌더링 및 가로 넘침·런타임 오류 검사
- 관련 파일:
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`
  - `src/components/ExternalApiPartnerManagementModal.tsx`
  - `server/cafe24/external-events-api.js`
  - `server/cafe24/migrations/2026-07-26-external-api-permissions-and-environment.sql`
- 관련 커밋: pending

## 2026-07-26 외부 일정 주소 입력 목적 및 비용 구조 정정

- 상태: 해결
- 범위: 외부 일정 API 주소 필드, 파트너 안내 페이지
- 증상: 주소를 이미지 없는 소셜의 필수값으로 한정하고 Dance Billboard가 카카오 주소 검색을 대행해, 실제 목적보다 제한이 강하고 외부 호출 비용 가능성이 생겼다.
- 원인: 주소의 목적을 모든 일정의 상세 장소·지도 연동 정확성 안내가 아니라 특정 화면의 대체 콘텐츠로 잘못 해석했다.
- 조치:
  - 주소를 이미지·분류와 무관한 선택값으로 변경했다.
  - Dance Billboard의 카카오 주소 대행 API와 등록 시 재검색을 제거했다.
  - 부정확한 주소는 상세 지도 검색의 첫 결과가 사용되어 다른 위치가 표시될 수 있음을 안내 페이지와 계약 문서에 명시했다.
  - 파트너가 원하면 다음 우편번호, 카카오맵, 네이버지도 또는 공공 도로명주소 검색으로 미리 확인할 수 있게 예시를 제공하되 확인 수단 입력은 강제하지 않았다.
  - Google Maps의 `formatted_address`, 영문 주소, Plus Code와 장소명은 카카오 주소검색 호환이 보장되지 않음을 확인하고, Google을 쓰는 파트너도 대한민국 도로명주소로 정리해 보내도록 안내했다.
- 검증: 외부 API 단위 테스트, TypeScript, ESLint, 프로덕션 빌드, 배포 후 브라우저 실사용 확인
- 관련 파일:
  - `server/cafe24/external-events-api.js`
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`
  - `docs/external-event-api.md`
  - `docs/external-event-api.openapi.yaml`

## 2026-07-26 외부 일정 요청 예시값과 CRUD 연결 설명 보강

- 상태: 해결
- 범위: 외부 일정 API 공개 안내 페이지와 전달용 문서
- 증상: 등록 코드 예시에서 어떤 값이 자유 입력이고 어떤 값이 고정 형식인지 바로 알기 어려웠으며, 수정·삭제가 등록값과 어떻게 연결되는지 설명이 분산돼 있었다.
- 조치:
  - 요청 주소·메서드·헤더·필드명은 유지하고 예시 필드값은 실제 일정에 맞게 바꾼다는 원칙을 등록 예시 바로 아래에 추가했다.
  - 자유 문자열, 날짜 형식, 허용 분류, HTTPS URL, 이미지 방식별 변경 규칙을 표로 정리했다.
  - 등록·수정·삭제는 동일 API Key와 동일 `external_id`로 연결되고, 수정은 전체 본문 재전송, 삭제는 본문 없이 식별자만 사용한다는 공통 규칙을 명시했다.
- 검증: 외부 API 단위 테스트, TypeScript, 대상 ESLint, 프로덕션 빌드, 운영 브라우저 확인
- 관련 파일:
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`
  - `docs/external-event-api.md`

## 2026-07-26 외부 API 신청 담당자 연락처 필수화

- 상태: 해결
- 범위: 외부 API 연동 신청 폼과 서버 검증
- 증상: 기술 담당자 연락처가 이메일 또는 전화번호 한 칸으로 되어 있어 긴급 연락과 기술 안내에 필요한 두 연락수단이 모두 확보되지 않았다.
- 조치:
  - 기술 담당자 이메일과 전화번호를 별도 필수 입력으로 변경하고 서버에서도 형식을 검증한다.
  - 로그인 계정에 유효한 이메일이 있으면 해당 이메일 사용 체크박스를 기본 선택하고 읽기 전용으로 자동 입력한다.
  - 소셜 로그인에서 이메일이 제공되지 않거나 형식이 유효하지 않으면 체크박스를 비활성화하고 직접 입력하게 한다.
  - 전화번호는 로그인 제공자 정보와 무관하게 항상 직접 입력한다.
- 검증: 연락처 정규화 단위 테스트, TypeScript, 대상 ESLint, 프로덕션 빌드, 운영 브라우저 확인
- 관련 파일:
  - `server/cafe24/external-events-api.js`
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`
  - `src/pages/external-api/ExternalEventApiGuidePage.css`

## 2026-07-26 파트너 API Key 보관 안내 명확화

- 상태: 해결
- 범위: 외부 API 공개 안내 페이지와 전달용 문서
- 증상: `파트너 서버에서만 사용`이라는 표현만으로는 `.env`만 사용해야 하는지, 다른 서버 비밀 저장 기술도 가능한지 판단하기 어려웠다.
- 조치:
  - 기술을 강제하지 않고 방문자에게 키가 노출되지 않아야 한다는 보안 원칙을 먼저 설명했다.
  - HTML·브라우저 JavaScript·공개 앱 번들·공개 저장소는 금지하고, 서버 환경변수·호스팅 비밀변수·Secret Manager·암호화 설정을 가능한 예로 제시했다.
  - `process.env`는 Node.js 서버 예시이며 다른 언어와 플랫폼에서는 동등한 비밀 저장 기능을 사용한다고 명시했다.
- 검증: TypeScript, 대상 ESLint, 프로덕션 빌드, 모바일·데스크톱 브라우저 렌더링 확인
- 관련 파일:
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`
  - `docs/external-event-api.md`

## 2026-07-26 cURL 예시와 실제 연동 코드 구분

- 상태: 해결
- 범위: 외부 API 등록 요청 안내
- 증상: cURL 예시가 모든 파트너가 그대로 사용해야 하는 고정 소스코드인지, 플랫폼별로 바꿔 구현해야 하는지 설명이 부족했다.
- 조치:
  - cURL은 터미널 시험용 예시이며 실제 서버 구현 코드는 언어와 플랫폼에 따라 달라진다고 명시했다.

## 2026-07-26 서버 환경별 외부 일정 등록 예시

- 상태: 해결
- 증상: 공개 안내 페이지가 cURL과 플랫폼 차이만 설명하고 있어, 파트너 개발자가 자신의 서버 환경에 맞는 최초 등록 코드를 직접 변환해야 했다.
- 해결:
  - Node.js, PHP, Python, Java 서버용 등록 예시를 선택형 탭으로 추가했다.
  - 프론트엔드 프레임워크와 무관하게 API Key는 서버 또는 서버리스 함수에서만 사용한다는 경계를 예시 바로 위에 표시했다.
  - 언어별 구현은 달라도 요청 주소·메서드·인증 헤더·JSON 계약은 동일하다고 문서화했다.

## 2026-07-26 외부 API 안내 검색 접근성

- 상태: 해결
- 증상: 안내 내용이 길어지면서 파트너가 이미지·장르·수정·오류 같은 필요한 항목을 목차만으로 빠르게 찾기 어려웠다.
- 해결:
  - 상단에 모든 방문자가 사용할 수 있는 문서 검색 버튼과 모달을 추가했다.
  - 섹션 제목·요약·API 키워드를 입력 즉시 자동완성하고 선택하면 해당 섹션으로 이동하게 했다.
  - 바깥 영역 클릭, 닫기 버튼, Escape 키를 지원하고 모바일에서는 하단 시트 형태로 표시한다.

## 2026-07-26 카카오톡 공유 링크 PWA 초기화 오류

- 상태: 해결
- 증상: Android 카카오톡 인앱 브라우저에서 외부 API 안내 링크를 열면 가상 PWA 모듈의 `registerSW`가 없는 상태에서 구조 분해되어 앱 오류 화면이 표시됐다.
- 원인: 일부 WebView에서 `virtual:pwa-register` 모듈 결과가 정상 export를 제공하지 않는데도 서비스 워커 등록을 무조건 실행했다.
- 해결:
  - 카카오톡을 포함한 인앱 브라우저에서는 서비스 워커 등록 자체를 건너뛴다.
  - PWA 모듈과 `registerSW` 함수 존재 여부를 검사하고 import 실패도 앱 부팅을 막지 않도록 처리했다.
  - Android 카카오톡에서 API 안내 링크를 열면 Chrome 외부 브라우저 Intent를 우선 실행한다.
  - Intent가 차단된 경우에는 반복 이동을 막는 fallback URL로 인앱에서도 문서를 정상 표시한다.

## 2026-07-26 모바일 API 문서 검색창 키보드 가림

- 상태: 해결
- 증상: Android에서 문서 검색 입력창에 포커스하면 소프트 키보드가 모달 위를 덮고, 자동완성 항목 수에 따라 모달 전체가 밀려 입력창이 가려졌다.
- 원인: 검색 모달이 레이아웃 뷰포트의 `100vh`와 화면 하단을 기준으로 배치되어 키보드가 줄인 `visualViewport` 높이와 위치를 반영하지 못했다.
- 해결:
  - 검색 배경과 모달을 `visualViewport`의 top·left·width·height CSS 변수에 맞춰 고정했다.
  - 제목과 입력창은 고정 영역으로 유지하고 자동완성 결과만 별도 세로 스크롤되도록 분리했다.
  - 모달이 열린 동안 배경 문서 스크롤을 잠그고 결과 영역에 터치 스크롤·overscroll 제한을 적용했다.

## 2026-07-26 API 문서 자동완성 띄어쓰기 불일치

- 상태: 해결
- 증상: `연동 신청`은 검색되지만 같은 의미의 `연동신청`은 검색 결과가 없었다.
- 원인: 검색어와 색인 문자열을 소문자로만 변환하고 공백과 구분 문자를 그대로 비교했다.
- 해결:
  - 검색어와 문서 색인을 유니코드 NFKC로 정규화한다.
  - 공백·줄바꿈·하이픈·슬래시·일반 구두점을 제거한 뒤 비교해 띄어쓰기와 간단한 구분 기호 차이를 무시한다.

## 2026-07-26 API 문서 복합 키워드 검색과 결과 구분

- 상태: 해결
- 증상: `API`는 검색되지만 `연동api`처럼 한글과 영문 키워드를 붙여 입력하면 두 단어 사이에 다른 문구가 있는 결과를 찾지 못했다. 검색 결과 카드도 입력창과 시각적으로 비슷하고 목록 스크롤 가능 여부가 명확하지 않았다.
- 해결:
  - 한글과 영문·숫자의 경계에서 검색어를 자동 분리하고, 여러 키워드가 순서와 간격에 관계없이 모두 포함된 항목을 찾는다.
  - 검색 결과를 별도 패널로 감싸고 결과 제목·건수·순번·스크롤 안내를 표시한다.
  - 결과 패널에 항상 세로 스크롤 영역과 눈에 보이는 스크롤바를 적용하고 입력창과 다른 배경·카드 구조를 사용한다.
  - WAI-ARIA combobox/listbox 패턴에 맞춰 결과 건수를 실시간 알리고 방향키·Home·End로 결과를 이동하며 Enter로 선택할 수 있게 했다.

## 2026-07-26 모바일 검색 선택 핸들·결과 표시 간격

- 상태: 해결
- 증상: Android 텍스트 선택 핸들이 검색창 바로 아래 안내 문구와 겹쳤고, `검색 결과`와 `1개`가 양쪽에 분리되어 결과 건수를 한눈에 읽기 어려웠다.
- 해결:
  - 입력창과 안내 문구 사이에 모바일 선택 핸들을 위한 간격을 확보했다.
  - 결과 머리말을 `검색 결과 N개` 한 문장으로 표시한다.
  - 결과 개수와 관계없이 결과 패널 오른쪽에 스크롤 레일과 하단 영역 안내를 항상 표시한다.

## 2026-07-26 외부 API 안내 헤더 고정 실패

- 상태: 해결
- 증상: 헤더에 `position: sticky`가 선언되어 있었지만 실제 운영 페이지를 1,800px 스크롤하면 헤더의 top이 `-1,800px`가 되어 화면에서 사라졌다.
- 해결:
  - 헤더를 viewport 기준 `position: fixed`로 전환하고 충분한 z-index와 그림자를 적용했다.
  - 페이지에 데스크톱 66px, 모바일 58px 상단 여백을 추가해 첫 콘텐츠가 헤더 아래에 가려지지 않도록 했다.

## 2026-07-26 모바일 헤더 아이콘 의미 불명확

- 상태: 해결
- 증상: 모바일 헤더가 검색·관리·공유 기능을 아이콘만 표시해 기능 이름을 바로 알아보기 어려웠고, 문서 섹션으로 이동하는 전용 메뉴가 없었다.
- 해결:
  - 모바일 헤더에 `검색`, `섹션`, `관리`, `공유` 이름을 직접 표시한다.
  - `섹션` 버튼과 1~9번 섹션 이름·요약을 보여주는 별도 선택 화면을 추가한다.
  - 섹션을 선택하면 해당 문서 위치로 이동하고 고정 헤더 아래에 맞춰 표시한다.
  - 플랫폼과 무관하게 고정되는 항목을 HTTP 메서드, URL, 인증 헤더, Content-Type, JSON 필드 계약으로 구분했다.
  - 브라우저 코드가 아닌 파트너 서버에서 동일한 HTTP 계약을 구현하도록 안내했다.
- 검증: TypeScript, 대상 ESLint, 프로덕션 빌드, 브라우저 렌더링 확인
- 관련 파일:
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`
  - `docs/external-event-api.md`

## 2026-07-26 Android 키보드 전환 중 검색 결과 하단 잘림

- 상태: 해결
- 증상: 같은 Android Chrome에서도 주소창과 키보드 애니메이션 상태에 따라 검색 결과 패널 하단이 키보드 뒤에 가려졌다.
- 원인: 전역 `visualViewport` CSS 변수가 한 번 갱신된다는 전제에 의존해, 키보드가 단계적으로 열리거나 브라우저가 늦게 viewport 값을 보고하면 모달 높이가 이전 화면 높이에 남았다.
- 해결:
  - viewport 메타에 `interactive-widget=resizes-content`를 선언해 지원 브라우저가 키보드 표시 시 레이아웃 영역도 줄이도록 했다.
  - 검색 모달이 열려 있는 동안 `visualViewport.height`, `innerHeight`, 문서 표시 높이 중 가장 작은 값을 사용하고 키보드 애니메이션 800ms 동안 매 프레임 다시 측정한다.
  - resize·focus·회전 이벤트에도 재측정하며, 낮아진 화면에서는 제목과 도움말을 압축하고 남은 높이를 결과 목록의 내부 스크롤에 배정한다.
- 관련 파일:
  - `index.html`
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`
  - `src/pages/external-api/ExternalEventApiGuidePage.css`

## 2026-07-26 공개 API 안내 진입 경로

- 상태: 해결
- 증상: API 안내가 관리자 하위 메뉴에만 있어 일반 사용자가 공개 문서에 접근하기 어렵고, 비로그인 방문 시 로그인 창이 자동 표시되었다.
- 해결:
  - 햄버거 메뉴의 빠른 기능 영역에 누구나 볼 수 있는 `API 연동` 버튼을 추가하고 일정 등록 버튼 왼쪽에 배치했다.
  - 화면 모드 버튼은 한 줄짜리 작은 버튼으로 축소했다.
  - 관리자 하위 메뉴의 중복 링크를 제거하고, 공개 문서는 로그인 없이 읽을 수 있게 했다.
  - 로그인은 연동 신청이나 테스트 한도 요청을 누를 때만 요구한다.
- 관련 파일:
  - `src/components/SideDrawer.tsx`
  - `src/styles/domains/overlays.css`
  - `src/styles/theme-completion.css`
  - `src/pages/external-api/ExternalEventApiGuidePage.tsx`

## 2026-07-26 햄버거 메뉴 API 버튼 의미 불명확

- 상태: 해결
- 증상: `API 연동`이라는 짧은 이름만으로는 외부 사이트 일정을 자동 등록하는 기능인지 바로 알기 어려웠다.
- 해결: 버튼 제목을 `자동등록 API`로 변경하고 `외부 사이트 일정 연동` 설명을 별도 줄로 표시해 제목과 용도를 한눈에 구분하도록 했다.
- 관련 파일:
  - `src/components/SideDrawer.tsx`
  - `src/styles/domains/overlays.css`

## 2026-07-26 햄버거 메뉴 전체 가독성

- 상태: 해결
- 증상: 영문 구역명과 비슷한 대비의 카드가 연속되어 빠른 기능, 사이트 현황, 관리자 기능을 한눈에 구분하기 어려웠다.
- 해결:
  - 구역명을 `빠른 기능`, `사이트 현황`, `관리자 메뉴`처럼 한국어 중심으로 변경하고 영문은 작은 보조표기로 낮췄다.
  - 각 구역에 배경·간격·경계선을 적용해 정보 위계를 분리했다.
  - 빠른 기능 카드의 높이와 대비를 통일하고 화면 모드 버튼은 한 줄 보조 기능으로 유지했다.
  - 관리자 하위 기능을 그룹 카드와 행 형태로 정리하고 메뉴 스크롤 위치를 확인할 수 있는 얇은 스크롤바를 표시했다.
  - 메뉴 이미지와 일반 요소의 불필요한 드래그 및 텍스트 선택을 차단했다.
- 관련 파일:
  - `src/layouts/MobileShell.tsx`
  - `src/components/SideDrawer.tsx`
  - `src/styles/domains/overlays.css`

## 2026-07-26 햄버거 메뉴 로그아웃 영역 높이 점유

- 상태: 해결
- 증상: 로그아웃과 버전 영역이 스크롤 영역 밖에 고정되어 좁은 모바일 화면에서 메뉴가 사용할 수 있는 높이를 계속 줄였다.
- 해결: 하단 영역을 메뉴 스크롤의 마지막 항목으로 이동해 끝까지 스크롤했을 때만 로그아웃 버튼이 보이도록 했다.
- 관련 파일:
  - `src/components/SideDrawer.tsx`
  - `src/styles/domains/overlays.css`

## 2026-07-26 햄버거 메뉴 명도 대비 부족

- 상태: 해결
- 증상: 다크 모드에서 메뉴 바탕, 기능 카드, 통계 카드의 명도가 비슷하고 보조 문구가 어두워 카드 경계와 설명을 읽기 어려웠다.
- 해결:
  - 다크 모드의 전체 배경, 구역 배경, 기능 카드, 통계 카드를 네 단계 명도로 분리했다.
  - 카드 테두리와 그림자를 강화하고 제목은 흰색, 보조 설명은 밝은 회색으로 대비를 높였다.
  - 일반 메뉴 아이콘에도 밝은 아이콘 타일을 적용해 메뉴 행을 빠르게 구분하도록 했다.
  - 라이트 모드는 회색 메뉴 배경 위에 흰색 카드를 사용하고 테두리와 본문 대비를 강화했다.
- 관련 파일:
  - `src/styles/domains/overlays.css`

## 2026-07-26 프로필 지역·외부 링크 줄 낭비

- 상태: 해결
- 증상: Instagram 대표 링크, 지역, Website 링크가 각각 별도 행을 사용해 충분한 가로 공간이 있는데도 프로필 영역이 여러 줄로 늘어났다.
- 해결:
  - 지역, 관심 장르, 모든 외부 링크를 하나의 가로 칩 행으로 통합했다.
  - Instagram과 Website 등 외부 링크는 접근 가능한 이름과 툴팁을 유지하면서 아이콘 전용 버튼으로 축소했다.
  - 공간이 부족한 경우에만 칩 행이 자동 줄바꿈되도록 했다.
- 관련 파일:
  - `src/components/SideDrawer.tsx`
  - `src/styles/domains/overlays.css`

## 2026-07-26 프로필 외부 링크 클릭 가능성 불명확

- 상태: 해결
- 증상: 축소된 SNS·웹사이트 아이콘이 모두 같은 색이면 서비스 구분이 어렵고 단순 상태 표시처럼 보여 클릭 가능한 링크인지 알기 어려웠다.
- 해결:
  - Instagram은 브랜드 그라데이션과 채움 아이콘, YouTube는 빨강, 웹사이트는 청록, 카카오 오픈채팅은 노랑, 기타 링크는 보라색 버튼으로 표시한다.
  - 호버와 키보드 포커스 시 밝은 테두리, 외곽선, 이동 효과를 표시해 클릭 가능한 링크임을 명확히 했다.
- 관련 파일:
  - `src/components/SideDrawer.tsx`
  - `src/styles/domains/overlays.css`

## 2026-07-26 자유게시판 신규·건의·숨김 글 식별 개선

- 상태: 해결
- 범위: 자유게시판 목록과 전역 하단 포럼 메뉴
- 증상:
  - 최근 등록 글, 건의사항, 관리자에게만 보이는 숨김 글의 시각적 구분이 약했다.
  - 다른 페이지에 있을 때 자유게시판에 새 글이 등록돼도 하단 메뉴에서 알 수 없었다.
- 조치:
  - 등록 후 14일 동안 데스크톱·모바일 목록 제목 옆에 `NEW`를 표시한다.
  - 건의/신청 말머리를 주황 계열로 분리하고 행 왼쪽 강조선을 추가했다.
  - 숨김 글도 목록의 정렬 위치와 신규 글 숫자에는 포함한다.
  - 숨김 글은 행의 존재, NEW, 날짜, 댓글 수 등만 표시하고 제목·작성자 아이디/닉네임·본문·이미지는 일반 사용자에게 전달하지 않는다.
  - 관리자와 해당 글 작성자에게만 원래 목록 정보와 상세 진입 권한을 제공한다.
  - 등록 후 14일 이내의 모든 자유게시판 글 수를 실제 모바일 하단 자유게시판 메뉴의 숫자 배지로 표시하며, 숨김 글의 실시간 신규 등록도 반영한다.
  - 자유게시판에 방문해도 14일 기간 안에는 숫자 배지를 유지한다.
- 검증:
  - `npx tsc --noEmit`
  - `npm run build:only`
- 관련 파일:
  - `src/pages/board/components/StandardPostList.tsx`
  - `src/pages/board/board.css`
  - `src/pages/board/hooks/useBoardPosts.ts`
  - `src/hooks/useFreeBoardUnreadCount.ts`
  - `src/layouts/BottomNavigation.tsx`
- 관련 커밋: pending

## 2026-07-26 다장르 혜택 이벤트 수집·노출 경로 완성

- 상태: 해결
- 범위: 스윙·살사·바차타·탱고·스트릿 무료·할인 이벤트 및 정기권 판매 수집
- 증상:
  - 혜택 판별과 전용 화면은 있었지만 등록된 검색어가 실제 일일 수집기에서 실행되지 않았다.
  - 후보의 `benefit_eligible`가 관리자 승인 뒤 운영 `events` 행으로 전달되지 않아 화면에 노출되지 않았다.
- 원인:
  - 정적 계정 소스 수집과 동적 혜택 검색이 연결되지 않았고, 검색엔진 차단 시 대체 발견 경로가 없었다.
  - 수집 후보와 운영 이벤트 사이의 혜택 메타데이터 계약이 누락됐다.
- 조치:
  - 스윙 전용 4개와 확장 장르 8개의 단계별 혜택 검색 소스를 추가했다.
  - Google → Bing → Naver 프로필 발견 → 실제 Instagram 게시물 검증 순서의 폴백을 구현했다.
  - 실제 본문·미래 날짜·원본 이미지·명시적 무료/정기권 표현을 모두 통과한 후보만 허용한다.
  - 확장 장르는 `expanded-research`에서 저장 없이 검증하고 `expanded-ingestion`에서만 저장 가능하게 했다.
  - 승인 시 `benefit_eligible`와 허용된 `benefit_kind`를 운영 이벤트에 보존하고 알 수 없는 값은 차단한다.
  - 관리자 수집센터 상단을 `신규 | 완료 | 중복 | 무료, 할인 이벤트` 독립 탭으로 구성하고, 해당 탭에는 검증 대기 중인 무료·할인·정기권 후보를 표시한다.
  - 일정 전체가 유료여도 무료 강습·입장·체험·제공·주차 등 명시된 무료 요소가 있으면 혜택 후보로 분류한다.
  - 동호회와 바의 정기권·시즌권·월정액·멤버십 판매도 같은 탭으로 분류한다.
  - 후보 등록은 기존 운영 이벤트 등록 흐름을 그대로 사용하며, 운영 일정 목록에 노출되는 동시에 메인의 `무료, 할인 이벤트` 버튼이 연결된 혜택 목록에도 추가 노출한다.
  - 08시 우선순위 1, 09시 우선순위 2에 이어 10시 우선순위 3(무료·정기권), 11시 우선순위 4(할인) 자동 실행을 추가하고 `benefit_search`만 실행해 일반 수집 예산과 분리한다.
  - 3·4단계 스윙 혜택 검색을 무료 강습·입장·워크숍·동호회 혜택, 할인·특가·얼리버드·쿠폰, 바·동호회·댄스홀 정기권/패스 등 14개 검색 축으로 확장했다.
  - 소셜 및 모든 혜택 후보(무료·할인·정기권·시즌권·멤버십·패스)는 원문·미래 날짜 등 나머지 조건이 확인되면 이미지가 없어도 검수 후보로 저장하고 이미지 보완 경고를 남긴다.
  - 기존 Instagram 접근 지연, 연속 실패 회로 차단, 20분 실행 제한은 그대로 유지해 봇 판정과 장기 실행 위험을 제어한다.
  - 같은 후보를 다시 검증했을 때 새 혜택 메타데이터가 기존 대기 후보에 갱신되도록 했다.
  - 수집 summary에 소스별 발견·검사·일치 건수를 추가했다.
- 검증:
  - 수집 표준 및 오탐·URL·프로필·필드 계약 테스트 통과
  - TypeScript, 관련 서버 테스트 29개, 프로덕션 빌드 통과
  - 스윙 무료 강습 실브라우저 드라이런에서 2026-08-01 후보 2건 확인
  - 로그인된 브라우저에서 2026-11-04 DDP 무료 스윙 이벤트 원본 본문·이미지 확인 및 후보 저장
  - 살사·바차타·탱고·스트릿 확장 드라이런에서 무결과, 명시 혜택 불일치, Instagram 접근 실패를 저장 없이 분리 기록
- 관련 파일:
  - `scripts/ingestion/collection-registry.mjs`
  - `scripts/ingestion/benefit-search-utils.mjs`
  - `scripts/ingestion/swing-daily-native.mjs`
  - `scripts/com.rhythmjoy.codex-ingestion-priority3.plist`
  - `scripts/com.rhythmjoy.codex-ingestion-priority4.plist`
  - `scripts/ingestion/candidate-utils.mjs`
  - `server/cafe24/ingestion-benefit-fields.js`
  - `server/cafe24/function-api.js`
  - `src/pages/admin/v2/EventIngestorV2.tsx`
  - `src/pages/v2/components/HomeNavButtonsSection.tsx`
  - `src/styles/components/HomeNavButtonsSection.css`
  - `src/pages/benefit-events/BenefitEventsPage.tsx`
- 관련 커밋: `78af6020`, `16d8e111`, `a796a641`, `db9dc114`

## 2026-07-26 메인 혜택 버튼 위치 누락 수정

- 상태: 해결
- 범위: 모바일·데스크톱 메인 신규 이벤트 영역
- 증상: `무료, 할인 이벤트` 진입점이 하단 메뉴에는 표시됐지만, 지정된 `원데이 모집` 버튼 아래에는 표시되지 않았다.
- 원인: 버튼용 반응형 스타일만 구현되어 있고 `NewEventsBanner`의 실제 버튼 마크업이 누락됐다.
- 조치:
  - `원데이 모집` 버튼 바로 아래에 `무료, 할인 이벤트` 버튼을 추가했다.
  - 기존 혜택 목록 `/benefit-events`로 연결하고 버튼 내부 요소는 드래그 대상이 되지 않도록 구성했다.
  - 모바일 혜택 목록의 가로 전체 이미지를 72px 정사각형 썸네일로 축소하고 날짜·이미지·정보를 한 행에 정렬했다.
  - 혜택 카드를 누르면 전체 설명이 포함된 상세창을 열고, 목록과 상세창의 외부 이동 명칭을 `원본 링크`로 통일했다.
  - 첫 미래 이벤트 자동 스크롤을 제거해 진입 시 항상 페이지 제목이 보이는 최상단에서 시작하게 했다.
  - 메인 빠른 진입 버튼 두 개를 하나의 세로 그룹으로 묶어 모바일에서는 오늘일정 왼쪽, 데스크톱에서는 기존 좌측 고정 영역에 `원데이 모집` 바로 아래 혜택 버튼이 유지되게 했다.
  - 상세창에 잘리지 않는 대형 포스터를 추가하고, 포스터를 누르면 원본 이미지를 단독으로 열어 모바일 브라우저 기본 확대를 사용할 수 있게 했다.
  - 페이지별 액션이 없는 화면에서도 사이드 메뉴 빠른 기능에 `이벤트 등록`을 기본 표시하고 기존 등록 선택창을 직접 열게 했다.
  - 상세 포스터에 원본 링크 버튼의 알약형 CSS가 잘못 적용되던 선택자 충돌을 제거하고 직사각형 원본 비율로 표시했다.
  - 상세 포스터는 목록용 `micro` 대신 `image_full → image → image_medium` 순서로 고해상도 파일을 사용한다.
- 검증: TypeScript, 프로덕션 빌드, 모바일 실화면 노출 및 이동 확인
- 관련 파일:
  - `src/pages/v2/components/NewEventsBanner.tsx`
  - `src/pages/v2/components/NewEventsBanner.css`
- 관련 커밋: pending

## 2026-07-26 상시 정기권 판매의 과거 게시물 수집 허용

- 상태: 해결
- 범위: 3단계 무료·정기권 혜택 검색
- 증상: 현재도 계속 판매하는 정기권·월정액·멤버십 안내가 과거 게시물 날짜 또는 미래 행사 날짜 부재 때문에 후보에서 제외됐다.
- 원인: 일회성 행사와 상시 판매 상품에 동일한 미래 날짜 검증을 적용했다.
- 조치:
  - 상시 판매·현재 구매 가능·정기권 가격 안내 등 지속 판매 근거를 별도로 판별한다.
  - 판매 종료·마감·중단·폐지·품절 표현은 상시 판매에서 제외한다.
  - 지속 판매가 확인되면 원 게시물 날짜는 `source_post_date`로 보존하고 수집일을 후보 날짜로 사용한다.
  - 당일 행사 시작 시간 검증을 상시 판매 상품에는 적용하지 않는다.
- 검증: 과거 게시물의 현재 판매 중 정기권은 통과하고 판매 종료 게시물은 거부하는 자동 테스트 추가
- 관련 파일:
  - `scripts/ingestion/candidate-utils.mjs`
  - `scripts/ingestion/swing-daily-native.mjs`
  - `scripts/test-ingestion-standards.mjs`
- 관련 커밋: pending

## 새 항목 템플릿

```md
## YYYY-MM-DD 제목

- 상태: 조사중 | 해결 | 보류 | 재발감시
- 범위:
- 증상:
- 원인:
- 조치:
- 검증:
- 재발 방지:
- 관련 파일:
- 관련 커밋:
```
## 2026-07-26 — Compact multi-day social notice collapsed to one candidate

- Status: resolved
- Context: A Naver Cafe notice titled `7월 25,26일` described Saturday and Sunday as separate social sessions with different DJ/time sections, but only one candidate appeared.
- Root cause: the social section parser required a numeric date beside each body section. Compact dates in the title plus weekday-only body headings fell back to generic date extraction, losing per-session details; already-past dates were then correctly filtered by the future-only policy.
- Resolution: added title-date/weekday-section matching so each future/current social session becomes its own candidate with section-scoped details. Social candidates remain exempt from first-date expansion collapse.
- Verification: covered by `scripts/test-ingestion-standards.mjs`.

### Follow-up: past session hidden after manual recovery

- The 2026-07-25 session was successfully stored during a 2026-07-26 manual recovery but did not appear in the admin new-candidate list because the normal future-only list filter hid it.
- Added a narrowly scoped `manual_recovery_until` exception. It keeps an explicitly recovered past candidate visible only through the specified review date; ordinary automated past candidates remain hidden.

## 2026-07-26 모바일 달력 연속 일정과 날짜 헤더 간격 겹침

- 상태: 해결
- 범위: 모바일 월간 달력의 날짜·요일 헤더 및 연속 일정 표시
- 증상: 연속 일정이 2줄 이상 표시될 때 첫 일정이 날짜·요일 표시부에 너무 가까워지고, 연속 일정 아래의 일반 이벤트와도 간격이 불균일했다.
- 원인: 모바일 날짜 헤더의 실제 최대 높이는 오늘 날짜 배지를 포함해 20px인데, 연속 일정의 시작 위치는 행 상단 18px로 고정되어 있었다. 연속 일정 뒤 본문 여백도 별도 기준값을 사용했다.
- 조치: 연속 일정 시작 위치를 날짜 헤더 아래 28px로 이동했다. 연속 일정의 실제 줄 수에 20px 줄 간격을 곱한 높이를 런타임 CSS 변수로 전달하고, 그 높이 뒤에 16px의 공통 여백을 적용해 1·2·3줄 이상에서도 아래 이벤트가 자동으로 밀리게 했다. 기존 3줄 표시 제한도 제거했다.
- 검증: CSS 규칙 확인 및 프로덕션 빌드
- 관련 파일:
  - `src/pages/calendar/components/FullEventCalendar.tsx`
  - `src/pages/calendar/styles/FullEventCalendar.css`
- 관련 커밋: pending

## 2026-07-26 인스타그램 소셜 캘린더 릴스 화질·배치·커버 오류

- 상태: 해결
- 범위: `swingenjoy.com/calendar` 기반 15초 Instagram Reel 생성 및 게시
- 증상: 데스크톱 비율 캡처 때문에 사이트 글자가 작고 화면이 잘렸으며, 색 보정으로 원본 색이 과장됐다. 글자 상자가 작거나 중앙에서 벗어났고 외곽선·라운드가 일관되지 않았다. 프로필 그리드에서는 영상 커버가 의도한 구도로 보이지 않았다.
- 원인: 모바일 페이지를 실제 모바일 viewport로 재배치하지 않은 캡처, 고정 좌표 기반 오버레이, 불필요한 색상 필터, Instagram 프로필 커버 크롭 미설정이 함께 발생했다.
- 연결 불안정 원인:
  - Mac 잠금 자체는 ADB를 끊지 않지만 잠자기에 들어가면 에뮬레이터와 ADB 실행이 정지한다.
  - Instagram 편집기는 앱 메모리 회수, 네트워크 지연, 로그인 세션 갱신 때 편집 화면 상태를 잃을 수 있다.
  - 장시간 UI 제어 연결이 끊겨도 에뮬레이터 앱 상태와 영상 파일은 남지만, 진행 단계 확인 없이 다시 누르면 중복 게시 위험이 생긴다.
- 조치:
  - Android 모바일 환경의 390×844 CSS viewport와 5배 device scale로 캘린더를 다시 캡처해 사이트 글자를 약 10% 키우고, 4K 축소 단계에서 약한 선명화만 적용한다.
  - 오늘 파란 날짜의 실제 DOM 좌표를 읽어 글자 상자를 위쪽 좌·우 중 여유 있는 곳에 배치하고 화살표 방향을 동적으로 계산한다.
  - 글자 상자를 1080 기준 440×150, 100px·굵기 500 글자, 흰색 배경, 20px 라운드, 4px 연회색 외곽선으로 고정하고 가로·세로 중앙 정렬한다.
  - 배경 색상 보정은 제거하고 BT.709 limited H.264 2160×3840, 30fps, 15초, CRF 16으로 출력한다.
  - 첫 프레임과 동일한 4K 커버를 별도 생성하고 Instagram 프로필 크롭은 확대·이동 없이 기본 원본 상태를 사용한다.
  - 실행 연결이 끊겨도 같은 날짜 결과를 중복 생성하지 않도록 잠금·재시도·검증·실행 상태 파일을 추가한다.
  - Instagram UI는 단계별 화면 확인 후 다음 동작으로 진행하고, 게시 완료 화면과 프로필 그리드를 각각 검증한다.
- 검증:
  - 동적 레이아웃 자동 테스트 7개 통과.
  - 2026-07-26 생성 영상의 2160×3840, 30fps, 15초, H.264 High Profile 및 BT.709 메타데이터 확인.
  - 기존 저가독성 Reel을 삭제하고 개선본을 다시 게시했다. 새 게시물에서 `Take Five — Dave Brubeck` 음악, 4K 캘린더 화면과 프로필 첫 번째 그리드의 기본 원본 크롭 노출을 확인했다.
- 재발 방지: `npm run social-reel:run`으로 동일한 생성 규칙과 레이아웃 테스트를 재사용하고, 게시 전 4K 커버와 기본 원본 프로필 크롭을 확인한다. 음악은 게시 회차마다 다른 재즈곡을 선택한다.
- 관련 파일:
  - `scripts/social-reels/generate-social-reel.mjs`
  - `scripts/social-reels/run-social-reel.mjs`
  - `scripts/social-reels/layout.mjs`
  - `scripts/social-reels/layout.test.mjs`
  - `docs/social-reel-automation.md`
  - `docs/decisions/2026-07-26-social-reel-dynamic-layout.md`
- 관련 커밋: `7badf4d0`

## 2026-07-26 Instagram Reel 자동 게시 예약과 연결 안정화

- 상태: 배포 완료
- 범위: Mac Android 에뮬레이터 기반 Instagram Reel 자동 등록, 화·목·토 12:30 예약 실행
- 문제:
  - 수동 UI 제어가 길어지고 연결이 끊길 때 현재 단계를 알 수 없었다.
  - 녹화 좌표 방식은 Instagram 팝업, 앱 업데이트, 네트워크 지연 때 다른 버튼을 누를 위험이 있었다.
  - 공유 직후 연결이 끊기면 무조건 재실행할 경우 같은 Reel이 중복 게시될 수 있었다.
- 해결:
  - 매 단계에서 Android UI 계층의 접근성 ID·텍스트·설명을 확인하는 ADB 상태 기반 제어기를 추가했다.
  - 실행 중인 `Medium_Phone` AVD를 재사용하고, 꺼져 있으면 자동으로 시작하도록 했다.
  - 로그인 계정과 게시물 수, 최신 영상, 정확한 음악 제목·아티스트, 4K 커버, 공유 버튼을 각각 검증한다.
  - 직전 게시 곡과 다른 재즈곡을 순환 선택하고, 검색 결과가 없으면 다음 후보를 사용한다.
  - 공유 전 실패와 공유 후 불명확 상태를 분리했다. 공유 후에는 프로필 게시물 수 증가가 확인되지 않으면 자동 재시도를 차단한다.
  - 화·목·토 12:30 KST LaunchAgent와 `caffeinate`, 실행 잠금, 상태 파일, 로그·알림을 구성했다.
- 검증:
  - `npm run test:social-reel`: 동적 배치·UI XML·음악 순환 테스트 10개 통과
  - 실제 Instagram 439.0.0.37.89에서 `Do What You Wanna — Ramsey Lewis` 선택, 4K 커버 설정, 기본 프로필 크롭 유지, `Share` 직전 화면 확인
  - 드라이런은 실제 게시 없이 편집 내용을 자동 폐기했고 UI 구간 106.5초 소요
  - 2160×3840, 30fps, 15초, H.264, yuv420p, BT.709 결과 재검증
  - `com.rhythmjoy.social-reel-publish`를 Mac 사용자 LaunchAgent로 설치하고 화(2)·목(4)·토(6) 12:30 트리거, `caffeinate`, 작업 경로와 로그 경로를 `launchctl print`로 확인
- 알려진 운영 상태:
  - 초기 드라이런에서 Telegram Bot 환경 값이 HTTP 404를 반환했다. 원인은 토큰 만료가 아니라 캘린더 동기화 설정의 셸 기본값 문법을 `dotenv`가 확장하지 않은 것이었다. 셸 호환 환경 로더를 추가해 기존 캘린더 동기화 봇과 채팅 ID를 함께 사용하도록 수정했다.
  - Telegram 전송 자체가 실패해도 게시 성공을 실패 처리하지 않으며 macOS 알림·로그로 대체한다.
  - Mac 잠금은 허용되지만 잠자기·종료·로그아웃·네트워크 단절 중에는 실행할 수 없다.
- 관련 파일:
  - `scripts/social-reels/instagram-reel-adb.mjs`
  - `scripts/social-reels/run-scheduled-social-reel.mjs`
  - `scripts/social-reels/install-macos-launch-agent.sh`
  - `ops/macos/com.rhythmjoy.social-reel-publish.plist`
  - `docs/social-reel-automation.md`
  - `docs/decisions/2026-07-26-social-reel-dynamic-layout.md`
- 관련 커밋: `7badf4d0`

## 2026-07-28 Instagram Reel 화·목·토 예약 게시 실패

- 상태: 코드 수정 및 회귀 테스트 완료, 2026-07-28 누락분 재게시 대기
- 현상: 화요일 12:30 예약 작업이 영상·커버 생성까지 완료했지만 Instagram 공유 단계로 진입하지 못했다.
- 원인:
  - 키오스크 ADB 연결과 `Medium_Phone` 에뮬레이터가 동시에 존재했다.
  - 게시기가 모든 ADB 명령을 serial 지정 없이 실행해 `more than one device/emulator`로 중단됐다.
  - 실패 뒤 남은 에뮬레이터 Quick Boot 스냅샷은 ADB `offline` 상태로 복원돼 자동 재게시도 진행할 수 없었다.
- 조치:
  - `adb devices` 결과를 파싱하고 실행 중인 AVD 이름을 확인해 `Medium_Phone` serial만 선택하도록 변경했다.
  - 이후 모든 ADB 명령에 `-s <serial>`을 적용해 키오스크나 다른 에뮬레이터가 함께 연결돼도 게시 대상을 혼동하지 않도록 했다.
  - `Share` 이전 실패만 최대 3회 재시도하고, `Share` 이후 검증 불명확 상태는 중복 게시 방지를 위해 재시도하지 않도록 실행기를 보강했다.
  - 다중 장치와 offline 장치가 섞인 회귀 테스트를 추가했다.
- 검증:
  - `npm run test:social-reel`: 12개 테스트 통과
  - 예약 작업 실행 기록에서 2026-07-28 영상은 2160×3840, H.264, 30fps, 15초, BT.709로 정상 생성됐고 실패 지점은 공유 전 ADB 대상 선택임을 확인
- 후속:
  - 실제 게시 환경은 `Medium_Phone`의 영구 사용자 데이터다. Quick Boot
    스냅샷은 게시 환경 또는 앱 설치 상태의 근거로 사용하지 않는다.
  - 당시 스냅샷 RAM 문자열만으로 `com.instagram.android` 설치 상태를
    재확인했다는 기록은 잘못이었다. 설치 여부의 단일 기준은 실행 중인
    대상 AVD에서 조회한 Android Package Manager의 `pm path` 결과다.
  - `Medium_Phone_2`는 게시 환경으로 사용하지 않으며 `Medium_Phone`은
    항상 snapshot load/save를 비활성화하고 cold boot한다.
  - Mac 잠금 해제에 의존하지 않고 emulator gRPC 화면 제어로 기존 스냅샷 상태를 확인할 수 있음을 검증했다.
  - 이후 모든 장애 대응은 AVD 이름·snapshot 경로·ADB serial·Instagram 패키지 확인 결과를 먼저 기록하고 진행한다.
- 관련 파일:
  - `scripts/social-reels/instagram-reel-adb.mjs`
  - `scripts/social-reels/instagram-reel-adb.test.mjs`

## 2026-07-28 Instagram 앱·로그인 상태 Quick Boot 롤백

- 상태: 원인 확인 및 재발 방지 적용, 앱 재설치·로그인 복구 필요
- 현상: 이전에 Instagram 설치와 `korea_swing_social` 로그인을 완료한
  `Medium_Phone`에서 앱 패키지가 보이지 않아 자동 게시가 중단됐다.
- 원인:
  - `default_boot` Quick Boot 스냅샷은 2026-07-26 14:56 KST에 저장됐고
    해당 스냅샷 화면과 Package Manager 상태에는 Instagram이 없었다.
  - 실제 Instagram 자동화 드라이런 성공은 같은 날 19:43 KST였다.
  - 게시기 시작 옵션은 snapshot save만 막고 snapshot load는 허용했다.
    따라서 2026-07-28 시작 시 14:56 스냅샷이 로드되어 그 뒤 설치·로그인한
    상태가 이전 시점으로 되돌아갔다. 앱 삭제 명령이나 AVD 재생성 기록은
    발견되지 않았다.
  - RAM 바이너리 문자열을 앱 설치 증거로 잘못 판단해 Package Manager
    검증이 코드에 추가되기 전까지 상태 손실을 조기에 차단하지 못했다.
- 조치:
  - 자동 실행 인자에 `-no-snapshot-load -no-snapshot-save`를 함께 적용해
    Quick Boot 상태를 읽거나 저장하지 않도록 했다.
  - 단 하나의 에뮬레이터만 실행 중이어도 AVD 이름이 `Medium_Phone`과
    일치하지 않으면 게시 대상으로 사용하는 fallback을 제거했다.
  - 게시 전에 `pm path com.instagram.android`, AVD 이름, ADB serial을
    검증하고 패키지가 없으면 미디어 선택이나 `Share` 전에 중단한다.
  - `Medium_Phone` 설정도 force cold boot로 변경해 Android Studio에서
    수동 시작해도 오래된 Quick Boot 스냅샷을 복원하지 않도록 했다.
- 검증:
  - 7월 26일 스냅샷 저장 시각과 실제 드라이런 성공 시각의 선후관계를
    상태 파일과 AVD 파일 메타데이터로 교차 확인했다.
  - 두 AVD 모두 `pm path com.instagram.android`가 비어 있음을 확인했다.
  - 중복 조사용 `Medium_Phone_2` 실행을 종료하고 `Medium_Phone`만 남겼다.
  - `Medium_Phone`을 실제 cold boot로 다시 시작해 실행 인자에
    `-no-snapshot-load -no-snapshot-save`가 기록되고 snapshot load 로그가
    생성되지 않는 것을 확인했다.
  - `npm run test:social-reel`: 17개 테스트 통과
  - 실제 게시 사전검증은 AVD·serial을 정확히 선택한 뒤 Package Manager에서
    앱 누락을 `failed-before-share`로 기록하고 공유 전에 중단했다.
- 남은 복구:
  - Instagram을 공식 경로로 다시 설치하고 `korea_swing_social`에 한 번
    로그인해야 오늘 누락 게시와 이후 예약 게시를 재개할 수 있다.
- 관련 파일:
  - `scripts/social-reels/instagram-reel-adb.mjs`
  - `scripts/social-reels/instagram-reel-adb.test.mjs`
  - `docs/social-reel-automation.md`
  - `docs/decisions/2026-07-28-instagram-avd-state-persistence.md`

# 2026-07-26 — 키오스크 홈 광고 하단 UI 정렬

- 상태: 수정 완료, 1080×1920 키오스크 화면 및 프로덕션 빌드 검증 완료
- 현상: 1024px 이상 세로형 키오스크에서 일반 데스크톱 2열 배치가 적용되어 원데이 모집/무료·할인 버튼과 광고 제목의 기준선이 어긋나고, 오늘 일정이 광고 오른쪽에 표시됨. 이미지가 없는 소셜 이벤트 광고는 장소 정보 대신 기본 누락 이미지만 노출됨.
- 원인: 키오스크 전용 레이아웃 안에서도 `.NEB-quickActions`의 데스크톱 `position: fixed` 규칙이 남아 하단 그리드 배치를 벗어남.
- 조치: 키오스크를 모바일과 같은 단일 열 순서로 변경하고 버튼 묶음을 하단 그리드로 되돌려 두 버튼의 폭과 간격을 통일함. 오늘 일정은 전면 광고 하단에 표시함. 이미지가 없는 소셜 광고는 전용 배경 위에 이벤트 제목·날짜·시간·장소를 표시하도록 대체 화면을 추가함. 키오스크 내 이미지는 드래그되지 않도록 제한함.
- 관련 파일: `src/styles/kiosk-mode.css`, `src/pages/v2/components/NewEventsBanner.tsx`, `src/pages/v2/components/NewEventsBanner.css`

## 후속 수정

- 소셜 상세 화면은 포스터 미등록뿐 아니라 등록된 포스터 URL이 모두 로딩 실패한 경우에도 주소가 있으면 카카오맵 장소 화면으로 전환하도록 보완함.
- 키오스크 오늘 일정에 모바일과 같은 카드형 목록 스타일, 고정 높이 내부 스크롤, 위치 표시 스크롤바를 적용함. 키오스크 폭이 모바일 미디어쿼리를 벗어나 내부 스타일이 누락되던 문제를 수정함.
- 후속 확인에서 키오스크 전용 오늘 일정 패널을 220px로 확대한 값이 광고와 하단 영역의 기존 비율을 바꾸는 문제가 확인되어, 모바일과 같은 152px 패널·105px 목록 높이로 복원함. 광고와 버튼 배열은 유지하고 오늘 일정 목록만 내부 스크롤되도록 제한함.
- 키오스크 캘린더에서도 모바일 셀 배치를 강제하고 날짜 헤더를 일반 흐름에 배치함. 연속 일정이 없는 셀에서 절대 배치된 날짜와 첫 이벤트가 겹치던 문제를 수정함.
- 일반 데스크톱 캘린더는 날짜 아래 기본 여백을 모든 주에 적용하고, 한 주의 일부 날짜에만 연속 일정 막대가 있어도 그 주 전체 셀이 동일한 막대 높이를 예약하도록 수정함. 연속 일정이 없는 토·일 카드가 날짜와 겹치거나 다른 요일보다 위로 올라오던 문제를 해결함.
- 관련 파일: `src/pages/v2/components/EventDetailModal.tsx`, `src/pages/calendar/components/FullEventCalendar.tsx`, `src/pages/calendar/styles/FullEventCalendar.css`

## 2026-07-26 휴무·졸공 과거 재수집 백테스트

- 상태: 백테스트 완료, 운영 저장 미적용
- 목적: 기존 저장자료가 아니라 현재 `swing-daily` 수집 대상을 다시 방문해 과거 휴무·졸업공연을 실제로 발견할 수 있는지 검증함.
- 조치: 운영의 미래 일정 규칙과 분리된 읽기 전용 백테스트 모드를 추가했다. 최대 180일(실행 인자로 조정 가능) 과거 날짜, 휴무·취소·쉬어감·졸업공연 문구, 게시 시각 기준 상대 요일을 판별하며 결과는 로컬 JSON 보고서에만 기록한다.
- 오탐 방지:
  - `6/6 취소, 6/13 정상`처럼 한 본문에 여러 날짜가 있을 때 휴무 표현과 가장 가까운 명시 날짜만 선택한다.
  - `이번 주 금요일`은 게시 시각과 서울 시간대를 기준으로 날짜를 계산한다.
  - 여러 달 휴무는 임의의 하루에 연결하지 않고 월 단위 기간으로 보존한다.
  - Instagram 추천 게시물이 다른 계정 소스에 섞이면 작성자 계정을 대조해 제외한다.
  - 미래 졸업공연을 과거 강습 시작일에 잘못 연결하지 않는다.
- 실제 재수집 확인:
  - 네오스윙: 2026-07-17 금햅 휴무
  - 피에스타: 2026-06-06 발보아 소셜 취소
  - 봉천살롱: 2026년 7~8월 서울 발보아 클럽 휴무 기간
  - 스윙타운: 2026-06-27 졸업파티
  - 스윙스캔들: 2026-07-04 졸업파티
- 검증: 선택한 현재 Instagram·Naver Cafe·Daum Cafe 대상에 재실행했으며 접근 실패 0건, 운영 API/DB 쓰기 0건을 확인했다. `npm run test:ingestion` 통과.
- 관련 파일: `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/run-swing-exception-backtest.mjs`

## 2026-07-26 정규 소셜 미래 일정 공백

- 상태: 수정 및 운영 적용 준비 완료
- 현상: 실제로 매주 운영되는 정규 소셜도 날짜가 명시된 DJ 공지가 아직 나오지 않으면 미래 캘린더에서 사라졌다.
- 원인: 기존 수집 정책은 이미지와 날짜가 있는 개별 포스트만 저장하며, 반복 운영 규칙을 미래 개별 일정으로 구체화하지 않았다.
- 조치: 공식 공지에서 반복성이 확인된 소셜 15개 규칙을 오늘부터 90일간 개별 일정으로 생성하는 롤링 조정기를 추가했다. 실제 DJ·졸공 소셜이 같은 날짜와 장소에 등록되면 기본 일정을 제거하며, 휴무·취소 후보가 있으면 해당 날짜를 억제한다.
- 범위 제한: 정규 소셜만 적용한다. 강습·워크숍·파티·기타 행사는 기존 수집 흐름을 유지한다.
- 검증: 운영 데이터 읽기 전용 미리보기에서 15개 규칙, 생성 대상 190건, 이미지 누락 0건을 확인했다. 조정기 단위 테스트 3건, 수집 표준 테스트, Cafe24 프로덕션 빌드가 통과했다.
- 관련 파일: `server/cafe24/regular-social-rules.js`, `server/cafe24/regular-social-reconciler.js`, `scripts/run-cafe24-cron-notifications.mjs`

## 2026-07-26 외부 API 정규 소셜 반복·예외 지원

- 상태: 배포 완료
- 배경: 기존 외부 API는 모든 장르의 날짜별 개별 일정만 지원해, 매주 반복되는 정규 소셜도 매 날짜를 다시 전송해야 했다.
- 조치: `social` 권한 API Key에 한해 반복 규칙과 날짜별 `closure`·`override` 예외 API를 추가했다. DJ 확정은 해당 날짜의 override만 전송하고, 졸공·별도 행사는 기존 개별 일정 API를 유지한다.
- 데이터 우선순위: 공식 개별 일정 → 공식 날짜별 예외 → 수집 변동 → 공식 반복 규칙 → 내부 기본 규칙.
- 안전장치: 기존 파트너 인증, 소유권, 테스트 모드, 호출 제한과 요청 로그를 동일하게 적용한다. 규칙 삭제·비활성화 및 예외 삭제 시 생성된 미래 일정도 조정한다.
- 배포 점검에서 인증 없는 요청이 인증보다 본문 검증을 먼저 수행해 `400`을 반환하는 순서 문제를 발견했다. 기존 API와 동일하게 인증·호출 제한을 먼저 수행하고 입력을 검증하도록 수정했다.
- 중복 처리: 수집 시 기존 공식 API 일정을 먼저 대조한다. 소셜은 날짜·정규화 장소가 같으면 DJ·제목 차이와 무관하게 공식 일정을 유지하고 수집 후보를 중복 처리한다. 다른 분류는 같은 날 한 장소의 복수 일정을 보존하기 위해 제목 유사도까지 확인한다.
- 입력 순서 보정: 수집본이 먼저 등록된 뒤 공식 API 일정이 들어온 경우에도 캘린더 응답에서 같은 중복 판정을 다시 수행한다. 수집 기록은 감사용으로 보존하고 화면에는 공식 API 일정만 노출한다.
- 기본 이미지 정책: 정규 일정이 과거 회차의 DJ 포스터를 임의로 재사용하던 동작을 제거했다. 확인 전에는 `DJ 미정`, 포스터 없음, 장소 카카오맵을 노출하고 해당 날짜의 공식 API 개별 일정이나 수집본이 있을 때만 대체한다.
- 공개 문서 보완: 등록 예시 최상단에 모든 장르의 날짜별 일정과 정규 소셜 반복 규칙을 구분하는 선택 메뉴를 추가했다. 반복 규칙·DJ 예외·휴무·포스터 회차의 전체 cURL과 우선순위·중복 규칙을 본문 및 OpenAPI에 맞췄다. 데스크톱 목차는 첫 문서 섹션 옆의 기존 위치에서 시작해 스크롤을 따라오고 현재 읽는 섹션을 파란색으로 표시하며, 모바일은 기존 섹션 모달을 유지한다.
- 등록 방식 가독성: `/events`와 `/regular-socials` 카드 위에 하나의 선택 지점에서 두 카드로 나뉘는 분기선을 추가했다. 모바일에서는 가로 스크롤을 유지하기 위해 분기선만 축약한다. 문서 전체에 텍스트 선택을 명시적으로 허용해 제목·본문·표·코드·카드·목차를 마우스로 선택하고 복사할 수 있게 했다.
- 실행 예시 검증: 왼쪽 `/events` 분기는 같은 섹션 상단 대신 실제 요청 예시 앵커로 조금 내려가도록 수정했다. Node.js·PHP·Python·Java 코드를 독립 실행 가능한 형태로 보완하고, 로컬 모의 API에 대한 cURL 포함 5개 실제 요청 테스트를 추가했다. PHP는 공식 PHP 8.3 Docker 환경에서 cURL 확장을 구성해 실행했으며, 모든 예시에서 POST·Bearer 인증·JSON 본문·2xx 응답 처리를 확인했다.
- 검증: 관련 단위 테스트 40개, 수집 표준 테스트, ESLint, Cafe24 프로덕션 빌드가 통과했다. 운영에서 기본 일정 190건을 교체한 뒤 포스터 0건, `DJ 미정` 190건, 지도 주소 190건을 확인했고 재실행 드라이런은 생성 0·삭제 0·유지 190건이었다.
- 관련 파일: `server/cafe24/external-regular-socials-api.js`, `server/cafe24/regular-social-reconciler.js`, `server/cafe24/migrations/2026-07-26-external-regular-socials-api.sql`, `docs/external-event-api.md`

## 2026-07-27 홈 데스크톱 3열 레이아웃 복원

- 상태: 수정 및 데스크톱 화면 검증 완료
- 현상: 데스크톱 홈에서 원데이 모집·무료/할인 버튼이 메인 광고 아래쪽에 고정되어, 기준 레이아웃의 왼쪽 보조 메뉴·가운데 광고·오른쪽 오늘 일정 구조가 무너졌다.
- 원인: 메인 배너와 오늘 일정만 외부 2열 그리드에 배치하고, 보조 버튼은 배너 내부 하단 영역에서 화면 기준 고정 위치를 사용했다.
- 조치: 1200px 이상 일반 데스크톱에서 배너 내부를 왼쪽 보조 메뉴와 가운데 광고의 2열로 나누고, 외부 오늘 일정 열과 합쳐 3열이 되도록 구성했다. 데스크톱 본문과 헤더의 최대 폭을 1060px로 통일하고 좌측 보조 열 146px·가운데 간격 18px·우측 일정 열 360px로 조정했다. 태블릿·모바일·키오스크 규칙은 제외했다.
- 검증: 운영 데이터 프록시를 연결한 1422×800 화면에서 햄버거와 원데이 메뉴 왼쪽이 181.1px, 사용자 썸네일과 오늘 일정 오른쪽이 1241.1px로 일치했다. 활성 광고·`모아보기 + 숫자` 오른쪽은 837.1px로, 활성 광고와 하단 제목의 중심은 720.6px로 일치하는 것을 실측했다.
- 관련 파일: `src/pages/v2/components/NewEventsBanner.tsx`, `src/styles/components/MobileShell.css`

## 2026-07-27 데스크톱 홈 제목 및 캘린더 헤더 정렬 보정

- 상태: 수정 및 로컬 화면 검증 완료
- 현상: 홈 데스크톱에서 광고 아래 제목 요소의 박스는 가운데였지만 실제 글자 정렬이 왼쪽으로 남아 있었다. 캘린더 데스크톱 헤더는 보기 전환 버튼과 오른쪽 버튼 묶음이 달력 영역이 아닌 화면 전체 폭 기준으로 배치되어 가이드 선과 맞지 않고 좁은 화면에서 겹칠 수 있었다.
- 원인: 홈 제목에는 이전 데스크톱 규칙의 `text-align: left`가 더 높은 우선순위로 남아 있었다. 캘린더 헤더는 달력 본문 폭과 별도 그리드를 사용했고 `오늘` 버튼이 보기 전환 버튼 옆에 있어 오른쪽 아이콘 묶음과 독립적으로 움직였다.
- 조치: 일반 데스크톱 홈 제목과 메타 정보를 실제 텍스트까지 가운데 정렬하도록 보정했다. 캘린더 헤더는 달력 본문과 같은 폭의 트랙을 사용해 보기 전환 버튼은 왼쪽, 월 선택은 중앙, `오늘`·번역·알림·검색·프로필 묶음은 오른쪽에 정렬하고, 721~860px 구간에서는 버튼 폭과 간격을 줄이도록 조정했다.
- 검증: 로컬 1422px 화면에서 캘린더 그리드 361.99~1059.97px, 헤더 트랙 361~1061px로 맞았고, 보기 전환 왼쪽 361px·월 선택 중심 711px·오른쪽 버튼 묶음 오른쪽 1061px를 확인했다. 800px 화면에서도 보기 전환 24~204px, 월 선택 306~494px, 오른쪽 버튼 묶음 598.47~776px로 겹침이 없었다. 홈 광고 제목 실제 글자 중심과 제목 박스 중심 오차는 0px였다.
- 후속 수정: 최초 배포 검증이 가로 좌표 중심이라 캘린더 헤더 세로 정렬 문제를 놓쳤다. 데스크톱 캘린더 헤더 트랙을 42px 단일 행으로 고정하고 보기 전환·월 선택·오른쪽 버튼·햄버거를 같은 y축 중심선에 배치했다.
- 관련 파일: `src/layouts/MobileShell.tsx`, `src/styles/components/MobileShell.css`

## 2026-07-27 관리자 콘텐츠 조회수 미집계

- 상태: 해결
- 현상: 관리자가 비공개 문의 게시물을 상세 화면에서 읽어도 조회수가 `0`으로 유지됨.
- 원인: 방문 분석에서 관리자 트래픽을 제외하는 보호 조건이 게시물·이벤트 콘텐츠 조회수에도 적용됐고 서버도 관리자 세션의 조회 기록을 거부함.
- 조치: 방문 분석의 관리자 제외는 유지하되 콘텐츠 조회수는 관리자도 사용자별 최초 1회만 집계하도록 분리함. 같은 관리자가 다시 열거나 새로고침해도 중복 증가하지 않음.
- 관련 파일: `src/hooks/useViewTracking.ts`, `server/cafe24/generic-data-api.js`

## 2026-07-27 데스크탑 캘린더 날짜 숫자 세로 정렬

- 상태: 해결
- 현상: 데스크탑 월간 캘린더의 날짜 머리글에서 작은 요일 표시는 중앙에 맞지만 큰 날짜 숫자는 위아래 중앙에서 벗어나 보였다.
- 원인: 요일은 flex 정렬을 사용하지만 날짜 숫자는 일반 inline 박스를 사용해 글꼴 행간 기준이 서로 달랐다.
- 해결: 날짜 숫자를 1em 높이의 inline-flex 박스로 바꾸고 수직·수평 중앙 및 고정 행간을 적용했다.
- 관련 파일: `src/pages/calendar/styles/FullEventCalendar.css`

## 2026-07-27 오전 이벤트 수집 사후 점검

- 상태: 정상, 운영 수정 불필요
- 점검 범위: 08:00 실행 `20260727_080005_53216`과 09:00 실행 `20260727_090005_61465`의 메타데이터, native collector 출력, Telegram summary, cleanup 기록을 대조함.
- 결과: 두 실행 모두 `swing-daily` native collector가 실행되었고 exit code 0, summary 블록, 전체 소스 처리, `deadlineReached=false`를 확인했다. 신규 저장은 각각 0건이며 스킵은 42건과 29건이었다. 접근 실패와 Instagram 회로 차단은 없었다.
- cleanup: deprecated cleanup은 정책대로 `legacy cleanup retired`로 스킵되었으며 삭제는 0건이었다.
- 검증: 외부·저장소 wrapper의 `bash -n`, `node scripts/test-ingestion-standards.mjs`가 통과했다. 20초 예산의 Telegram/native dry-run도 exit code 0과 summary 블록을 남겼다. dry-run의 `deadlineReached=true`와 잔여 18개 소스는 의도한 짧은 검증 예산에 따른 결과이며 운영 장애가 아니다.
- 남은 위험: Instagram fallback의 작성자 불일치 스킵과 날짜가 명시되지 않은 원데이 공지는 계속 보수적으로 제외된다. Chrome의 updater/new-tab 진단 메시지는 수집 결과에 영향을 주지 않았다.

## 2026-07-27 캘린더 DJ 미정 문구 노출

- 상태: 해결
- 현상: DJ가 확정되지 않은 정규 소셜 일정에 캘린더가 `DJ 미정`을 표시했다.
- 원인: 소셜 캘린더가 `dj_name`과 제목에서 추출한 DJ 값을 실제 이름인지 확인하지 않고 표시했다.
- 조치: DJ 값이 `미정` 또는 `DJ 미정`이면 빈칸으로 처리하고, 실제 이름이 있을 때만 `DJ 이름`을 표시한다.
- 관련 파일: `src/pages/calendar/components/FullEventCalendar.tsx`

## 2026-07-27 무료·할인·정기권 검색 후보 전량 작성자 불일치

- 상태: 해결
- 범위: 10시 무료·정기권 및 11시 할인 혜택 자동수집
- 증상: 검색 결과에서 인스타그램 게시물과 프로필을 발견했지만 무료·할인·정기권 후보가 한 건도 본문 판정 단계로 진행되지 않았다. 과거 게시물의 현재 판매 중 정기권도 같은 이유로 누락됐다.
- 원인: 혜택 검색 소스의 URL은 Google 검색 URL인데, 인스타그램 원문 작성자 검증이 이 URL의 첫 경로인 `search`를 기대 계정명으로 사용했다. 따라서 실제 모든 인스타그램 작성자가 `instagram author mismatch (search)`로 거부됐다.
- 조치: 혜택 검색에서 직접 발견한 게시물은 Google 경로를 작성자 계정명으로 간주하지 않는다. 등록된 인스타그램 프로필 소스와 검색 결과에서 발견한 프로필 소스의 실제 계정명 검증은 그대로 유지한다.
- 검증: 혜택 검색 URL은 기대 작성자 없음, 실제 인스타그램 프로필 URL은 기대 작성자 유지 조건을 자동 테스트에 추가했다.
- 관련 파일: `scripts/ingestion/benefit-search-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`

## 2026-07-27 혜택 탭 부가 경품 오판 및 만료 후보 잔존

- 상태: 해결
- 범위: 관리자 무료·할인·정기권 후보 탭
- 증상: DDP 무료 행사를 제외하면 유료 파티의 무료강습권 경품과 이미 종료된 무료 원데이 강습이 현재 혜택 후보처럼 표시됐다.
- 원인: 무료강습권·무료 음료·주차 등 부가 제공도 행사 자체의 무료 혜택으로 판정했고, 혜택 탭은 날짜가 지난 모든 무료·할인 후보를 계속 표시했다.
- 조치: 행사 참가·입장·관람·수강 자체가 무료인 명시적 표현만 무료 이벤트로 인정한다. 부가 경품·편의 혜택은 제외하고, 과거 후보 예외는 현재 판매 중임이 확인된 정기권·상시 혜택에만 적용한다.
- 검증: 유료 파티의 무료강습권·무료 주차·음료·상담·대관 혜택 거부, DDP의 누구나 무료 유지, 만료 후보 숨김 및 상시 정기권 유지 테스트를 추가했다.
- 후속 조치: 검색엔진이 노출한 2024년 게시물의 연도 없는 월·일을 2026년 미래 일정으로 해석한 재수집 후보 2건을 제외 처리했다. 날짜형 혜택은 게시 후 180일이 지난 원문이면 저장하지 않고, 현재 판매가 본문에서 확인된 상시 정기권만 예외로 허용한다.

## 2026-07-27 정기 할인권 유사 표현 검색 누락

- 상태: 검색어·판정 규칙 개선 및 재수집 검증 완료
- 증상: 이용자가 약 한 달 전 스윙프렌즈 인스타그램에서 본 정기권성 판매 공지를 기존 `정기권` 중심 검색이 찾지 못했다.
- 원인: 검색·판정 어휘가 `정기권`, `시즌권`, `월정액`, `멤버십`에 치우쳐 `정기 할인권`, `다회권`, `N회권`, `월간권`, `시즌패스`, `프리패스`, `티켓북`, 입장권 묶음 같은 표현을 포괄하지 못했다. Google의 최신 Instagram 색인에서도 해당 공식 계정 결과가 반환되지 않았다.
- 조치: 위 유사 표현을 정기권성 혜택으로 판정하고, 일반 다회권 검색과 스윙프렌즈 공식 계정 전용 검색을 추가했다. 부정·종료 표현과 상시 판매 판정에도 같은 어휘를 적용했다.
- 검증: 유사 표현 7종 자동 테스트가 통과했다. 신규 검색 2개 재수집은 정상 종료했으나 Google에서 검증 가능한 Instagram 게시물 링크가 반환되지 않아 신규 후보는 0건이었다. 로그인된 공식 계정의 2026년 5월 말~6월 게시물 36개 캡션도 직접 대조했지만 해당 문구는 확인되지 않았다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/collection-registry.mjs`, `scripts/test-ingestion-standards.mjs`

## 2026-07-27 수동 일정의 무료·할인 노출 선택 부재

- 상태: 해결
- 증상: 사용자가 일정을 직접 등록하거나 수정할 때 무료·할인 이벤트 여부를 명시적으로 선택할 수 없어 혜택 페이지 노출을 제어할 수 없었다.
- 원인: `benefit_eligible`, `benefit_kind` 필드는 자동수집 후보에만 연결되어 있었고 공통 일정 등록 모달에는 입력 UI와 저장 연결이 없었다.
- 조치: 모든 일정 등록 진입점이 공유하는 상세 등록 화면 하단에 `일반`, `무료`, `할인 이벤트` 선택을 추가했다. 무료·할인을 선택한 경우에만 혜택 필드를 저장하고, 일반을 선택하면 혜택 노출을 해제한다. 수정 시 기존 무료·할인 값을 복원하며 혜택 페이지의 종류 라벨도 저장값을 우선 사용한다.
- 검증: Cafe24/MySQL 운영 설정의 프로덕션 빌드와 수집 기준 테스트, diff 검사가 통과했다.
- 관련 파일: `src/components/EventRegistrationModal.tsx`, `src/components/EditableEventDetail.tsx`, `src/styles/components/EditableEventDetail.css`, `src/lib/cafe24Client.ts`, `src/pages/benefit-events/BenefitEventsPage.tsx`
- API 후속 조치: 외부 일정 등록·수정 API에 선택 필드 `benefit_kind`를 추가했다. `free_event`, `discount_event`, `null`만 허용하고 서버가 `benefit_eligible`을 자동 계산한다. API 가이드의 요청 필드표와 서버 예제를 갱신했으며 외부 API 테스트 27개가 통과했다.
- 등록 경로 후속 수정: 최초 조치는 공통 행사·강습 폼에만 적용되어 별도 구현인 소셜 일정과 원데이 모집 링크 등록에는 선택지가 없었다. 공용 혜택 선택 컴포넌트를 분리해 행사·강습, 소셜 일정, 원데이 모집 링크 세 등록 경로에 모두 적용했다. 소셜은 이벤트 혜택 필드를 직접 저장하며, 무료·할인으로 등록한 상시 원데이 모집 링크도 혜택 페이지 데이터에 합쳐 노출한다.

## 2026-07-28 수집 소셜 등록 시 정규일정 대체 지연

- 상태: 해결
- 증상: 실제 DJ 공지를 수집 후보에서 등록해도 같은 날짜의 자동 생성 정규 소셜이 다음 일일 동기화 전까지 함께 남을 수 있었다. 장소가 비어 있고 출처 키워드만 있는 후보는 동기화 후에도 정규일정을 대체하지 못할 수 있었다.
- 원인: 정규일정 대체 판단이 일일 동기화에만 있었고, 장소 또는 요일이 포함된 정규 제목 중심으로만 업장을 비교했다.
- 조치: 수집 등록 API가 날짜와 소셜 분류를 확인한 뒤 장소·제목·출처 키워드로 같은 업장의 자동 정규 소셜을 찾아 즉시 제거한다. 이미 등록된 후보를 재처리하는 경우에도 같은 보정을 수행하며, 관리자에게 대체 건수를 알린다.
- 검증: 출처 키워드만으로 스윙타운 정규 소셜을 찾는 경우와 다른 날짜·다른 업장을 보존하는 경우를 포함한 정규 소셜 테스트 9개 및 프로덕션 빌드가 통과했다.
- 관련 파일: `server/cafe24/function-api.js`, `server/cafe24/regular-social-reconciler.js`, `server/cafe24/regular-social-reconciler.test.js`, `src/pages/admin/v2/components/EventEditModal.tsx`

## 2026-07-28 수집 후보 장소 일관성 실측

- 상태: 수집 규칙 개선 완료
- 범위: 운영 DB의 미래 대기 후보 16건
- 측정: 장소 필드 존재율과 `location`/`venue_name` 일치율은 각각 100%였지만, 14건(87.5%)은 원문 장소 확인값이 아니라 검색 소스명을 그대로 두 필드에 복사한 값이었다. 소셜 후보 14건 중 자동 정규일정과 단일 매칭되는 후보는 9건(64.3%), 복수 정규일정과 모호하게 매칭되는 후보는 0건이었다.
- 발견: `스윙패밀리 강습/행사`, `무료 강습 검색`, `스윙타운` 같은 소스명이 장소로 저장됐다. 2026-07-28을 수요일로 해석한 과거 스윙타임 공지 2건과 강습 모집을 소셜로 분류한 3건도 포함됐다.
- 조치: 장소를 찾지 못했을 때 소스명을 장소로 대입하는 동작을 제거했다. 스윙타운·스윙패밀리 소스의 검증 장소를 `봉천살롱`으로 등록하고, 장소가 본문·소스 레지스트리·소스 별칭 중 어디에서 결정됐는지 `venue_provenance`로 기록한다. 날짜와 명시 요일이 다른 후보 및 모집·강습을 소셜로 분류한 후보는 저장 전에 거부한다.
- 재측정: 개선된 검증 규칙을 기존 16건에 재적용했을 때 오분류·날짜 불일치 5건(31.3%)이 자동 거부 대상으로 식별됐다. 기존 운영 후보는 읽기 전용으로 측정했으며 자동 수정하지 않았다.
- 검증: 수집 표준 테스트, 정규 소셜 테스트 9개, 프로덕션 빌드 및 diff 검사를 수행한다.
- 관련 파일: `scripts/ingestion/audit-ingestor-consistency.mjs`, `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/candidate-utils.mjs`, `scripts/test-ingestion-standards.mjs`
- 운영 상태 후속 수정: 스윙패밀리는 현재 운영되지 않는다는 사용자 확인에 따라 자동수집·발견용 소스와 원데이 모집 링크에서 제거했다. 과거 스윙패밀리 메뉴·카페·Linktree 및 후보 식별자는 운영 종료 소스로 차단한다. 봉천살롱 장소 매핑은 현재 운영 중인 스윙타운 소스에만 유지한다.
- 장소 역할 후속 수정: `스윙스캔들`은 장소가 아닌 집단명이다. 스윙스캔들 수집 소스의 검증 장소를 `사보이볼룸`으로 분리하고 `사보이`, `사보이홀`, `사보이볼룸(사당)` 표기를 `사보이볼룸`으로 정규화한다.
- 승인 후보 전수조사 후속 수정: 승인 완료 150건 중 URL+날짜로 운영 일정과 직접 연결되는 후보는 84건(56%)이었다. 연결된 후보의 제목 94%, 장소 89.3%, 분류 86.9%, DJ 94%가 유지됐다. 상세 시간은 사이트 수집·조회 대상이 아니므로 평가에서 제외했다. 여러 장소를 쓰는 스윙팝·올어바웃스윙·골든스윙에는 소스 단일 장소 기본값을 강제하지 않고 원문 장소만 사용한다. 플랫폼·게시판 소제목은 후보 제목으로 거부하며, 후보는 실제 일정 저장 성공 후에만 승인 완료 처리하고 `registered_event_id`를 영구 기록한다.
- 자동등록 준비도 후속 조치: 수정 확률은 제목 6.0%, 장소 10.7%, 분류 13.1%, DJ 6.0%로 측정됐다. 이 수정 패턴을 저장 전 검증 규칙으로 반영하고, 시간 정보는 조건에서 제외했다. 승인 이력이 충분한 스윙스캔들·경성홀만 `shadow` 대상으로 지정하되 이미지·검증 장소·구체적 제목·분류가 모두 필요하고 소셜은 DJ가 있어야 준비 완료로 판정한다. 실제 자동등록은 아직 수행하지 않으며 후보에 준비 여부와 차단 이유만 기록한다.
- shadow 범위 후속 확대: 네오스윙과 쏘셜클럽도 실제 자동등록 없이 준비도만 측정하는 대상으로 추가했다. 검증 장소는 각각 `해피홀`, `소셜클럽`이며 나머지 후보 단위 필수조건은 동일하다.
- shadow 범위 2차 확대 및 재검토: 스윙타운, 스윙프렌즈 공식 카페·인스타, 스위티스윙도 준비도 측정 대상으로 추가했다. 재검토 결과 스윙프렌즈는 `스윙타임` 외 `해피홀`, `243` 사용 이력이 확인돼 단체별 고정 장소를 자동 대입하는 방식은 오등록 위험이 있었다. 세 단체 모두 소스 기본 장소를 제거하고 원문 본문·포스터에서 실제 장소가 명시된 후보만 shadow 준비 완료로 판정한다. 실제 자동등록은 계속 비활성 상태다.
- 쏘셜클럽 수요일 수집원 재검토: 기존 `@sosyalclub_swing`은 반복 접근 불가였고 실제 수요 정규 소셜 원본이 아니었다. 공식 홈페이지가 연결한 `@thesocialcluba`를 `Balboa in Social Club`의 주 수집원으로 교체하고 우선순위를 높였다. `socialclubseoul.co.kr` 일정 페이지는 날짜·DJ·신청 링크 대조 경로로 사용하며, 스윙피크닉은 별도 비정기 행사이므로 정규 수요일 수집원에서 분리했다.
- 쏘셜클럽 재수집 검증 후속: 공식 Instagram 최신 8개 시험 수집에서 수요 소셜 외 `8/10·8/17` 다이나믹발보아 강습과 게시일로 추정되는 `7/28`이 후보화됐다. 실제 저장은 중단하고 해당 소스에 `Balboa in Social Club` 계열 제목, 소셜 분류, 수요일 날짜를 모두 요구하도록 제한했다. 이미지와 DJ까지 있는 후보만 자동등록 shadow 준비 완료가 된다.
- 운영 후보 확인 후속: `2026-07-29` 중복 후보의 DJ가 `Mungun Application link`로 붙은 상태에서도 과거 shadow 준비 완료로 기록된 것을 확인했다. 후보는 `duplicate`, `is_collected=false`라 일정 등록에는 반영되지 않았다. DJ 추출 시 영문 `Application/Registration link`와 한글 신청·입금 안내 앞에서 값을 자르고, 기존처럼 안내 문구가 섞인 DJ는 검증 및 자동등록 준비 판정에서 거부한다.
- 자동등록 가능성 전수 재측정: 승인 후보를 운영 일정과 다시 연결해 소스별 완전 무수정률을 산출했다. 연결 표본 기준 스윙타운 66.7%(16/24), 네오 33.3%(1/3), 경성홀 12.5%(2/16), 스윙프렌즈 0%(0/23), 스윙스캔들 0%(0/10)였다. 과거 스윙스캔들 장소명 보정과 여러 소스의 DJ 공란·정제 차이가 낮은 수치의 주원인이며, 시간은 평가에서 제외했다. 미래 대기 후보 13건 중 현 규칙상 shadow 준비 완료는 0건으로 오등록은 차단됐다. 경성홀에서 반복된 `🎉 파티`처럼 내용 없는 단독 제목도 저장 전 거부하도록 추가했다.
- 자동등록 확률 비교식·분류 후속 개선: 운영 일정은 DJ를 별도 필드가 아니라 제목의 `DJ … |` 접두부에 저장하므로 이를 DJ 없음으로 계산하던 감사 오류를 수정했다. 재측정 완전 무수정률은 스윙프렌즈 87.0%, 경성홀 87.5%, 스윙타운 79.2%로 정정됐다. 스윙스캔들의 과거 10%는 집단명을 장소로 저장했던 이전 오류이며 현재는 사보이볼룸으로 보정된다. 실제 잔여 오류인 `경성 클래스 : 소셜을 더 즐기기 위한 1시간`의 소셜 오분류를 막고, `85F 스칼라 루비`, `호두 2026...` 같은 DJ 장식·날짜를 정제한다. 네오는 표본과 분류 정확도가 부족해 자동 전환 대상에서 계속 제외한다.
- 95% 선행 게이트: 실제 자동등록 실행 경로 작업은 철회하고 모든 대상 소스를 `shadow`로 유지했다. 과거 승인 150건 중 운영 일정과 연결 가능한 표본은 85건이며, 원본 그대로 무수정 일치는 60건(70.6%)이다. 현재 규칙의 좁은 안전 활동군은 17/17(100%)이지만 전체 연결 표본 커버리지는 20%에 불과하므로 95% 달성으로 보지 않는다. 감사 도구의 통과 조건을 `안전군 정확도 95% 이상`과 `연결 표본 커버리지 95% 이상`을 동시에 만족하도록 수정했다. 실제 일정 자동 생성은 계속 비활성 상태이며 시간 정보는 수집·평가하지 않는다.
- 과거 85건 원인 재학습: 불일치 25건을 전부 펼쳐 재검토하고 정기권 판매, 주년 행사, 클래스·원데이 모집을 기존 명시 분류보다 제목의 강한 증거로 먼저 보정하도록 바꿨다. 게시물 전체의 부가 판매·신청 문구가 정상 소셜을 덮어쓰지 않도록 제목 우선으로 제한했다. 여러 날짜와 여러 DJ가 한 후보에 합쳐지는 소셜은 날짜별 후보 분리 전에는 거부하며, 여러 Daum 카페를 호스트명만으로 첫 소스에 잘못 귀속하던 매칭도 차단했다. 현재 규칙 전체 재생 무수정 일치는 71/85(83.5%)로 상승했지만 95%에는 아직 미달하므로 자동등록은 비활성 상태를 유지한다.
- 과거 표본 2차 안정화: 과거 승인 일정의 분류값을 무조건 정답으로 취급하지 않고 제목에 `정기권`, `주년 파티`, 명시적 클래스·모집·소셜이 있는 경우 현재 의미 규칙을 기준으로 재검증하도록 감사식을 보강했다. 복수 장소 행사는 후보와 등록 일정의 장소 집합이 하나 이상 일치하면 유지로 판정하고 `Big Apple/빅애플`, `Savoy/사보이볼룸` 표기를 정규화했다. 현재 규칙 전체 재생은 76/85(89.4%), 현재 검증을 통과한 표본은 32/35(91.4%)다. 남은 불일치 9건 중 캡션 조각 제목, 잘못 섞인 DJ, 다중 날짜 미분리, 운영 종료 스윙패밀리 후보는 현재 규칙이 차단한다. 95% 정확도와 95% 커버리지 동시 조건은 아직 미달이므로 실제 자동등록은 계속 비활성 상태다.
- 신규 수집 교차검증: 네오·스윙타운·스윙프렌즈·스위티·소셜클럽·스윙스캔들·경성홀 7개 원본을 저장 없는 dry-run으로 다시 수집했다. 네오의 `7/5~8/16 6주 과정, 8/23 졸업파티` 일정 문구를 8/16 단일 행사 제목으로 오인한 신규 위험을 발견해 날짜 범위·주기만 적힌 캡션 조각 제목을 차단했고, 재수집에서 네오 신규 후보 0건·위험 후보 안전 차단을 확인했다.
- 전체 승인 데이터 재검토 정정: 앞선 42/42(100%) 수치는 과거 스윙타운 후보의 `스윙패밀리 강습/행사`, `스윙타운` 같은 소스·단체명을 실제 명시 장소로 인정한 오류가 있어 철회한다. 운영 후보 150건 전체를 다시 조사했고 엄격 연결 85건, 링크 1~3·제목·날짜·장소·DJ 보조 연결 후 88건만 실제 일정과 연결됐다. 나머지 62건은 과거 등록 API가 일정 저장 성공 전에 후보를 `collected`로 표시한 고아 이력일 가능성이 높아 성공 표본으로 세지 않는다. 현재 규칙 재생 무수정 일치는 연결 88건 중 63건(71.6%), 검증 통과 표본은 36/53(67.9%)다. 안전하게 유지할 수 있는 기존 shadow 소셜군은 경성홀 7건과 스윙스캔들 9건으로 16/16(100%)이지만, 전체 승인 연결률은 58.7%, shadow 활동군 커버리지는 69.6%이므로 95% 게이트는 미달이다. 스윙타운·스윙프렌즈 shadow 확대는 철회하고 실제 자동등록은 비활성 상태로 유지한다.
- 반복 감사 후속: 후보와 등록 일정을 등록 ID→URL/날짜→엄격 유사도 순서로 한 일정당 후보 하나만 연결하도록 바꿨다. 제목 중간의 `DJ 이름` 형식과 이벤트 링크 1~3을 모두 읽도록 보강한 최종 보수 연결은 150건 중 87건(58.0%)이다. 관리자의 `일괄 완료 처리`가 등록 일정 없이 `collected=true`를 저장할 수 있었던 재발 경로를 차단하고, 실제 `registered_event_id`가 있는 후보만 완료 처리할 수 있게 서버와 UI를 수정했다.
- 신규 원문 진단 후속: 경성홀·스윙스캔들 원문을 고한도 dry-run 진단 모드로 다시 수집했을 때 경성홀 주간 게시물의 7/25·7/26·7/28 DJ를 한 7/28 후보에 합치고 클래스 문장 제목을 사용하면서도 자동등록 준비 완료로 판정하는 오류를 발견했다. 여러 날짜·여러 DJ 미분리와 `activity_type=social`인데 `event_type/category=class`인 충돌을 저장 전에 차단했다. 수정 후 같은 원문 재수집은 신규 0건·29건 안전 차단이었다. 단순 규칙만으로 혼합 게시물을 안정 분리하기 어렵다고 판단해, 향후 자동등록에는 원문 근거만 허용하는 AI 구조화 판정과 결정적 규칙 재검증을 함께 요구한다.

## 2026-07-28 키오스크 사전 부팅 복구

- 상태: 수정 및 운영 검증 진행
- 현상: 운영 서버와 `/kiosk` 정적 자산은 정상이어도 미니PC Chrome이 오래된 서비스워커 자산을 잡고 있으면 애플리케이션이 시작되기 전에 빈 화면으로 멈출 수 있다.
- 원인: 기존 서비스워커·Cache Storage 정리는 React의 `KioskModeController`가 렌더링된 뒤 실행되어, 오래된 번들 자체가 로드되지 않는 장애에는 도달할 수 없었다.
- 조치: `index.html`의 애플리케이션 번들보다 앞에서 키오스크 경로의 서비스워커 등록과 Cache Storage를 정리하고, 기존 서비스워커가 현재 페이지를 제어 중인 경우 한 번만 캐시 무효화 URL로 재진입한다. 저장소나 정리 API 실패 시에는 정상 부팅을 계속한다.
- 검증: 운영 `/kiosk`의 현재 서버·자산 응답과 새 브라우저 렌더링을 확인하고, 프로덕션 빌드 후 캐시 제어 상태를 재검증한다.

## 2026-07-28 미니PC 키오스크 원격 접속·기동 복구

- 상태: 해결
- 현상: 같은 내부망의 미니PC `kiosk-j@172.30.1.13`은 응답하고 SSH 22번 포트도 열려 있었지만, 현재 관리 키가 승인되지 않아 키오스크를 원격 재시작할 수 없었다.
- 원인: 2026-06-15 작업은 비밀번호 인증으로 접속한 뒤 당시 임시 공개 키를 등록했으며, 현재 보관된 미니PC 관리 키는 이후 생성되어 미니PC의 `authorized_keys`에 없었다. 내부망 연결 여부와 SSH 인증은 별개이므로 같은 회선만으로는 접속할 수 없었다.
- 조치: 과거 세션 기록의 인증 절차를 비밀정보가 출력되지 않도록 복구해 접속하고, 현재 전용 관리 공개 키를 등록했다. `kiosk-display.service`와 `kiosk-chrome.service`를 재시작하고 자동 시작을 활성화했으며 중복 Chrome 탭을 한 개로 정리했다.
- 안정성: Chrome 서비스는 `Restart=always`, 자동 시작 `enabled`, 사용자 세션 linger `yes` 상태다. `/kiosk` 진입은 키오스크 모드 플래그를 저장한 뒤 `/`로 이동하도록 설계되어 최종 탭 URL이 `/`인 것은 정상이다.
- 검증: 현재 관리 개인 키의 비대화형 SSH 인증 성공, Chrome·화면 서비스 `active`, 자동 시작 `enabled`, 운영 `/kiosk` HTTP 200, Chrome 원격 디버깅 탭 1개를 확인했다.
## 2026-07-28 — 수집 후보가 실제 일정으로 자동등록되지 않음

- 상태: 수정 완료, 배포 전
- 현상: 수집기가 `auto_registration.ready=true`를 계산해도 후보 저장 API만 호출하여 실제 일정 등록은 관리자의 수동 등록 버튼에 의존했다.
- 원인: 수집 토큰은 `/api/scraped-events` 저장만 허용했고 `/api/ingestor-register-event`는 관리자 세션만 허용했다. 두 API를 잇는 안전한 서버 검증 경로가 없었다.
- 해결: 수집기가 후보를 먼저 저장한 뒤 준비 완료 후보에 한해 자동등록 API를 호출하도록 연결했다. 서버는 저장된 후보를 다시 읽고 허용 출처/활동, 정확한 날짜, 장소, DJ, 이미지, 대기 상태, 시간 필드 부재를 독립 검증한다. 성공 시에만 일정 ID를 연결하고 완료 처리한다.
- 현재 허용 범위: `kyungsunghall/social`, `swingscandal-cafe/social`. 그 밖의 출처와 중복·제외·완료 후보는 자동등록하지 않는다.
- 검증: 자동등록 허용/차단 테스트 통과, 수집 표준 검사 통과, 프로덕션 빌드 통과.
- 관련 파일: `scripts/ingestion/swing-daily-native.mjs`, `server/cafe24/function-api.js`, `server/cafe24/ingestor-registration-link.test.js`, `docs/decisions/2026-07-28-ingestion-automatic-registration.md`

### AI 이중 판정 보강

- 수집 후 등록 후보 분류 단계에 구조화 출력 AI 판정기를 추가했다.
- AI는 외부 검색 없이 저장 대상 원문만 보고 날짜, 활동, 장소, DJ, 원문 근거를 반환한다.
- 자동등록은 기존 규칙과 AI가 전부 합의하고 AI 신뢰도 0.95 이상이며 근거가 원문에 실제 존재할 때만 허용한다.
- 실제 모델 시험에서 명확한 `2026-07-29 경성홀 소셜 DJ 뉴야` 표본은 승인되었고, 여러 날짜와 DJ 및 강습 문구가 혼합된 표본은 검토 대상으로 차단되었다.
- 모델 호출 실패 시 자동등록은 닫히고 후보 저장만 계속하는 실패 안전 방식이다.
- 후속 재검토에서 자동등록 임계값을 0.98로 상향했다. 또한 원문에 존재하는 임의 근거 한 줄만으로 통과하지 못하도록, AI 근거 안에 후보 날짜·장소·각 DJ·소셜 유형이 모두 명시되어야 하는 필드별 근거 검증을 수집기와 서버 양쪽에 추가했다.
- 자동등록 범위 재확정: 경성홀·스윙스캔들·네오·The Social Club·스윙타운·스윙프렌드만 서버 허용 목록에 둔다. 스위티스윙은 명시적으로 제외했다. 소셜뿐 아니라 허용된 강습·행사·판매 유형도 각 유형을 나타내는 원문 문구가 AI 근거에 반드시 있어야 하며, The Social Club 수요일 제한과 스윙타운·스윙프렌드 실제 장소 명시 조건을 서버에서 재검증한다. 표준·AI·서버 테스트를 10회 반복해 모두 같은 결과로 통과했다.
- 당일 운영 후보 AI 실전 검증: 2026-07-28 운영 DB에 당일 생성된 후보는 The Social Club 1건이었다. 실제 저장 원문을 AI 판정기에 그대로 입력한 결과 신뢰도 0.99에서도 `register`가 아닌 `review`가 반환됐다. 원문에 `7월 29일`과 `July 28, 2026`이 함께 있고 수집 DJ가 `Mungun Application link`로 오염돼 있어 날짜·장소 표기·DJ가 수집값과 완전히 합의하지 않았기 때문이다. 현재 결정 규칙도 DJ 오염으로 준비 실패, 서버도 AI 미승인·중복 상태로 등록 거부하여 실제 일정은 생성되지 않았다.
- The Social Club 원문 재수집 개선: 실제 Instagram 게시물을 로그인 세션으로 다시 확인했다. 한국어 본문은 `7월 29일(매주 수요일)`, `쏘셜클럽`, `D J : 멍군`으로 일관되지만 뒤의 영문 번역에 `July 28, 2026` 오기가 있었다. 이 공식 계정의 완결된 한국어 첫 구간을 일정 근거로 선택하고 영문 번역·신청 마감 구간을 제외하도록 변경했다. `D J`처럼 띄어 쓴 표기와 `사전신청` 경계를 인식해 DJ를 `멍군`으로 정제하고, `쏘셜클럽`/`소셜클럽`은 같은 장소로 검증한다. 수정 후 실제 소스 dry-run은 `2026-07-29 / 소셜클럽 / DJ 멍군` 1건만 생성했고 AI는 0.99로 승인했다. AI 승인된 실제 게시물이 자동 생성 정규 소셜과 중복될 때는 중복 후보를 안전하게 다시 열어 실제 게시물 일정으로 대체할 수 있도록 서버 경로도 보강했다.
## 2026-07-28 Android 키오스크 USB 디버깅 승인 모달

- 상태: 연결 주체 차단 및 복구 완료, TV 디버깅 포트 비활성화는 후속 과제
- 현상: Android 키오스크 화면에 USB 디버깅 허용(RSA 승인) 모달이 표시됐다.
- 원인: 미니PC `172.30.1.13`에는 ADB가 설치되어 있지 않고 ADB/scrcpy 프로세스도 없었다. 관리 Mac의 ADB 서버가 Android 키오스크 `172.30.1.28:5555`에 연결된 상태였으며 장치는 `unauthorized`로 응답해 승인 모달을 표시했다.
- 조치: 관리 Mac에서 `172.30.1.28:5555` 연결을 명시적으로 끊고 ADB 서버를 종료했다. launchd에 ADB/scrcpy 자동 재연결 항목이 없고 미니PC에도 연결 주체가 없음을 확인했다. 이미 떠 있던 모달은 Android TV Remote v2를 6자리 코드로 페어링한 뒤 `BACK` 키를 두 번 전송해 제거했다. TV 앱이 HDMI 입력 `com.google.android.tv.inputplayer`로 복귀했고 현장에서 모달 제거를 확인했다.
- 잔여 위험: Android 키오스크의 TCP 5555 포트는 아직 열려 있다. 승인되지 않은 상태에서는 원격 명령으로 기기 설정을 변경할 수 없으므로, 모달의 절대 재발 방지는 기기 설정에서 무선/USB 디버깅을 끄거나 기존 승인 관리 키로 포트를 닫아야 한다.
- 보안 원칙: 이 키오스크는 운영 중 ADB를 사용하지 않는다. 유지보수가 필요하면 현장에서 일시적으로만 활성화하고 작업 후 즉시 비활성화한다.
- 재발 방지 확인: 관리 Mac의 ADB 서버는 종료 상태이고 ADB/scrcpy 자동실행 launchd 항목이 없으며, Mini PC에도 ADB가 없다. 운영 문서에서 `adb connect` 안내를 제거했다. TCP 5555가 열린 현재 상태에서 제3의 ADB 클라이언트가 새로 접속하면 모달이 다시 뜰 가능성은 남으므로, 절대적 차단은 TV 설정에서 디버깅을 끈 뒤 완료된다.
- 재발 대응: Mini PC Chrome/HDMI를 재시작하지 말고 ADB 연결 주체부터 종료한 다음, 저장된 Android TV Remote v2 인증으로 `BACK`을 두 번 보낸다. 실제 화면 확인 전에는 해결로 보고하지 않는다.

## 2026-07-29 키오스크 외부 링크 안내가 관리자·일반 데스크톱에 노출됨

- 상태: 해결
- 현상: 관리자 수집 화면에서 외부 원문을 열 때 키오스크 전용 QR 안내창이 표시됐고, `/kiosk`를 사용한 적이 있는 일반 데스크톱의 다른 탭에도 키오스크 동작이 이어졌다.
- 원인: `/kiosk` 진입 상태를 origin 전체가 공유하는 `localStorage`에 영구 저장했고, 관리자 경로와 관리자 인증 상태를 키오스크 활성 조건에서 제외하지 않았다.
- 조치: 키오스크 상태를 전용 탭의 `sessionStorage`에만 저장하고 기존 `localStorage` 값은 활성 조건에서 제거했다. `/admin` 계열 경로와 인증된 관리자 상태에서는 키오스크 컨트롤러 및 외부 링크 안내를 즉시 비활성화하고 해당 탭의 키오스크 상태를 정리한다.
- 검증: 레거시 영구 플래그 무시, 키오스크 진입·탭 세션 유지, 관리자 경로·관리자 인증에서 안내창 미표시, 실제 키오스크 탭에서 안내창 유지 조건을 자동 테스트로 추가했다.
- 관련 파일: `src/lib/kioskMode.ts`, `src/components/KioskModeController.tsx`, `src/layouts/MobileShell.tsx`, `src/components/LoginModal.tsx`, `src/utils/analyticsGuards.ts`, `src/lib/kioskMode.test.ts`, `src/components/KioskModeController.test.tsx`

## 2026-07-30 스윙스캔들 목요 소셜 자동등록 누락

- 상태: 해결
- 현상: 공식 카페에 `2026.07.30 스윙스캔들 목요소셜 DJ` 공지가 있었지만 운영 일정은 자동 생성 정규 소셜의 `DJ 미정` 상태로 남았다.
- 원인 1: 2026-07-29 수집은 해당 공지를 후보 `7cd2d516eace25d8`로 만들었으나, 원문 텍스트에 장소가 없고 장소 표기는 포스터 이미지의 `SAVOY BALLROOM BAR`에만 있었다. AI 판정 입력에는 포스터 OCR이 포함되지 않아 레지스트리 장소 `사보이볼룸`의 원문 근거를 확인하지 못했고 신뢰도 0.98 게이트를 통과하지 못했다. AI 미승인 후보는 기존 자동 생성 정규 소셜 중복을 다시 열 수 없어 실제 일정으로 대체되지 않았다.
- 원인 2: 2026-07-30 08:00 재수집은 당일 후보에 미래 시작 시간이 원문 텍스트로 명시되어야 한다는 검증을 적용했다. 실제 포스터에는 `19:30 - 23:00`이 있었지만 텍스트 추출에는 포함되지 않아 `same-day event has no future time or is already past`로 후보 생성 전에 탈락했다. 이는 달력이 날짜만 수집·조회하며 시간 필드를 자동등록에서 금지하는 운영 정책과 충돌한다.
- 확인: 공식 카페 게시글 `102575`의 1080×1350 원본 포스터에서 `DJ 테일`, `SAVOY BALLROOM BAR`, `19:30 - 23:00`을 확인했다. 2026-07-30 예약 실행은 08시·09시·10시·11시 모두 종료 코드 0으로 완료됐으므로 예약 실행 실패나 원본 부재가 원인은 아니다.
- 운영 영향: `regular-social:scandal-thu:2026-07-30`이 `사보이볼룸`, `DJ 미정`으로 유지됐다. 같은 실행의 AI 장소 불일치 후보 `307fd103055f64a3`은 2026-08-01 토요 소셜이므로 7월 30일 누락과 구분한다.
- 조치: 당일 포함 미래 여부를 날짜만으로 판단하고 수집 후보의 시간 필드를 제거했다. 검증된 단일 장소 공식 소스는 레지스트리 장소를 AI·서버의 제한된 장소 근거로 공유하되, 복수 장소 소스의 명시 장소 정책은 유지한다.
- 사전 검증: 수집 표준 검사, AI·서버 자동등록 테스트 17개, 프로덕션 Cafe24 빌드가 통과했다. 수정 수집기는 7월 30일 후보 `7cd2d516eace25d8`을 다시 생성하고 로컬 AI 0.98 게이트를 통과했으며, 배포 전 서버가 이전 근거 규칙으로 422를 반환하는 것까지 재현했다.
- 1차 운영 반영 후속: 서버 배포와 재수집으로 7월 30일·8월 1일 후보가 자동등록됐지만, 네이버 카페 작성자 등급·닉네임 `57F 밍밍`이 DJ명 앞에 붙은 것을 운영 조회에서 발견했다. `숫자F + 닉네임` 접두어를 DJ에서 제거하고, 동일 원본 URL·날짜·등록 일정 ID를 가진 AI 검증 완료 후보만 기존 일정을 안전하게 갱신하도록 보정했다.
- 최종 운영 검증: 실행 `20260730_142624_64804`에서 후보 `7cd2d516eace25d8`, `307fd103055f64a3`이 모두 AI·서버 검증을 통과해 기존 일정 ID를 중복 없이 갱신했다. 운영 조회 결과 2026-07-30은 `DJ 테일 | 스윙스캔들 목요소셜`, 2026-08-01은 `DJ 째 | 스윙스캔들 토요소셜`, 장소는 모두 `사보이볼룸`, 시간은 모두 `null`이다.
- 장소 정책 후속 정정: 사용자 확인에 따라 스윙타운의 고정 장소를 `봉천살롱`, 스윙프렌즈 공식 카페·Instagram의 기본 장소를 `스윙타임`으로 등록했다. 스윙프렌즈 토요일 해피홀 일정처럼 원문에 다른 장소가 명시된 경우에는 해당 장소를 별도 일정으로 우선한다. 단체명 자체를 장소로 쓰는 후보와 레지스트리 고정 장소가 불일치하는 후보는 AI·서버에서 거부한다.
- 구형 완료 이력 복구: 실제 재수집에서 스윙타운 후보 `9aaa6987183f9147`은 과거 일정이 `스윙타운` 장소로 잘못 등록됐지만 후보에 `registered_event_id`가 없어 안전 갱신 대상에서 빠졌다. 등록 ID가 없는 구형 완료 후보도 원본 URL과 날짜가 정확히 같은 운영 일정이 단 하나이고 새 후보가 현재 AI·서버 검증을 전부 통과하는 경우에 한해 다시 열어 연결·정정한다. 운영 자동등록은 기존 시간값도 `null`로 정리한다.
- 고정 장소 운영 검증: 실행 `20260730_143007_66073`에서 스윙프렌즈 8월 23일 워크숍이 `스윙타임`, 시간 `null`로 자동등록됐다. 구형 복구 배포 뒤 실행 `20260730_143424_67522`에서 스윙타운 후보 `9aaa6987183f9147`이 기존 일정 `9830dffe-c12c-4337-98a4-b4b634994e77`을 중복 없이 갱신했고, 운영 조회 결과 장소 `봉천살롱`, 시간 `null`, DJ `사복`이다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/ai-candidate-adjudicator.mjs`, `server/cafe24/function-api.js`

## 2026-07-30 경성홀 8월 1일 주간 소셜 분리·자동등록 누락

- 상태: 해결 및 운영 자동등록 완료
- 현상: 경성홀 공식 Instagram 주간 게시물의 `2026-08-01 DJ 북실` 소셜이 날짜별로 분리되지 않았고 자동 생성 정규 소셜을 대체하지 못했다.
- 원인: 같은 게시물에 `8/1 DJ 북실`, `8/2 DJ 메이저`, `8/4 DJ 스톰`과 별도의 `8/4 경성 클래스` 안내가 함께 있었다. 수집기가 날짜별 DJ 일정 분리보다 게시물 전체 활동 분류를 먼저 수행해 클래스 문구를 게시물 전체 분류로 사용했고, 하나의 `8/1 강습` 후보에 DJ 세 명을 합쳤다. 경성홀 자동등록은 소셜만 허용하므로 결정 규칙 단계에서 탈락해 AI 검증도 실행되지 않았다.
- 조치: 두 날짜 이상에서 날짜와 DJ가 모두 완결된 주간 일정은 게시물 전체 분류보다 날짜별 소셜 분리를 우선한다. 각 후보는 해당 날짜·DJ 근거 구간만 AI 2차 판정에 전달하되 저장되는 원문은 전체 게시물을 유지한다. 공통 구간에 서로 다른 요금이 함께 있으면 특정 날짜 요금으로 임의 배정하지 않는다. 날짜·DJ가 하나라도 불완전하면 기존처럼 자동등록하지 않는다.
- 건식 검증: 실행 `20260730_152516_74698`에서 8월 1일 북실, 8월 2일 메이저, 8월 4일 스톰의 소셜 후보 3건이 각각 생성됐고 모두 결정 규칙 `ready=true`였다. 시간 필드는 생성되지 않았고, 여러 요금이 섞인 8월 4일 후보에도 잘못된 요금이 붙지 않았다.
- 운영 검증: 실행 `20260730_152900_75780`에서 후보 `b776b8e769a4f726`, `d7d497e0fbc83331`, `fb116eab9f5bd118`이 모두 AI 0.98 및 서버 재검증을 통과해 자동등록됐다. 운영 API는 각각 `DJ 북실 | 경성홀 토요 소셜`, `DJ 메이저 | 경성홀 일요 소셜`, `DJ 스톰 | 경성홀 화요 소셜`을 반환하며 장소는 `경성홀`, 시간은 모두 `null`이다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`

## 2026-07-30 네오 7월 31일 소셜·8월 2일 휴무 혼합 공지 누락

- 상태: 해결 및 운영 반영 완료
- 현상: 네오스윙 공식 Instagram의 7월 5주차 공지에 `7/31 금햅 DJ 호두`가 있었지만 실제 수집 일정으로 등록되지 않았고, 같은 게시물에서 휴무로 공지한 8월 2일 정규 일요 소셜은 운영 일정에 남아 있었다.
- 원인: 게시물에 정상 소셜과 다른 날짜의 휴무 문구가 함께 있으면 수집기가 휴무 문구를 먼저 감지하고 게시물 전체를 조기 종료했다. 일반 다중 날짜·DJ 파서를 단순 적용하면 포스터 OCR 순서 때문에 `호두`와 프로그램명 `스윙베이비 [프랑]`을 뒤바꾸고 휴무인 8월 2일까지 소셜 후보로 만드는 위험도 확인됐다.
- 조치: 네오 위클리 공지는 `금햅/일햅 DJ` 문구를 실제 해당 요일 날짜와 직접 연결한다. 프로그램명은 DJ로 사용하지 않고, 날짜 구간에 `쉬어갑니다`가 명시된 날짜는 소셜 후보에서 제외해 별도 `closure` 예외로 저장한다. 정상 소셜과 휴무 예외가 섞인 게시물은 두 결과를 동시에 보존하며, 후보별 날짜·DJ·해피홀 근거만 AI 2차 판정에 전달한다.
- 검증: 확장 dry-run `20260730_174930_95468`에서 `2026-07-31 / 네오스윙 금요 소셜 / DJ 호두 / 해피홀` 한 건만 생성되고 자동등록 사전판정 `ready=true`를 확인했다. 휴무 날짜와 DJ가 뒤바뀌었던 중간 결과는 운영 저장 전에 폐기했다.
- 운영 검증: 실행 `20260730_175407_96644`에서 7월 31일 후보 `dcff91528478274d`이 AI 0.98 및 서버 검증을 통과해 자동등록됐고, 8월 2일 휴무 예외도 저장됐다. 정규 소셜 재조정 실행 `4d187810-467a-4749-8d2d-fd46041bdf48`이 휴무 날짜의 자동 생성 일정 1건을 제거했다. 운영 API에는 `DJ 호두 | 네오스윙 금요 소셜`, 장소 `해피홀`, 시간 `null`만 남고 8월 2일 네오 일요 소셜은 노출되지 않는다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`

## 2026-08-01 08:00·09:00 수집 예약 실행 누락

- 상태: 원인 확인, 코드 수정 없음
- 현상: 08:00 priority1과 09:00 priority2 LaunchAgent 실행 뒤 생성돼야 할 `.meta`, `.jsonl`, `.last.txt`가 없었고 공용 로그도 2026-07-30 17:54 이후 갱신되지 않았다.
- 원인: Mac은 2026-07-31 06:40에 부팅됐지만 사용자 Aqua 세션 로그인은 2026-08-01 15:02였다. 두 작업은 사용자 GUI 세션의 LaunchAgent라 예약 시각에 로드되지 않았고, 로그인 후 `launchctl print`에서도 각각 `runs = 0`, `last exit code = (never exited)`였다. 래퍼 실패나 timeout, summary 누락이 아니라 실행 자체가 시작되지 않은 경우다.
- 직전 정상 실행: `20260730_175407_96644`는 `swing-daily` 네이티브 수집기를 실행해 exit 0으로 끝났고 결과 JSON과 Telegram summary 블록을 모두 남겼다. deprecated 수집 흐름은 사용하지 않았다.
- 검증: 설치·저장소 양쪽 `run-ingestion.sh`의 `bash -n`, `node scripts/test-ingestion-standards.mjs`가 통과했다. `TELEGRAM_DRY_RUN=1`, `INGESTION_NATIVE_DRY_RUN=1`, cleanup skip, 120초 예산으로 실행한 `20260801_150448_1929`도 네이티브 결과 JSON과 summary를 남기고 exit 0으로 종료했다. 제한에 사용한 소스 ID는 현재 실행 목록과 일치하지 않아 실제 조회 대상은 0개였으며 저장은 발생하지 않았다.
- 잔여 위험: 사용자가 예약 시각 전에 로그인하지 않은 날에는 GUI LaunchAgent가 실행되지 않는다. 이를 보완하려면 로그인 시 catch-up 실행 여부와 같은 운영 정책 결정이 필요하며, 중복·예정 외 실행 위험 때문에 이번 점검에서는 `RunAtLoad`를 임의 추가하지 않았다.

## 2026-08-02 스윙타임 일요 소셜 수집·자동등록 누락

- 상태: 수정 및 실제 게시물 재검증 완료
- 현상: 스윙타임 공식 Instagram의 7월 30일 게시물에 8월 2일 일요 소셜과 DJ 훔머가 명시됐지만 수집 후보와 자동등록 일정이 생성되지 않았다.
- 원인: 같은 게시물의 8월 1일 스위티스윙 26주년 파티 문구가 게시물 전체 활동 분류를 행사로 선점했다. 날짜별 DJ 구간을 한 건만 찾으면 소셜 분리를 우선하지 않았고, `8월 1,2일` 묶음 날짜 표기는 분리 뒤에도 복수 날짜 오류로 다시 차단됐다. 실제 1053×1317 Instagram 이미지 URL의 크롭 파라미터도 저화질 이미지로 잘못 판정됐다. `swingtimebar`는 자동등록 출처 목록에도 없었다.
- 조치: 완결된 날짜/DJ 대응이 한 건이어도 해당 구간을 소셜로 우선 분리한다. 묶음 날짜 머리글과 해당 요일/DJ 구간만 후보 증거로 보존해 다른 날짜의 파티·강습 문구를 배제한다. 스윙타임 공식 계정을 고정 장소 `스윙타임`, 자동등록 활동 `social`로 등록했다. 소셜은 미래 날짜와 DJ가 명확하고 AI 0.98 및 서버 검증을 통과하면 이미지 없이도 자동등록하며 시간 필드는 계속 금지한다.
- 검증: 실제 게시물 `DbbCeEdGhIO` 재수집 실행 `20260802_133435_34812`에서 `2026-08-02 / 스윙타임 일요 소셜 / DJ 훔머 / ready=true` 후보 1건이 생성됐다. 시간 필드는 없고 이미지 원본은 1053×1317이었다. 같은 날짜별 증거로 AI 판정은 `register`, 신뢰도 0.99, 결정 검증 오류 0건이었다. 수집 표준 검사와 AI·서버·정규 일정 회귀 테스트 32개가 통과했다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/ai-candidate-adjudicator.mjs`, `scripts/ingestion/collection-registry.mjs`, `server/cafe24/function-api.js`

## 2026-08-03 과거 혜택 오수집·정기권 누락 및 연속 일정 과밀 표시

- 상태: 수정·운영 후보 정리 및 배포 완료
- 현상: 2024년 Instagram 게시물과 2026년 5월의 이미 지난 단일 무료 행사가 연도 없는 날짜를 미래 연도로 보정받아 신규 후보로 저장됐다. 반대로 과거 사용 기간이 적힌 정기권 판매 글은 날짜가 지났다는 이유로 단일 행사처럼 탈락했고, 혜택 검색도 Instagram 주소만 추출해 검색 결과에 노출된 다음카페 원문을 확인하지 못했다. 모바일 월간 캘린더는 연속 일정 띠 아래 여백과 날짜별 카드가 함께 누적돼 과밀한 주의 세로 길이가 과도하게 늘어났다.
- 원인: 날짜 없는 검색 결과와 원문 게시 연도를 교차검증하지 않고 지난 월을 다음 해로 넘겼으며, 정기권의 상품 수명과 단일 행사의 개최일을 같은 기준으로 검사했다. 정기권 후보 ID에도 검사 날짜를 포함해 같은 상품을 다시 수집할 여지가 있었다. 월간 캘린더는 모바일 날짜별 표시 개수 상한이 없었다.
- 조치: 날짜형 혜택은 원문 게시일의 연도에 맞춰 날짜를 결정하고 게시 연도도 확인할 수 없으면 저장하지 않는다. 정기권·다회권·월간권·티켓북 등은 명시적 판매 종료가 없는 한 상품형 혜택으로 분류하고 원본 URL과 `season-pass` 고정 식별자로 중복 제거한다. `출빠 정기권` 검색에서 Daum·Naver·Facebook 실제 문서 주소도 추출하도록 확장했다. 모바일 캘린더는 날짜별 카드 5개까지만 우선 표시하고 나머지는 `더보기`로 열며, 연속 일정 띠 아래 여백을 줄였다. 홈 왼쪽 바로가기 문구는 `원데이&동호회`로 변경했다.
- 운영 정리: 잘못 저장된 후보 `61e1831bff66e17c`, `9f2deb757358f14f`와 연결된 수집 이미지를 API로 삭제했다. 스윙타임빠 정기권은 검색 결과 주소가 아닌 실제 원문 `https://m.cafe.daum.net/sweetyswing/5lqO/1732`을 사용해 `season_pass` 대기 후보 `9882ac19190cfc4c`로 등록했다. 운영 DB에서 두 오수집 ID가 없고 새 후보의 원문 URL과 혜택 종류가 일치함을 확인했다.
- 검증: `node scripts/test-ingestion-standards.mjs`, 관련 수집기 구문 검사, `npm run build:only`가 통과했다.
- 관련 파일: `scripts/ingestion/benefit-search-utils.mjs`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `src/pages/calendar/components/FullEventCalendar.tsx`, `src/pages/calendar/styles/FullEventCalendar.css`, `src/pages/v2/components/NewEventsBanner.tsx`, `src/pages/v2/components/NewEventsBanner.css`

### 정기권 최신 검색 누락 후속

- 상태: 로컬 수정 및 무저장 수집 검증 완료, 배포 전
- 현상: Google에서 `스윙타임 정기권`을 직접 검색하면 7·8·9월 정기권 다음카페 글이 보이지만, 자동수집은 과거 4·5월 정기권을 후보로 사용하고 최신 글을 안정적으로 발견하지 못했다. 관리자 폼도 `판매이벤트` 선택 시 대분류를 `파티/행사`로 강제해 `소셜 + 판매이벤트` 조합을 유지하지 못했다.
- 원인: 혜택 수집이 관련도순 결과의 제한된 링크만 검사했고, 여러 정기권 검색축이 Instagram으로 제한돼 있었다. 검색 결과의 Instagram 프로필을 카페 원문보다 먼저 확장해 시간 예산을 소모했으며, 사용 종료일이 명시된 정기권도 판매 종료 문구가 없으면 상시 상품으로 간주했다. Google 비정상 트래픽 차단도 접근 실패가 아니라 수집 대상 없음으로 기록됐다.
- 조치: 모든 혜택 검색에서 Google 최신순을 먼저 조회하고 관련도순을 교차 병합한다. 한국어·한국 지역 결과를 사용하고, 등록된 실제 장소에서 `장소명 + 정기권` 검색 소스를 자동 생성하며, 스윙바 정기권 검색의 Instagram 제한을 제거했다. 카페·블로그 원문을 Instagram 프로필 확장보다 먼저 검사하고, 명시된 이용 기간 종료일이 오늘 이전이면 만료 후보로 거부한다. Google 차단은 즉시 해당 엔진 재시도를 중단하고 접근 실패로 보고한다. 관리자 분류에서는 판매 여부와 소셜/행사 대분류를 독립 선택하도록 강제 전환을 제거했다.
- 검증: `node scripts/test-ingestion-standards.mjs`, 수집기 구문 검사, `npm run build:only`가 통과했다. 무저장 `스윙타임 정기권` 실행에서 검색 결과 원문 `https://m.cafe.daum.net/sweetyswing/5lqO/1759`가 과거 원문보다 먼저 발견됐고 후보 1건 생성 경로를 확인했다. 이후 Google이 비정상 트래픽을 반환한 실행에서는 같은 상태가 `접근불가`로 명시됐으며 운영 API/DB 쓰기는 없었다.
- 관련 파일: `scripts/ingestion/benefit-search-utils.mjs`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `src/pages/admin/v2/components/EventEditModal.tsx`

### 혜택 검색과 자동등록 경계 후속

- 상태: 수정·회귀 검증 및 배포 완료
- 현상: 혜택 검색 소스 자체는 수동 검수 정책이지만, 검색 결과가 자동등록 허용 공식 계정의 원문 URL이면 후보 판정이 검색 경로를 잃고 공식 계정 정책을 상속할 수 있었다. 무료·할인·정기권 검색 다수에도 Instagram 한정 검색어가 남아 카페·블로그 원문 발견 범위가 불균일했다.
- 원인: 후보에는 실제 원문 URL만 남고 해당 원문을 직접 순회했는지 검색으로 발견했는지 나타내는 발견 출처가 없었다. 자동등록 준비 판정은 실제 원문 URL로 레지스트리를 다시 찾아 정책을 결정했다.
- 조치: 모든 후보에 `source_id`, `discovery_source_id`, `discovery_source_type`을 보존한다. `benefit_search`로 발견한 후보는 실제 원문이 자동등록 허용 계정이어도 수집기와 서버 양쪽에서 자동등록을 거부하고 검수 대기로만 저장한다. 혜택 검색어는 생성 단계에서 Instagram `site:` 제한을 제거해 무료·할인·정기권 22개 검색축이 최신순·관련도순 및 카페·블로그 원문을 공통으로 확인한다. 한 검색 결과 원문 접근 실패가 다음 결과 검사를 중단하지 않도록 대상별 실패를 격리하고, Google 차단 시 Bing 결과가 있어도 Naver 최신순·관련도순을 함께 병합한다. 스위티스윙 `타임빠 통신`의 게시판 내부 `정기권` 검색을 수동 검수 전용 직접 소스로 추가해 검색엔진 색인에 의존하지 않는다. 혜택 종류는 `free_event`, `discount_event`, `season_pass`로 독립 분류하되 같은 `scraped_events` 검수 큐와 승인된 `events` 저장소를 사용한다. `sale` 활동과 대분류도 분리해 소셜 입장 정기권은 `category=social + activity_type=sale`을 유지한다. 쇼핑의 `shops`·`featured_items`는 소유권과 상품 등록 흐름이 다른 별도 데이터이므로 자동 생성·복제하지 않는다.
- 검증: 수집 표준 검사에서 혜택 검색 22개가 모두 Instagram 비한정이며 무료 6개, 할인 4개, 정기권 12개로 분리됨을 확인했다. 공식 스윙프렌즈 원문이더라도 혜택 검색으로 발견된 후보는 수집기 준비 판정과 서버 자동등록 재검증 양쪽에서 차단되는 회귀 테스트를 추가했다. Google 차단 상태의 무저장 검색 실행은 발견 원문 5개를 모두 검사해 첫 실패 뒤 중단하지 않음을 확인했다. 게시판 직접 무저장 실행은 `2026-06-30 / 스윙타임빠 수요일 타임빠 정기권(7,8,9월) / category=social / activity_type=sale / season_pass / auto-registration=false` 후보 1건을 생성했으며 운영 DB 쓰기는 없었다. 서버 자동등록 테스트 14개가 통과했다.
- 관련 파일: `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `server/cafe24/function-api.js`, `server/cafe24/ingestor-registration-link.test.js`

### 혜택 후보 AI 보조 판정 누락 후속

- 상태: 수정·실제 AI 검증·운영 검수 후보 저장 및 배포 완료
- 현상: 기존 AI 판정 호출이 `auto_registration.ready=true` 후보에만 실행돼, 안전상 수동 검수로 고정한 `benefit_search`와 정기권 직접 수집 후보에는 AI가 전혀 개입하지 않았다.
- 원인: AI의 원문 재검증과 자동등록 권한이 하나의 조건으로 결합돼 있었다. 기존 AI 스키마도 날짜·활동·장소·DJ 자동등록 검증용이라 혜택 종류, 이용 기간, 현재 유효성, 판매 활동과 대분류의 독립성을 판단하지 않았다.
- 조치: 혜택 후보 전용 AI 검토 스키마와 판정기를 추가했다. AI는 원문만 사용해 `free_event`, `discount_event`, `season_pass`, 현재 유효성, 종료일, 제목, 장소, `category`, `activity_type`을 독립 판정하고 정확한 원문 근거를 반환한다. 고신뢰·원문 근거가 있는 명백한 거절 후보만 저장 전에 제외하며, 불일치·애매함·AI 오류는 후보를 없애지 않고 `AI 재검토` 또는 `AI 오류`로 관리자 검수에 남긴다. AI 확인 후보도 검색 발견 정책상 자동등록하지 않는다. 관리자 수집 목록에는 `AI 확인`, `AI 재검토`, `AI 오류` 상태를 표시하고 근거 사유를 툴팁으로 제공한다.
- 검증: 스위티스윙 정기권 직접 소스를 무저장·AI 활성화로 실행해 `season_pass`, `category=social`, `activity_type=sale`, `active_on_today=true`, 종료일 `2026-09-30`, 신뢰도 `0.99`, 원문 근거 3개로 `AI 확인`된 후보 1건을 확인했다. 후보의 자동등록 상태는 계속 `ready=false`였고 운영 API/DB 쓰기는 없었다. 혜택 AI 승인·만료 거절·분류 불일치 재검토 테스트를 포함한 AI/서버 테스트 27개가 통과했다.
- 운영 수집 화면 검증: 무저장 시험만으로 수집 화면 반영을 확인했다고 볼 수 없었던 점을 정정했다. 실행 `20260804_000658_77339`을 실제 저장 모드로 수행해 후보 `ef5c493f33ba3b77` 1건을 운영 `scraped_events`에 저장했다. 운영 DB 읽기 전용 조회에서 `is_collected=false`, `benefit_eligible=true`, `benefit_kind=season_pass`, `category=social`, `activity_type=sale`, `benefit_lifecycle=evergreen`, `ongoing_sale=true`, 혜택 AI `approved/0.99`, 자동등록 정보 없음으로 확인했다. 현재 운영 서버의 `free` 탭 필터는 `season_pass`를 포함하고, 지난 작성일이어도 `evergreen + ongoing_sale` 혜택은 숨기지 않으므로 이 후보가 무료·할인·정기권 검수 탭 조건을 통과함을 운영 배포 코드와 대조했다. 관리자 로그인 세션이 없는 별도 브라우저에서는 목록 GET이 차단되어 화면 DOM 자체는 확인하지 못했다.
- 혼합 게시물 분리 후속: 같은 원문에는 `7월 2일 수 소셜/DJ 훔머`, 스트리밍 안내, `7·8·9월 정기권`이 함께 있었다. 수집기가 정기권 소스 이름과 글 전체를 한 후보에 사용해 소셜과 판매 중 하나만 남기거나 소셜을 정기권으로 오염시키던 문제를 수정했다. 글머리표 블록을 먼저 분리해 날짜·DJ 소셜과 정기권 판매를 독립 후보로 만들고, 정기권 블록에는 다른 소셜의 DJ·스트리밍·이미지를 상속하지 않는다. 이미지 없는 정기권도 원문 판매 근거가 있으면 검수 후보로 허용한다. `INGESTION_TEST_TODAY=2026-06-30`인 무저장 실제 원문 시험은 소셜 후보 `30738a38e83bab41`과 정기권 후보 `ef5c493f33ba3b77` 두 건을 생성했고 자동등록은 둘 다 차단했다. 오늘 `2026-08-04` 기준으로는 지난 소셜을 제외하고 유효한 정기권만 유지한다. 운영 갱신 실행 `20260804_002144_79212` 및 이미지 정리 실행 `20260804_002303_79439`에서 정기권 후보의 본문은 정기권 블록 307자만 남았고 `훔머=false`, `스트리밍 서비스=false`, `poster_url=''`, AI `approved/0.99`, 종료일 `2026-09-30`, 자동등록 `false`를 운영 DB에서 확인했다.
- 혜택 전용 소스 범위 정정: 정기권·무료 혜택을 찾기 위한 소스에서는 같은 글이나 검색 결과에 함께 노출된 일반 유료 소셜을 수집하지 않는다. 날짜별 소셜 후보 자체의 제한된 근거에서 요청한 혜택 종류가 확인될 때만 저장하고, 이웃한 정기권 블록의 문구를 일반 소셜이 상속하지 못하게 했다. 실제 스위티스윙 원문 무저장 실행 `20260804_113622_90153`에서 일반 목요·일요 소셜 2건은 `no confirmed season_pass benefit`으로 제외되고 정기권 후보 `ef5c493f33ba3b77` 한 건만 유지됐다. 과거 잘못 저장된 일반 소셜 후보 `e87a84553a70b0bc`, `3d708f4e5672d4bb`와 무료 혜택이 확인되지 않은 검색 후보 `09a1309fc567531c`는 공식 수집 API를 통해 삭제했으며 운영 DB에는 세 ID가 남지 않았다.
- 운영 배포: 관련 변경 커밋 `c3250dac`를 `origin/main`에 푸시하고 Cafe24에 배포했다. 서비스는 `active`, 외부 헬스는 `ok`, 공개 버전은 `2026-08-04T02:57:13.493Z`(`buildTime=1785812229618`)다. 운영 일정 API에서 8월 4일 안토니와 8월 8일 파인 일정만 확인되고 대응하는 스윙타운 자동 생성 정규 소셜 ID는 남지 않았다.
- 관련 파일: `scripts/ingestion/ai-benefit-review.schema.json`, `scripts/ingestion/ai-candidate-adjudicator.mjs`, `scripts/ingestion/ai-candidate-adjudicator.test.js`, `scripts/ingestion/swing-daily-native.mjs`, `src/pages/admin/v2/EventIngestorV2.tsx`, `src/pages/admin/v2/EventIngestorV2.css`

### 스윙타운 8월 4일 DJ 안토니 정규 소셜 교체 누락

- 상태: 수정 및 운영 교체 완료
- 현상: 스윙타운 공식 공지에서 2026-08-04 DJ 안토니와 2026-08-08 DJ 파인을 수집했지만 자동 생성된 `DJ 미정` 정규 소셜을 교체하지 못했다.
- 원인: 네이버 카페 본문의 작성자 문맥이 DJ명 뒤에 반복돼 `안토니 스윙타운 DJ 안토니 20`, `파인 스윙타운 DJ 파인 20`이 각각 두 번째 DJ처럼 저장됐다. 복수·불일치 DJ 안전 게이트가 후보 자동등록을 차단했다.
- 조치: `이름 + 스윙타운 DJ + 같은 이름 + 시간 조각` 형태의 반복 문맥을 단일 DJ명으로 축약하고 일반 DJ 표기는 유지했다.
- 검증: 무저장 실행 `20260804_113355_89926`에서 두 후보 모두 DJ가 각각 `안토니`, `파인` 한 명이고 `ready=true`였다. 실제 실행 `20260804_113651_90292`에서 후보 `5f351f76b591752b`, `7e4b03a07f5fa490`이 자동등록됐다. 운영 일정은 8월 4일 `DJ 안토니 | 스윙타운 DJ 안토니`(`8c021244-74ef-40c7-a12c-a6b0d2906ac8`), 8월 8일 `DJ 파인 | 스윙타운 DJ 파인`(`e4559a0a-8780-437a-b833-5e1fcac5993e`)만 남고 해당 날짜의 `regular-social:swingtown-*` 일정은 제거됐다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`

### 이미지 없는 혜택 등록 및 목록 깨진 이미지 후속

- 상태: 수정·회귀 검증 및 운영 배포 완료
- 현상: 무료·할인 혜택을 일반 일정 등록창에서 저장할 때 이미지가 무조건 필수였고, 이미지 없이 등록된 정기권은 혜택 목록 카드에서 깨진 이미지 아이콘으로 표시됐다.
- 원인: 일반 일정과 혜택 일정이 같은 이미지 필수 검증을 사용했다. 혜택 목록은 실제 이미지가 없는 일정에도 공통 기본 썸네일 URL을 반환했는데, 코드 경로 `/uploads/images/default-thumbnails`와 실제 정적 파일 경로 `/default-thumbnails`도 서로 달랐다.
- 조치: `free_event`, `discount_event`, `season_pass`는 이미지 없이 등록할 수 있도록 이미지 필수 검증에서 제외했다. 혜택 목록은 실제 저장 이미지가 있을 때만 `<img>`를 만들며, 이미지가 없거나 로드에 실패하면 가짜 이미지·아이콘·빈 미디어 영역을 만들지 않고 본문이 해당 공간을 사용한다. 공통 기본 썸네일 경로도 실제 배포 경로로 바로잡았다.
- 검증: 이미지 필수 정책, 혜택 이미지 선택, 무이미지 카드 및 이미지 로드 실패 전환 테스트 6개가 통과했다. `npm run build`가 성공했고 빌드 결과에 `/default-thumbnails/default_thumbnail.webp`가 포함됨을 확인했다.
- 운영 배포: 변경 커밋 `e1b6a3e3`을 `origin/main`에 푸시하고 Cafe24에 배포했다. 운영 API에서 정기권 `0e803bac-f59f-40c0-8e01-23165a976e80`의 모든 이미지 필드가 `null`임을 확인했고, 공개 번들에는 `has-no-image` 분기가 있으며 빈 이미지 자리표시자 클래스는 없다. 서비스는 `active`, 내부·외부 헬스는 모두 `ok`, 공개 버전은 `2026-08-04T06:59:21.509Z`다.
- 관련 파일: `src/components/EventRegistrationModal.tsx`, `src/lib/eventRegistrationRules.ts`, `src/pages/benefit-events/BenefitEventsPage.tsx`, `src/utils/getEventThumbnail.ts`

### 캘린더 날짜·소셜 뱃지 세로 간격 후속

- 현상: 모바일에서 스크롤 고정 날짜줄과 소셜 섹션의 돌출 뱃지가 2px까지 가까워졌고, 소셜이 없는 일반 이벤트도 날짜와의 간격 규칙이 별도로 관리됐다.
- 원인: 날짜 헤더, 기간 일정 띠, 소셜 뱃지의 돌출 높이, 주간 행 간격이 서로 다른 하드코딩 값으로 배치됐다. 오늘 주 자동 스크롤도 일정 본문을 고정 날짜줄 경계에 여백 없이 맞췄다.
- 조치: 최소 세로 간격 3px, 날짜-콘텐츠 간격, 소셜 뱃지 돌출 높이를 레이아웃 변수로 분리했다. 소셜 섹션은 돌출 뱃지 전체 높이와 안전 간격을 문서 흐름에서 예약한다. 모바일 자동 스크롤 기준점에는 고정 날짜줄과 콘텐츠 사이의 실제 3px 여백을 반영하고, 키오스크에도 같은 규칙을 적용했다.
- 검증: 390px 모바일 브라우저 실측에서 고정 날짜 원형 뱃지-첫 콘텐츠 7px, 고정 날짜줄 경계-첫 콘텐츠 3px, 셀 내부 날짜-첫 콘텐츠 3px, 기간 띠-본문 6px, 모든 주간 행 간격 3px를 확인했다. 1024px 데스크톱에서도 날짜-첫 콘텐츠와 주간 행 간격의 최솟값이 각각 3px였다.
- 운영 반영: 커밋 `283a57bd`를 `origin/main`에 푸시하고 Cafe24에 배포했다. 공개·내부 헬스 체크가 모두 `ok`였고 서비스는 `active`, 운영 버전은 `2026-08-03T09:00:38.707Z`다.
- 관련 파일: `src/pages/calendar/page.tsx`, `src/pages/calendar/styles/FullEventCalendar.css`

## 2026-08-03 알림 미발송·빈 알림함 및 사용자별 읽음 상태 부재

- 상태: 로컬 수정 및 회귀 검증 완료, 배포 전
- 현상: 관리자가 오늘 일정과 새 등록 알림을 모두 선택하고 유효한 FCM 구독 2개를 보유했지만 단말 알림이 오지 않았고 우측 상단 종에도 수신 내역이 없었다. 자유게시판의 숫자 배지와 `NEW` 표시는 글을 읽어도 계속 남았다.
- 운영 확인: `notification_queue`에는 129건이 있었고 최근 대기 5건도 처리되지 않았다. `user_notifications`는 전 사용자 0건이었으며 서버에는 알림 cron 파일이 설치되지 않았다. 관리자 구독 행의 `pref_today_digest`, `pref_new_event_alerts` 및 세부 대상 선택은 모두 활성 상태여서 사용자 설정 누락이 원인이 아니었다.
- 원인: 업로드된 cron 호출 스크립트를 실제 스케줄러에 설치하지 않아 큐 소비자가 없었다. 새 이벤트 알림은 단말 구독 행에 붙은 설정만 보고 푸시를 시도했으며 인앱 알림함에는 저장하지 않았다. 자유게시판 배지는 사용자 읽음 기록 없이 최근 14일 글을 전부 세었다. 앱 포커스·일반 이동에서 운영체제 알림을 일괄 제거하는 동작도 읽음 의미를 훼손했다. 관리자 전용 대상도 관리자 계정이 아니라 관리자 단말 구독 행에서 역산해, 단말이 없는 관리자는 인앱 알림함에서 제외되는 결합 오류가 있었다.
- 조치: 사용자 알림 설정과 기기별 Web Push 구독을 분리하고, 사용자 설정을 정규 테이블에 저장·기존 구독에서 이관한다. 새 이벤트는 푸시 신청 여부와 무관하게 모든 로그인 사용자의 인앱 알림함에 사용자별 미확인 항목으로 저장하고, 아직 종료되지 않은 이벤트만 종의 배지와 목록에 표시한다. 푸시 신청자에게만 선택한 소셜·강습·동호회 조건으로 모든 보유 기기에 전송한다. 큐는 결정적 ID, 24시간 Push TTL, Topic, 성공 기기 기록, 제한 재시도와 지수 백오프를 사용하며 오래된 Push는 보내지 않되 이전 방문 뒤 등록된 미확인 이벤트는 인앱 알림함에 보존한다.
- 읽음 규칙: 종을 열면 현재 사용자에게 표시된 항목만 읽음 처리되어 다음 열기부터 사라진다. 운영체제 알림을 탭하면 해당 한 건만 사용자별 서버 읽음 상태와 로컬 기록에 반영한다. 앱 실행·포커스·일반 클릭은 알림을 지우지 않는다. 자유게시판은 `user_id + post_id` 읽음 키로 관리해 숫자 배지와 `NEW`를 같은 사용자에게만 제거하며 직접 링크 진입도 읽음으로 기록한다.
- 운영 작업 준비: 1분 주기의 `flock` cron, 알림 설정·자유게시판 읽음 마이그레이션, 기존 구독 설정 이관 스크립트를 배포 흐름에 포함했다. 아직 배포하지 않았으므로 운영 cron과 신규 테이블은 변경되지 않았다.
- 관리자 안전 검증: 운영 정보를 읽기 전용으로 확인해 관리자 계정 2명, 한 관리자 소유 FCM 단말 2대, 모든 관련 알림 선택 활성 상태를 확인했다. 실제 운영 단말이나 일반 사용자에게 시험 알림은 보내지 않았다. 이 형태를 익명화한 격리 테스트와 장르 4종 × 전체 알림 2상태 × 세부 선택 2상태 × 단말 0~3대의 64개 조합을 실행했다. 1차에서 단말 0대인 16개 조합이 실패해 위 관리자 계정/단말 결합 문제를 발견했고, 관리자 ID를 `users.is_admin`에서 직접 읽도록 수정한 뒤 64개를 처음부터 재실행해 전부 통과했다. 운영 형태 회귀에서는 관리자 2명 모두 인앱 대상, 해당 관리자 단말 2대만 Push 대상, 일반 사용자는 인앱·Push 모두 0건임을 확인했다.
- 전체 구독 운영 확인: 운영에는 단말 구독 38개, 구독 사용자 37명, 그중 비관리자 36명이 있었다. 명시적으로 `오늘 일정` 또는 `새 이벤트` 상위 스위치를 켠 사용자는 관리자 1명뿐이고, 나머지 36명은 상위 선택값이 없는 구형 구독이다. 알림함 저장은 전 사용자 0건이고 신규 사용자 설정 테이블과 cron은 아직 없으므로 관리자만의 문제가 아니라 전체 자동 전달 구조가 미배포·미작동 상태다. 큐의 `sent` 표시는 121건이지만 성공 단말 수, 대상 수, 처리 시각과 결과가 모두 비어 있어 실제 수신 성공 증거로 사용할 수 없다. 대기 5건과 실패 3건이 남아 있다.
- 방문 경계 후속: 종 알림함의 “이전 방문 이후 등록” 조건이 24시간을 넘긴 오래된 큐에만 적용되던 문제를 추가로 발견했다. 모든 큐에서 등록 시각과 사용자 마지막 방문 시각을 비교하도록 수정했고, 등록 전 방문 사용자는 인앱 대상에 포함되고 등록 후 방문 사용자는 제외되며 단말 Push 선택은 독립적으로 유지되는 회귀 테스트를 추가했다.
- 배포 폭주 방지: 첫 알림 배포 전에 존재하는 모든 `pending` 항목은 기준선 스크립트가 `expired / pre_delivery_rollout_baseline`으로 종료하고 단말 Push와 종 저장을 모두 0건으로 기록한다. 기준선은 배포당 반복하지 않도록 서버 마커로 보호한다. cron 파일은 활성 경로에 바로 올리지 않고 앱 내부에 먼저 준비하며, 기존 cron을 중지하고 잠금 작업 종료를 기다린 뒤 마이그레이션·설정 이관·기준선·서비스 재시작이 모두 성공한 마지막 단계에서만 설치한다. 이후 한 실행에 처리 대상이 4건 이상이면 개별 단말 Push를 모두 억제하고 사용자별 종에만 저장하며, 동시 cron·관리자 실행은 하나의 처리 Promise로 합쳐 같은 단말 중복 발송을 막는다.
- 설정 초기화 안전성: 최초 전환에서는 관리자 포함 기존 구독자 37명의 마스터·오늘 일정·새 이벤트 설정을 모두 끄고 서버의 기존 단말 구독 38개를 제거한다. 사용자가 알림 설정을 다시 저장해야 새 구독과 사용자 설정이 활성화된다. 이 초기화는 별도 서버 마커로 한 번만 수행하며 이후 배포는 재설정한 값을 덮어쓰지 않는다.
- 상태변경 공지: 모든 기존 사용자에게 `알림 기능 재설정 안내`를 고정 출처 ID로 정확히 한 건 생성한다. 본문은 `알람기능이 재설정되었습니다. 사용하기위해서는 재설정 저장해주세요`이며 단말 Push는 발생하지 않는다. 브라우저의 구형 정적 공지와 로컬 미읽음 복제본을 제거해 배포 직후 종 숫자는 이 공지 1건만 반영하고, 사용자가 종을 열면 사용자별 서버 읽음으로 전환된다.
- 자유게시판 읽음 동기화 후속: 목록에서 연 상세뿐 아니라 알림함·직접 상세 URL 등 모든 자유게시판 상세 컴포넌트가 실제 글 표시 시 같은 읽음 API를 호출한다. 성공한 읽음은 현재 탭 CustomEvent, 다른 탭 BroadcastChannel·storage, 다른 기기/복귀 시 focus·visibility 재조회로 바텀 포럼 배지, 홈 게시판 배지, 글별 `NEW`에 동시에 반영한다. 읽음 조회·저장은 HTTP 캐시를 금지한다.
- 검증: 관리자 안전 매트릭스를 포함한 Push/인앱·배포 기준선·설정 이관 핵심 테스트 81개가 통과했다. Vitest 대상 전체 197개와 별도 Node 소셜 릴스 테스트 19개가 각각 통과했고, 수집 기준 검사, 안전 관련 lint, 서버·서비스워커·배포 스크립트 구문 검사, 프로덕션 빌드와 Cafe24 서버 번들, 변경 공백 검사도 통과했다. 전체 `*.test.*`를 한 번에 Vitest로 실행하면 Node 전용 `*.test.mjs` 2개를 잘못 수집하는 저장소 실행기 충돌이 있어, 공식 `test:social-reel` Node 실행과 Vitest 대상을 분리해 검증했다. 알림 기능 실패는 없었다.
- 1차 배포 호환성 후속: Cafe24 구형 MySQL이 한 테이블의 `DEFAULT CURRENT_TIMESTAMP`와 `ON UPDATE CURRENT_TIMESTAMP` 두 컬럼을 거부해 정적 파일 전송 뒤 신규 설정 테이블 생성에서 배포가 중단됐다. cron·초기화 마커·신규 테이블이 모두 없는 상태를 확인해 기존 구독 삭제나 알림 발송은 발생하지 않았다. `created_at`을 명시 저장하는 `DATETIME`으로 바꾸고 `updated_at` 하나만 자동 갱신하도록 호환했다.
- 2차 배포 종료 후속: 설정 초기화 스크립트가 37명의 설정을 모두 비활성화한 뒤 MySQL 연결 풀을 닫지 않아 프로세스가 종료되지 않았다. cron은 계속 미설치 상태였고 단말 Push는 0건이었다. 배포 프로세스를 중지하고, 기존 단말 구독은 레거시 행별 식별자 차이에 영향받지 않도록 `generic_records.table_name`의 정확한 범위 삭제로 바꿨다. 초기화·기준선·공지 스크립트는 CLI 완료 후 연결 풀을 명시적으로 종료한다.
- 관련 파일: `server/cafe24/push-api.js`, `server/cafe24/notification-preferences.js`, `server/cafe24/board-read-api.js`, `server/cafe24/migrations/2026-08-03-notification-delivery-standard.sql`, `server/cafe24/migrations/2026-08-03-user-board-post-reads.sql`, `src/lib/notificationStore.ts`, `src/lib/pushNotifications.ts`, `src/hooks/useFreeBoardUnreadCount.ts`, `src/layouts/MobileShell.tsx`, `public/service-worker.js`, `scripts/run-cafe24-notification-cron.mjs`, `scripts/deploy-cafe24.sh`
- UI 후속: 우측 종 알림함 하단에 `단말 알림 설정` 바로가기를 고정했다. 미확인 이벤트가 없는 빈 알림함에서도 표시되며, 선택하면 알림함을 닫고 기존 단말 Push 설정 화면을 연다.

## 2026-08-03 미니PC 키오스크 503 화면 고정

- 상태: 운영 복구 완료
- 현상: 키오스크가 댄스빌보드 화면을 벗어나 `503 Service Unavailable` 화면에 고정됐다.
- 원인: 미니PC의 DHCP 주소가 기존 `172.30.1.13`에서 `172.30.1.14`로 바뀌어 이전 주소의 SSH가 타임아웃됐고, 6일간 유지된 Chrome 탭에는 과거 운영 503 응답이 남아 있었다. 디스플레이·Chrome 서비스 자체는 active/enabled 상태였다.
- 조치: mDNS `kiosk-host.local`로 현재 주소를 찾고 전용 관리 키로 접속해 `kiosk-display.service`와 `kiosk-chrome.service`를 재시작했다. 운영 문서의 접속 기준을 고정 IP보다 mDNS 우선으로 변경했다.
- 검증: 두 서비스가 모두 active/enabled, Chrome 원격 디버깅 탭 1개, 탭 제목 `댄스빌보드`, 최종 URL `https://swingenjoy.com/`, 운영 `/kiosk` HTTP 200을 확인했다.

## 2026-08-04 그루브 랩 하단 고정 메뉴 강제 노출

- 상태: 수정 및 재배포
- 현상: 새 `개발중` 그루브 랩 앱이 사용자의 메뉴 편집 선택과 무관하게 하단 고정 메뉴에 강제로 추가됐다.
- 원인: 커스텀 메뉴 앱 목록 등록과 기본 고정 메뉴 노출을 같은 작업으로 잘못 해석해 `quickMenuItems` 필수 항목에 그루브 랩을 넣었다.
- 조치: 그루브 랩은 전체 커스텀 메뉴 목록에만 `BETA` 앱으로 유지하고, 사용자가 기존 메뉴 편집 기능으로 직접 고정할 때만 하단에 나타나도록 강제 추가를 제거했다.
- 검증: 고정 메뉴 계산이 기존 항목과 혜택 메뉴만 유지하며 `groove-lab`은 `HOME_MENU_ITEMS` 카탈로그에만 존재함을 확인했다. 그루브 엔진 테스트 10개와 프로덕션 빌드가 통과했다.
- 관련 파일: `src/pages/v2/components/HomeV2MenuPanel.tsx`, `docs/decisions/2026-08-04-instrument-groove-lab.md`

## 2026-08-04 그루브 랩 모바일 스크롤 및 메뉴 정렬

- 상태: 수정 및 재배포 준비
- 현상: 그루브 랩이 모바일 셸 안에서 세로 스크롤되지 않아 아래 프리셋과 조사 근거를 볼 수 없었다. 선택 영역도 재생기 아래에 있어 리듬 탐색이 늦었고, 확장 메뉴 상단에는 외곽선이 남았으며 `강습&행사` 합성 아이콘과 문구의 가로 중앙 기준이 명시되지 않았다.
- 원인: 일반 셸은 전역 스크롤을 막지만 그루브 랩 경로에 높이·내부 스크롤 계약이 없었다. 프리셋 선택 영역은 DOM 후반의 기본 배치였고, 확장 메뉴 패널의 inset 그림자가 외곽선처럼 보였다.
- 조치: 그루브 랩 전용 셸을 뷰포트 높이로 고정하고 페이지 내부에 관성 세로 스크롤과 하단 안전 여백을 부여했다. 리듬·악기 선택을 첫 화면으로 올리고 8계열 28프리셋으로 확장했다. 확장 메뉴의 외곽 inset을 제거하고 모든 메뉴 라벨 및 `강습&행사` 합성 아이콘에 명시적 중앙 정렬을 적용했다.
- 검증: 390×844 모바일 브라우저에서 첫 화면에 리듬 선택이 노출되고, 최하단 조사 근거까지 스크롤되며, 확장 메뉴에서 `개발중`은 커스텀 목록에만 있고 `강습&행사` 아이콘·라벨이 카드 중앙에 배치됨을 확인했다. 엔진 테스트 12개와 프로덕션 빌드가 통과했다.
- 관련 파일: `src/layouts/MobileShell.tsx`, `src/styles/components/MobileShell.css`, `src/pages/groove-lab/GrooveLabPage.tsx`, `src/pages/groove-lab/groove-lab.css`, `src/pages/groove-lab/grooveEngine.ts`, `src/pages/v2/components/HomeV2MenuPanel.css`, `src/styles/theme-completion.css`

## 2026-08-04 그루브 랩 합성 음질·연구 설명 불일치

- 상태: 로컬 수정 및 근거 기반 적응형 감사 진행 중, 배포 전
- 현상: 대부분의 악기가 단일 파형과 매 타격마다 새로 만든 난수에 의존해 반복 음색이 얇고 동시 타격의 출력 보호가 부족했다. 판데이루 화면은 비균등 삼바 미세타이밍을 설명했지만 실제 이벤트는 균등 16분음표였고, 드러머 라이드에서 측정된 스윙 곡선의 적용 범위도 솔리스트와 구분되지 않았다.
- 원인: 첫 버전은 리듬 위치 검증을 우선해 음원 모델과 출력단을 최소 구현으로 두었다. 연구 근거가 설명과 이벤트 생성·합성 파라미터까지 연결되는 자동 감사가 없었다.
- 조치: 발현 현악 합성, 다중 모드 타악 합성, 재사용 결정적 노이즈, DC 차단·다이내믹 압축 출력단을 분리 구현했다. 삼바 실연의 23.0/23.6/22.8/30.7% 길이를 정규화한 onset 기준을 실제 이벤트에 반영하고, 스윙 UI를 드러머 라이드 측정 모델로 한정했다. 출처 없는 전역 톤 EQ는 제거하고 측정값이 있는 보이스 전용 필터만 유지했다. 내부 수치 탐색 100스텝을 연구 감사 100회처럼 표시하던 문구와 테스트를 제거했다.
- 초기 감사 한계: 네 모델의 감쇠·밝기를 내부 목표로 조정한 100스텝은 논문 검토나 청취 평가가 아니었다. 이 숫자는 연구 회차에서 제외하고 역사 기록에 한계를 명시했다.
- 검증: 그루브 단위 테스트 33개가 통과했다. Chromium `OfflineAudioContext`에서 17보이스 45개 단일 렌더, 28개 프리셋 1~2마디 전체 루프, 22개 발음 쌍 기술량을 검사했다. 전체 믹스 최대 피크 0.788751에서 클리핑이 없고, 픽업 필터 뒤 현 조율 최대 오차는 0.2784 cents다. 화면의 고유 근거 URL 44개를 실제 요청해 자동 접근 39개, 출판사 자동화 제한 5개, 실제 끊김 0개를 확인했다.
- 관련 파일: `src/pages/groove-lab/grooveAudio.ts`, `src/pages/groove-lab/grooveQuality.ts`, `src/pages/groove-lab/grooveEngine.ts`, `src/pages/groove-lab/GrooveLabPage.tsx`, `docs/research-data/groove-lab-quality-audit-2026-08-04.md`

## 2026-08-04 Instagram 캘린더 릴스 게시 성공 오판

- 상태: 수정 및 실제 게시 상태 복구 완료
- 현상: 화요일 12:30 예약 실행이 오늘 캘린더 영상을 생성하고 Instagram `Share`까지 수행했지만 10분 뒤 `verification-required`와 exit 1로 끝나 자동 게시가 실패한 것처럼 보고됐다.
- 원인: 게시 직후 Instagram이 새 릴스 화면과 알림 권한 요청을 표시했는데, 검증기는 현재 화면에서 하단 `Profile` 탭만 찾았다. 프로필 화면으로 돌아가지 못해 게시물 수를 읽지 못했으며, `verification-required` 상태도 다음 수동 실행에서 자동으로 차단되지 않아 중복 게시 위험이 있었다.
- 조치: 공유 뒤 20초 동안 업로드의 백그라운드 전환을 기다리고, 정확한 `korea_swing_social` 프로필 딥링크를 열어 권한창을 처리한 뒤 게시물 수를 검증한다. `sharing`과 `verification-required` 상태는 다음 실행에서 먼저 현재 게시물 수와 이전 기준값을 대조하고, 증가가 확인되면 재게시 없이 `published`로 복구하며 증가가 없으면 명시적 강제 복구 전까지 차단한다.
- 검증: 오늘 게시 전 기록은 16건이었고 실제 Instagram 프로필은 17건이며, 계정의 최신 릴스에서 오늘 캘린더 영상과 `4 hours ago` 표시를 확인했다. 수정된 예약 실행을 같은 날짜로 다시 수행해 4.5초 만에 새 게시물을 만들지 않고 기존 실행을 `published / profile-post-count`로 복구했다. LaunchAgent도 재기동해 `already-published`로 중복 없이 종료했고 실행 횟수 2, 마지막 종료 코드 0을 확인했다. 소셜 릴스 테스트 20개가 통과했다.
- 관련 파일: `scripts/social-reels/instagram-reel-adb.mjs`, `scripts/social-reels/run-scheduled-social-reel.mjs`, `scripts/social-reels/instagram-reel-adb.test.mjs`, `docs/social-reel-automation.md`

## 2026-08-04 그루브 랩 악기 모델 과장 및 반복 연습 부족

- 상태: 적응형 개선 진행 중, 배포 전
- 현상: 라이드의 스윙 비율이 네 박 리듬 기타에도 적용되는 것처럼 보였고, Four-to-bar 기타는 완전한 3음 코드를 반복했다. 블루 노트 기타형 벤드는 단순 톱니파였으며 반복 연습은 계속 듣기만 가능했다. 모든 프리셋에 동일한 현·타악 합성 근거를 붙여 실제 적용 범위도 구분되지 않았다.
- 원인: 첫 버전은 패턴 탐색 범위를 먼저 넓히며 악기별 발음 모델, 주장 단위 출처 연결, 능동 반복 훈련을 분리하지 않았다. 공식 공개 합성 구현과 앱의 축약 범위를 대조하는 게이트도 없었다.
- 조치: 라이드 비율을 실제 분할 제어가 있는 프리셋으로 제한했다. 플럭 위치·두 편극 현, 주법별 베이스, 해머 현, 다중 모드 타악을 도입했다. 공식 STK·Faust 구현과 연결 논문을 대조해 기타의 몸통 IR·현 결합 부재를 화면에 명시했다. 블루 노트는 현 버퍼 전체 피치 벤드, Four-to-bar는 대학원 전사 분석에 따른 한 음 중심·뮤트 질감으로 수정했다. 이후 STK 원코드 재검토로 플럭 콤의 편도 지연 계산을 교정하고, 피아노에는 음역별 1/2/3현 미세 비팅, 전기 베이스에는 업라이트와 분리된 자기 픽업 위치 콤 필터를 추가했다. 실제 Clavinet D6 측정에 맞춰 매 5번째 배음 픽업 노치를 넣고 두 픽업 선택·비선형·톤 스위치 부재를 구분했다. 2+2 콜백과 +2 BPM 사다리 연습, 프리셋 주장 범위·출처 등급 자동 감사를 유지한다.
- 검증: 그루브 테스트 26개, 관련 lint, 프로덕션 빌드가 통과했다. Chromium OfflineAudioContext의 17보이스 42케이스는 피크 0.026706–0.258072, 유한값·비무음·무클리핑이었다. 390×844 실제 브라우저에서 내부 스크롤, 기타의 `PULSE`, 2+2 무음 전환, 모델 포함/제외 범위와 대학원 논문 링크 노출을 확인했다.
- 추적: 실제 수정과 브라우저 검증이 연결된 회차만 `docs/research-data/groove-lab-adaptive-1000-ledger-2026-08-04.md`에 누적하며 현재 9/1000이다.
- 관련 파일: `src/pages/groove-lab/GrooveLabPage.tsx`, `src/pages/groove-lab/groove-lab.css`, `src/pages/groove-lab/grooveAudio.ts`, `src/pages/groove-lab/grooveEngine.ts`, `src/pages/groove-lab/grooveEvidence.ts`, `src/pages/groove-lab/grooveEvidence.test.ts`

## 2026-08-04 Android 화면 복귀 시 `Failed to fetch` 전역 오류창

- 상태: 수정 및 검증 완료, 배포 준비
- 현상: Android Chrome에서 `/groove-lab`을 열어 둔 채 다른 화면으로 나갔다가 돌아오면 `TypeError: Failed to fetch`가 전역 치명 오류창으로 표시됐다.
- 원인: 자유게시판 미읽음 배지의 `focus`·`visibilitychange` 리스너가 화면 복귀 즉시 `/api/board/free/unread`를 호출했고, Android 네트워크가 아직 안정되지 않은 순간의 fetch 거부를 처리하지 않아 `unhandledrejection`으로 전파했다. 알림 IndexedDB의 구버전 탭 경합도 일부 비동기 호출에서 같은 전역 경로로 전파될 수 있었다.
- 조치: 복귀 배지 조회를 800ms 디바운스하고 조회 실패 시 기존 상태를 유지한다. 전역 오류 정책은 동적 청크 실패와 일반 앱 오류는 기존대로 처리하면서 일시적 네트워크 실패와 정확한 IndexedDB 하위 버전 경합만 비치명으로 분류한다. 알림 저장소는 IndexedDB를 열 수 없을 때 서버 알림함 fallback을 사용하며 오래된 로컬 알림 정리 실패도 명시적으로 처리한다.
- 검증: 오류 정책 8개와 로그인 사용자의 화면 복귀 fetch 실패 회귀 테스트 1개가 통과했다. 프로덕션 빌드가 성공했다. 411×803 모바일 브라우저에서 API 연결을 끊은 뒤 다른 탭에서 돌아오는 조건을 재현했고, 기존 화면이 유지되며 `#crash-fallback-overlay`가 0개임을 확인했다.
- 관련 파일: `src/hooks/useFreeBoardUnreadCount.ts`, `src/utils/globalErrorPolicy.ts`, `src/main.tsx`, `src/lib/notificationStore.ts`, `src/App.tsx`

## 2026-08-09 PWA 혼합 버전 오류 및 로컬 자동화 공백 재검토

- 상태: 원인 재현·수집 보강·Instagram 상태 복구 완료. PWA 구조 수정 및 운영 배포 완료
- PWA 현상: Android Chrome에서 `The requested version (1) is less than the existing version (2)`와 `Failed to fetch` 전역 오류창이 배포 전후 반복됐다.
- PWA 구조 원인: 앱은 `notification-history` DB v2, 서비스워커는 같은 DB v1을 연다. 서비스워커와 PWA 등록은 새 버전을 즉시 활성화·리로드하고 구 캐시를 정리하며, 배포는 `rsync --delete`로 구 해시 자산을 즉시 삭제한다. 존재하지 않는 `/assets/*` 요청도 SPA fallback이 `index.html`을 200 HTML로 반환한다. 이 조합은 구버전 탭과 신버전 서비스워커·서버가 섞이는 정상적인 배포 중첩 구간을 지원하지 않는다.
- PWA 재현: 격리 브라우저에서 현재 방식인 즉시 활성화·구 캐시/구 자산 삭제 뒤 구버전 탭의 지연 import가 `FAILED:TypeError`로 실패했다. 새 서비스워커를 대기시키고 구 자산을 유지한 비교군은 `lazy-1-ok`로 성공했다. 앱 DB v2를 연 뒤 v1 경로를 열면 `VersionError`, v2로 정렬하면 성공했다. 운영의 과거 해시 `assets/index-CuhdXVuW.js` 요청도 HTTP 200 `text/html`로 확인됐다.
- PWA 제안: 직전 릴리스 중첩 보존, `/assets/*` 정확한 404, 자산 선업로드·검증 후 진입 파일 전환, 단일 갱신 코디네이터, 서비스워커의 알림 DB 소유 제거를 불변 조건으로 정리했다. DB 숫자만 올리거나 캐시 초기화를 강화하는 단기 처방은 다음 마이그레이션·배포에서 재발하므로 채택하지 않았다.
- PWA 재발 확인: 2026-08-10 운영 로그에서 앱 부팅 직후 `[MobileShell]`과 `[App]`의 알림 조회가 다시 `The requested version (1) is less than the existing version (2)`로 실패하고 같은 오류가 `unhandledrejection`으로 전파됐다. 당시 `Online: true`였으므로 네트워크 단절이 아니라 운영에 남은 동일 IndexedDB 소유권 충돌의 재발로 판정했다.
- PWA 구조 수정: 운영 알림함의 유일한 원본을 서버 `user_notifications`로 고정하고 앱·서비스워커의 `notification-history` IndexedDB 사용을 모두 제거했다. 서비스워커는 운영체제 Push 표시와 서버 출처 식별자 전달만 담당하며, 관리자 미리보기는 새로고침 시 사라지는 메모리 저장소만 사용한다. 알림 조회·읽음은 캐시하지 않는 서버 API로 통일했다.
- PWA 릴리스 수정: 클라이언트 해시 자산을 진입 파일보다 먼저 올리고 구 해시를 즉시 삭제하지 않으며, 없는 `/assets/*`는 SPA HTML이 아닌 정확한 404로 종료한다. PWA 등록은 `prompt`로 바꾸고 설치 단계의 무조건 `skipWaiting()`을 제거했다. 기존 모달·입력·숨김 탭 보호를 유지하는 단일 갱신 코디네이터가 안전 시점에만 대기 중 서비스워커를 활성화하고 리로드한다.
- PWA 검증 및 배포: 서버 알림 조회·읽음, 서비스워커 저장소 비소유, 배포 순서·404·안전 활성화 계약, 갱신 재시도 및 전역 오류 정책을 포함한 관련 테스트 98개가 통과했다. 커밋 `ad681a7e`를 `origin/main`에 푸시하고 Cafe24에 배포했다. 운영 서비스와 내부·외부 헬스는 정상이고 공개 버전은 `2026-08-10T03:45:17.762Z`다. 공개 서비스워커에서 `notification-history`, `indexedDB`, `notification_local_id`가 모두 0건이며, 새 진입 자산과 8월 4일 구 해시 자산은 HTTP 200, 없는 `/assets/*`는 `text/plain` HTTP 404임을 확인했다.
- 예약 수집 감사: 2026-08-05~09에는 매일 08·09·10·11시 네 LaunchAgent 실행이 모두 종료 코드 0으로 남아 있어 3일 전체 미실행은 아니었다. 다만 컴퓨터 전원 종료 공백을 소스별 마지막 성공 위치에서 이어가는 cursor가 없고, 우선순위 1은 최신 2개·우선순위 2는 최신 1개만 보는 고정 범위라 여러 게시물이 올라오면 누락될 구조적 가능성이 있다.
- 수집 보강: 기준 테스트와 source guard 통과 후 우선순위 1·2 공식 Instagram 33개 소스를 게시물 4개 범위로 두 번에 나눠 실행했다. `20260809_174023_43759`는 신규 후보 2건을 저장하고 기존 보류 1건을 포함한 자동등록 3건을 성공시킨 뒤 남은 16개를 기록했다. `20260809_175753_45982`는 그 16개만 이어서 확인해 신규 0건, 남은 소스 0, 종료 코드 0으로 끝났다. 운영 일정 API에서 8월 12일 소셜클럽 `DJ 쵸리`, 8월 9일 네오스윙 `DJ 조커`, 스윙타임 `DJ 메이져` 반영을 확인했다.
- Instagram 복구: 8월 8일 실행은 Share를 눌렀지만 게시물 수 검증이 끝나지 않아 `verification-required`였다. 같은 날짜 안전 복구를 실행해 재게시 없이 프로필 게시물 수 17→18을 확인하고 `published / profile-post-count`로 복구했다. 소셜 릴스 Node 테스트 20개가 통과했다.
- Telegram 승인 검토: 릴스 전용 봇은 webhook 없음, pending update 0이며 Codex 대화용 봇과 분리돼 있다. 최종 Share 화면·릴스 미리보기를 보내고 단일 chat ID, 난수 nonce, 10~15분 TTL, 일회용 inline `게시 승인`/`취소` callback을 사용하는 fail-closed 상태머신이 가능하다. 승인·검증 전에는 Share를 누르지 않고 거절·시간초과·재시작은 자동 게시하지 않는 구조를 권고했다.
- 관련 기록: `docs/decisions/2026-08-09-pwa-release-consistency.md`, `docs/decisions/2026-08-09-local-automation-catchup-and-approval.md`

### 2026-08-11 구 PWA 탭의 DB VersionError 선발생 후 지연 정상화

- 상태: 구조 수정·운영 배포·검증 완료
- 현상: Android Chrome의 오래 열린 탭을 전면으로 가져오면 `The requested version (1) is less than the existing version (2)` 오류 화면이 먼저 나타나고 약 1초 뒤 사이트가 정상 로딩됐다. 새로 생성한 로그에도 과거 오류가 남아 같은 오류가 계속 발생한 것처럼 보였다.
- 운영 근거: 2026-08-10 14:39:08 UTC 오류 시점의 reload diagnostic은 실행 중 클라이언트를 빌드 `1785315591396`(2026-07-29, 커밋 `95cf4ff6`)으로, 서버를 `1786342147739`로 기록했다. 14:39:09에 리로드가 실행된 직후 현행 코드만 사용하는 `/api/notifications?unread=1` 요청이 성공했다. 14:40:53에 복사된 로그의 오류 항목 시각은 여전히 14:39:08이므로 리로드 뒤 새 오류가 아니라 localStorage에서 복원된 이전 부트 기록이었다.
- 직접 원인: 7월 29일 번들은 모듈 평가 시 `notification-history` v1을 열고, 이미 다른 탭이 v2로 올린 DB와 충돌했다. App의 처리되지 않은 `deleteOld()` 거부가 전역 오류 화면을 만들었고 기존 버전 폴러가 1.5초 뒤 현행 번들로 리로드했다. 이미 메모리에 실행 중인 구 JS에는 이후 배포한 catch와 DB 소유권 제거가 적용될 수 없었다.
- 전환 교착: 구 `autoUpdate` 등록 코드는 새 `prompt` 서비스워커에 `SKIP_WAITING`을 보내지 않는데 새 서비스워커도 설치 시 자동 활성화를 중단해, 구 탭이 남은 브라우저에서 새 worker가 영구 waiting 상태가 될 수 있었다. 전체 JS·HTML 프리캐시와 활성화 시 무조건 `clients.claim()`도 이전 앱 셸 재부팅 가능성을 남겼다.
- 구조 조치: CacheStorage 영속 marker를 둔 1회용 호환 브리지가 기존 active worker가 있는 브라우저에서만 강제 활성화·선점하고 모든 안전한 구 탭을 복구 URL로 이동시킨다. marker 완료 뒤 이후 릴리스는 다시 `prompt` 대기와 앱 코디네이터의 안전 활성화를 사용한다. 서비스워커의 HTML·JS·CSS 프리캐시와 navigation fallback을 모두 제거하고 Web Share Target·Push만 유지했다.
- 부트·복구 조치: `index.html` 진입점을 가벼운 pre-React bootstrap으로 바꿔 서버 빌드가 현재 번들과 다르면 React와 알림 코드를 평가하기 전에 캐시 정리·worker 갱신·cache-busted navigation을 수행한다. 오류 화면의 초기화 버튼은 worker unregister, 구 앱 셸 캐시와 `notification-history` 삭제를 모두 기다린 뒤 재시작한다. 자동 복구가 두 번 실패하면 자동 unregister·reload를 반복하지 않고 수동 재시도 화면에서 멈춘다.
- 진단 조치: 클라이언트 로그에 build ID와 boot ID를 기록한다. 현재 부트와 이전 부트를 복사 텍스트에서 분리하며 오류 점·오류/경고 수는 현재 부트만 계산한다.
- 로컬 검증: PWA 릴리스 계약, 1회 브리지 조건, 코디네이터 동시성·재시도, 구 캐시·DB 정리, 로그 부트 경계를 포함한 Vitest 30개가 통과했다. 실제 Chromium에서 7월 29일 worker가 제어하는 구 탭 2개, DB v2, 구 runtime·Workbox 캐시를 만든 회귀도 통과했다. 두 탭이 약 3.5초 안에 최신 `__APP_STARTED`로 수렴했고 구 캐시는 삭제, Share·무관 캐시는 보존됐으며 `notification-history` 삭제 뒤 v1 open이 성공하고 VersionError·crash overlay는 0건이었다. 이 회귀가 activate에서 `client.navigate()` 완료를 기다리던 교착을 검출했고 navigation을 예약만 하도록 고친 뒤 통과했다. 프로덕션 빌드가 성공했고 생성 서비스워커는 약 6.4KB, inject manifest 1개이며 앱 자산 프리캐시·cached index fallback·알림 DB open이 없음을 확인했다.
- 운영 배포 및 검증: 수정 커밋 `d3850c6a`를 `origin/main`에 푸시한 뒤 동일 커밋의 clean worktree에서 Cafe24에 배포했다. 공개·서버·배포 원본의 버전은 `1786378011134`로 일치하고 서비스워커 SHA-256도 `49b34b66fd03828be33dd211ddeffc65d55c958a76bcc416ebed5eb8acd72d27`로 모두 같았다. 공개 서비스워커는 `no-store`이며 전환 marker를 포함하고 앱 자산 프리캐시·cached navigation·`notification-history` 문자열은 0건이다. 외부 헬스와 서비스 상태는 정상, 미존재 자산은 `text/plain` HTTP 404다. 운영 Chromium 스모크 테스트에서 앱 본문과 active worker를 확인했고 VersionError·치명 오류 화면·관련 console/page error는 0건이었다.
- 관련 파일: `src/bootstrap.ts`, `src/lib/pwaRecovery.ts`, `public/service-worker.js`, `vite.config.ts`, `src/lib/pwaUpdateCoordinator.ts`, `src/components/DeploymentAutoRefresh.tsx`, `src/utils/clientLogBuffer.ts`, `src/main.tsx`

## 2026-08-11 오늘 일정 알림의 비개최 일정 혼입 및 설정 오류 오표시

- 상태: 시작일 기준 재수정·운영 배포·검증 완료
- 현상: 2026-08-11 아침 Push와 알림함에 실제 화요일 개최 일정뿐 아니라 목요일·토요일 반복 수업과 7월/9월 구간 축제까지 포함돼 `오늘 일정 11개`로 표시됐다. 일정 카드에는 7월의 시리즈 시작일이 나타나 과거 등록분을 다시 보낸 것처럼 보였다. 설정 API가 실패하면 저장된 계정 설정 대신 기본 OFF 화면을 표시할 수도 있었다.
- 원인: 알림 서버가 명시 개최일 `event_dates`에 오늘이 없더라도 `start_date <= 오늘 <= end_date`이면 개최로 인정했다. 캘린더는 명시 개최일을 우선하므로 화면과 알림의 날짜 계약이 달랐다. 알림함은 오늘 요약 날짜 대신 이벤트 시리즈 시작일을 표시했고, 설정 모달은 조회 성공 여부와 설정값 표시 상태를 구분하지 않았다.
- 1차 오판: `event_dates`를 실제 개최 회차의 원본으로 보고 11건을 6건으로 줄였지만, 제품이 요구한 오늘 일정은 “오늘 회차”가 아니라 “시작일이 오늘”인 일정이었다. 이 때문에 7/7 시작 엘라스틴·JAZZBUZZ와 7/28 시작 한보가 8/11 목록에 계속 남았다.
- 최종 조치: `event_dates`와 종료 범위를 확장하지 않고 기본 시작일이 요약 날짜와 같은 일정만 포함한다. 8/11 운영 데이터 기준 대상은 `DJ 후안`, `New✖Balup`, `경성홀 화요 소셜` 3건이다. 카드와 설정 문구도 `오늘 시작`으로 명확히 바꾼다. 설정 조회 실패 시 스위치·저장 UI를 숨긴 재시도 화면을 표시하며, 해제 API 실패 시 모달을 성공으로 닫지 않는다. 새 v2 보정 marker가 기존 미확인 오늘 알림을 다시 계산한다.
- 검증: 관련 Vitest 90개와 보정 Node 테스트 3개가 통과했다. 운영 DB의 2026-08-11 일정에 배포된 판정 함수를 직접 적용한 결과 시작일이 같은 `DJ 후안`, `New✖Balup`, `경성홀 화요 소셜` 3건만 남고 7/28 시작 한보를 포함한 이전 시작 다회차 일정은 모두 제외됐다. v2 보정 marker 적용을 확인했다. 수정 커밋 `571e2bd6`를 `origin/main`에 푸시한 뒤 Cafe24에 배포했으며 공개·서버·배포 원본 버전은 `1786410214890`, 서비스워커 SHA-256은 `5c7f1b8bf2213f397f4f07373986628c678a1776451928cfaee6d8d159eb6879`로 일치한다. 외부 헬스와 `swingenjoy`, `crond`, `httpd` 서비스가 모두 정상이다.
- 관련 결정: `docs/decisions/2026-08-11-notification-delivery-lifecycle.md`
- 관련 파일: `server/cafe24/push-api.js`, `scripts/reconcile-daily-notification-occurrences.mjs`, `src/components/NotificationHistoryModal.tsx`, `src/components/NotificationSettingsModal.tsx`, `scripts/deploy-cafe24.sh`

## 2026-08-09 지난 금요일 정규 소셜 자동 제거

- 상태: 원인 확인, 수정·배포 전
- 현상: 8월 월간 캘린더에서 지난 금요일인 2026-08-07의 소셜이 전부 비어 보였다.
- 운영 확인: 8월 7일 소셜은 운영 일정 API에 0건이다. 반면 8월 14일에는 `Busan Balboa Social`, `네오스윙 금요 소셜`, `드림발 금요 소셜`이 있고, 8월 21일·28일에도 네오스윙·드림발 금요 소셜이 남아 있다.
- 원인: 정규 소셜 조정기가 `regular-social:*` 자동 생성 일정 중 `eventDate < today`인 행을 제거 목록에 넣고 실제 `events`에서 삭제한다. 따라서 날짜가 지난 정규 소셜은 월간 캘린더의 역사에서 사라진다. 8월 9일 확대 재수집은 legacy cleanup을 스킵했고 삭제 건수도 0이므로 이번 수집 실행이 원인이 아니다.
- 판단: 미래 90일 롤링 생성과 과거 일정 보존을 같은 정리 조건으로 처리한 구조 문제다. 월간 캘린더가 지난 일정을 보여야 한다면 자동 생성 정규 소셜도 과거 보존 기간 또는 월 경계까지 유지하도록 별도 정책이 필요하다.
- 관련 파일: `server/cafe24/regular-social-reconciler.js`, `server/cafe24/regular-social-rules.js`

## 2026-08-10 캘린더 소셜 유무 날짜의 첫 이벤트 윗선 불일치

- 상태: 재배포 및 운영 검증 완료
- 현상: 소셜이 없는 날짜의 첫 일반 이벤트 윗선이 인접 날짜의 초록색 소셜 라운드 컨테이너가 아니라 그 위로 돌출된 `소셜 N` 뱃지 윗선에 맞춰져 보였다.
- 조치: 소셜이 있는 날짜와 없는 날짜를 셀 본문 클래스에서 구분하고, 소셜 라운드의 상단 여백과 소셜이 없는 날짜의 첫 일반 이벤트 상단 여백이 같은 CSS 변수 하나를 사용하도록 통일했다. 뱃지의 돌출 위치는 유지했다.
- 검증: 레이아웃 계약 테스트와 프로덕션 빌드가 통과했다. Chromium 실측에서 일반 이벤트 윗선과 소셜 라운드 윗선의 차이는 390px 모바일과 1024px 데스크톱 모두 0px였고, 뱃지는 라운드보다 각각 5px·6px 위에 유지됐다. 대상 ESLint는 오류 0건이며 기존 경고만 남았다.
- 연속 일정 색상 후속: 연속 일정 막대가 모두 `event` 분류의 단일 색을 공유하던 구조를 분리했다. 단일 다일 일정은 이벤트 ID, 여러 행을 묶은 일정은 제목·장소·주최자 기반 시리즈 키를 해시해 고정 HSL 색상을 직접 생성한다. 같은 일정은 월·필터·정렬 순서 및 주 경계가 바뀌어도 동일한 색을 유지하며, 제한된 팔레트 인덱스 충돌로 다른 일정이 같은 색을 받는 문제도 피한다.
- 연속 일정 검증: 이벤트 ID와 시리즈 키의 결정적 색상 배정 및 월별 정렬 인덱스 비의존 계약 테스트를 추가했다. 캘린더 레이아웃 테스트와 색상 단위 테스트 및 프로덕션 빌드가 통과했다.
- 운영 검증: 커밋 `751c5fa1`을 `origin/main`에 푸시하고 Cafe24에 배포했다. 공개·서버 버전은 `2026-08-10T05:05:24.530Z`로 일치하고 서비스와 헬스 체크가 정상이다. 운영 9월 화면에서 서로 다른 연속 일정 2건이 서로 다른 고정 HSL 색상으로 표시됐으며, 8월 오늘 이동 후 표시 중복 없음, `+N 더보기` 0건을 확인했다.
- 오늘 표시 중복 후속: 모바일에서 오늘 버튼 또는 주간 스크롤 정착 위치로 이동하면 고정 요일줄의 파란 오늘 원과 그 아래 실제 날짜 원의 하단이 동시에 보였다. 콘텐츠 기준 스크롤의 의도된 5px 여백은 유지하되, 고정 컨트롤이 그 경계 여백을 같은 배경색으로 덮어 실제 날짜 원이 비치지 않게 했다.
- 오늘 정렬 재검증: 551px 운영 화면을 좌표로 다시 측정한 결과 오늘 날짜 칩이 일반 날짜보다 6px 높은 탓에 오늘 칸의 첫 일반 이벤트가 소셜 라운드보다 6px 내려갔고, 이를 가리던 5px 고정 컨트롤 그림자가 소셜 뱃지 상단 2px도 함께 덮었다. 모바일 셀 헤더를 14px로 고정하고 오늘 칩을 흐름 높이와 분리했으며, 추가 가림막 없이 셀 본문을 고정 요일줄 바로 아래에 정착시켰다. DB 연결 로컬 화면에서 390·420·551·720px 모두 일반 이벤트와 소셜 라운드 윗선 차이 0px, 소셜 뱃지 완전 노출, 오늘 칩 비노출을 확인했다.
- 정렬 재배포 검증: 커밋 `7881c5f6`을 `origin/main`에 푸시하고 Cafe24에 재배포했다. 공개·서버 버전은 `2026-08-10T05:28:07.389Z`로 일치하고 서비스와 헬스 체크가 정상이다. 운영 화면의 359px 및 551px 오늘 이동 상태에서 일반 이벤트와 소셜 라운드 윗선 차이 0px, 소셜 뱃지 완전 노출, 오늘 칩 비노출, 고정 컨트롤 그림자 없음, `+N 더보기` 0건을 확인했다.
- 날짜 칸 표시 후속: 모바일의 5개 제한과 `+N 더보기`를 제거해 기간 막대로 대체되는 중복 항목 외에는 날짜별 모든 일정을 셀에 직접 렌더링한다.
- 관련 파일: `src/pages/calendar/components/FullEventCalendar.tsx`, `src/pages/calendar/styles/FullEventCalendar.css`, `src/pages/calendar/styles/CalendarPage.css`, `src/pages/calendar/components/FullEventCalendar.layout.test.ts`, `src/pages/calendar/utils/calendarSpanTone.ts`, `src/pages/calendar/utils/calendarSpanTone.test.ts`

## 2026-08-10 과거 혜택·대관 가능일정 오수집 및 예약 실행 누락

- 상태: 구조 수정·로컬 예약 설정 반영 및 배포 준비 완료
- 현상: 무료·할인·정기권 후보에 `evergreen` 또는 `ongoing_sale`이 붙으면 과거 후보 날짜와 오래된 원문 게시일을 통과했다. 경성홀 Instagram 게시물 `Damg6VxktkS`처럼 실제 이벤트가 아닌 대관 가능일정도 이벤트 후보로 분류될 수 있었다.
- 원인: 상시 혜택 판정이 혜택 종류 분류뿐 아니라 미래 날짜·원문 신선도 검사의 예외로도 사용됐다. 비이벤트 운영 공지 중 대관 가능일정에 대한 결정적 제외 규칙이 없었고, 서버 수집 API는 수집기 검증 결과를 신뢰해 동일 정책을 재검사하지 않았다.
- 조치: 후보 날짜는 혜택 생명주기와 관계없이 오늘 이후만 허용하고, 오래된 혜택 원문에도 `evergreen` 예외를 제거했다. 과거 상시 혜택은 관리자 혜택 탭에서도 숨긴다. `대관/공간·홀·연습실·스튜디오 대여`와 `가능일·가능일정·빈 시간·스케줄·캘린더`가 같은 문맥에 있는 게시물은 비이벤트 공지로 제외한다. 일반 이벤트의 `사전 예약 가능` 문구는 이 규칙에서 제외한다. 이 정책을 수집기 중앙 검증과 서버 후보 저장 단계가 함께 사용하며, 이미 저장된 미수집 대관 공지도 관리자 후보 목록에서 숨긴다.
- 예약 실행 원인: 메인 Mac 절전으로 우선순위 실행이 중간에 멈춘 뒤 남은 소스가 있어도 종료 코드 0을 반환했고, 뒤 예약은 전역 잠금과 충돌했다. 미니 PC는 키오스크 표시 장치이며 수집을 실행하지 않는다.
- 신뢰성 조치: 우선순위별 남은 소스와 마지막 완주 시각을 체크포인트하고 다음 실행에서 남은 소스를 먼저 확인한다. 공백 기간에는 Instagram 게시물 확인 범위를 최대 8개까지 확대한다. 부분 실행은 종료 코드 75를 반환하고 네 LaunchAgent를 `caffeinate -dimsu`로 감싸 실행 중 절전을 막는다.
- 품질 조치: 모든 후보에 실제 이미지를 의무화하고, 소셜은 DJ 또는 구체적 운영 근거를 요구한다. 설명 문장형 제목을 차단하고 같은 소스·날짜·장소의 DJ 부분집합 후보는 더 완전한 한 건으로 병합한다. AI는 이 결정 규칙 뒤 자동등록 직전의 최종 안전판으로만 유지한다.
- 검증: `node scripts/test-ingestion-standards.mjs`가 통과했고, 경성홀 보고 URL의 대관 가능일정 차단과 미래 예약 가능 원데이 클래스 허용 회귀 사례를 포함한다. AI 판정·자동등록·공식 일정 중복 테스트 31개가 통과했다. 경성홀·네오스윙·스윙타임 실제 Instagram 경로 무저장 실행에서 15건을 품질 규칙으로 제외하고 저장 후보 0건, 접근 오류 0건을 확인했다.
- 관련 파일: `server/cafe24/ingestion-candidate-policy.js`, `server/cafe24/ingestion-date-expansion.js`, `server/cafe24/function-api.js`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/benefit-search-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`

## 2026-08-11 수집 부분 실행 실패 오표기 및 경성홀 이벤트 누락

- 상태: 원인 수정 및 운영 후보 갱신 완료
- 현상: 08:00·09:00 우선순위 수집이 각각 마지막 소스 1개를 남기고 종료 코드 75를 반환하면서 Telegram에는 `이벤트 수집 실패 / 중복 실행 차단`으로 표시됐다. 경성홀 공식 Instagram의 2026-08-15 `광복절 특별 워크숍`도 실제 게시물과 원본급 이미지가 있는데 후보로 저장되지 않았다.
- 원인: 새 진행상태에 완주 시각이 없으면 기본 2개 대신 Instagram 게시물 4개를 확인해 20분 예산을 반복 초과했다. 운영 래퍼는 부분 실행용 종료 코드 75를 과거의 중복 실행 코드로만 해석했다. 경성홀은 `계정명 on Instagram: "캡션"` 제목 형식의 첫 줄을 파싱하지 못했고, 본문의 후속 소셜 문구가 주 활동을 소셜로 뒤집었다. 프로필 화면에 섞인 다른 계정 추천 URL도 경성홀 확인 개수를 소모했다.
- 조치: 완주 이력이 없는 최초 상태에서는 설정된 게시물 수를 그대로 사용하고 실제 완주 공백이 계산될 때만 확인 범위를 늘린다. 종료 코드 75와 `remaining sources`가 함께 있으면 운영 알림을 실패가 아닌 부분 완료·다음 회차 재개로 표시한다. Instagram 양쪽 제목 형식에서 캡션 첫 줄을 추출하고, 그 첫 줄이 워크숍·강습이면 본문 후반 소셜 문구보다 강습 분류를 우선한다. 계정명이 포함된 permalink는 대상 handle과 일치하는 것만 프로필 확인 창에 포함한다.
- 검증: 수집 표준 테스트와 Node 구문 검사, 운영 래퍼 `bash -n`, 변경 공백 검사가 통과했다. 경성홀 단독 무저장 실행에서 대관 가능일정은 계속 제외되고 `광복절 특별 워크숍`은 `class / 경성홀 / 2026-08-15` 후보 1건으로 통과했다. 이어 Cafe24 공식 수집 API 실실행에서 후보 ID `7abe53e13177a57b`가 `refreshed`로 확인되어 원문 이미지와 정규화 데이터가 운영 후보에 갱신됐다. 오늘 예산으로 빠졌던 LQ스튜디오·KP댄스홀도 별도 실행해 남은 후보가 없음을 확인하고 우선순위 1·2 진행상태를 완주로 마감했다.
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/benefit-search-utils.mjs`, `scripts/ingestion/ingestion-progress.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `/Users/inteyeo/scripts/run-ingestion.sh`

## 2026-08-11 경성홀 소셜 자동등록 중단 및 과거 게시물 미래 일정 오인

- 상태: 구조 수정·운영 정리·배포 검증 완료
- 현상: 경성홀 공식 경로에서 날짜·DJ·장소가 명확한 소셜을 수집해도 자동등록되지 않았다. 오래된 무료 행사와 종료된 모집 게시물의 연도 없는 날짜는 수집일 기준 2027년으로 바뀌어 신규 후보에 나타났고, 고정 최신 N개 확인 방식 때문에 게시물이 연속으로 올라오면 아직 확인하지 않은 글이 수집 창 밖으로 밀릴 수 있었다.
- 원인: 수집기가 결정 규칙을 통과한 소셜도 AI로 다시 판정해 AI 오류·재검토 결과로 `ready`를 취소했고, 서버는 AI 승인과 0.98 신뢰도를 필수로 요구했다. 생성형 정규 소셜 중복을 실제 게시물로 교체하는 경로에도 같은 AI 조건이 중복 적용됐다. 연도 없는 날짜는 원문 게시일이 아닌 현재 수집일로 연도를 정했고, 소스별 확인 게시물 체크포인트가 없었다. 혜택 후보는 AI 호출 실패만으로 저장 전에 유실됐다.
- 조치: 날짜별 소셜 근거에 날짜·고정 또는 원문 장소·DJ·소셜 표지가 모두 있으면 결정 규칙과 서버가 원문 근거를 직접 재검증해 자동등록한다. AI는 비정형·애매한 자동등록 후보와 혜택의 보조 판정으로만 사용하며, 혜택은 근거가 있는 명시적 거절만 버리고 AI 오류·불가·재검토는 관리자 후보로 보존한다. 연도 없는 날짜는 원문 게시일과 가장 가까운 연도로 맞추고 서버도 게시일 대비 400일 초과 및 180일을 넘긴 혜택 원문을 차단한다. Instagram은 소스별 확인 URL을 보존해 최신 고정 개수가 아니라 아직 확인하지 않은 게시물부터 처리하며, 후보 저장 또는 자동등록 오류가 없는 경우에만 체크포인트를 확정한다.
- 운영 정리: 과거·후기·은퇴 소스에서 잘못 생성된 후보 8건과 연결 후보 이미지를 운영 API로 삭제했고 잔존 후보 0건을 확인했다. 경성홀 실제 경로 무저장 실행은 대관 가능일정을 제외하고 `광복절 특별 워크숍`과 당일 강습을 강습 후보로, 2026-08-11 `DJ 테일` 일정을 자동등록 가능한 소셜로 분리했다.
- 검증: `node scripts/test-ingestion-standards.mjs`와 AI 판정·자동등록 링크·정규 소셜 교체 Vitest 37개가 통과했다. 경성홀 운영 재수집에서 접근 오류 없이 소셜 후보 `49a06ee1089eb445`가 자동등록됐고, 공개 일정 API에서 `DJ 테일 | 경성홀 화요 소셜`과 운영 일정 ID를 확인했다. 강습 후보의 `auto_registration.ready`는 계속 `false`다. 프로덕션 빌드와 Cafe24 배포 후 내·외부 헬스가 정상이며 공개 버전은 `2026-08-11T01:29:40.320Z`다.
- 관련 커밋: `60ccae83`
- 관련 결정: `docs/decisions/2026-08-11-deterministic-ingestion-checkpoints.md`
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/ingestion-progress.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/ai-candidate-adjudicator.mjs`, `server/cafe24/ingestion-candidate-policy.js`, `server/cafe24/function-api.js`

## 2026-08-11 일정 등록 분류·장르 시트의 목록 소실과 저장 동작 부재

- 상태: 구조 수정·회귀 검증 완료
- 현상: 세부 장르 버튼을 누르면 그 장르가 검색어로 강제 입력되어 하위 목록이 검색 결과 몇 개로 축소됐다. `혹시 스윙인가요?` 자동 교정 버튼이 같은 값을 다시 제안했고, 분류 시트에는 완료·저장 동작이 없어 사용자가 선택 종료 방식을 알 수 없었다. 필수 장르 누락 뒤 등록을 누르면 실제로 존재하지 않는 `genre` 모달을 열어 시트가 나타나지 않는 경로도 있었다.
- 원인: 선택 상태와 검색 입력 상태를 하나의 `genreQuery`로 공유했고, 시트 선택을 부모 폼에 즉시 반영하면서 별도 완료 동작을 두지 않았다. 운영 일정 전체의 기존 `genre` 값까지 선택지로 합쳐 `강습`, `DJ`, `소셜` 같은 활동값이 세부 장르로 섞였다.
- 조치: 검색 입력·직접 입력·퍼지 추천·`혹시 ...인가요?` 교정 UI를 등록 시트에서 제거했다. 분류·범위·세부 장르는 시트 내부 임시 상태로 선택하고 `저장`할 때만 폼에 반영하며, 취소·배경 닫기는 변경을 버린다. 새 등록에는 정식 프리셋만 표시하고 기존 일정 수정 시 해당 일정의 레거시 장르 한 개만 보존한다. 범위 변경 시 맞지 않는 세부 장르를 비워 잘못된 범위·장르 조합을 막고, 모바일의 5개 범위는 숨은 가로 스크롤 대신 3열 그리드로 모두 노출한다. 장르·분류 누락 검증은 항상 같은 분류 시트를 열도록 통합했고, 폼 초기화의 중복 분류 설정과 전달된 초기 그룹을 다시 `null`로 덮던 코드도 정리했다.
- 비확장 검토: 스윙 외 살사·바차타·탱고·스트릿 범위와 기존 프리셋은 유지하되 새 장르나 추론 규칙은 추가하지 않았다. 수동 선택 UI는 범위와 세부 장르가 일치할 때만 저장되며, 과거 일정에서 수집한 비정식 장르는 신규 선택지로 확산되지 않는다.
- 검증: 신규 시트 회귀 테스트 5개를 포함한 Vitest 55개 파일·335개 테스트가 통과했다. 프로덕션 빌드와 대상 ESLint가 오류 없이 통과했고, 390×844 모바일 렌더에서 분류 3개·범위 5개·스윙 세부 장르 7개 및 고정 `취소/저장` 동작이 한 시트에 노출됨을 확인했다.
- 관련 파일: `src/components/EditableEventDetail.tsx`, `src/components/EventRegistrationModal.tsx`, `src/styles/components/EditableEventDetail.css`, `src/components/EditableEventDetail.classification.test.tsx`

## 2026-08-13 자유게시판 동일 작성자의 이전 글 덮어쓰기

- 상태: 서버 수정·운영 복원·배포 검증 완료
- 현상: 관리자가 자유게시판에 `경성홀 입장료변경안내`를 새로 등록한 직후 기존 글 일부가 목록과 DB에서 사라졌다. 새 글 등록 뒤 자유게시판 전체 건수가 증가하지 않았다.
- 운영 증거: Apache 접근 로그에는 2026-08-13 10:21:23 KST 요청이 수정이 아닌 `POST /api/cafe24-data/board_posts/insert`와 HTTP 201로 기록됐다. 그러나 새 글은 작성자 사용자 ID와 같은 기존 레코드 ID를 사용했다. 2026-07-14부터 2026-08-13까지의 로컬 전체 DB 백업을 대조한 결과 같은 레코드가 `이벤트수집 규칙`에서 `제주스윙캠프 2026 취소소식`, 다시 `경성홀 입장료변경안내`로 교체됐다.
- 원인: 범용 레코드 ID 계산기가 명시적 ID와 자연 키가 없는 모든 행에서 `user_id`를 최종 식별자로 사용했다. 따라서 소유권 필드만 가진 `board_posts`, `board_comments` 등 다건 테이블은 같은 사용자의 두 번째 insert가 첫 번째 행과 같은 `(table_name, record_id)`를 만들었고 `ON DUPLICATE KEY UPDATE`가 기존 행을 덮어썼다.
- 조치: 복합 키 테이블은 선언된 전체 키, 기존 행은 명시적 ID, 신규 upsert는 전체 conflict 키, 자연 키 테이블은 자연 키를 사용한다. `user_id` 단독 식별은 `board_admins`, `board_users`, `user_home_menu_settings`처럼 사용자당 한 행인 테이블에만 허용하고 나머지 ID 없는 행에는 새 UUID를 부여한다. 생성 응답의 JSON에도 계산된 ID를 항상 기록한다.
- 복원: SHA-256이 일치하는 2026-08-05·13 전체 DB 백업에서 마지막 정상 본문과 메타데이터를 추출했다. 단일 트랜잭션에서 현재 `경성홀 입장료변경안내`를 새 ID `8ca978c6-cb7b-4ba9-aaac-bc8f11c9512f`로 보존하고, 가장 최근 과거 글 `제주스윙캠프 2026 취소소식`은 기존 ID `5d7fe39c-4172-4247-9188-a15366cf033b`로 되돌려 기존 링크를 보존했으며, `이벤트수집 규칙`은 새 ID `b7ff8709-e59b-4921-a492-8eb7e7b06d3c`로 복원했다. 댓글·좋아요·싫어요·즐겨찾기는 세 글 모두 0건이었고 제주 글의 기존 읽음 2건은 그대로 유지했다. 변경 전 행과 복원 원본은 로컬 incident-recovery 백업에 별도 보존했다.
- 검증: 범용 ID 정책 회귀 테스트 5개를 포함한 Cafe24 서버 테스트 19개 파일·188개 테스트, 대상 ESLint, 프로덕션 프런트엔드·서버 함수 빌드가 통과했다. 수정 커밋 `10a7c1e0`을 푸시하고 Cafe24에 배포했으며 서비스와 내·외부 헬스가 정상이고 공개 버전은 `1786585286461` (`2026-08-13T01:41:31.260Z`)이다. 운영 런타임에서 같은 작성자의 ID 없는 두 글이 서로 다른 UUID를 받고 JSON ID와 물리 ID가 일치함을 확인했다. 복원 후 공개 API는 게시물 16건·자유게시판 14건을 반환하고 세 글의 제목·작성 시각·본문 길이 및 서로 다른 ID가 백업·현재 행과 일치한다.
- 관련 커밋: `10a7c1e0`, `eecdc3bc`
- 관련 결정: `docs/decisions/2026-08-13-generic-record-identity.md`
- 관련 파일: `server/cafe24/generic-data-api.js`, `server/cafe24/generic-record-id.test.js`, `scripts/recover-2026-08-13-board-posts.mjs`

## 2026-08-13 자유게시판 UUID 머릿말 수정 누락 및 상세 새로고침 403

- 상태: 수정·운영 데이터 교정·배포 검증 완료
- 현상: 관리자 메뉴에서 자유게시판 머릿말 `뉴스`를 추가한 뒤 `경성홀 입장료변경안내` 글의 머릿말을 변경했지만 목록에 적용되지 않았다. UUID 게시글 상세 주소를 새로고침하면 간헐적으로 흰색 `Forbidden` 화면이 표시됐다.
- 운영 증거: `뉴스` 머릿말은 ID `b8e2e936-f6c6-487e-8880-3fa0c1ae0d5a`로 2026-08-13 10:54 KST 생성됐다. 대상 글은 10:55 KST 수정됐지만 `prefix_id=null`이었다. Apache 감사 로그에서 10:54:41·10:54:43 KST의 `/board?category=free&postId=8ca978c6-cb7b-4ba9-aaac-bc8f11c9512f` 요청이 ModSecurity CRS 2.2.9 규칙 `981173`의 SQL 문자 이상 탐지로 403 처리됐으며, UUID의 하이픈 네 개가 오탐 근거였다. 같은 규칙은 `/calendar?id=<UUID>` 직접 진입도 차단할 수 있었다.
- 원인: 게시글 편집기가 머릿말 선택값에 `Number()` 또는 `parseInt()`를 적용해 UUID를 `NaN`으로 만들었고 JSON 직렬화 과정에서 `null`이 됐다. 머릿말 관리 모달은 자체 목록만 다시 불러와 게시판 정적 데이터 캐시를 갱신하지 않았다. Apache에는 SPA 상세 이동용 UUID 쿼리 인수에 대한 좁은 예외가 없었다.
- 조치: 머릿말 ID를 `number | string` 불투명 식별자로 통일하고 숫자 레거시 값만 안전하게 숫자로 복원한다. 추가·수정·삭제 후 게시판 정적 데이터를 함께 갱신한다. ModSecurity 규칙 `981173`에서 `/board`의 `postId`와 `/calendar`의 `id` 인수만 검사 대상에서 제외하며 API와 다른 인수의 검사는 유지한다. 이 서버의 `REQUEST_URI`에는 쿼리 문자열도 포함되므로 정확 경로와 `?` 또는 값 끝을 일치시키는 좁은 정규식으로 예외를 등록했다. 운영 교정 스크립트는 대상 글과 `뉴스` 머릿말의 제목·ID·카테고리를 잠근 뒤 해당 `prefix_id`만 복구했다.
- 운영 교정: 미리보기에서 대상 글의 `prefix_id`가 `null`이고 교정 대상이 `뉴스` UUID임을 확인한 뒤 한 트랜잭션으로 적용했다. 교정 전 게시글·머릿말·이벤트·수집 후보 원본은 권한 0600 백업 `/opt/swingenjoy/backups/data-fixes/2026-08-13-prefix-benefit-2026-08-13T02-22-16-161Z.json`에 보존했다. 재실행 결과는 `alreadyApplied=true`였다.
- 검증: 머릿말 UUID·보안 설정 회귀 테스트, Node 테스트 25개, 대상 ESLint, 프로덕션 빌드와 전체 Vitest 64개 파일·372개 테스트가 통과했다. 운영 API에서 `경성홀 입장료변경안내`의 머릿말 UUID와 `뉴스` 조인이 일치했고, 관리자 브라우저의 목록·상세에도 `뉴스`가 표시됐다. 게시글과 달력 UUID 직접 주소는 모두 HTTP 200이며 Apache 설정은 `Syntax OK`, 서비스는 정상이다. 공개 빌드는 `1786587934399` (`2026-08-13T02:25:38.951Z`)이다.
- 관련 커밋: `2bb4b35c`, `efc57b29`
- 관련 결정: `docs/decisions/2026-08-13-generic-record-identity.md`
- 관련 파일: `src/utils/boardPrefixId.ts`, `src/pages/board/components/UniversalPostEditor.tsx`, `src/components/BoardPrefixManagementModal.tsx`, `deploy/cafe24/apache/00-swingenjoy-modsecurity-exceptions.conf`, `scripts/repair-2026-08-13-prefix-and-benefit.mjs`

## 2026-08-13 수집 일정의 얼리버드 할인 오분류 및 상세 교정 기능 누락

- 상태: 수정·운영 데이터 교정·배포 검증 완료
- 현상: 수집 후보에서 등록한 `대전 피버 토 졸파`가 일반 유료 행사인데 무료·할인 이벤트에 노출됐다. 달력 상세 인라인 수정 화면에는 혜택 분류를 해제하거나 바꾸는 기능이 없었다.
- 운영 증거: 운영 이벤트 ID `4cc850cb-3d38-458c-9ef2-3d30b7baa221`와 수집 후보 ID `3e37414b5bf363f2`가 모두 `benefit_eligible=true`, `benefit_kind=discount_event`였다. 후보의 AI 검토도 `review`, 신뢰도 0.94로 수동 검수를 요구했으며 할인 근거로 `얼리버드 5인: 6,000원 / 사전 신청: 12,000원 / 현장 신청: 20,000원` 입장료 구간을 제시했다.
- 원인: 결정 규칙이 `얼리버드` 또는 `early bird` 단어만 있어도 할인으로 확정했다. 등록용 전체 편집기에는 혜택 선택기가 있었지만 사용자가 실제로 연 상세 인라인 편집기는 별도 구현이며 해당 필드를 제공하지 않았다.
- 조치: `얼리버드`·`조기 등록`은 할인·특가·혜택 또는 할인율이 같은 근거 범위에 있을 때만 할인으로 판정한다. 상세 인라인 편집 상단에 `일반/무료/할인 이벤트/정기권` 선택을 추가하고 `일반` 저장 시 두 혜택 필드를 일관되게 해제한다. 운영 교정 스크립트는 이벤트와 연결된 수집 후보를 같은 트랜잭션에서 일반 일정으로 바꾸고 대상 원본을 적용 전에 백업했다.
- 운영 교정: `대전 피버 토 졸파` 이벤트는 `benefit_eligible=false`, `benefit_kind=null`로 바꾸고 연결된 수집 후보에서는 혜택 분류·수명주기 필드를 제거했다. 적용 전 미리보기와 적용 후 `alreadyApplied=true` 재검증을 모두 통과했다.
- 검증: 실제 입장료 문구가 일반 일정으로 남는 수집 회귀 사례, 상세 편집 선택기 테스트, 혜택 상태 정규화 테스트를 추가했다. 수집 표준 테스트, 대상 ESLint, 프로덕션 빌드와 전체 Vitest 64개 파일·372개 테스트가 통과했다. 공개 이벤트 API가 `false/null`을 반환하고 혜택 모아보기에는 해당 일정이 0건이며, 관리자 상세 수정 화면에서 현재 `일반`과 네 가지 선택지가 표시됨을 확인했다. 공개 빌드는 `1786587934399`다.
- 관련 커밋: `2bb4b35c`
- 관련 결정: `docs/decisions/2026-08-13-benefit-classification-evidence.md`
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/test-ingestion-standards.mjs`, `src/pages/v2/components/EventDetailModal.tsx`, `src/pages/v2/components/EventEditBottomSheet.tsx`, `src/utils/eventBenefitKind.ts`, `scripts/repair-2026-08-13-prefix-and-benefit.mjs`

## 2026-08-13 자유게시판 머릿말 목록 배치 및 상세 변경 즉시 반영 누락

- 상태: 추가 조정·배포 검증 완료
- 현상: 자유게시판 머릿말이 글 왼쪽의 고정 폭 열을 차지해 제목 영역과 분리돼 보였다. 상세 수정창에서 머릿말을 바꿔도 상세만 새로 조회하고 뒤 목록은 이전 머릿말을 계속 표시했으며, 목록 페이지를 다시 불러와야 변경값이 보였다.
- 원인: 자유게시판 데스크톱 그리드와 모바일 제목 행이 머릿말을 독립 열로 설계했다. 상세 수정 완료 콜백은 `refreshPost()`만 호출하고 `useBoardPosts`의 목록 조회를 호출하지 않았으며, Cafe24 호환 실시간 채널은 실제 목록 갱신을 제공하지 않는다. 따라서 제목·썸네일·공지/비공개 상태도 같은 조건에서 오래된 목록 값으로 남을 수 있었고 삭제·숨김 전환·댓글 수 변경에도 동일한 누락이 있었다.
- 조치: 기존 머릿말 열을 제거하고 머릿말을 데스크톱과 모바일 모두 제목 왼쪽 위의 절대 위치 배지로 분리했다. 후속 조정에서 배지를 24×14px로 줄여 제목보다 8px 왼쪽, 세로로 약 1px만 겹치게 배치하고 제목 영역에는 3px의 내부 여유만 둬 배지가 카드 높이를 늘리지 않도록 했다. 비공개 표시는 메타데이터 줄로 되돌리고 긴 작성자명이 안전하게 줄어들도록 정리했으며, 모바일 메타데이터에 눈 아이콘과 조회수를 댓글 수 앞에 추가했다. 상세 모달은 목록 변경 콜백으로 수정·삭제·숨김 전환·댓글 추가·삭제 후 목록을 다시 조회한다.
- 검증: 머릿말 DOM 구조·모바일 조회수·이미지 비드래그 및 상세 변경 목록 동기화 회귀 사례를 포함한 전체 Vitest 66개 파일·376개 테스트, 대상 ESLint와 Cafe24 운영 빌드가 통과했다. 운영 DB 연결 로컬 사이트의 390×844 모바일 렌더에서 배지는 24×14px, 배지 왼쪽 4px·제목 왼쪽 12px, 세로 겹침 약 1px로 측정됐다. 이미지 없는 카드와 비공개 카드는 70px, 썸네일 카드는 약 72.5px로 복원됐고 작성자·날짜·조회수·댓글 메타데이터 넘침과 드래그 가능한 썸네일은 모두 0건이었다. 후속 커밋 `07444a1d`를 푸시·배포한 뒤 운영 모바일 351×760 화면에서도 동일한 24×14px 배지, 제목과 약 1px 겹침, 70~73px 카드 높이, 조회수 표시, 메타데이터 넘침 0건과 드래그 가능한 썸네일 0건을 재확인했다. 외부 상태 확인과 자유게시판 응답은 정상이었으며 공개 빌드는 `1786590058891` (`2026-08-13T03:01:03.414Z`)이다.
- 관련 커밋: `c867db26`, `07444a1d`
- 관련 파일: `src/pages/board/BoardMainContainer.tsx`, `src/pages/board/board.css`, `src/pages/board/components/StandardPostList.tsx`, `src/pages/board/components/BoardDetailModal.tsx`, `src/pages/board/components/UniversalPostEditor.tsx`, `src/pages/board/components/CommentSection.tsx`, `src/pages/board/hooks/useBoardDetail.ts`

## 2026-08-13 알림함 오늘 일정·신규 등록 중복 기준과 개수 표시 혼선

- 상태: 수정·배포 검증 완료
- 현상: 알림함 상단과 각 섹션 제목에 `오늘 일정`·`신규 등록` 건수가 반복 표시됐다. 오늘 열리는 일정이라도 알림 원본이 신규 등록 경로이면 `신규 등록` 구역에 남아, 일정 날짜와 알림 생성 경로 중 무엇이 분류 기준인지 혼동될 수 있었다.
- 원인: 화면 분류가 일정의 실제 개최일을 확인하지 않고 `daily_schedule` 또는 `new_event` 알림 종류만 그대로 사용했다. 별도의 경로별 숫자 요약과 섹션 제목의 괄호 숫자도 같은 값을 중복 노출했다.
- 조치: 상단의 경로별 숫자 요약을 제거하고 모든 섹션 제목에서 괄호 개수를 뺐다. 신규 등록 알림은 알림 데이터와 최신 일정 미리보기의 시작일·종료일·명시 개최일을 달력과 같은 KST 날짜 규칙으로 판정하며, 오늘 일정이면 `오늘 일정`에 먼저 배치하고 그 밖의 항목만 `신규 등록`에 남긴다. 전체 읽지 않은 알림 수는 알림함 제목에만 유지한다.
- 검증: 오늘 날짜와 미래 날짜의 신규 등록을 함께 둔 회귀 테스트에서 오늘 신규 일정은 `daily_schedule`, 미래 신규 일정은 `new_event` 구역에 각각 한 번만 분류되고 상단 숫자 요약과 섹션별 괄호 숫자가 렌더되지 않음을 확인했다. 대상 ESLint·TypeScript 검사, 전체 Vitest 66개 파일·376개 테스트와 Cafe24 프로덕션 빌드가 통과했다. 운영 DB 연결 로컬 사이트의 390×844 모바일 렌더에서 상단 요약 0개, 숫자가 붙은 섹션 제목 0개, 가로 넘침 0px와 드래그 가능한 이미지 0건을 확인했다. 구현 커밋 `7120e880`을 푸시한 뒤 Cafe24에 배포했으며 서비스는 `active`, 외부 헬스와 알림 미리보기 응답은 정상이다. 공개 JS에서 새 문구 `오늘 진행되는 일정`이 포함되고 제거 대상 `nhm-route-summary`가 없음을 확인했으며 공개 빌드는 `1786591023345` (`2026-08-13T03:17:07.946Z`)이다.
- 관련 커밋: `7120e880`
- 관련 파일: `src/components/NotificationHistoryModal.tsx`, `src/components/NotificationHistoryModal.test.tsx`, `src/styles/components/NotificationHistoryModal.css`

# 2026-08-21 수동 등록한 정규 소셜이 반복본으로 복원된 문제

- 상태: 근본 원인 수정, 운영 데이터 복구·배포 완료
- 현상: 관리자가 2026-08-22 사보이볼룸 수집 후보를 직접 등록했지만 이후 실제 일정이 사라지고 `DJ 미정` 정규 반복 일정만 남았다. 2026-08-21 인더무드신림은 정규 반복 일정만 있고 실제 공지 반영본은 수집되지 않았다.
- 기존 보호 목적: 정규 반복 일정은 공식 공지가 없는 날짜를 채우며, 실제 수집·수동 일정이 등록되면 반복본만 제거한다. 실제 중복 일정은 기존 행을 보정해 중복 생성하지 않는다.
- 근본 원인: 관리자 화면의 날짜·장소 중복 검색이 정규 반복본 ID를 `existingEventId`로 전달했고, 서버가 그 반복본을 실제 일정처럼 제자리 갱신하면서 `automation.generated_by=regular-social-rolling-v1`을 보존했다. 다음 정규 일정 재조정은 이를 여전히 생성본으로 판단해 관리자 입력을 기본 반복본으로 되돌렸다.
- 수정: 등록 대상이 동일 슬롯의 생성 반복본이면 기존 실제 이벤트 보정 경로로 보내지 않고 새 실제 일정으로 등록한 뒤 기존 반복본만 제거한다. 생성본이 아닌 실제 중복 일정의 기존 보정 동작은 유지한다.
- 인더무드신림 조사: 8월 20일과 21일 우선순위 1 실행은 공식 프로필에서 보이는 12개 게시물을 이미 확인한 상태였다. 별도 체크포인트를 쓴 무저장 재수집으로 최신 게시물 4개를 다시 열었으나, 오늘 금요 소셜 문구에는 정확한 달력 날짜 없이 매주 금요일 운영 문구만 있어 날짜 필수 정책이 실제 일정 후보 생성을 차단했다. 운영 DB에는 `regular-social:dreambal-fri:2026-08-21`만 존재한다. 반복 운영 문구를 날짜별 실제 공지로 승격할지는 오등록 방지 정책 변경이므로 별도 판단 대상으로 남긴다.
- 검증 범위: 생성 반복본의 대체 판정 회귀 테스트, 기존 정규 일정 조정 테스트, 등록 링크/자동등록 안전 테스트를 실행한다. 실제 중복 일정 보정은 변경하지 않아 기존 등록 링크 테스트로 회귀 확인한다.
- 운영 결과: 구현 커밋 `384baf12`를 배포한 뒤 2026-08-22 사보이 `DJ 짜장 | 스윙스캔들 토요소셜`을 실제 등록했다. 공개 API에서 일정 ID `53aee968-192c-403c-82cd-c329422f89e8`, 공식 원문 링크, 저장 이미지와 `automation=null`을 확인했고 같은 슬롯의 생성 반복본은 제거됐다.

## 2026-08-21 소셜 후보 판별·자동등록 단계 불투명 및 AI 한도 차단

- 상태: 근본 원인 수정·운영 배포·재수집 완료
- 현상: 공식 원문 링크와 후보는 존재하지만 2026-08-21 해피홀 실제 소셜이 자동등록되지 않았고, 스윙타운 7·8월 월간 일정의 미래 소셜도 후보에만 남았다. 실행 요약만으로 링크 수집, 활동 분류, 날짜별 분해, 저장, 자동등록 중 어느 단계에서 막혔는지 확인하기 어려웠다.
- 기존 보호 목적: 다중 이미지·복수 일정·DJ 누락 같은 비정형 소셜은 AI와 서버가 독립 검증하고, 행사·졸업파티를 일반 소셜로 오등록하지 않는다. 후보 저장과 실제 일정 등록은 분리하며 실제 일정 ID가 생긴 뒤에만 후보를 완료 처리한다.
- 판정 경로: 두 공식 Naver 원문을 직접 열고 무저장 수집을 실행했다. 스윙타운 링크 `articles/156478`은 정상 접근됐고 8월 22일 조춘식이, 25일 미우, 29일 후안 세 구간으로 분해됐다. 해피홀 `articles/56188`도 8월 21일 후보까지 생성됐으나 `DJ는 사보이지기 ★테일★님`에서 역할명 `사보이지기`를 DJ로 오인했다. 운영 로그에서는 저장된 해피홀 후보의 2차 AI 판정이 Codex 사용량 제한으로 실패해 자동등록 준비 상태가 해제됐다.
- 근본 원인: 링크나 날짜 파서가 아니라 정책 연결과 가시성 문제였다. 8월 15일 변경이 모든 준비 완료 소셜을 AI 필수 경로로 되돌렸고 서버도 `ai_verified`를 무조건 요구했다. 스윙타운 월간 게시판은 혼합 공지라는 이유로 출처 전체가 수동 전용이어서 완결된 날짜별 정규 소셜도 함께 막혔다. 실행 결과에는 단계별 상태가 없었다.
- 수정: 완결된 공식 `date_scoped_social`만 저장 원문 기반 서버 재검증으로 AI 가용성과 분리했다. 스윙타운 월간 게시판은 정규 소셜만 shadow 허용하고 졸업파티·졸공·행사는 수동 검수한다. 역할명 뒤 장식된 실제 DJ를 추출하도록 보정했다. 실행 결과와 Telegram 요약에 발견·판별·분해·저장·등록 건수 및 구조화된 단계 차단 사유를 추가했다. 새 DB·큐·상태값은 만들지 않고 기존 후보 원장과 진행 체크포인트를 재사용했다.
- 유지한 불변조건: 공식 출처, 정확한 날짜, 소셜 표지, 전체 DJ, 검증 장소가 모두 저장 원문에 있어야 한다. 비정형·DJ 미공지·검색 발견·혜택·강습·행사는 기존 AI/이미지/수동 경계를 유지하고, 특수 행사는 일반 소셜로 자동등록하지 않는다. 같은 원문·날짜 ID와 운영 일정 중복 방지도 유지한다.
- 검증: `node scripts/test-ingestion-standards.mjs`, 자동등록·정규 소셜 회귀 38개, 대상 ESLint와 프로덕션 빌드가 통과했다. 구현 커밋 `e38beda0`을 푸시하고 Cafe24에 배포했으며 서비스는 `active`, 공개 빌드는 `1787286775626` (`2026-08-21T04:32:58.862Z`)이다. AI를 끈 운영 재수집 `20260821_133535_69050`은 `발견 9 → 판별 9 → 분해 4 → 저장/갱신 4 → 자동등록 2 → 정책차단 1`을 기록했다. 공개 API에서 8월 21일 `DJ 테일 | 스윙프렌즈 해피홀 게시판 금요 소셜`(ID `73a4c5d8-31a5-435e-b933-2741fa971fba`)과 8월 25일 `DJ 미우 | 스윙타운 월간 일정 화요 소셜`(ID `167e07aa-bb51-4ca9-883c-80a46ed7e485`)을 확인했다. 스윙타운 8월 22일은 기존 개별 원문 일정과 같은 날짜·장소·DJ라 중복 차단됐고, 8월 29일 졸업파티는 후보만 갱신돼 공개 이벤트가 생성되지 않았다.
- 관련 결정: `docs/decisions/2026-08-21-staged-social-ingestion-diagnostics.md`
- 관련 파일: `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/collection-registry.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `server/cafe24/function-api.js`, `server/cafe24/ingestor-registration-link.test.js`

## 2026-08-21 메인 광고 지난 시작일 과다 보충 및 인더무드 소셜 판별 누락

- 상태: 근본 원인 수정·운영 배포 및 누락 일정 복구 완료
- 현상: 최초 시작일이 지난 다회차 강습이 뒤쪽 보충이 아니라 일반 미래 광고 후보로 남아 메인 광고에 반복 노출됐다. 인더무드신림의 2026-08-22 공식 `Slow Social` 게시물은 원문·포스터·날짜·장소·DJ가 모두 있었지만 수집 후보가 0건이었다.
- 기존 보호 목적: 메인 광고는 장르·작성자/장소 중복을 제한하면서 가까운 행사와 미래 일정을 우선하고, 지난 일정은 광고가 부족할 때만 낮은 빈도로 보충한다. 수집 날짜 필터는 신청·접수·입금·마감일을 실제 행사일로 오등록하지 않는다.
- 근본 원인: 광고 판정이 `start_date`보다 `event_dates`의 남은 회차를 우선해 최초 시작일이 지난 일정도 미래 후보로 되살렸고, 화면 조합은 미래 후보 수와 관계없이 지난 후보를 최대 설정 개수까지 합쳤다. 수집 날짜 검사는 `Slow Social 2026.08.22`의 연도와 월·일 사이를 강한 소셜 날짜 문맥으로 인식하지 못해 뒤의 `사전신청`만 강한 신청일 문맥으로 판정했다.
- 수정: 광고는 `start_date → date → event_dates의 최초일` 순으로 단일 최초 시작일을 사용한다. 현재·미래 후보는 최대 15개까지 그대로 노출하고 10개 미만일 때만 최근 지난 시작일 후보를 필요한 수만 추가해 총 10개까지 채운다. 수집기는 소셜·파티 표지와 연도가 붙은 날짜를 강한 행사 날짜로 인정하고, 공식 원문에 날짜·소셜·장소·전체 DJ·포스터가 함께 있는 후보는 기존 `date_scoped_social` 서버 재검증을 재사용한다.
- 유지한 불변조건: 광고의 장르·출처/장소 중복 제거와 관리자 최대 개수는 유지한다. 수집은 공식 출처·미래 날짜·이미지·DJ·검증 장소를 계속 요구하고, 신청일만 있는 공지·DJ 미확정·졸업파티/행사·비정형 후보는 기존 AI 또는 수동 검수 경계를 유지한다. 새 DB·큐·상태값은 추가하지 않았다.
- 검증: 광고 시작일·10/15개 경계 회귀 22개와 설정 회귀 1개, 수집 표준 검사, 자동등록·정규 소셜 서버 회귀 38개, Node 전용 31개, 키오스크 단독 3개가 통과했다. 전체 Vitest 실행의 제품 단언 444개도 통과했으며 `node:test` 파일 2개는 올바른 Node 러너로 분리 확인했다. 대상 ESLint는 오류 0건, Cafe24 정책 감사와 프로덕션 빌드는 성공했다. 실제 공식 원문 4개 소스를 무저장 실행해 스윙타운 8월 22·25일, 해피홀 8월 21일, 사보이 8월 22일 후보와 졸업파티 수동 차단을 확인했다. 수정 전 0건이던 인더무드신림 원문은 무저장 재검증에서 `발견 1 → 판별 1 → 분해 1`, 날짜 `2026-08-22`, 장소 `인더무드신림`, DJ `비비비`, 자동등록 준비 상태로 통과했다. 구현 커밋 `0c2b0d06`을 푸시하고 Cafe24 공개 빌드 `1787314297433` (`2026-08-21T12:11:41.173Z`)을 배포했으며 서비스와 공개 홈은 정상이다. 운영 수집 `20260821_211501_74621`은 인더무드신림을 `발견 1 → 판별 1 → 분해 1 → 저장 1 → 자동등록 1`, 차단·오류 0으로 등록했다. 이벤트 `08d14a53-c2c2-4e9f-bb39-4bb74abd1e38`(`DJ 비비비 | Slow Social`)을 공개 API에서 확인했고, 재시도 `20260821_212255_74822`는 같은 후보와 이벤트를 갱신해 공개 이벤트가 정확히 1건임을 확인했다. 같은 API에서 사보이 8월 22일 `53aee968-192c-403c-82cd-c329422f89e8`, 해피홀 8월 21일 `73a4c5d8-31a5-435e-b933-2741fa971fba`, 스윙타운 8월 25일 `167e07aa-bb51-4ca9-883c-80a46ed7e485`도 유지됨을 재확인했다.
- 관련 파일: `src/pages/v2/components/EventList/utils/homeAdPriority.ts`, `src/pages/v2/components/EventList/utils/homeAdPriority.test.ts`, `src/pages/v2/components/EventList/components/EventPreviewSection.tsx`, `src/pages/v2/components/NewEventsBanner.tsx`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `server/cafe24/ingestor-registration-link.test.js`

## 2026-08-22 경성홀 날짜별 소셜·휴관·올어바웃스윙 졸업공연 누락

- 상태: 근본 원인 수정·운영 배포·졸공 표기 보정 완료
- 현상: 경성홀 공식 주간 공지에 2026-08-22 `DJ 제니스` 소셜이 명시됐지만 실제 원문 일정은 등록되지 않고 `DJ 미정` 정규 생성본만 남았다. 같은 날 올어바웃스윙 `98학기 SWING FESTIVAL` 졸업공연도 후보에서는 장소 미정으로 표시되고 실제 일정에 반영되지 않았다.
- 판정 경로: 8월 19일 경성홀 실행은 원문을 열었으나 게시물 뒤쪽의 8월 29~30일 `MT 휴관` 문구로 문서 전체를 제외했고, 오류 없는 제외로 판정돼 게시물 URL이 확인 완료 체크포인트에 들어갔다. 8월 22일 재실행은 그 URL을 이미 확인한 글로 건너뛰었다. 졸업공연 원문은 후보 ID `416f140c3801ab8b`까지 생성했지만 별칭 목록에 없는 `📍 마포구청 대강당`을 장소 근거로 보존하지 못해 등록 안전정책을 통과하지 못했다.
- 기존 보호 목적: MT·대관·휴관 자체를 이벤트로 오등록하지 않고, 후보 저장·자동등록 실패만 재시도하며 정상적으로 판정이 끝난 원문은 반복 수집하지 않는다. 출처 고정 장소는 명시적 원문 장소가 없을 때만 안전한 기본값으로 사용하고, 수동 검수 출처를 임의로 자동등록 출처로 승격하지 않는다.
- 근본 원인: 날짜별 소셜 분해는 문서 전체 금지어 검사보다 먼저 성공했지만 뒤의 전역 `MT` 검사 결과가 분해된 후보까지 폐기했다. 장소 판별은 알려진 별칭 또는 출처 기본 장소만 인정해 일반 `📍/장소:` 라벨의 새 장소를 누락했다. 휴관은 기존 원장·정규 소셜 제거 경로가 있었지만 `휴관` 동사가 빠졌고 과거 백테스트용 미래일 제한과 문서 첫 휴관 신호를 모든 날짜에 결합하는 방식 때문에 정확한 날짜별 예외가 생성되지 않았다. 졸공은 이미 캘린더의 `소셜/졸공` 장르와 수동검수 안전장치를 갖고 있었지만, `행사/공연` 판정이 먼저 굳어 회차가 DJ 표시값으로 연결되지 않았다. 수집 링크나 Instagram 접근 실패는 아니었다.
- 수정: 완전한 날짜·DJ 소셜 구간이 존재할 때는 문서 뒤쪽 MT·휴관 문맥을 해당 구간에 전파하지 않고 날짜별 소셜만 보존한다. 순수 MT 게시물과 날짜·DJ가 불완전한 게시물은 계속 제외한다. `📍`, `장소:`, `개최지:`, `venue/location:`의 명시적 한 줄 장소를 원문 근거로 보존하고, 미정·추후 공지·강습별 상이·URL/DM 값은 거부한다. 원문 장소는 출처 기본 장소보다 우선한다. 잘린 이모지 surrogate가 DJ 이름에 저장되지 않도록 추출·검증도 보강했다. 휴관은 제목·날짜 구간 안에서만 `휴무/휴관/취소`를 결합하고 짧은 날짜 범위를 일자별로 확장해 기존 예외 원장과 정규 소셜 조정기에 전달한다. 졸공은 근거가 있을 때만 `social/졸공`으로 정규화하고 `98학기·12회·32기` 같은 원문 회차를 `졸공 98회` 형식의 DJ 표시값으로 사용한다.
- 유지한 불변조건: 순수 MT·대관·과거 일정 차단, 날짜·DJ·장소·이미지 필수, 같은 원문·날짜 후보 ID와 실제 일정 중복 방지, 수동 출처의 자동등록 금지와 졸공·특별행사 자동등록 차단을 유지했다. 대회·일반 페스티벌은 졸공 근거가 없으면 행사로 남는다. 새 DB·큐·상태값과 특정 게시물 ID 예외는 추가하지 않았다.
- 운영 반영: 수집 실행 `20260822_205732_91752`에서 경성홀 `8/22 제니스`, `8/25 BBB`, `9/1 해림` 실제 소셜을 등록했다. 휴관 보정 실행 `20260822_213936_95459`은 `8/23`, `8/29`, `8/30` 예외 후보와 세 실제 소셜을 같은 원문에서 분리했고, 공개 API에서 세 휴관일의 경성홀 일정이 0건임을 확인했다. 올어바웃스윙 후보 `416f140c3801ab8b`는 `2026-08-22 / 마포구청 대강당`으로 갱신했다. 공통 졸공 정규화를 배포한 뒤 기존 저장 경로와 단일 트랜잭션을 사용해 운영 이벤트 `e4a2df91-db61-4ceb-8287-62ac8a347259`를 `social/졸공`, `group_id=2`, 제목 `DJ 졸공 98회 | 98학기 SWING FESTIVAL🎶`로 보정했으며 날짜·장소·원문 링크·이미지는 그대로 보존했다.
- 검증: 수집 표준 회귀에서 혼합 공지의 세 소셜과 세 휴관일이 서로 오염 없이 분리되고 순수 MT와 과거 백테스트 경계는 계속 차단됨을 확인했다. 실제 원문 무저장 실행은 `8/23·8/29·8/30 휴관`, `8/22 제니스·8/25 BBB·9/1 해림` 정확히 6건이었다. 졸공 `98학기 → 졸공 98회`, 실제 DJ가 있는 `12회 졸업파티 → 졸공 12회`, 졸공 근거 없는 98학기 강습·일반 페스티벌 비변경을 추가했다. 수집 표준, 관리자 매핑 2건, 자동등록·정규 소셜 서버 회귀를 합친 Vitest 41건, 대상 ESLint와 프로덕션 빌드가 통과했다. 전체 `tsc -b`는 이 변경 파일 밖의 기존 보드·메트로놈 등 타입 오류로 실패했으며 변경한 졸공 매핑 줄의 신규 타입 오류는 없었다. 공개 API와 운영 캘린더 DOM에서 날짜 `2026-08-22`, 장소 `마포구청 대강당`, `DJ 졸공 98회` 표시를 재확인했다.
- 관련 커밋: `133257c2`, `e7ccc0ef`, `2c048fff`, `58e2caa8`
- 관련 파일: `src/utils/graduationEvent.mjs`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/test-ingestion-standards.mjs`, `src/pages/admin/v2/utils/ingestorMapping.ts`, `src/pages/admin/v2/components/EventEditModal.tsx`, `src/pages/admin/v2/EventIngestorV2.tsx`

## 2026-08-22 일정 상세 수정·캘린더 검색·메인 광고 우선순위 연결 오류

- 상태: 공통 원인 수정·운영 배포·화면 검증 완료
- 현상: 검색에서 연 일정의 상세 수정 시 장르를 저장하면 상세와 검색 화면이 함께 닫힐 수 있었다. 캘린더 검색 결과는 선택해도 해당 월 이동과 카드 강조만 하고 상세를 열지 않았다. 밝은 전역 검색 입력창은 흰 글자를 상속해 검색어가 보이지 않았다. 메인 광고에서는 정규강습이 오늘 일정 또는 가까운 시작일 우선순위로 앞에 배치될 수 있었다.
- 판정 경로: 장르 저장 버튼 클릭 → 포털 하단 시트의 클릭 전파 → 상위 검색 오버레이 `onClose` 실행 → 상세를 소유한 검색 모달 언마운트였다. 캘린더 검색은 `CalendarSearchModal.onSelectEvent` → 월 변경·강조까지만 연결되고 `useEventModal.selectedEvent` 갱신이 없었다. 검색 입력은 전역 다크 테마의 입력 글자색 → 밝은 `.search-input` 배경 조합이었다. 광고는 기존 `rankHomeAdEvents`의 오늘·이번 달·근미래 정렬 뒤 동호회만 후순위로 분할했다.
- 기존 보호 목적: 검색 상세를 닫아도 검색 결과는 유지하고, 캘린더 검색 선택 시 목표 월과 카드를 함께 보여주며, 메인 광고는 시작일이 지난 일정을 현재·미래 후보가 10개 미만일 때만 최대 10개까지 보충한다. 이번 달 행사와 가까운 미래 일정, 동호회 후순위도 유지한다.
- 근본 원인: 중첩 포털 모달의 이벤트 경계가 빠졌고, 캘린더 검색의 기존 선택 콜백에 상세 상태 연결이 누락됐다. 검색 입력은 배경만 지정하고 명시적 전경색을 두지 않았다. 광고 정렬은 정규강습을 식별할 구조화 분류·태그·명시 문구 판정과 별도 최후 그룹이 없었다.
- 수정: 상세 오버레이와 하단 수정 시트에서 클릭 전파를 종료하고 저장 결과만 기존 `eventUpdated` 경로로 전달한다. 캘린더 검색 선택 시 월 이동·강조와 함께 같은 `useEventModal`의 선택 이벤트를 설정한다. 밝은 검색 입력에 글자·커서·WebKit 전경색을 명시한다. 기존 광고 정렬 함수 안에서 `regular`, `club_regular`, `정규강습`, `academy_regular` 등 명시적 정규강습 근거만 판정해 일반 후보, 동호회, 정규강습 순으로 안정 분할한다.
- 유지한 불변조건: 원데이·워크샵과 일반 강습은 정규강습으로 낮추지 않는다. 현재·미래 후보의 기존 상대 순서와 작성자·장소 중복 제거, 최대 15개, 지난 시작일의 10개 미달 보충 및 8회 중 1회 자동 전면 제한을 유지한다. 새 모달·라우트·DB 필드·특정 일정 예외는 추가하지 않았다.
- 검증: 광고의 `created_at`·`date` 두 정렬에서 오늘 정규강습도 일반 행사·원데이·동호회 뒤로 가고, 원데이는 정상 우선순위에 남는 회귀를 추가했다. 장르 저장 클릭이 상위 오버레이로 전파되지 않는 컴포넌트 회귀를 추가했다. 졸공 매핑·자동등록·정규 소셜 인접 회귀를 포함한 대상 Vitest 67건, 수집 표준 검사와 프로덕션 빌드가 통과했다. 커밋 `58e2caa8`을 푸시한 뒤 Cafe24 공개 빌드 `1787407034529` (`2026-08-22T13:57:17.907Z`)을 배포했고 외부 헬스가 정상이다. 운영 Chromium에서 전역 검색 입력값 `졸공`이 흰 배경과 짙은 글자·커서로 표시되고, 캘린더 검색의 졸공 결과 선택 뒤 URL이 `/calendar`에 남은 채 상세가 열리는 것을 확인했다. 운영 메인 광고 DOM은 12개 후보 중 명시적 정규강습 두 건을 11·12번째에 배치했다.
- 관련 커밋: `58e2caa8`
- 관련 파일: `src/pages/v2/components/EventList/utils/homeAdPriority.ts`, `src/pages/v2/components/EventList/utils/homeAdPriority.test.ts`, `src/pages/v2/components/EventDetailModal.tsx`, `src/pages/v2/components/EventEditBottomSheet.tsx`, `src/pages/v2/components/EventEditBottomSheet.test.tsx`, `src/pages/calendar/page.tsx`, `src/components/GlobalSearchModal.css`, `src/pages/v2/components/NewEventsBanner.tsx`

## 2026-08-26 공식 졸업파티 후보 자동등록 단절

- 상태: 근본 원인 수정·운영 배포·대상 후보 자동등록·공개 API 검증 완료
- 현상: 09:00 `swing-daily` 실행이 공식 스윙타운 2026-08-29 졸업파티 후보를 정상 저장했지만 `special event classification requires manual review instead of social auto-registration`으로 차단해 등록 API를 호출하지 않았다.
- 기존 보호 목적: 일반 행사·대회·근거가 불완전한 졸업파티를 소셜로 오등록하지 않고 후보 저장과 실제 일정 등록을 분리한다. 저장 직전 2차 AI는 비정형 소셜의 실제 DJ 명단을 독립 대조한다.
- 근본 원인: 공통 졸공 정규화는 후보를 `social/졸공`, `group_id=2`, 표시값 `DJ 졸공`으로 정확히 변환했지만 자동등록 수집기와 서버가 모든 졸공을 다시 일괄 차단했다. 첫 수정 뒤에는 저장 직전 일반 소셜 AI가 분류 표시값 `졸공`과 원문 실제 DJ `후안`을 비교해 다시 준비 상태를 껐다.
- 해결: 공식 자동등록 출처에서 정확한 미래 날짜, 검증 장소, 원본 이미지와 명시적 졸업파티 근거를 모두 갖춘 졸공만 기존 자동등록 API에 연결한다. 졸공 표시값은 실제 DJ 비교 AI에 넣지 않고 서버가 저장 원문에서 날짜·장소·이미지·졸공 근거를 재검증한다. 이미지 없는 졸공, 수동 출처, 일반 행사·대회는 기존 차단을 유지한다.
- 검증: 수집 표준 검사, 자동등록 서버 회귀 36건, 대상 구문 검사, `git diff --check`, 프로덕션 빌드가 통과했다. 배포 빌드 `1787751550324`, 서비스 `active`를 확인했다. 공식 원문 재수집은 후보 `0f21a765d9766d25`를 갱신하고 이벤트 `30034f70-329c-4ac4-815d-7986aba344f8`을 자동등록했다. 공개 API에서 2026-08-29 `DJ 졸공 | 봉천살롱 토요 소셜`, `social/졸공`, `group_id=2`, 봉천살롱과 저장 이미지를 확인했다.
- 관련 커밋: `278ad193`, `8a748f63`
- 관련 결정: `docs/decisions/2026-08-26-grounded-graduation-social-registration.md`
- 관련 파일: `src/utils/graduationEvent.mjs`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `server/cafe24/function-api.js`, `scripts/test-ingestion-standards.mjs`, `server/cafe24/ingestor-registration-link.test.js`

## 2026-08-23 캘린더 졸공 DJ 슬롯 공백

- 상태: 공통 표시 원인 수정·표시 정책 재조정·회귀 검증·운영 재배포 완료
- 현상: 2026-08-23 해피홀의 `네오 8/23 졸업파티`는 이미 자동등록됐고 운영 API도 `category=social`, `genre=졸공`, `group_id=2`를 반환했지만 월간 캘린더 카드에는 장소 `해피홀`만 보이고 그 아래 DJ 슬롯이 비었다.
- 판정 경로: 운영 이벤트 조회 → `FullEventCalendar`의 소셜 카드 판정 → 구조화 `djs` 또는 제목의 `DJ ...`만 추출 → 과거 등록본에는 두 값이 없어 빈 문자열 생성 → 캘린더 두 번째 줄 공백이었다. 수집·후보 판정·자동등록 실패가 아니었다.
- 기존 보호 목적: 일반 소셜은 구조화 DJ를 우선하고 제목을 레거시 보조 근거로 사용하며, `DJ 미정`은 캘린더에서 숨긴다. 졸공 공통 판정기는 명시적 졸업공연·졸업파티·졸공 근거가 있을 때만 졸공으로 분류하고 회차 메타데이터를 보존한다.
- 근본 원인: 공통 졸공 판정기는 수집·관리자 등록 경로에만 연결돼 있었고 월간 캘린더 표시 경로는 이를 사용하지 않았다. 따라서 배포 전에 저장돼 `djs`가 없는 정상 졸공 일정은 장르가 정확해도 표시할 값이 없었다.
- 수정: 기존 `calendarEventKind` 표시 책임에 소셜 두 번째 줄 계산을 모으고 기존 졸공 판정기를 재사용했다. 졸공 여부만 판정한 뒤 캘린더에서는 저장된 기수·회차와 관계없이 항상 `졸공`을 반환하며 `DJ` 접두어도 붙이지 않는다. 일반 소셜만 기존처럼 `DJ 이름`으로 표시한다. 수집·관리자 경로의 회차 메타데이터는 유지한다.
- 유지한 불변조건: 수집 후보·자동등록·운영 이벤트 원장은 변경하지 않는다. 졸공이 아닌 일정은 회차 숫자가 있어도 바꾸지 않고, 일반 DJ 우선순위와 `DJ 미정` 숨김을 유지한다. 특정 이벤트 ID나 해피홀 전용 예외, 새 필드·테이블은 추가하지 않았다.
- 검증: 실제 운영 데이터와 같은 `네오 8/23 졸업파티 / NEO SWING 140기 / social / 졸공`과 저장 DJ가 `졸공 98회`인 레거시 일정이 모두 캘린더에서 `졸공`으로만 표시되고, 회차 없는 졸공도 `졸공`, 일반 소셜은 `DJ 메이저`, 미정은 공백으로 남는 회귀를 추가했다. 캘린더 종류·레이아웃 Vitest 10건, 대상 ESLint 오류 0건과 프로덕션 빌드가 통과했다. 수집·DB 원장은 변경하지 않아 수집 실행과 데이터 보정은 제외했다. 커밋 `09013c0b`를 푸시한 뒤 Cafe24 공개 빌드 `1787415415254` (`2026-08-22T16:16:58.532Z`)을 배포했고 외부 헬스가 정상이다. 운영 Chromium에서 해피홀 140기 일정과 마포구청 98회 일정의 두 번째 줄이 모두 정확히 `졸공`이고, 화면의 모든 졸공 줄에 기수·회차·`DJ` 접두어가 없으며 일반 소셜은 `DJ 충하`로 유지됨을 확인했다.
- 관련 커밋: `bc71db85`, `09013c0b`
- 관련 파일: `src/pages/calendar/utils/calendarEventKind.ts`, `src/pages/calendar/utils/calendarEventKind.test.ts`, `src/pages/calendar/components/FullEventCalendar.tsx`

## 2026-08-23 정규 소셜 휴무 회차가 일정에서 사라지는 문제

- 상태: 공통 원인 수정·운영 배포·실데이터 검증 완료
- 현상: 정규 소셜의 휴무·휴관 공지가 날짜별 후보로 정상 수집돼도 해당 날짜의 기본 일정이 삭제돼 캘린더와 리스트에서 완전히 사라졌다. 사용자는 수집 누락과 공식 휴무를 구분할 수 없고 공지 원문도 일정 화면에서 열 수 없었다. 수정 전 운영 공개 API에서 경성홀 휴관 후보가 있는 2026-08-23·29·30의 경성홀 일정이 0건임을 읽기 전용으로 재확인했다.
- 판정 경로: 공식 원문 수집 → `scraped_events`의 `closure` 후보 저장 → `regular-social-reconciler.exceptionInfo`의 날짜·출처 판정 → 같은 규칙·날짜 생성본을 `removes`에 추가 → `events` 삭제 → 캘린더·리스트 공백이었다. 링크와 수집 근거는 후보 원장에 있었지만 사용자 일정으로 전달되지 않았다.
- 기존 보호 목적: 휴무일에 `DJ 미정` 기본 소셜을 노출해 실제 운영하는 것으로 오해하지 않게 하고, 실제 개별 소셜이 등록되면 생성 반복본만 제거한다. 생성본은 결정론 ID로 재실행해도 중복되지 않고 과거 포스터를 빌려오지 않는다.
- 근본 원인: `closure`가 사용자에게 보여줄 날짜 상태가 아니라 생성 여부를 끄는 삭제 신호로만 모델링됐다. 조정기가 후보의 제목·사유·원문 URL을 읽지 않아 표시용 휴무 회차를 만들 수 없었다.
- 수정: 기존 후보·외부 API 예외·정규 소셜 조정기와 결정론 ID를 그대로 재사용해 `closure`를 `social/휴무` 회차로 물질화한다. DJ 슬롯과 리스트 배지는 `휴무`만 표시하고, 후보 또는 API 예외의 원문 URL은 `휴무 공지` 링크로, 수집 근거·설명은 상세 내용으로 보존한다. 같은 날짜에 실제 개별 소셜이 있으면 기존 우선순위를 유지해 휴무 중복을 만들지 않는다.
- 유지한 불변조건: 후보 원장과 외부 예외 원장은 변경하지 않는다. 휴무 회차에는 포스터를 재사용하지 않고, 실제 개별 소셜 우선·90일 범위·지난 생성본 정리·Cron 인증·동일 ID 재시도 안전성을 유지한다. 새 DB 필드·테이블·큐·상태값과 특정 장소·날짜 예외는 추가하지 않았다.
- 검증: 수집 휴무의 생성본 대체·원문 링크·사유 보존, 같은 입력 재시도 무변경, 실제 소셜 충돌 시 미생성, 공식 API 휴무 링크, 캘린더 DJ 슬롯과 리스트 배지 `휴무`, 기존 졸공·일반 소셜 경계를 검증했다. 정규 소셜·외부 API·캘린더 대상 Vitest 29건과 수집 표준 검사, 대상 ESLint 오류 0건, 프로덕션 빌드가 통과했다. 커밋 `a2ca355e`를 푸시한 뒤 Cafe24 공개 빌드 `1787419963798` (`2026-08-22T17:32:47.175Z`)을 배포했고 외부 헬스가 정상이다. 운영 조정기는 경성홀 2026-08-23·29·30을 각각 동일 규칙 ID의 `휴무` 회차로 생성했고 세 건 모두 공식 Instagram 원문 URL·설명·예외 원장 ID를 보존했다. 운영 Chromium에서 월간 셀 `경성홀 / 휴무`, 리스트 배지 `휴무`, 상세 `휴무 공지` 링크와 실제 원문 URL을 확인했으며 기존 `졸공`과 일반 `DJ 충하` 표시도 유지되고 페이지 오류는 없었다. 수집 파서는 변경하지 않아 실제 원문 재수집은 중복 검증에서 제외하고 기존 후보 원장을 운영 조정 입력으로 사용했다.
- 관련 커밋: `a2ca355e`
- 관련 결정: `docs/decisions/2026-08-23-regular-social-closure-visibility.md`
- 관련 파일: `server/cafe24/regular-social-reconciler.js`, `server/cafe24/regular-social-reconciler.test.js`, `src/pages/calendar/utils/calendarEventKind.ts`, `src/pages/calendar/components/CalendarListView.tsx`

## 2026-08-23 수집 후보 중복·과거 원문·게시판 탐색 날짜 재노출

- 상태: 공통 원인 수정·운영 원장 보정·배포·재실행 검증 완료
- 현상: 이미 운영 일정으로 등록된 미오새 Killer Boogie와 여러 소셜 후보가 관리자 `신규` 탭에 계속 남았다. 2018년 Facebook 무료 워크숍은 AI 검토 실패 뒤 `2027-01-01`로 보였고, 실제 8월 15일 다음카페 원데이 게시물은 게시판의 다른 글 날짜인 9월 13일 후보로 표시됐다.
- 판정 경로: 원문 수집 → 게시일·미래 날짜 정책 → 운영 일정/후보 중복 판정 → `scraped_events.status` 저장 → 관리자 신규·중복 탭 필터다. 운영 읽기 전용 감사에서 pending 80건 중 기존 중복 함수만으로도 운영 일정과 일치하는 14건이 확인됐다. 같은 ID의 pending 후보 갱신 분기가 이 판정을 건너뛰어 상태를 계속 pending으로 덮었고, 정책 탈락·다중 날짜 축약 후보는 원장 상태를 저장하지 않아 재수집될 수 있었다.
- 기존 보호 목적: 수동·자동 등록 완료와 제외 상태는 재수집으로 되돌리지 않고, 결정론 후보 ID와 날짜별 분리를 유지한다. `regular-social:` 생성본은 날짜·장소·DJ 근거가 완전한 실제 공식 소셜만 교체할 수 있으며, DJ가 다른 같은 장소 소셜과 서로 다른 강습은 합치지 않는다. AI 장애 후보는 결정론 근거가 없으면 수동 검토 대상으로 남긴다.
- 근본 원인: 2026-07-26에 후보 내용을 최신 원문으로 갱신하도록 추가한 same-ID 분기가 이후 도입된 운영 중복 판정보다 앞에서 종료됐다. 비소셜 중복은 운영 제목 필드만 비교해 상세 본문에 후보 제목이 그대로 있어도 놓쳤다. 다음카페 수집기는 `document.body.innerText`를 사용해 실제 `#article` 뒤의 다른 글 목록까지 날짜 근거로 섞었다. 게시일 정책은 명시 필드만 읽어 Facebook 텍스트 헤더의 2018년 게시일과 다음카페의 2자리 작성일을 사용하지 못했다.
- 수정: 기존 중복 판정기와 `duplicate/excluded` 상태를 재사용한다. same-ID pending 갱신도 운영 일정과 이전 후보 원장을 다시 판정하며, 날짜·장소·활동이 같고 12자 이상의 구체 후보 제목이 운영 상세에 그대로 있는 비소셜만 고신뢰 중복으로 처리한다. 정책 탈락과 다중 날짜 부수 후보는 제외 사유·단계를 원장에 저장한다. 다음카페는 실제 게시글 컨테이너의 본문·이미지만 읽고, 다른 글 경계 뒤에서만 나온 날짜는 후보 검증에서 차단한다. 게시일은 2자리 작성일과 플랫폼 텍스트 헤더의 초기 독립 날짜까지 정규화한다.
- 유지한 불변조건: 기존 URL·날짜, 소셜 날짜·장소·DJ, 공식 API 우선, 유사 제목 중복 규칙과 정규 생성 소셜 교체 안전장치를 유지한다. 소셜 DJ 불일치와 서로 다른 비소셜 제목은 중복 처리하지 않는다. 새 테이블·큐·상태값과 특정 이벤트 ID 예외는 추가하지 않았고 운영 일정 삭제도 하지 않는다.
- 검증: 미오새와 동일한 운영 본문 포함 중복, same-ID pending 재판정, 후보 간 다른 원본 중복, 소셜 DJ 불일치, 정규 생성 소셜 교체, 2018년 플랫폼 게시일, 다음카페 2자리 작성일, 실제 본문 날짜와 다른 글 날짜 경계를 추가했다. 자동등록 연결·정규 소셜 Vitest 45건과 전체 수집 표준 검사, 대상 ESLint, 프로덕션 빌드가 통과했다. 운영 드라이런은 신규 판정 대상 80건 중 `duplicate` 4건·`excluded` 67건·유효 신규 9건을 산출했고, 트랜잭션 적용 전 원본은 권한 0600 백업 `/opt/swingenjoy/backups/data-fixes/2026-08-23-ingestor-ledger-2026-08-22T19-29-59-216Z.json`에 보존했다. 미오새 후보 `0e5757737e6aa18a`는 운영 이벤트 `183734f4-90f6-496c-8cc6-cf562219999a`의 중복, 스위티스윙 `2460d0eab2fdff58`은 다른 글 영역 날짜, 2018년 Facebook `737670cf350edd72`는 게시일 대비 비현실적 날짜로 각각 저장됐다. 적용 직후 재실행은 대상 9건·전환 0건·`alreadyReconciled=true`였다.
- 배포 후속: 전체 수집 스크립트 배포 뒤 `candidate-utils.mjs`가 참조하는 공용 졸공 모듈이 운영 경로에 없는 패키징 누락을 드라이런이 DB 변경 전에 차단했다. 기존 `src/utils/graduationEvent.mjs`를 동일 경로에 배포하고 원격 import 검사를 배포 절차에 추가했다. 커밋 `b34a88b4`, `35b26777`, `b085fa78`, `019ca470`을 푸시했으며 공개 빌드 `1787426961067` (`2026-08-22T19:29:24.507Z`), 외부 헬스, 공용 모듈 로컬·운영 SHA-256 일치를 확인했다.
- 관련 결정: `docs/decisions/2026-08-23-ingestion-ledger-adjudication.md`
- 관련 파일: `server/cafe24/function-api.js`, `server/cafe24/ingestion-candidate-policy.js`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/reconcile-2026-08-23-ingestor-ledger.mjs`, `server/cafe24/ingestor-registration-link.test.js`, `scripts/test-ingestion-standards.mjs`

## 2026-08-23 재사용 원본 링크와 게시판 발행물의 복합 중복 판정

- 상태: 공통 원인 수정·운영 배포·원장 보정 완료
- 현상: 원본 사이트가 한 게시물 URL에서 날짜별 일정을 계속 갱신할 수 있는데도 같은 URL·날짜를 절대 중복으로 취급해 서로 다른 내용을 덮거나 누락할 수 있었다. 반대로 이미 게시판에 발행된 `경성홀 입장료변경안내`는 이벤트 원장에 없다는 이유로 수집 관리자 `신규` 탭에 다시 나타났다.
- 판정 경로: 수집 후보 생성 → 결정론 ID 및 배치 병합 → 운영 일정·후보 중복 판정 → 자동등록 대상 선택 → `scraped_events.status`와 관리자 탭이다. 기존 구현은 후보 ID, 같은 원본 URL·날짜, 같은 출처·날짜를 내용 충돌 확인 전에 종료 조건으로 사용했고, 게시판 원장은 중복 비교 대상에 포함하지 않았다.
- 기존 보호 목적: 반복 실행이 등록 완료·제외·중복 같은 종결 상태를 다시 열지 않게 하고, 같은 원문에서 재발견한 동일 일정의 후보 ID와 등록 대상을 안정적으로 재사용한다. 날짜·장소·활동·DJ가 충돌하는 소셜은 합치지 않고 `registered_event_id` 연결은 자동 갱신보다 우선한다.
- 근본 원인: 링크·ID 안정성과 이벤트 동일성이 하나의 규칙에 묶여 있었다. URL 재사용과 ID 충돌에서 제목·본문·장소·활동·시간·DJ의 호환성을 확인하지 않아 같은 링크의 다른 이벤트를 구분할 수 없었고, 이벤트가 아닌 운영 안내를 소유하는 `board_posts` 원장은 판정 경로에 연결되지 않았다.
- 수정: 기존 원장과 상태값을 유지하고 공통 복합 식별자를 추가했다. 같은 URL·날짜라도 내용이 호환될 때만 중복이며, 내용이 다르면 콘텐츠 지문을 포함한 안정 ID로 별도 후보를 보존한다. 수집 배치·V3·서버 저장·자동등록·감사 스크립트가 같은 판정기를 사용한다. 게시판 중복은 가격·입장료 변경 같은 운영 안내에 한해 호환 제목과 로컬 이미지 시각 일치가 모두 있을 때만 `board_posts` 중복으로 연결한다.
- 유지한 불변조건: 등록 완료 연결, 종결 상태 비재개방, 날짜별 후보 분리, 소셜 DJ·장소·활동 충돌 차단, 공식 API 우선순위와 수동·자동 등록 경계를 유지했다. 게시판 제목만 같거나 원격 이미지를 가져올 수 없는 경우에는 자동 중복 처리하지 않으며 새 테이블·큐·상태값과 특정 URL·이벤트 ID 예외를 추가하지 않았다.
- 검증: URL 재사용의 동일/상이 내용, ID 충돌, DJ 충돌, 후보 배치 병합, V3, 등록 대상 선택, 게시판 안내의 제목·이미지 양쪽 근거와 일반 이벤트 비차단을 포함한 Vitest 80건, Node 4건, V3 안전성·수집 표준 검사, 대상 ESLint 오류 0건과 프로덕션 빌드가 통과했다. 커밋 `8c5d1a70`을 푸시하고 Cafe24 공개 빌드 `1787458336631` (`2026-08-23T04:12:20.020Z`)을 배포했으며 외부 헬스가 정상이다. 운영 dry-run은 pending 11건 중 `📢 입장료 변경 안내` 1건만 기존 게시판 글 `8ca978c6-cb7b-4ba9-aaac-bc8f11c9512f`의 중복으로 산출했다. 적용 전 원본은 `/opt/swingenjoy/backups/data-fixes/2026-08-23-ingestor-ledger-2026-08-23T04-15-25-936Z.json`에 보존했고, 후보 `cca4c0a0c0f35b1e`는 `duplicate/board_posts`로 저장됐다. 제목 유사도는 `0.86`, 이미지 평균 절대오차는 `2.175`였고 적용 후 재실행은 pending 10건·전환 0건·`alreadyReconciled=true`였다.
- 관련 결정: `docs/decisions/2026-08-23-ingestion-composite-content-identity.md`
- 관련 파일: `server/cafe24/ingestion-duplicate-identity.js`, `server/cafe24/function-api.js`, `server/cafe24/ingestor-v3-api.js`, `scripts/ingestion/candidate-utils.mjs`, `scripts/ingestion/swing-daily-native.mjs`, `scripts/ingestion/expanded-genre-native.mjs`, `scripts/ingestion/tango-scene-map.mjs`, `scripts/reconcile-2026-08-23-ingestor-ledger.mjs`, `src/pages/admin/v2/EventIngestorV2.tsx`

## 2026-08-23 Instagram 릴스 커버 편집 UI 변경으로 예약 게시 누락

- 상태: 공통 UI 경로 수정·오늘 릴스 게시·재발 방지 완료
- 현상: 2026-08-22 12:30 예약 실행은 4K 영상·커버 생성과 AVD·Instagram 검증을 마쳤지만 Share 전에 세 번 실패해 릴스가 올라가지 않았다. 같은 증상은 8월 20일에도 발생했다.
- 판정 경로: LaunchAgent → 날짜별 영상 생성·검증 → `Medium_Phone` AVD·계정 확인 → 영상·음악 선택 → 커버 편집 → Share 상태 기록 → 프로필 게시물 수 확인이다. 8월 22일 원장은 `failed-before-share`였고 오류 화면에는 `Edit cover`, `Add from camera roll`, `Done`이 이미 표시됐지만 자동화는 과거 중간 오버레이 `clip_thumbnail_layout`만 기다렸다.
- 기존 보호 목적: 정확한 AVD·계정·Instagram 설치를 Share 전에 확인하고, Share 이전 실패만 안전 재시도한다. Share 이후에는 프로필 게시물 수 증가가 확인되기 전까지 자동 재게시를 금지해 중복 게시를 막는다. 날짜별 영상·게시 상태와 음악 이력은 원자적으로 보존한다.
- 근본 원인: Instagram이 커버 미리보기를 더블탭하면 중간 썸네일 오버레이를 거치지 않고 곧바로 전체 `Edit cover` 화면을 여는 UI로 변경했지만 자동화가 기존 오버레이 경로만 지원했다. 수동 복구에서도 과거 날짜를 명시하면 실제 게시를 허용해, 누락 원인 재현과 오늘 게시를 구분하지 못하는 운영 위험이 있었다.
- 수정: 더블탭 뒤 `Add from camera roll`이 이미 보이면 편집기가 열린 것으로 판정하고 바로 4K 커버 선택을 진행하며, 기존 `clip_thumbnail_layout` 오버레이도 계속 지원한다. 실제 게시는 KST 오늘 날짜만 기본 허용하고 과거·미래 날짜는 드라이런 또는 명시적 `--allow-noncurrent-date`에서만 허용한다.
- 유지한 불변조건: `Medium_Phone`·Package Manager 설치·계정 검증, Share 전/후 재시도 경계, 프로필 수 기반 게시 완료, 날짜별 잠금·상태, 4K 규격과 음악 순환은 유지했다. UI 변화가 있어도 계정 로그인이나 불확실한 Share를 추측해 진행하지 않는다.
- 검증: 신규 직접 커버 편집·레거시 오버레이·현재 날짜 차단 경계를 포함한 소셜 릴스 Node 회귀 24건, 대상 구문·ESLint와 `git diff --check`가 통과했다. 2026-08-23 영상은 `23일 소셜`, 2160×3840, H.264, 30fps, 15초로 새로 생성했고 드라이런에서 4K 커버와 `Do What You Wanna — Ramsey Lewis`를 적용해 Share 직전까지 통과한 뒤 정상 폐기했다. 실제 게시 뒤 프로필 수가 `22 → 23`으로 증가했고 최신 게시물을 직접 열어 `23일 소셜` 화면과 해당 음악을 확인했다. 복구 중 명시적으로 게시된 22일 릴스는 사용자가 삭제했으며 22일 게시 원장은 `published`로 유지해 자동 재게시를 막았다.
- 관련 커밋: `8386bb6f`, `2dfee0b7`
- 관련 파일: `scripts/social-reels/instagram-reel-adb.mjs`, `scripts/social-reels/instagram-reel-adb.test.mjs`, `scripts/social-reels/run-scheduled-social-reel.mjs`

## 2026-08-30 전역 행사 검색의 붙여 쓴 날짜·분류 검색 누락

- 상태: 공통 원인 수정·Cafe24 운영 배포·공개 검색 검증 완료
- 현상: 전역 검색에서 `28일소셜`을 입력하면 2026-08-28 소셜 일정이 운영 원장에 있어도 결과가 없었다. `28일 소셜`처럼 띄어 쓴 복합 검색어도 Cafe24 행사 API에서는 같은 문제를 보였다.
- 판정 경로: 전역 검색 입력 → `fetchCafe24Events(q)` → `/api/events`의 `buildWhere` → 제목·설명·장소에 대한 원문 전체 `LIKE` → 클라이언트의 전체 토큰 재검증 → 결과 표시다. 서버 후보 조회에서 붙인 원문 전체가 먼저 탈락해 화면 재검증까지 도달하지 못했다. 조회 경로이므로 DB·큐·알림 쓰기는 없었다.
- 기존 보호 목적: 여러 단어 검색은 모든 단어가 있는 행만 허용해 글자 사이 임의 와일드카드가 만드는 오탐을 막고, 미래만/과거 포함 선택, 검색 결과 제한, 공식 API 일정 우선순위를 유지한다.
- 기존 구현 판정: 일부 있음. 전역 검색의 다중 토큰 교집합 판정과 API 안내 검색의 NFKC·공백/구분 기호 정규화는 있었지만, Cafe24 행사 API가 검색어를 원문 한 덩어리로만 처리했고 날짜·분류 구조화 값과 공통 정규화를 공유하지 않았다.
- 근본 원인: 2026-06-05 느슨한 글자 와일드카드를 다중 토큰 교집합으로 교체한 뒤, 2026-06-11 Cafe24 조회 경로가 클라이언트 검색어 원문만 `q`로 전달하면서 토큰 의미가 서버 후보 조건에 연결되지 않았다.
- 수정: 클라이언트와 서버가 같은 행사 검색 정규화·토큰·날짜/분류 별칭 판정기를 사용한다. `28일소셜`은 `28일 + 소셜`로 분리하고 날짜 토큰은 저장된 시작·종료 날짜, 분류 토큰은 저장된 `category/activity_type`의 한국어 별칭으로 판정한다. 일반 검색은 공백과 기본 구분 기호를 제거한 연속어 일치 또는 모든 토큰 포함 규칙을 사용한다. 새 필드·테이블·인덱스·큐·상태값은 추가하지 않았다.
- 유지한 불변조건: 복합 검색의 모든 토큰 필수 조건, 미래만/과거 포함 기준, 최대 조회 수, 날짜 정렬, 공식 일정 우선과 상세 조회 흐름을 유지한다. 설명에 다른 날짜나 `무료 라인 강습 없음`이 있어도 저장 날짜·분류가 다르면 `29일소셜`, `28일강습`으로 오탐하지 않는다.
- 검증: 붙여 쓴/띄어 쓴 날짜·분류, 공백·구분 기호, 구조화 날짜·분류, 모든 키워드 필수, 설명 속 다른 날짜·분류 오탐 경계 회귀와 공식행사 우선·행사 보안 인접 회귀 12건이 통과했다. 대상 ESLint 오류 0건과 Cafe24 프로덕션 빌드가 통과했다. 운영 DB 읽기 전용 조회와 배포 뒤 공개 API 모두에서 `28일소셜`과 `28일 소셜`이 동일한 9건을 반환하고 8월 28일 소셜 3건을 포함했으며, `29일소셜`이 28일 행을 제외하고 `28일강습`이 소셜 행을 제외했다. Cafe24 서비스는 재시작 뒤 active였고 공개 버전 `1788053593121`을 확인했다.
- 관련 파일: `server/cafe24/event-search.js`, `server/cafe24/event-search.d.ts`, `server/cafe24/events-api.js`, `server/cafe24/event-search.test.js`, `server/cafe24/events-api.search.test.js`, `src/components/GlobalSearchModal.tsx`

## 2026-08-30 날짜가 지난 정규 소셜 휴무 회차 삭제

- 상태: 공통 원인 수정·Cafe24 운영 배포·공개 복구 검증 완료
- 현상: 2026-08-29 토요일 `경성홀 휴무`가 전날에는 캘린더에 표시됐지만 날짜가 바뀐 뒤 사라졌다. 같은 원문에서 만든 8월 30일 일요일 휴무는 아직 표시됐고, 운영 `scraped_events`에는 8월 23·29·30일 휴무 후보가 모두 남아 있었다.
- 판정 경로: 공식 원문 → `scraped_events` 휴무 후보 원장 → `planRegularSocialReconciliation`의 날짜별 휴무 물질화 → `events` 표시용 행 → 날짜가 지난 생성본 정리 → 공개 `/api/events` → 캘린더다. 8월 30일 00:00 KST 조정 로그의 `removes: 1`은 `eventDate(generated) < today` 조건에 걸린 8월 29일 표시용 휴무였고, 후보 원장이나 검색 기능이 삭제한 것이 아니었다.
- 기존 보호 목적: 커밋 `4756938c`에서 도입한 과거 생성본 정리는 근거 없는 `DJ 미정` 반복 회차가 계속 누적되지 않게 한다. 커밋 `a2ca355e`의 휴무 물질화는 휴무일을 정상 영업으로 오해하지 않게 하면서 실제 개별 소셜 우선, 결정론 ID 재시도 안전성, 포스터 미재사용을 보장한다.
- 기존 구현 판정: 일부 있음. 휴무 후보·외부 API 예외를 표시용 `social/휴무`로 만드는 경로와 `automation.exception_type=closure`, 결정론 ID, 과거 일반 생성본 정리는 이미 있었지만 과거 정리에서 근거 있는 휴무를 구분하고 삭제된 표시 행을 원장에서 복구하는 연결만 빠져 있었다.
- 근본 원인: 최종 정리 조건이 모든 `regular-social:*` 과거 회차를 같은 임시 생성본으로 취급했다. 휴무 표시 행도 날짜가 지나면 삭제됐고, 조정 범위가 오늘부터 시작해 원장에 휴무 후보가 남아 있어도 과거 행을 다시 만들 수 없었다.
- 수정: 기존 후보·외부 API 예외와 정규 소셜 조정기를 그대로 재사용한다. 규칙별 미래 일정에 더해 실제 과거 휴무 근거가 있는 날짜만 조정 대상으로 포함하고, 해당 근거와 연결된 `automation.exception_type=closure` 행은 날짜가 지나도 보존한다. 표시 행이 이미 삭제됐으면 같은 결정론 ID로 다시 만들며, 외부 API의 과거 `closure` 예외도 읽는다. 장소·날짜 전용 예외, 새 함수 소유자, DB 필드·테이블·큐·상태값은 추가하지 않았다.
- 유지한 불변조건: 근거 없는 과거 일반 생성본은 계속 삭제한다. 실제 개별 소셜이 있으면 휴무를 중복 생성하지 않고, 규칙 유효기간·미래 90일 범위·공식 규칙 우선·동일 ID 멱등성·휴무 포스터 미재사용·후보 및 외부 예외 원장 불변을 유지한다. 매분 작업이 과거 전체 날짜를 순회하지 않고 실제 휴무 근거 날짜만 추가 검사한다.
- 검증: 기존 휴무 재시도 테스트를 날짜 경과와 표시 행 삭제 복구 경계까지 확장해 수집 휴무 보존·복구, 일반 과거 생성본 삭제, 다음 미래 회차 생성을 한 테스트에서 확인했다. 정규 소셜 14건과 외부 API 인접 3건의 Vitest, 대상 ESLint와 프로덕션 빌드가 통과했다. 배포 전 운영 DB 읽기 전용 `dryRun`은 `creates: 3`, `removes: 0`이었고, 커밋 `4e52aff1`을 `origin/fix/sitewide-image-drag-integration-20260811`에 푸시한 뒤 Cafe24 공개 빌드 `1788074923221`을 배포했다. 공개 API에서 `regular-social:kyungsung-sat:2026-08-29`와 근거 원장이 남은 과거 휴무 2건이 `휴무`·공식 원문 링크와 함께 복구되고 기존 8월 30일 경성홀 휴무가 유지됨을 확인했다. 배포 뒤 재실행 `dryRun`은 `creates: 0`, `removes: 0`, `retained: 194`였고 `swingenjoy`, `crond`, `httpd`가 모두 active이며 배포 서버와 로컬 조정기 SHA-256이 일치했다.
- 관련 결정: `docs/decisions/2026-08-23-regular-social-closure-visibility.md`
- 관련 파일: `server/cafe24/regular-social-reconciler.js`, `server/cafe24/regular-social-reconciler.test.js`

## 2026-08-31 오늘 일정 없는 홈 광고 반응형 정렬

- 상태: 공통 반응형 원인 수정·로컬 운영 DB 화면 검증 완료, 운영 배포 대상
- 증상: 오늘 일정이 0개인 홈에서 데스크탑 광고 묶음이 화면 왼쪽으로 치우쳤고, 761~1199px 데스크탑에서는 바로가기 3개가 광고 아래로 내려갔다. 모바일에서는 바로가기 3개가 세로로 표시됐으며 광고 제목이 활성 포스터 중심과 어긋났다.
- 판정 경로: `todaySchedules.length` → 기존 `home-neb-desktop-grid--single` 상태 → 폭별 `MobileShell.css` 덮어쓰기 → 광고·제목·바로가기 최종 좌표 순으로 확인했다. 오늘 일정과 광고 원장은 읽기만 하며 DB·큐·알림 쓰기는 없다.
- 기존 보호 목적: 오늘 일정이 있으면 데스크탑의 광고/일정 2열과 모바일의 기존 일정 패널·버튼 배치를 유지하고, 키오스크 전용 배치는 일반 홈 규칙에서 제외한다. 비드래그 이미지 보호와 광고 슬라이드 동작도 유지한다.
- 기존 구현 판정: 일부 있음. 일정 0개용 단일 화면 클래스, 광고의 `--neb-active-left`·`--neb-card-width`, 모바일·데스크탑 버튼 묶음은 이미 있었다. 정확한 빈틈은 최종 1200px 규칙이 단일 열을 다시 2열로 덮고, 761~1199px 규칙이 제목을 왼쪽 정렬하며, 일정 0개 모바일 전용 가로 3열 연결이 없었던 것이다.
- 수정: 기존 클래스와 광고 좌표 변수를 재사용했다. 일정 0개 데스크탑은 전체 단일 열과 광고 왼쪽 세로 버튼을 유지하고, 일정 0개 모바일에서만 버튼 3개를 같은 높이의 가로 3열로 배치한다. 제목·날짜·장소는 모든 폭에서 활성 광고 폭과 중심을 사용한다. 새 컴포넌트·상태·필드·테이블·큐는 추가하지 않았다.
- 검증: 운영 DB 연결 로컬 빌드와 프로덕션 빌드가 통과했다. 390px에서 버튼 3개의 상단 좌표가 모두 `475.6px`, 활성 광고와 제목 중심이 모두 `301.5px`였다. 900px에서는 버튼 3개가 동일한 `left/right=36/176px`로 세로 유지됐고 광고와 제목 중심이 모두 `645.5px`였다. 1200px 이상 기존 단일 화면 중앙 정렬도 확인했으며, 오늘 일정 1개 이상 버튼은 단일 화면 선택자 밖이라 기존 배치를 유지한다.
- 관련 파일: `src/styles/components/MobileShell.css`

## 2026-08-31 Groove Lab 재즈 스윙의 단독 악기 재생

- 상태: 공통 원인 수정·Cafe24 운영 배포·공개 검증 완료
- 현상: `/groove-lab`의 `재즈 스윙` 계열이 라이드 단독으로 시작했고, 워킹 베이스나 Four-to-bar 기타를 선택하면 균등한 네 박만 들려 재즈 스윙 전체가 아닌 것처럼 들렸다.
- 판정 경로: 재즈 스윙 계열 선택 → 첫 단독 악기 프리셋 선택 → 악기별 `buildGrooveBar` 이벤트 → Web Audio 합성 출력이다. 악기별 타이밍 적용 범위는 맞았지만 최종 앙상블 조립이 없어 연구용 분해 결과가 사용자에게 완성된 스윙처럼 제시됐다.
- 기존 보호 목적: 템포별 롱–숏 비율은 드러머 라이드 측정 경향에만 적용하고, 워킹 베이스와 리듬 기타의 균등한 네 박, 고정 2:1 셋잇단, 셔플과 스트레이트를 구분한다. 무작위 humanize를 스윙으로 오인하지 않는다.
- 기존 구현 판정: 일부 있음. 라이드·2·4 하이햇·워킹 베이스·Four-to-bar 이벤트, 음색, 근거와 테스트가 모두 있었고 재즈 스윙의 기본 청취 단위로 합치는 연결만 빠져 있었다.
- 수정: 기존 세 프리셋을 재사용하는 `재즈 스윙 앙상블`을 추가해 첫 프리셋과 페이지 기본값으로 지정했다. 라이드의 적응형 비율만 변경되며 베이스·기타의 4분음표 위치는 유지한다. 화면의 표현 명칭도 `연구 기반 라이드 비율`과 `고정 2:1 셋잇단`으로 분리했다. 새 음원·타이밍 엔진·데이터 구조는 만들지 않았다.
- 유지한 불변조건: 단독 악기 분석 프리셋, 악기별 타이밍 범위, 셔플·셋잇단·스트레이트 구분, 결정적 합성과 출력 보호를 유지했다.
- 검증: 단위·근거 감사 23개, 관련 ESLint와 프로덕션 빌드가 통과했다. 실제 Chromium 오디오 감사에서 17보이스 45렌더, 29프리셋 전체 루프와 22개 발음 쌍이 통과했고 최대 프리셋 피크 0.788751에서 무음·비유한값·클리핑이 없었다. 커밋 `0d41cb2f`를 원격 브랜치에 푸시한 깨끗한 체크아웃에서 Cafe24에 배포했다. 공개 `/groove-lab`은 HTTP 200, 공개 빌드 `1788186973376`의 자산에서 `재즈 스윙 앙상블`, `연구 기반 라이드 비율`, `고정 2:1 셋잇단`을 확인했고 `swingenjoy`·`httpd`·`crond`와 내부 헬스가 모두 정상이다.
- 관련 결정: `docs/decisions/2026-08-31-groove-lab-swing-ensemble.md`
- 관련 파일: `src/pages/groove-lab/grooveEngine.ts`, `src/pages/groove-lab/GrooveLabPage.tsx`, `src/pages/groove-lab/grooveEngine.test.ts`, `scripts/groove-lab/browser-audio-audit.mjs`

## 2026-08-31 캘린더 헤더 검색의 구형 검색창 분기

- 상태: 공통 검색 연결 수정·Cafe24 운영 배포·공개 검증 완료
- 증상: 캘린더 헤더에서 `rsf`를 검색하면 설명 속 게시판 공통 문구에만 `RSF`가 있는 과거 일정이 관련도순 결과에 섞였고, 기존 `과거 포함` 옵션도 보이지 않았다.
- 판정 경로: 헤더 검색 클릭 → 캘린더 경로 판정 → `openCalendarSearch` 이벤트 → 구형 `CalendarSearchModal`의 전체 일정 조회 → 제목·설명·장소·주최자 단순 부분 일치 → 날짜순 원장 결과 표시다. `Bunny Hop`과 `Follower Fitness`의 설명에는 공통 안내문 `서린페, RSF 등`이 있어 화면에 보이지 않는 설명 문자열로 일치했다.
- 기존 보호 목적: 캘린더 검색 결과 선택 시 일정 상세를 열 수 있어야 한다. 통합 검색은 검색어 정규화와 모든 토큰 필수 판정, 오늘 이후 기본 조회, 사용자가 명시하는 `과거 포함`, 날짜 정렬과 상세 열기를 이미 소유한다.
- 기존 구현 판정: 기존 구현 있음. `GlobalSearchModal`에 필요한 검색·날짜 옵션·상세 흐름이 모두 있었고, 캘린더일 때만 별도 구형 검색 이벤트로 우회하는 헤더 연결이 정확한 빈틈이었다.
- 수정: 헤더 검색은 현재 경로와 무관하게 기존 통합 검색 모달을 연다. 새 검색 함수·옵션·필드·테이블·큐·상태값은 추가하지 않았으며 페이지 내부의 레거시 이벤트 경로는 다른 잠재 호출부의 호환을 위해 이번 변경에서 삭제하지 않았다.
- 유지한 불변조건: 일정 상세 열기, 과거 일정의 명시적 포함, 복합 검색의 모든 토큰 필수 조건, 오늘 이후 기본값, 다른 화면의 통합 검색 동작을 유지한다. DB·운영 데이터·알림에는 쓰기 부작용이 없다.
- 검증: 행사 검색 정규화 회귀 7건, 대상 ESLint 오류 0건, 프로덕션 빌드와 변경 공백 검사가 통과했다. 커밋 `4007f5bc`를 원격 브랜치에 푸시한 깨끗한 작업공간에서 Cafe24에 배포했다. 공개 빌드 `1788187469586`과 서비스·웹 서버 active를 확인했고, 공개 미래 검색에서 `rsf`는 2026-09-10 `RSF 2026 Theater Show: The First Beat`와 2026-09-11 `Rockin’ & Swingin’ Festival 2026` 2건만 반환했다. 캘린더 페이지의 헤더 연결만 변경되므로 장소·쇼핑·게시판 검색 데이터와 페이지 내부 달력 이동·강조 경로는 변경 경계에서 제외했다.
- 관련 파일: `src/layouts/MobileShell.tsx`, `src/components/GlobalSearchModal.tsx`

### 화면 후속: 동시 발음 병합과 음가 없는 점 표시

- 상태: 공통 표시 원인 수정·운영 DB 연결 로컬 화면 검증 완료, 배포 전
- 증상: 앙상블의 라이드·하이햇·베이스·기타가 같은 박에서 울릴 때 타임라인이 이를 한 점과 `3`·`4` 같은 개수로 합쳐 첫 이벤트의 색만 남겼다. 후속 악기별 점 레인도 음가·셋잇단 쉼·온/오프비트·강세·백비트를 읽을 수 없어 교육용 리듬 표기로는 부족했다.
- 판정 경로: `buildGrooveBar`의 정상 악기별 이벤트 → `timelinePoints`의 위치 단독 그룹화 또는 점 좌표 변환 → 음가와 기능이 없는 화면 → 사용자 해석이다. 오디오 합성·DB·큐·외부 전송은 이 표시 경로에 관여하지 않는다.
- 기존 보호 목적: 기존 단일 악기 프리셋에서는 같은 위치의 복수 발음을 한 점으로 압축해 좁은 화면에서도 박 위치를 읽게 한다. 이 동작과 오디오 타이밍은 유지한다.
- 기존 구현 판정: 일부 있음. 악기별 이벤트·색상·타임라인과 반응형 컨테이너는 있었지만, 앙상블 프리셋을 악기별 레인으로 연결하는 표시만 빠져 있었다.
- 수정: 악기별 레인을 폐기하고 단일 리듬 악보에서 매 카운트의 ON과 뒤로 밀린 OFF를 표시한다. 기본 앙상블의 실제 예약 이벤트도 네 카운트 모두 같은 ON/OFF 위치를 사용하며 2·4 백비트는 별도 강세 층으로 둔다. 적응형 모드는 라이드 연구의 교육용 보간 비율만큼 화면 간격과 소리를 함께 이동하고 셋잇단 괄호를 표시하지 않으며, 고정 2:1 모드에서만 셋잇단 괄호를 표시한다. 예약 큐의 기존 이벤트에 위치를 연결해 실제 오디오 시각의 음표만 활성화한다.
- 유지한 불변조건: 라이드에만 적용되는 스윙 비율, 하이햇 2·4, 베이스·기타의 균등한 네 박, 재생 활성점, 비드래그 화면과 단일 악기 표시를 유지했다.
- 후속 리듬 섹션 판정: 드럼과 베이스는 재즈 시간축의 기본 조합으로 유지하되 Four-to-bar 기타는 특정 전통 컴핑 선택지이지 필수 기본 파트가 아니다. Charleston 피아노도 가능한 컴핑 어휘 중 하나이므로 이름을 `컴핑 예시`로 제한하고 기본 리듬 섹션에는 넣지 않았다. 강제 4파트 합주는 폐기하고 드럼 시간축·워킹 콘트라베이스·선택 기타를 독립 토글로 연결했으며 기본값은 드럼과 베이스 ON, 기타 OFF다.
- 후속 음색 수정: 기존 워킹 베이스는 현 지연선과 저주파 사인파 중심이라 얇은 전기 베이스처럼 들렸다. 기존 웨이브가이드 소유자를 재사용해 밝기를 낮추고 감쇠를 늘렸으며 저차 몸통 공명과 짧은 손가락 공격을 추가해 피치카토 콘트라베이스 방향으로 보강했다. 실제 몸통 IR·브리지 어드미턴스·현/브리지 결합을 구현했다는 주장은 하지 않는다.
- 후속 실음 교체: 위 합성 보강도 음마다 같은 92 Hz 몸통 공명과 미세 디튠 이중 플럭을 더해 반복적이고 인공적인 음색을 만들었다. 기존 구현은 전기 베이스 주법·실패 시 합성 대체 경로로는 일부 재사용하되 워킹 콘트라베이스 연결은 빠진 것으로 판정했다. CC0인 VSCO 2 CE 솔로 콘트라베이스 피치카토의 E1·G1·B-flat1·C2 두 번갈이 녹음을 로컬 자산으로 연결하고, 워킹 라인을 E1–B1 실제 저음역으로 내렸다. 라이드·하이햇·기타의 이벤트 위치, 레이어 토글, 출력 압축과 샘플 로드 실패 시 무음 방지 합성 대체 경로는 유지했다. 새 DB·큐·상태값이나 외부 쓰기 부작용은 없다.
- 검증: 엔진·근거 회귀 23건, 대상 ESLint, 프로덕션 빌드와 변경 공백 검사가 통과했다. 운영 DB 연결 로컬 Chrome에서 적응형 2.50:1은 셋잇단 괄호 0개, 고정 2:1은 4개임을 확인했다. 1.5초 재생 표본에서 실제 예약 위치에 연결된 활성 음표가 32회 감지됐고, ON/OFF 네 쌍과 2·4 백비트가 단일 악보에 표시됐다. 운영 배포·커밋·푸시는 사용자 요청에 따라 제외했다.
- 후속 검증: 엔진·근거·합성 품질 회귀 34건, 대상 ESLint와 프로덕션 빌드가 통과했다. Chromium 오프라인 음원 감사에서 17보이스 45렌더·29프리셋·22발음 비교가 통과했고 비유한값·무음·클리핑이 없었다. 운영 DB 연결 로컬 화면에서 세 레이어의 독립 `aria-pressed` 상태와 기타 OFF→ON 전환을 확인했다.
- 실음 검증: 엔진·근거·음원 품질 회귀 35건과 대상 ESLint, 프로덕션 빌드가 통과했다. Chromium 오프라인 감사가 실제 콘트라베이스 MP3를 디코딩한 경우를 포함해 17보이스 45렌더·29프리셋·22발음 비교에서 무음·비유한값·클리핑 없이 통과했다(최대 단독 피크 0.377, 최대 합주 피크 0.789). 운영 DB 연결 로컬 8888 화면에서 드럼 OFF·베이스 ON 단독 재생이 6마디 이상 지속되고 베이스 관련 콘솔 오류가 없음을 확인했다. 배포·커밋·푸시는 요청 범위에서 제외했다.
- 상단 합주 제어 재검토: 증상은 악기 프리셋 카드와 아래 별도 레이어 믹서가 같은 선택을 중복 소유하고, 앙상블 라이드가 모든 박의 ON/OFF를 직접 연주해 표준 `1 · 2-& · 3 · 4-&`와 달랐던 것이다. 판정 경로는 상단 단일 `presetId` 선택 → 하단 `swingLayers` 필터 → 잘못 확장된 앙상블 이벤트 → Web Audio 출력·단일 악보였다. 기존 구현은 `일부 있음`으로 판정해 현재 `swingLayers`, 개별 라이드·베이스·피아노·기타 빌더와 예약 큐를 재사용했다. 상단 카드를 드럼·베이스·피아노·기타의 다중 ON/OFF와 전체 켜기/끄기의 유일한 조작부로 연결하고 하단 중복 믹서를 삭제했다. 합주 라이드에는 Butterfield의 표준 패턴과 2·4 하이햇을 복원했으며, 모든 박의 긴 ON·뒤로 밀린 강한 OFF와 2·4 백비트 스냅은 Carnegie Hall의 분리 연습 방식에 따라 합주 악기가 아닌 독립 교육용 가이드로 표시·재생한다. 악기 토글은 실제 예약 이벤트·근거·음표 표시를 같은 필터로 공유한다. 라이드 비율의 악기별 적용 범위, 워킹·Four-to-bar의 균등 4박, 선택 컴핑, 실음 베이스와 출력 보호는 유지했다. 엔진·근거·합성 품질 35건, 대상 ESLint, 프로덕션 빌드가 통과했고 Chromium 오디오 감사 17보이스·45렌더·29프리셋·22발음 비교에서 최대 합주 피크 0.789, 무음·비유한값·클리핑 없음이었다. 로컬 운영 DB 화면에서 개별 다중 선택, 전체 OFF→ON, 가이드 독립 ON, 재생 중 25개 표본 중 11개의 음표 활성화를 확인했다. 배포·커밋·푸시는 하지 않았다.
- ON/OFF 위치 후속 검증: 오디오는 140 BPM·2.50:1에서 OFF를 박의 71.4%에 정상 예약했지만, CSS가 두 길이 칸의 중앙에 음표를 놓아 화면 ON–OFF 중심 거리를 약 42%로 축소하고 정박처럼 보이게 했다. 음표와 카운트의 좌표를 실제 onset 비율에 직접 연결해 화면 OFF를 내부 악보 구간 71.4%에 배치하고 모든 OFF에 강세표를 표시했다. 브라우저 가이드 단독 측정은 ON→OFF 약 291–311ms, OFF→다음 ON 약 124–149ms로 긴–짧은 간격을 확인했다. 연구 보간값 자체는 임의로 과장하지 않았다.
- 셋잇단 기준선·기타 제거 후속: 스윙 전용 Four-to-bar 기타는 베이스와 역할이 다르지만 현재 축약 화면에서는 같은 네 정박만 표현해 학습 정보가 중복됐다. 기존 `균등 셋잇단음` 프리셋이 이미 정확한 `0·1/3·2/3` 전자 클릭을 소유하므로 새 엔진 없이 이를 합주 필터의 독립 `triplet` 레이어와 4박×3음 악보에 연결했다. 스윙 합주와 카드에서는 전용 기타 프리셋만 제거하고 다른 장르의 기타 음원·프리셋은 유지했다. 스윙 악보 계산에서 셋잇단 비교 이벤트를 제외해 기존 적응형 OFF와 라이드·백비트 불변조건을 보존했다. 엔진·근거·합성 품질 35건, 대상 ESLint와 프로덕션 빌드가 통과했다. 운영 DB 연결 로컬 화면에서 기타 카드 부재, 셋잇단 OFF→ON, 네 박 각 세 음과 재생 활성 표시를 확인했고, Chromium 오디오 감사 17보이스·45렌더·28프리셋·22발음 비교에서 최대 프리셋 피크 0.789, 무음·비유한값·클리핑 없음이었다. 배포·커밋·푸시는 하지 않았다.

## 2026-09-10 재수집 최종 검증과 날짜 해석 보완

- 증상 → 판정 → 부작용 → 결과: 공식 9/9 소셜의 신청 마감 문구 ‘9월 8일 23시’ → 구분자가 선택 사항인 일자 목록 정규식이 23을 날짜로 해석 → AI 근거 검사와 서버 등록 검사도 같은 구문 허용 → 9/23 잘못된 후보 생성. 정확한 원문 단독 재수집(저장 없는 검사)에서 재현했다.
- 기존 구현 일부 있음: d61966c87의 날짜 추출과 fe2b77c68의 압축된 복수 날짜 근거 지원이 원래 목적이다. 기존 세 계층의 구문을 고쳐 다음 날짜 앞에는 구분자를 요구하며, 완전성 검사에는 이미 있는 신청 마감일 제외 함수를 연결했다. 새 필드·큐·저장 구조는 없다. 복수 날짜·상속 월·휴무 제외·등록 전 근거 검증을 유지한다.
- 관련 등록/AI 검사 65건, 기존 수집 표준과 신청 마감 경계 확장, lint 통과. 무관한 UI/미디어 작업은 포함하지 않았다.
- 운영 복구: 공식 원문과 이미지 확인 후 기존 관리자 수집 후보/등록 API로 9/8 경성홀 아드리안, 봉천살롱 안토니와 9/9 소셜클럽 쵸리, 스윙타임 윤슬 총 4건 복구. 9/10 스윙스캔들 굼바 기존 등록 유지. 확인한 공식 소스 9곳에서 9/7 소셜 공지는 발견되지 않았다. 과거 수동 복구만 허용하고 정상 자동 수집 미래 날짜 정책은 유지했다.
- 공개 주간 API·실제 캘린더에 위 5건 표시, 이미지 5건 HTTP 200. 운영 조정 dry-run retained=197, creates=0, removes=0, 기본 소셜 누락 링크=0, 다음날 기준 과거 삭제=0. 이미 삭제되어 원문 근거가 없는 과거 행을 추정 생성하지 않았다.

- 최종 실제 원문 단독 수집 검증: 발견 1 / 후보 1 / 기대 1 / 준비 1 / 누락 0 / 차단 0, 저장 없는 실행 성공. 9/9만 남고 신청 마감 9/8과 시각 오인식 9/23이 제거됨.

- 배포 완료: f6b8dd25를 origin/codex/social-retention-recovery-20260910에 푸시 후 Cafe24 배포. 운영 health 정상, version buildTime=1789019537895 확인. 배포 직후 복구 4건 및 목요일 기존 1건 공개 API·이미지 HTTP 200 재확인.
- 동일 날짜 오인식으로 생성된 9/23 중복 2건을 읽기 전용 조사: 원문과 동일한 정상 9/2·9/9 이벤트가 이미 존재함을 확인. 기존 관리자 API로 잘못된 후보를 제외 처리하고, 이미지 연결을 해제한 뒤 잘못된 중복 이벤트만 정정했다(삭제 이미지 0). 원장/정상 이벤트/이미지 백업·보존. 기존 조정기가 9/23 기본 소셜과 수집 위치 링크를 다시 생성했고 공개 API 확인. 미수집 기본 일정의 자동 삭제와는 별개인 근거 확인된 오등록 정정이다. 복구 전 원장은 운영 임시 백업 /tmp/social-date-correction-before.json에 보관했다.

## 2026-09-10 소셜 바로가기 대상 정정

사용자 정책: 소셜 바로가기는 개별 게시물이 아닌 수집 게시판/계정으로 이동한다. 증상→판정→표시→외부 결과: 수집 원문 link1 저장→기존 상세화면(3d33d060)이 그대로 href 사용→개별 글 열림. 기존 구현 일부 있음: collection-registry의 사보이 메뉴 501 주소 및 findSourceByUrl을 재사용하고 상세화면의 표시 연결만 수정했다. 원문 link1은 수집 근거·중복 판정 용도로 보존하므로 DB/필드/마이그레이션을 추가하지 않았다. 기존 정규 소셜의 게시판 링크도 같은 경로를 사용한다. 소셜 외 활동과 등록되지 않은 출처는 기존 링크를 유지한다. 기존 표준 검사에 네이버 글→게시판, Instagram 글→계정 경계를 추가하고 빌드/실제 화면으로 검증한다. 수집 원장과 등록 로직은 변경하지 않으므로 재수집/운영 데이터 수정은 제외한다.

- 검증 완료: 기존 수집 표준·Cafe24 빌드 통과. d6717b9a를 푸시한 뒤 배포, health 정상 및 buildTime=1789019993812 확인. 운영 상세 화면에서 9/10 사보이 굼바의 href가 메뉴 501 게시판, 9/9 소셜클럽 쵸리의 href가 thesocialcluba 계정임을 직접 확인했다. 두 링크 모두 수집 위치 바로가기 표시 및 draggable=false 확인.

## 2026-09-10 봉천살롱 9/12 자동 수집 미반영 후속 확인

- 증상: 공식 게시판 264에 9/7 게시된 글 156741(9/12 DJ 스밤)이 기본 소셜로만 표시됨. 판정/실행 경로: swingtown-cafe는 priority=2 → 14:06 동시 예약 4개 중 priority=1만 진행, 나머지는 구형 잠금의 exit 75 → 해당 소스 수집 실행 없음 → 운영 후보 e0d60fce9b7d52be 부재. 이전 작업의 진단 실행에서는 ready=true였지만 dryRun으로 저장하지 않았고 수동 복구 범위는 9/7~10이었다.
- 기존 구현 있음: 10e7a762에 적용된 잠금 대기 로직과 설치 실행기 연결을 재확인했다. 실제 수집 심볼 collectNaverArticleLinks → scrapeNaverArticle → 후보 판정 → 기존 후보 저장/automatic:true 등록을 재사용했다. 이번 증상은 수정 전 예약 누락과 진단/복구 범위 차이로, 새로운 판정 예외·코드·필드·우선순위 변경 없이 운영 재실행으로 해결했다. 기존 단일 브라우저 실행 보호, 오늘 이후 날짜, AI 근거 검증, 중복 방지를 유지한다.
- 처리: 9/10 기준 swing-daily를 swingtown-cafe 소스만 지정해 저장 활성화로 재실행. 사용자/관리자 수동 등록 우회 없음. 발견 3, 후보 1, 저장 1, 자동등록 1, 공개확인 1, 누락/차단 0. 지난 8/29·9/8 글은 정상 제외.
- 결과: event cf7dd06a-6680-4bf7-b97c-52c8ddfc9bd9, 2026-09-12 DJ 스밤 | 봉천살롱 토요 소셜. 공개 API에서 원문 156741 연결과 단일 등록, 이미지 HTTP 200 확인. 기존 수집 표준 통과. 코드 변경이 없어 추가 배포/중복 회귀 테스트는 제외했다. 예약 실행 전체 성공을 이번 단일 소스 재실행의 결과로 주장하지 않는다.

## 2026-09-10 프렌즈 카페 강습 수집 누락 조사 (원인 확인, 수정 전)

- 사용자 질문: 프렌즈 카페 강습 수집이 중단됐는지 확인. 설정상 swingfriends-cafe/swing_friends는 class 허용 유지. 운영 원장에 9/26 Big Apple·9/27 BANGKOK STRIKE 강습 collected 기록 존재하므로 전체 강습 비활성으로 판정하지 않는다.
- 증상→선정→실행→원장: 기존 카페 source는 타임바게시판(85), 강습소개(321)는 목록에 없음 → collectNaverArticleLinks의 제목 날짜 우선 정렬(fe2b77c68)과 resolveSourceScanLimit 기본 3개 → 9/10 진단에서 지난 9/9·9/5~6·9/2 소셜만 읽음 → 후보/저장 0. 예약 priority 2 묶음의 당일 구형 잠금 차단도 별도 확인됐다.
- 실제 게시판에서 강습소개 및 전체 4학기 공지 56164, 워크샵 56311을 확인. 전체 학기 공지는 looksLikeBroadScheduleNotice(d8ab0f9e8)로 차단하며, 복수 강습·신청 날짜를 단일 일정으로 잘못 등록하는 것을 막기 위한 기존 보호다. 개별 강습 추출·등록 책임은 일부 존재하지만 게시판 범위·게시물 선정·종합 시간표 분해에 빈틈이 있다. 새로운 소스/파서/날짜 예외는 아직 추가하지 않았다.
- 검증: 현재 설정 그대로 저장 없는 단일 소스 진단 성공, 발견/검사 3, 후보 0. 기대 일정 검사도 소셜 중심으로 0이라 강습 누락을 성공처럼 지나칠 수 있음을 확인했다. 구체적 누락 강습/게시판 링크를 사용자에게 요청했으며 대상 강습 복구 여부는 미확정. 이번 질문에 대한 조사로 운영 데이터 변경·추정 강습 등록·배포는 수행하지 않았다.

## 2026-09-10 승인된 데스크탑 상세보기 디자인 적용

- 사용자 승인: 별도 샘플 확인 후 적용·배포 지시. 기존 구현 일부 있음: EventDetailModal 및 5664a5728의 768px 이상 데스크탑 규칙, getCalendarSocialDisplayText, 확대 포털을 재사용한다. 기존 데스크탑 규칙을 교체해 중복 스타일 소유자를 만들지 않았다.
- 화면 경로: 실제 이벤트 → 기존 제목/DJ·날짜/장소·권한 판정 → 상세보기 표시 → 게시판/장소/공유·편집 동작. 데이터/외부 쓰기 로직은 변경하지 않는다. 포스터/정보 2열, 단순한 구분선, DJ 분리, 상단 도구, 하단 주 링크, 접는 관리자 정보로 승인 샘플을 적용한다. 미디어 쿼리 상태는 기본 접힘과 모바일 펼침·제목 표시를 동기화하는 컴포넌트 내부 상태다.
- 기존 보호 유지: 실제 안내문/다중 날짜/휴무 표현, 관리자 권한, 장소 상세, 공유·편집·삭제 처리, 모바일 기본 배치, 비드래그 이미지/링크. 전체 화면 확대 상태/포털은 존재했지만 열기 호출부가 없어 데스크탑 포스터 클릭·키보드에 연결했다. 이미지 편집은 기존 선택 모드에 한정한다.
- 검증: 공개 운영 데이터를 사용하는 로컬 화면 1440/820/390px, 포스터 로딩·확대·닫기, 원문 게시판 링크, 모바일 원래 제목, 가로 넘침 없음 확인. 기존 DJ/휴무/졸업 표시 회귀 9건, Cafe24 빌드 통과. ESLint 오류 0(기존 경고 59). 데이터/수집/등록 로직 무변경이므로 재수집과 운영 데이터 변경 검사는 제외했다.

- 배포 완료: 5da87634를 origin/codex/social-retention-recovery-20260910에 먼저 푸시한 뒤 Cafe24 배포. buildTime=1789021674911, 서비스 active 및 공개 health.ok=true. 운영 사이트에서 실제 사보이 굼바 상세의 제목/DJ 분리·메뉴 501 링크·가로 넘침 없음·포스터 확대/닫기를 재검증했다.

## 2026-09-10 데스크탑 월 표시 세로 정렬

- 증상 → 기존 grid 버튼의 세로 정렬 기본값 → 글자 영역 늘어남 → 데스크탑 월 표시 상단 치우침. DB/외부 쓰기 없음.
- 기존 구현 있음: MobileShell 제목 버튼과 21f47c87c의 데스크탑 헤더 정렬을 재사용. 721px 이상 기존 규칙에 align-content/align-items: center 두 속성만 추가한다. 버튼 크기·연도/월 기준선·월 선택 동작·모바일 스타일 유지, 새 구조 없음.
- 검증: 공개 데이터 연결 확인 화면에서 1440px/980px 글자 영역과 버튼 중앙 차이 0px, 390px 기존 normal 정렬 유지. CSS diff 검사 통과. 데이터 수집·등록 및 모바일 동작 변경이 없어 해당 회귀는 제외. 사용자 배포 승인에 따라 커밋/푸시 후 기존 Cafe24 배포 절차 실행.

## 2026-09-10 강습 수집 게시판 대상 정정 및 월 정렬 배포 확인

- 사용자가 제시한 원장은 cafe 10342583/menu 13. 앞선 프렌즈 cafe 10026855/menu 85 조사는 다른 게시판이며 이 URL의 원인이 아니다.
- 원인: cd838a488(2026-07-28)이 운영 종료 스윙패밀리 소스 제거 시 menu 13과 해당 menuid 원문을 retired-swingfamily 규칙에 포함했다. 현재 수집 목록에는 스윙타운 menu 264/233만 존재. 이 주소는 누락된 연결만이 아니라 명시적 제외 대상이다. 기존 목적은 종료 단체 후보 재유입 방지이며, 현재 게시판 운영 성격 확인 없이 제외 규칙을 해제하지 않았다. 코드·운영 데이터 변경 없음.
- 월 정렬: 48204e64를 origin/codex/social-retention-recovery-20260910에 먼저 푸시하고 Cafe24 배포 성공. 서비스 active, 공개 buildTime 1789023014933. 강습 수집 복구가 완료됐다는 의미는 아님.

## 2026-09-10 지정 강습 원장 menu 13 복원

- 원인: cd838a488의 운영 종료 단체 제외가 현재 외부 강습 게시판까지 차단. fe2b77c68의 월간 소셜/날짜 제목 우선순위도 최신 개별 강습 읽기를 방해했다. 브라우저에서 실제 게시판명과 최신 민트·호링·문이화 글을 확인했다.
- 기존 구현 일부 있음: 공용 registry/native collector/candidate validation 및 AI 재검증을 재사용. menu 13 URL 제외만 해제, 별도 class-only 메뉴 소스를 연결하고 원문 장소를 요구. 강습 전용 선택은 월간 공지보다 최신 개별 글 우선. 종료 단체 이름/보관 링크 제외, 소셜 월간 공지 우선, 이미지·첫 회 날짜·중복 방지는 유지한다. 새 파서/DB/큐 없음.
- 검증: 수집 표준 통과, 후보 활동 분류 2건 및 AI 근거 검증 28건 통과. 지정 게시판 최신 3건 선택 확인. 첫 실행은 월간/상단 공지에 소비되어 후보 0이었으며, 개별 글 우선 변경 후 호링 강습 후보가 발견됐다. 수집·등록 최종 결과는 배포 후 별도 기록한다.

### 강습 원장 복원 배포 및 실제 수집 결과

- 1d8c50ff를 origin/codex/social-retention-recovery-20260910에 푸시 후 Cafe24 배포 완료. 서비스 active, 공개 buildTime 1789023530362 확인. 예약 실행이 읽는 로컬 작업 트리에도 해당 변경만 패치했다.
- 실제 자동 경로: menu 13 최신 글 3건 검사 → 강습 후보 1건 저장 → 자동등록 0건. 원장 읽기 확인: 6f5f84a862d788eb / 호링쌤과 Fast Dynamic Swivel / 2026-10-16 / source_id swingtown-lessons-cafe / is_collected false. 운영 캘린더에 등록됐다고 주장하지 않는다.
- 남은 문제: 실제 글에 장소가 있으나 장소 표기 추출이 실패해 explicit venue 자동등록 검증에서 보류. 저장 없는 앞선 진단에서는 문이화 후보도 발견됐지만 실제 실행에는 포스터 미확보 문제가 남았다. 원장 연결 복원과 모든 강습 자동등록 완료를 구분한다. 장소 라벨 파서는 133257c26에서 줄 시작의 명시 장소만 인정해 잘못된 장소 추정을 막도록 도입됐으며, 이번 연결 변경에서 그 보호를 완화하거나 수동 강제등록하지 않았다.

## 2026-09-10 강습 후보 검수 등록 복구

- 사용자 지시: 지정 강습 게시판에서 수집해 캘린더 반영. 기존 prepareCandidate/중복 검사/관리자 검수 등록 API를 재사용했으며 파서 예외나 DB 직접 쓰기를 추가하지 않았다.
- 원문 근거: 156739에 10/16 첫 회와 사당 올웨이즈 연습실 A홀(방배천로4안길 84)이 명시됨. 156721에 9/17 시작 및 남부터미널 빅애플이 명시됨. 이전 수집기의 제목·장소 추출 실패를 이번 사용자 승인 검수로 보완했다. 자동 AI 승인으로 기록하지 않는다.
- 준비된 후보는 기준 검증과 기존 일정 중복 검사를 통과. 기존 후보 백업 후 /api/scraped-events 및 /api/ingestor-register-event 경유 저장. 원문 포스터는 운영 업로드로 보존, 후보가 등록 일정에 연결됨.
- 결과: 호링 Fast Dynamic Swivel (2026-10-16), event 2f45f604-c7cb-47dd-b386-805c6e0410a4 / candidate 6f5f84a862d788eb. 문이화 베이직원정대 Swing Out Variation (2026-09-17), event 97056589-4796-4fe3-a5fe-979049757847 / candidate 043fb0126d8d13ad. 공개 API에서 class 분류·날짜·장소·원문 링크 확인, 각 URL+날짜 단일 일정 및 포스터 HTTP 200 확인.
- 범위: 검수 가능한 2건을 반영했으며 게시판 전체 수집 완료라고 주장하지 않는다. 추가 브라우저 조회는 본문 준비 선택자 시간 초과로 검증하지 못했고, 민트 글은 앞선 수집에서 포스터 미확보였다. 기존 미래 첫 회·이미지 필수·중복 방지 유지. 운영 복구이므로 코드 배포/관계없는 전체 테스트는 수행하지 않았다.

## 2026-09-10 강습 원장 첫 페이지 15개 글 대조 및 누락 복구

- 사용자 요청: 목록의 미등록분이 많으므로 대조해 등록. 첫 페이지 일반 글 15개 전체 본문과 연결 신청서를 브라우저로 확인. 앞선 임시 점검은 주 페이지에서만 본문을 찾았으나 실제 cafe_main 프레임을 확인해야 했음. 운영 수집기는 이미 프레임 처리를 가지고 있어 새 파서 변경 없이 점검 절차를 바로잡았다.
- 결과: 기존 등록 6개 글 유지, 누락 7개 글에서 9개 일정 검수 등록(피에스타 3일 분리), 종료된 8/17 원데이 1개 제외, 민트 1개 포스터 없음. 목록 반영 범위에 진행 중인 강습을 포함해 실제 시작일로 복원했으며 미래 날짜로 바꾸지 않았다.
- 보호 목적 유지: 원문 날짜/장소와 원본 포스터 확인, URL+날짜 결정론 후보 ID, 기존 중복 검사, 관리자 검수 API를 재사용. 자동 AI 승인으로 꾸미지 않음. SUNNY UP 장소가 주차별로 바뀌므로 첫 회 장소와 주차별 안내를 설명에 보존. Swing Together는 게시판 연결 신청서의 실제 원본 이미지를 브라우저 응답에서 확보했다.
- 검증: 기존 후보 백업 후 수집 저장 및 등록 API 사용. 9건 전부 공개 API class 분류, 정확한 날짜/원문 링크, URL+날짜 단일 건, 포스터 HTTP 200 확인. 코드/스키마/예약 규칙 변경이 없어 추가 배포와 무관한 전체 회귀검사는 제외. 민트는 연결 신청서의 아지트 연습실까지 확인했지만 이미지 필수 지침 때문에 이미지 없는 등록 여부를 사용자에게 질문했다. 전체 과거 페이지의 완료를 주장하지 않는다.
- 2026-09-07 문탄·이화 Monthly Jazz 9월 / event e16684ab-5d80-4886-998c-1b2aab6e0f99 / candidate e0307a258f6b433c
- 2026-09-03 리코 솔로재즈 Let’s Get Tricky / event 5c80c5c9-16d9-4a1f-88b5-957364ddcd94 / candidate 32e26a84416d8107
- 2026-08-24 SUNNY UP 시즌 8 뮤지컬리티 / event 2e9f7f80-e0c5-4da2-a5c8-c20ee048272f / candidate 85cfd17682bb7791
- 2026-09-14 Swing Together 린디합 루틴 프로젝트 시즌 12 / event 7017fc6c-e9f3-4c16-90e5-12fc9882c1e7 / candidate 41acbd62f041d5f1
- 2026-09-08 솔로재즈 JAZZBUZZ 시즌 34 / event 07a6e3ef-0218-4c5e-9411-1f5fadf17beb / candidate a074c221cf4d3b68
- 2026-09-09 솔로재즈 초급 EASY BUZZ 시즌 12 / event 11fdcd91-360b-4682-92a7-a1c4206459ad / candidate a241940ce3ab9969
- 2026-09-25 피에스타 한가위 4인4색 워크숍 / event 0453f0ed-f653-4381-94fa-d20cc452e782 / candidate 9dea11e89becbffa
- 2026-09-26 피에스타 발보아 매직데이 워크숍 / event 9ff69a66-63ca-436a-9a1c-d2ae8b6c8a4c / candidate f6ba9d51c9d259d4
- 2026-09-27 피에스타 슬로우 린디합 & 블루스 워크숍 / event f2369c12-aad5-4752-b0a9-a4026999a771 / candidate ad5a3cc1e248ad0f


### 2026-09-10 — MINT 강습 포스터 없이 수동 등록 완료
- 사용자 요청: 포스터가 없어 보류된 메뉴 13의 MINT 강습(게시글 156749)을 등록.
- 경로/판정: 원문·신청서에 포스터 없음 → 기존 후보 검증의 `poster_url or imageData required`로 자동 수집 보류 → 사용자 명시 요청에 따라 기존 관리자 수동 등록 API 사용 → 수집 원장과 공개 events 반영. 기존 구현 있음; 코드·스키마 추가 없음.
- 기존 보호 목적: 자동 등록의 원본 이미지 및 검증 근거 확보. 이번 사용자 검토 등록만 이미지 없이 처리하며 자동 수집 정책은 변경하지 않음. 날짜·장소 검증, 제외 상태 보호, 출처/날짜 중복 방지는 유지.
- 결과: 후보 `bd124d788deeb80c`, 이벤트 `3a006d3e-9cef-4cde-93f1-6931b617916b`; MINT 린디합 트레이닝 9·10월 시즌, 2026-09-17 시작, 아지트 연습실. 원문 링크와 휴강 포함 전체 과정 안내 저장. 허위 이미지 없이 image/image_full=null.
- 검증: 후보 검증 오류는 이미지 누락 1개뿐. 운영 수동 등록 성공 및 공개 `/api/events`에서 강습 분류·날짜·장소·원문·이미지 없음·출처/날짜 유일성(1건) 확인. 데이터 복구만 수행하여 코드 배포/전체 테스트는 불필요. 기존 수동 경로 재사용으로 중복 구현 없음.


### 2026-09-10 — 포스터 없는 강습 자동수집·등록 정책 변경
- 상태: c0f187f2 커밋·origin/codex/social-retention-recovery-20260910 푸시 후 운영 배포 완료.
- 증상 → 판정 → 부작용 → 원장: 포스터 없는 원문 → native 이미지 차단/공통 후보 검증/자동등록 준비/서버 이미지 필수 → 후보 저장 또는 등록 중단 → 캘린더 누락. 이미지 원본 확보 목적의 기존 보호(후보 a77ca3fc, 소셜 예외 fe2b77c)를 사용자 요청에 맞춰 class에 한해 완화.
- 기존 구현 일부 있음: 기존 수집/수동/자동 API 재사용. 메뉴 13은 1d8c50ff에서 수집 레지스트리에 추가됐으나 서버 AUTOMATIC_REGISTRATION_SOURCE_RULES 연결이 누락됨. 기존 허용 목록(1a6211406)의 소스/활동 제한 목적을 유지하며 class-only, explicitVenue로 연결. 후보별 예외나 신규 데이터 구조 없음.
- 변경: 강습 이미지 누락 차단 해제(스윙/확장 native, 후보 검증, 준비 판정, 서버); 수집 스킬 및 소스 정책 문구 갱신. 있던 포스터 보존, 날짜/장소/AI 근거/종료 소스/중복/기등록 안전장치 유지.
- 검증: ingestion standards 통과(유료·무료 text-only 강습, 포스터 보존, 과거 일정/장소 누락 차단 및 기존 소셜/할인 보호); 서버 등록/중복·재시도와 AI 근거 검증 71개 통과. 양 native 문법 검사 통과. 수동 등록은 기존 37개 등록 연계 검사로 확인하며 중복 테스트나 스키마/전체 UI 검사는 영향 범위 밖이라 제외.

- 배포 확인: buildTime 1789028790402, 서비스 active, 공개 calendar HTTP 200 및 version 일치. 실제 MINT 후보를 배포 서버에서 읽기 전용 재검증하여 포스터 없이 수집/등록 준비 통과, 서버 소스 허용 통과, 실제 AI 승인 없이는 여전히 차단 확인. 기존 포스터 없는 MINT 공개 이벤트 보존 확인. 자동 실행이 참조하는 원래 작업 폴더에 동일 패치와 스킬 지침 반영 완료. 운영 검증용 신규 이벤트 삽입은 하지 않음.


### 2026-09-10 — 살사 고정 소스 자동수집 기초조사
- 상태: 1차 조사 완료. 서울·수도권 원본 확인 및 지방 발견 후보 정리. 운영 이벤트·스케줄·코드 변경/배포 없음.
- 기존 구현 있음/일부 연결: expanded-research·레지스트리·기존 씬맵 재사용. 스윙 daily scope 단독 및 salsa 연구 저장 비활성 확인. 과거 턴/보니따 계정과 현재 주간 안내의 연결 주소가 달라 원본 재검증 필요.
- 발견: 살사바·동호회·강사/학원 혼합 운영. 고정글, 협업 게시, 한 글의 다중 날짜/홀·장르, 오래된 개강 소개가 자동수집 오류 요인. 실제 강남 라틴 9/10 공지·JDC 9/12 강습·SIDF 공식 날짜 확인; 검증 수준과 접근 제한을 분리 기록.
- 결과: docs/salsa-scene-research-2026-09-10.md에 고정 URL, 근거, 최근 확인 범위, 보류 사유, 지역 확장 및 기존 경로 재사용 시범안 기록. 데이터 저장이나 코드 변경이 없어서 빌드/배포 대신 공개 원문·최신성·프로필 설정을 검증함.


## 2026-09-11 — 오늘 스윙 소셜 누락: 미독 게시물과 포스터 판독

- 상태: 원인 수정·오늘 2건 운영 복구 및 배포 검증 완료.
- 증상 → 판정 → 부작용 → 원장/외부 결과: 08:00 예약 실행은 미래 소셜 5건을 등록했지만 인더무드 신규 7개 중 앞 2개만 읽고 나머지를 remainingSources에 남기지 않았다. 오늘 드림발 공지는 세 번째였다. 네오 주간 공지는 본문이 ‘9월 2주 위클리네오 / 금햅 DJ 쓴귤’이고 정확한 날짜·장소가 포스터에 있었지만, AI 진입 조건이 ‘소셜/social/정모 + 숫자 날짜’를 요구해 후보 0건을 확인 완료로 기록했다. 공개 API에는 오늘 DJ 소셜 대신 정규 생성본 2건만 남았다.
- 기존 구현 판정: 일부 있음. dcd369e0의 신규 글 체크포인트·묶음 제한과 fe2b77c68의 실행 예산·원본 이미지 AI 검증을 재사용한다. 직접 경계는 swing-daily-native의 collectSource/main/buildAiSocialFallbackCandidates 및 ingestion-progress의 완료 판정 호출부다. 새 DB 필드·테이블·영속 큐·상태값·예약을 추가하지 않았다. 기존 AI 진입 함수는 AI 모듈로 이동해 동일 책임 아래 검사한다.
- 기존 보호 목적 및 불변조건: 반복 읽기 비용 억제, 2개 단위 처리·20분 전체 예산·접근 제한 회로, 공식 출처/날짜/장소/DJ 근거, AI 98% 검증, 서버 등록 허용 정책, 결정론 ID·중복 방지를 유지한다. 미수집 정규 일정 보존 및 게시판/계정 바로가기도 유지한다.
- 수정: 제한에 걸린 미독 원장은 기존 remainingSources에 유지한다. 성공한 묶음만 다른 수집원 뒤에 이어 읽고, 예산이 끝나면 체크포인트에서 다음 실행이 재개한다. 금햅/일햅 및 DJ 포스터를 기존 이미지 판독에 연결하되 등록 근거를 완화하지 않는다. AI 오류·미확정 판독은 기존 post 실패 경로로 남겨 확인 완료를 막는다. 충분한 확신으로 대상 없음이 확인된 글은 완료 처리한다.
- 운영 복구: 9/11 드림발 공식 DdHGtNey09h와 네오 공식 주간 DdDYinBzasB를 브라우저에서 읽고 포스터를 대조했다. 수집 원문을 수정된 기존 후보 생성·AI 검증·postCandidate에 재입력하여 후보 저장/automatic 등록 API로 2건 반영했다. DJ 충하(인더무드신림) 이벤트 4d20bfe7-dc48-4294-a630-a4e1c8aec740, DJ 쓴귤(해피홀) 이벤트 261114d5-5258-4dbc-be94-319c4dc5e398. 승인된 오늘 소셜만 반영했고 직접 DB INSERT나 후보 상태 초기화는 하지 않았다. 공개 날짜·장소·DJ·포스터·원문 확인.
- 한계: 해피홀 계정 단독 글은 장소 근거 부족으로 계속 보류하며 허용 정책을 임의 변경하지 않는다. 동일 일정은 날짜·장소가 적힌 네오 원본 포스터로 등록했다. 주간 공지의 ‘9월 2주’와 ‘라인강습 쉬어갑니다’로 만들어진 9/2 휴무 후보는 건조 실행에서 발견했으나 이번 복구에서는 저장하지 않았다. 별도 날짜/휴무 판독 조사 대상으로 남긴다. 아침의 7개 로그인 제한 원장이 이번 수정으로 복구됐다고 주장하지 않는다.
- 검증: 기존 ingestion standards의 완료/재시도·부분 실행 경계를 확장했다. AI 원문/이미지·DOM·서버 등록/재시도·정규 소셜 보존 95개, 자동등록 보고 Node 검사 3개, 표준 검사와 lint 통과. 실 native main/collectSource를 외부 브라우저만 모의한 실행에서 5개+1개 글을 2→다른 원장 1→2→1 순서로 모두 읽고 잔여 원장 0 및 완료 체크포인트를 확인했다. 실제 원문 재입력에서 네오 9/11·9/13 추출 및 독립 AI 검증을 확인했고 운영에는 승인된 9/11 두 건만 등록했다. 무관한 UI/살사 변경 및 전체 테스트는 제외한다.
- 배포 확인: 구현 7fdf4664를 origin/codex/social-retention-recovery-20260910에 먼저 푸시한 뒤 Cafe24 배포 성공. buildTime 1789120814006, 서비스 active, __health ok. 서버 수집 파일 2개의 SHA-256이 배포 작업본과 일치한다. 예약 실행이 사용하는 원래 작업 폴더에도 해당 패치만 동기화하고 표준 검사 통과. 공개 데이터 전후 비교에서 오늘 정규 생성본 2개만 실제 공지 2개로 교체됐으며 다른 일정 삭제/추가 없음. 실제 데스크톱 캘린더 오늘 칸에서 해피홀 DJ 쓴귤·인더무드신림 DJ 충하 표시 확인.


## 2026-09-15 — 소셜 상세보기의 수집 위치 우선 배치 (로컬 검증 완료)

- 요청/범위: 소셜 상세의 이미지를 줄이고 수집 위치 → 장소 → 설명 순서로 강조. 강습·일반 행사 상세는 기존 화면 유지. 후속 배포 요청에 따라 운영 반영 진행.
- 경로/기준: 기존 isEventDetailSocialLikeEvent 판정 → link1의 findSourceByUrl 매칭 → 상세 화면 링크 렌더링 → 수집 게시판/계정 열기. 큰 이미지가 첫 화면을 차지하고 고정 하단 링크가 기능 버튼과 공간을 나눠 출처명이 잘리는 배치가 원인이다.
- 기존 구현 일부 있음: EventDetailModal, collection-registry, 장소 열기, 이미지 확대 포털, 소유자/관리자 수정·삭제 및 calendar의 weeklySocial 등록 진입을 재사용했다. d6717b9a와 기존 9/10 출처 정정 기록의 게시판/계정 연결 목적, 58e2caa8의 중첩 모달 클릭 전파 방지 목적을 유지한다.
- 변경: 소셜 전용 EDM-socialDetail 범위에서 84×106 썸네일(좁은 화면 68×90), 상단 출처 카드, 장소/주소, 상세 설명, 하단 글자 있는 기능 버튼 적용. 모바일 사진 확대와 이미지·링크 드래그 방지. 출처 미등록은 빈 상태로 표시한다. 원본 link1, DB/스키마/수집 원장, 권한, 수정 모드의 삭제 확인·진행 중 중복 차단은 유지. 신규 저장 구조/등록 로직 없음.
- 등록 연결: 처음 확인한 기존 /social?action=register_social 페이지는 length 접근 오류가 발생했다. 이 페이지의 별도 수정은 범위 밖이며, 캘린더가 이미 사용하는 weeklySocial 모달로 연결하고 정상 표시를 확인했다.
- 검증: build:only 통과. 기존 calendarEventKind 9개 및 EventEditBottomSheet 2개 테스트 통과. 실제 운영 DB를 기존 SSH 터널로 연결한 로컬 8888에서 첨부와 같은 9/9 스윙타임 DJ 윤슬 상세, 게시판 대신 Instagram 계정 href 보존, 사진 확대/닫기, 등록 모달 진입 확인. 320px 화면에서 수평 넘침 없음(스크롤 폭=콘텐츠 폭), 출처/하단 버튼 화면 내 노출, img/a draggable=false 확인. 데스크톱에서도 소셜 썸네일 배치 확인. EASY BUZZ 강습 및 RSF 일반 행사는 소셜 클래스/출처 카드 없이 기존 큰 포스터 배치 유지 확인.
- 제외: 운영 등록·수정·삭제 실행과 수집/서버 전체 회귀는 데이터·권한·저장 로직을 변경하지 않아 제외. 비로그인 UI에서 관리자 버튼 실행은 검증하지 않았으며 기존 권한 조건 및 삭제 핸들러는 변경하지 않았다. 기존 미커밋 작업을 보존하고 관련 컴포넌트/CSS와 본 기록만 추가 수정했다.

- 배포 준비: 직전 운영 version 1789443495777의 기반 브랜치 5783d0b0에서 격리한 작업본에 소셜 상세 변경 2개 파일만 반영. 변경 전 스냅샷과 기반의 두 파일이 일치함을 확인했다. latest origin/main 포함 및 기존 운영 수집/등록 수정 보존. 커밋·푸시 후 기존 deploy:cafe24 절차를 사용한다.

- 운영 배포 완료: fa65ac14를 origin/codex/social-detail-source-20260915에 먼저 푸시한 후 deploy:cafe24 성공. version 1789471115247 (2026-09-15T11:18:39.113Z), 서비스 active, 공개 __health ok 및 버전 일치 확인. 운영 스윙타임 9/9 상세에서 84px 썸네일과 Instagram 수집 계정 링크 확인. 288px 모바일에서도 수평 넘침 없음(258/258px), 출처·하단 기능 버튼 화면 내 노출 및 draggable=false 확인. EASY BUZZ 강습·RSF 일반 행사는 소셜 레이아웃 없이 기존 약 587px 포스터 영역 유지 확인. 관련 기존 회귀 11개 통과. 등록·수정·삭제 데이터 쓰기는 검증 중 실행하지 않음.

## 2026-09-15 — 소셜 제목과 수집 바로가기 통합

- 사용자 후속 요청: 제목을 누르면 수집 링크로 이동하도록 제목/별도 출처 카드를 합치고, 이미지는 확대보기 유지.
- 기존 구현 일부 있음: fa65ac14의 소셜 제목·EDM-sourceCard·shortcutUrl·이미지 확대 포털을 재사용. 기존 두 영역의 분리가 원인이며 제목 안으로 기존 링크를 옮기고 중복 출처 섹션/CSS를 제거했다. 별도 링크/상태/저장 책임을 추가하지 않았다.
- 보호/불변조건: 소셜만 변경, 강습·행사 기존 제목 유지. 원본 link1와 게시판/계정 URL 매칭, 수정 모드 제목 편집, 링크 없는 제목의 일반 텍스트 표시, 권한/삭제/등록 경로 유지. 이미지에는 확대 표시를 넣되 지도 대체 이미지에는 확대 표시를 넣지 않는다. 이미지·링크 드래그 금지 유지.
- 로컬 검증: build:only 통과. 실제 DB 연결 스윙타임 상세에서 제목 안 링크 1개, 별도 출처 섹션 0개, 기존 Instagram 계정 href 확인. 포스터 확대/닫기 성공. 저장 경로 변경이 없어 운영 데이터 쓰기·수집 전체 회귀는 제외. 기존 배포 승인 범위의 후속 UI 조정으로 커밋·푸시 후 재배포한다.

- 배포 검증 완료: 1e0120d0를 origin/codex/social-detail-source-20260915에 선푸시 후 Cafe24 배포 성공. buildTime 1789476932215, 공개 version 일치·__health ok·서비스 active. 운영 390px 화면에서 제목 내부 수집 링크 1개/별도 출처 섹션 0개, 기존 Instagram 계정 href, 가로 넘침 없음(360/360px), 사진 확대 및 닫기 버튼 확인. 관련 회귀 11개 통과.

## 2026-09-15 — 소셜 상세 정렬·크기 시각 개선

- 요청: 실제 화면을 보면서 정렬·사이즈를 개선하고 운영 반영.
- 증상→판정→표시: 소셜 판정 → 기존 header 내부 장르/제목 배치와 외부 썸네일의 별도 행 기준 → 제목이 사진보다 29px 낮게 시작. 장소는 카드 내부 16px 들여쓰기, 날짜는 바깥 기준이라 기준선 불일치. 설명 13px/기능 라벨 11px로 상대적으로 작았다. 기준은 실제 운영 390px 렌더링과 fa65ac14·1e0120d0의 관련 CSS 이력이다.
- 기존 구현 일부 있음: 기존 소셜 전용 grid와 제목 링크/사진 확대를 재사용. header를 동일 grid에 참여시켜 제목/사진을 같은 2행으로 정렬하고, 제목 최소 높이·썸네일 높이를 112px로 통일. 기존 CSS 책임 안에서 간격 12px, 설명 14px·버튼 12px, 장소/날짜 기준선과 기능 버튼 영역을 정리했다. 소셜 전용 CSS만 변경하고 새 구조·상태·저장 책임 없음.
- 보호 목적/불변조건: 수집 위치 우선, 제목 클릭 링크와 사진 확대의 독립 동작, 링크/이미지 드래그 금지, 기존 작성자/관리자 권한·수정/삭제/등록 경로, 강습·일반 행사 기존 화면 유지. 관리자 편집 필드는 기존 전체 폭 행으로 유지.
- 검증: build:only 및 diff 검사 통과. 실제 DB 연결 로컬 화면에서 제목/사진 top 동일·기본 높이 112px 확인, 장소/날짜 본문 기준선 차이 1px 미만. 모바일 및 PC, 긴 소셜 제목·긴 설명에서 가로 넘침 0·하단 버튼 화면 내 노출 확인. 강습 상세는 소셜 클래스 없이 기존 약 587px 포스터 영역 유지. CSS 배치 변경이라 동일 로직 Vitest 반복/신규 테스트와 운영 데이터 쓰기·수집/서버 회귀는 제외했다.

- 운영 반영 완료: c6ed1b19를 origin/codex/social-detail-source-20260915에 선푸시 후 deploy:cafe24 성공. buildTime 1789482983474, 서비스 active·공개 health ok·version 일치. 운영 PC/390px 모바일에서 제목과 사진 top 차이 0px 및 높이 112px, 가로 넘침 0, 설명 14px, 동일 77.5×54px 버튼 4개 확인. 제목 수집 계정 href 보존.


## 2026-09-16 — 소셜 상세 정렬 및 별도 시간 입력/표시 제거

- 증상 → 판정 → 부작용 → 데이터: 기존 공통 상세의 time/편집모드 조건이 시간 행을 표시하고, 소셜 등록이 별도 time 입력을 저장 어댑터로 전달했다. 설명은 별도 문자열이며 시간 문구를 포함할 수 있다. 사용자 정책은 구조화된 시간 UI만 제외하는 것이다.
- 기존 구현 일부 있음: 일반 등록에는 별도 시간 입력이 없고 SocialScheduleModal, EventDetailModal, EventEditBottomSheet에 남은 입력·표시 연결만 제거했다. 설명 렌더링과 저장, 레거시 time/start_time 어댑터를 재사용한다.
- 관련 이력: 3d33d060의 공통 시간 표시, fa65ac14/1e0120d0/c6ed1b19의 소셜 구분·수집 링크 통합·정렬 목적을 유지했다. 소셜 전용 CSS의 채운 제목 카드와 하단 버튼 테두리가 불필요한 시각적 무게를 만들었다.
- 수정: 제목·DJ·수집 안내를 하나의 링크로 묶고 작은 확대 이미지와 정렬. 장소/날짜/설명 왼쪽 기준 통일. 하단 기능 버튼의 박스·테두리·배경 제거. 등록·수정·상세의 별도 시간 UI 제거, AGENTS와 결정문에 정책 명시.
- 불변조건: 설명/포스터의 시간 원문, 기존 시간 데이터, 날짜, 강습·일반 행사 디자인, 권한과 삭제 확인, 링크/이미지 드래그 방지 유지. 신규 스키마·필드·중복 소유자 없음.
- 검증: 기존 소셜 분류/편집 테스트 13개 통과(설명 원문·레거시 시간 호출 추가), 프로덕션 빌드 통과. 운영 DB를 읽는 로컬 화면에서 390px 상세·등록, 320px 긴 제목, 1280px 상세 확인. 버튼 border 0/배경 투명, 제목 수집 URL, 이미지 확대/닫기, 시간 입력 0개, 설명 속 시간 유지, 강습 소셜 스타일 미적용 확인. 긴 제목에 이미 포함된 DJ는 중복 표시하지 않는다. 데이터 저장 경로는 변경하지 않으므로 운영 쓰기/마이그레이션 검증 제외.


### 2026-09-16 — 소셜 상세 시각 구성 재검토

- 이전 시간 정책/박스 제거 변경 294e3488 커밋을 origin/codex/social-detail-source-20260915에 푸시 후 Cafe24 배포 완료(service active, buildTime 1789494356302).
- 사용자 추가 지시: 기능 추가 없이 세련된 소셜 상세 디자인, 인터넷 사례 참고.
- 기존 구현 있음: 제목 링크·이미지 확대·하단 기능은 기존 호출 그대로 사용. 추가 함수·필드·데이터 변경 없이 소셜 전용 스타일과 링크 내부 표시만 수정했다.
- 참고: Luma 공식 이벤트 예시 https://luma.com/eventbrite 의 제목 중심 구성과 보조 정보 대비. 이미지 크기는 사용자가 요청한 작은 썸네일로 유지하고 외부 디자인/이미지 자산을 복제하지 않았다.
- 시각적 원인: 제목과 바로가기 안내의 분리된 인상, 카테고리/이미지의 어긋난 시작점, 반복 구분선과 여러 버튼 색이 주목도를 분산했다. 기존 정보 우선순위(수집 제목 → 장소 → 설명)는 유지한다.
- 수정: 제목에 외부 링크 화살표 통합, 썸네일을 상단 정렬, 장소 라벨/중복 구분선 제거, 보조 아이콘과 하단 버튼 색 통일, 박스 없는 하단 링크·버튼과 여백 조정. 권한·등록·관심·공유·편집·삭제 호출은 변경 없음.
- 검증: DB 연결 로컬 390px/320px 스크린샷 검토, 긴 제목 가로 넘침 0, 기존 수집 URL 유지, 빌드 통과. 스타일 변경이므로 앞서 통과한 13개 동작 테스트는 반복하지 않고 영향 있는 화면·확대·링크만 확인. 시간 원문/저장 데이터 유지 정책은 그대로 적용.

- 최종 배포 검증: ebd6b411을 origin/codex/social-detail-source-20260915에 푸시한 뒤 Cafe24 배포 완료. 서비스 active, /__health 200·ok, buildTime 1789494702191. 운영 390px 화면에서 제목 수집 링크·이미지 확대/닫기·하단 버튼 테두리 0/투명 배경·별도 시간 행 없음·설명 PM 시간 원문 유지·등록 time 입력 0개 확인. 운영 데이터 제출 없이 검증.


## 2026-09-16 — 같은 날짜·장소 소셜 중복을 재검토로 전환

- 경로: 원본/포스터 → DJ별 콘텐츠 ID 분리 → ‘확정 중복 아님’ → 자동등록/알림 → events 2행과 collected 후보 2행. 운영 공개 데이터와 연계 후보를 읽기 전용 조사했다.
- 원인: e3f76dc3/8c5d1a70/ec0a7be1의 서로 다른 콘텐츠·DJ 보호 뒤에 같은 날짜·장소 충돌 보류가 없었다. 오인식한 카페 회원 표기/PM8/누락 DJ도 별도 행사로 자동등록됐다.
- 판정: 기존 구현 일부 있음. 날짜/장소·기본 소셜 교체, pending와 auto_registration.reasons, 기존 쓰기 잠금을 재사용했다. 신규 DB 구조 없이 자동등록 전과 잠금 안의 최종 저장 직전에 같은 장소·날짜 소셜을 다시 대조하며 충돌 시 422·검토함 보류. 기존 strict duplicate와 수동 검토 권한 유지.
- 운영 검토: 9/6 인더무드신림(굿맨), 9/16 스윙타임(유광), 9/17 사보이(몽룡), 9/19 인더무드신림(민·캐롤), 9/19 사보이(단미)의 원문·원본 포스터를 대조. 기존 복구 스크립트에 검토된 계획 입력을 연결했다. dry-run 5건 모두 정확한 후보 연계·관심 등록 0 확인. 원본 백업, 이미지·발송 원장 보존, 후보 duplicate 참조 보정 후 적용 예정.
- 검증: 등록 연계/실제 API 보류·동시 요청/쓰기에 앞선 잠금 검사/정규 소셜/관리자 삭제 80개 통과(같은 ID 보정 재시도 포함), 수집 표준 검사와 빌드 통과. 운영에 가짜 이벤트를 쓰는 검증과 무관한 전체 AI 수집·DB 마이그레이션은 제외했다.
- 결정: docs/decisions/2026-09-16-social-occurrence-conflict-review.md.

- 운영 반영 완료: f67224e4를 origin/codex/social-detail-source-20260915에 먼저 푸시 후 Cafe24 배포. 서비스 active, health ok, buildTime 1789496152476. 운영 function-api/generic-data-api SHA-256이 검증 작업본과 일치.
- 데이터 복구: 원문 검토 계획 5건 적용, 각 건의 원본 이벤트·후보·관련 참조·알림함 행을 backups/data-fixes/social-duplicate-*.json에 권한 0600으로 백업. 중복 이벤트 5행/중복 알림함 5행만 제거하고 발송 원장·후보·이미지 보존, 후보는 duplicate 및 유지 일정 참조로 보정. 사보이 9/19 유지 일정 c0fffc42-76fa-47b9-9127-43e14cd5b502와 후보 f192895ef95c6879의 DJ 표기를 단미로 보정.
- 최종 검증: 재실행 dry-run 5건 모두 alreadyApplied=true, 추가 쓰기 없음. 공개 9/1 이후 소셜의 같은 날짜·장소명 중복 그룹 0, 유지 5건 존재/제거 5건 부재, 설명·이미지 원문 동일. 실제 모바일 캘린더에서 유광/몽룡/민·캐롤/단미가 각각 1개이며 PM8 표시 없음. 무관한 과거 전체 데이터나 실제로 다른 행사는 합치지 않았다.


## 2026-09-16 — 사보이 9/19 중복 재발: 영문 장소 별칭 누락

- 상태: 수정 검증 완료, 배포 및 복구 준비.
- 증상 → 판정 → 부작용 → 원장: 08:05 KST에 같은 원문 102746의 `SAVOY BALLROOM`/DJ 누락 후보 453586d507d06310가 기존 `사보이볼룸`/DJ 단미 일정과 다른 장소로 판정되어 이벤트 4c1c0ea5-b3c3-4450-9592-7a14be3cd7dd와 알림이 추가됐다. 양쪽 venue_id는 null이다. 앞선 문자 그대로의 장소명 그룹 검사와 단일 한글 별칭 회귀가 이 경계를 빠뜨렸다.
- 기존 구현 일부 있음: candidate-utils의 toMapSafeVenueName/별칭 테이블이 수집 장소명을 소유한다(2b6a0ab0, ed4182c16, fe2b77c68). function-api의 sameVenue는 27bab702의 문자열 유사도만 쓰며 f67224e4의 충돌 검사도 이를 그대로 호출했다. 기존 venue DB는 한글 이름과 주소만 가지고 있어 영문→ID 해석이 없었다.
- 공통 원인 수정: 서버의 충돌 검사 양쪽 장소를 기존 수집기 normalizer로 해석하고, 기존 사보이 별칭 항목에 원본 포스터의 SAVOY BALLROOM/BAR 표기를 포함했다. 해피홀·소셜클럽 등 기존 영문 별칭도 동일 경로로 적용된다. 날짜/이벤트 ID 전용 예외, 새 함수·테이블·큐·중복 별칭 소유자 없음. 알 수 없는 지점명은 별칭에 추가하지 않는다.
- 유지한 보호: 엄격한 확정 중복·콘텐츠 ID 판정은 변경하지 않는다. 장소 ID가 서로 다르면 이름보다 우선하고, 다른 날짜/장소·비소셜·자기 수정·기존 정규 기본 일정 교체·관리자 수동 등록·잠금 안의 최종 검사 유지. 설명·포스터 시간 원문 및 저장 데이터 정책 불변.
- 근거: 중복/유지 이벤트의 원문 URL 일치, 포스터 파일 SHA-256 동일(83e37c34…), 이미지의 DJ 단미 및 SAVOY BALLROOM BAR/주소와 장소 DB 대조. 단순 날짜·장소 일치만으로 삭제하지 않는다. 기존 reviewed-plan 복구 스크립트와 백업/연계/관심 등록 검사 재사용.
- 검증: 기존 등록·충돌·잠금·정규 소셜·관리자 삭제 83개 통과. 한영 별칭 양방향, DJ 누락, 같은 원문, 다른 장소/ID, 사전 검사와 잠금 직전 새 충돌을 포함한다. 수집 표준 및 프로덕션 빌드 통과. 원래 이미지 다운로드가 테스트 경계를 가로막아 테스트 이미지 입력을 로컬화 없는 fixture로 교체했고 실제 네트워크 의존을 제거했다. UI/스키마는 변경하지 않아 전체 UI·DB 이관과 운영 가짜 이벤트 등록은 제외.


- 사보이 재발 운영 반영: 1973329b 선푸시 후 Cafe24 배포 완료, health ok·service active·buildTime 1789528985525. 동일 SHA-256 포스터와 원문을 확인한 새 중복 1건만 백업 후 제거하고 알림함 1행 정리, 후보 duplicate/유지 일정 참조로 보정. 재실행 alreadyApplied=true. 기존 DJ 단미 일정·설명·이미지 보존.

## 2026-09-16 — 자동등록 장소 상세/지도 연결 누락

- 증상 → 판정 → 부작용 → 원장: 추출된 장소명 → 자동등록 validateAutomaticRegistrationCandidate가 venue_id/address/location_link를 단순 복사 → venues 대조 없이 events와 collected 후보에 빈 ID/링크 저장 → 상세는 venue_id가 없으면 장소 상세를 열 수 없고 지도 링크도 없음. 삭제된 장소 ID 오류는 조사한 향후 일정에서 0건이다.
- 기준 데이터: 9/16 이후 수집 원장 연계 공개 일정 28건 중 ID/지도 링크 누락 26건. 사보이 중복 1건 제거 뒤 27건/누락 25건. 장소 목록과 유일하게 일치하는 15건, 나머지 10건은 현재 장소 레코드 미등록·추출 장소 불명확·주소 표기 차이(클럽 라틴 3, 강남역 라틴바 3, 아지트 연습실·원스빠·패스트엔터테인먼트·골든스윙 각 1). 자동·관리자 등록 후보가 섞여 있으며 숫자를 자동등록 건수로 단정하지 않는다.
- 기존 구현 일부 있음: c5036f8f의 venueNormalization.ts / 관리자 ingestorMapping의 matchVenueRecord 및 normalizeVenueStructuredData가 장소 ID 우선·주소/별칭·지도 JSON 해석을 이미 소유한다. 2d4e23c38의 서버 자동등록 경로에는 이 대조 호출이 빠졌다. 기존 복구 스크립트는 중복 삭제/후보 상태/이미지만 담당해 장소 연결 복구 책임은 없음.
- 수정: 기존 matcher를 동일 위치의 공용 .mjs로 옮겨 TS 진입점에서 재노출하고 수집기 별칭 테이블도 통합했다. 서버 자동등록에 기존 normalizeVenueStructuredData를 연결한다. 기존 수동 검색은 유지하고 자동 경로의 strict 옵션은 활성 장소 중 유일한 이름/주소 일치만 허용한다. 불명확·중복 이름·유효하지 않은 ID는 임의 연결하지 않는다. 새 테이블/필드/큐 없음. 공용 모듈을 배포 목록에 포함.
- 복구: backfill-ingested-event-venues.mjs는 검토한 eventId/venueId 계획만 받는다. 장소·수집 원장 연계를 다시 검증하고 기존 saveCafe24TableRow/쓰기 잠금을 재사용, 저장 직전 동시 변경 검사, 쓰기 전 0600 원본 백업. 장소 ID와 비어 있는 주소/지도만 채우고 이름·날짜·설명·포스터·후보의 원문 증거·알림은 변경하지 않는다. 기존 장소 소유권이 있으면 중단한다. 스키마 이관 없음, 코드 revert/백업 원본 복원으로 롤백 가능.
- 검증: 기존 및 확장 테스트 89개·수집 표준·프로덕션 빌드 통과. 한영 자동/수동 매칭, 주소 매칭, 비활성/동명/누락 ID 보류, 기존 수동 유사 검색, 자동등록 이벤트와 후보에 같은 장소 메타데이터 저장, 중복/잠금/수동/재시도 경로 포함. 운영 dry-run 15건 통과. UI 구조 변경이나 장소 신규 등록은 요청 범위의 연결 누락 해결에 필요 없어 제외했고 불명확 장소를 임의 생성하지 않는다.


- 장소 연결 운영 반영 완료: 95da66e3를 origin/codex/social-detail-source-20260915에 선푸시 후 Cafe24 배포. 서비스 active, health ok, buildTime 1789529504618. 서버 등록/공용 장소 모듈/수집 유틸 SHA-256이 작업본과 일치.
- 기존 일정 복구: 계획 15건에 장소 ID·빈 주소/지도 연결 적용, backups/data-fixes/ingested-event-venues-*.json에 원본 0600 백업. 재실행 변경 0건. 공개 API에서 15개 ID/링크 확인, 제목·날짜·설명·포스터·원문 링크·표시 장소명 동일 검증. 알려진 별칭으로 묶은 9/16 이후 소셜 충돌 그룹 0.
- 모바일 운영 확인: 캘린더에 사보이 DJ 단미 1개/영문 중복 0개. 해당 상세의 장소 버튼 클릭으로 사보이볼룸(사당) 장소 상세와 주소·네이버 지도 버튼 정상 표시. 잔여 미연결 10건은 장소 목록 미등록/추출 장소 문제 외에 주소 표기 차이(아지트의 서울 접두어 등)도 포함한다. 추정 링크 생성이나 무관한 장소 연결은 하지 않았다.

## 2026-09-18 자유게시판 새 글 1 표시 조사

- 상태: 읽기 전용 조사 완료. 사용자 계정/기기의 읽음 상태는 미확인, 코드·운영 데이터·배포 변경 없음.
- 증상 → 판정 → 부작용 → 원장/외부 결과: 하단 새 글 1이나 목록 변화 불명확 → useFreeBoardUnreadState 및 board-read-api가 최근 14일 공개 자유글에서 사용자별 읽음(회원은 본인 글도 제외)을 차감 → 메뉴 숫자와 목록 NEW 표시, 상세를 열었을 때만 읽음 저장 → 회원 user_board_post_reads 또는 게스트 localStorage에 기록. 목록 방문 자체는 읽음이 아니다.
- 기존 구현 있음: 기존 읽음 API/훅/목록 NEW를 확인. 78af6020의 방문 기준 배지가 3c0dae04에서 글별 읽음으로 변경됐으며, 8/3 이슈 기록의 사용자별 상세 읽음 보호 목적과 일치한다. 새 상태나 예외를 추가하지 않았다.
- 운영 검증: 공개 board_posts query(select) HTTP 200, 총 16건. 최근 14일 공개 글은 9/6 「인더무드신림 요금변경안내」 1건이며 공지 2건 다음 세 번째에 정렬된다. 9/18 신규 글은 없었다. 따라서 숫자 1은 해당 기존 글의 미읽음으로 설명 가능하지만 사용자별 원장 대조 없이는 확정하지 않는다.
- 범위/제외: 숫자 산정, 목록 정렬/NEW, 상세 읽음 호출과 직접 관련 이력만 조사. 코드 변경이 없어 회귀 테스트·빌드·배포는 제외했고, 계정 읽음 초기화나 실제 상세 열기로 조사 대상 상태를 바꾸지 않았다. 개인 기기의 갱신 실패 여부는 미확인.
## 2026-09-19 자유게시판 읽음 배지 응답 경합 및 연속 행사 장소 배지

- 상태: 로컬 수정·검증 완료, 배포 전. 9/18의 단순 미읽음 추정은 사용자의 이미 읽었다는 설명으로 재조사했으며 해당 추정을 확정 결론으로 사용하지 않는다.
- 증상 → 판정 → 부작용 → 원장/외부 결과: 이미 읽은 글인데 새 글 1 → AuthContext 인증 완료 전 guest 조회와 회원 조회, 읽음 저장 전후 조회가 동시에 진행 → 늦은 응답이 최신 unreadPostIds를 덮어씀 → 서버 읽음 행이 있어도 배지만 복구될 수 있음. 상세 읽음 저장 실패는 로그만 남기고 재시도하지 않았고, 탭 간 읽음 메시지에는 계정 범위가 없었다. 기준 데이터는 인증 완료 여부·회원 ID·게시물 ID·요청 순서·저장 성공 여부다.
- 기존 구현 판정: 일부 있음. useFreeBoardUnreadCount의 계정별 서버 원장/게스트 저장소/CustomEvent·BroadcastChannel·storage·복귀 조회를 재사용한다. 3c0dae04의 사용자별 상세 읽음과 7c68bd57의 일시 네트워크 장애 시 상태 보존 목적을 유지하며, 누락된 인증 대기·오래된 응답 배제·계정 범위·저장 재시도만 연결했다. 인증된 훅의 사용자 ID를 읽음 함수에 전달해 추가 auth/me 조회 실패가 게스트 저장으로 잘못 흐를 여지도 없앴다. 새 DB·큐·영구 읽음 저장소 없음. 기존 탭의 userId 없는 메시지는 원장 재조회로 호환한다.
- 운영 대조: 기존 SSH/DB 연결을 통한 SELECT만 수행, 종료 시 rollback. 원장 자유게시판 16건과 공개 API 목록이 일치하고 최신 글은 9/6이며 9월 댓글은 0건이다. 최신 글의 회원 읽음 행 2건이 실제 존재한다. 댓글은 배지 산정 입력이 아니다. 기록의 최근 updated_at만으로 새 글/댓글로 판정하지 않았다. 현재 사용자 계정·기기의 읽음 기록과 요청 순서는 식별 정보가 없어 미확정이다. DB는 READ ONLY transaction 구문을 지원하지 않아 일반 transaction에서 SELECT만 실행했다.
- 인접 경로: Cafe24의 createNoopChannel(0960c01c)은 실제 실시간 갱신을 하지 않는다. 다만 게시판/댓글 재진입은 직접 query를 수행하고 신규 글 작성 완료는 기존 목록 재조회 콜백을 사용한다. 이번 저장 원장 대조에서 목록에 누락된 최신 글/댓글은 확인되지 않았으므로 범용 실시간 전송 시스템을 새로 만들지 않았다. 기존 글/댓글 ID 충돌 방지 테스트도 확인했다.
- 수정: 인증 완료 후 조회/읽음 저장, 회원 전환·새 조회·읽음 성공 이전의 응답 폐기, 계정별 읽음 메시지 적용. 실패 저장은 상세가 열린 동안 800ms 뒤 한 번 자동 재시도하고 이후 네트워크/화면 복귀 때 재시도하며 상세 종료 시 타이머를 취소한다. 회원 실패를 게스트 성공으로 취급하지 않는다. 14일/비공개·본인 글 제외/상세 읽음/일시 조회 실패 보존 규칙과 운영 읽음 행을 유지한다.
- 캘린더: 기존 cf2b082e의 연속 행사 구간과 representativeEvent, 751c5fa1의 고정 색상·20px 줄 간격을 재사용. FullEventCalendar에서 venue_name → place_name → location 순서의 기존 장소 표시 기준으로 각 주 막대 제목 앞에 장소 배지를 붙였다. 장소가 없으면 배지를 생략하고 긴 이름은 막대 폭 45% 이내 말줄임, 전체 장소·제목은 title로 제공한다. 드래그 불가, 주 경계 이어짐·상세 클릭·전체 일정 노출 유지. 새 데이터 필드/스키마/행사 분류 변경 없음.
- 검증: 기존 훅 테스트를 확장해 수정 전 인증 대기/guest→회원 경합/읽음 이전 응답/다른 계정 방송/실패 저장 재시도 5건 실패를 확인했다. 수정 후 게스트 저장, 인증 완료 후 저장, 구형 탭, 회원 401, 상세 종료, 일시 장애 보존을 포함한 관련 7개 파일 36건 통과. 기존 서버 읽음·비공개/익명 보호·글/댓글 ID 및 달력 배치·색상 검사를 포함했다. build:only 통과, 대상 ESLint 오류 0(기존 경고 24), 대상 diff 공백 검사 통과. 전체 수집/배포 검사는 변경 경계 밖이라 제외. Mac 잠금으로 실제 브라우저 시각 검증은 수행하지 못했다. 사용자별 운영 기록 초기화·배포는 하지 않았다.
## 2026-09-19 자유게시판 새 댓글 +N 및 실제 화면 검증

- 상태: 로컬 구현·자동 검사·별도 브라우저 화면 검증 완료, 미배포. 실제 사용자 기기의 당시 로그인 세션을 재현한 것은 아니다.
- 관리자 증상 후속: 운영 SELECT 대조에서 최근 공개 글은 1건. 작성 관리자 계정은 본인 글 1·읽음 1로 정상 새 글 수 0이며, 다른 관리자 계정은 읽음 행이 없어 1이다. 사용자가 관리자 로그인이라고만 밝혀 정확한 계정/당시 요청 순서는 미확정이다. 댓글은 기존 새 글 숫자 입력이 아니었고 운영 9월 댓글 0건은 재확인된 이전 조사 결과다.
- 경로: 새 댓글 저장 → 기존 배지는 board_posts의 작성일/읽음 ID만 사용 → 댓글 추가가 배지/목록 NEW에 반영되지 않음 → 기존 board_comments 원장과 user_board_post_reads를 확장해 표시한 댓글 ID만 기록한다. 판정은 공개 부모 글·최근 14일 댓글·작성자 ID·읽음 ID이며 본문 NEW와 댓글 +N을 분리했다. 오래된 글 및 관리자 본인 글에 달린 타인의 새 댓글도 포함한다.
- 기존 구현 일부 있음: 기존 읽음 API·계정/게스트 훅·목록·CommentSection 재사용. 본문 read_at 하나로는 댓글 조회 실패와 뒤늦게 도착한 댓글을 구분할 수 없어 기존 읽음 행에 nullable read_comment_ids 한 필드만 추가한다. 새 테이블/큐/알림 없음. 레거시 NULL은 기존 read_at 기준을 보존한 후 최초 쓰기에 스냅샷으로 전환하며, 같은 행 잠금과 합집합으로 다중 탭의 읽음을 보존한다. 공통 판정 모듈을 회원 API/게스트가 함께 사용해 중복 소유자를 만들지 않았다.
- UI 연결: 데스크톱·모바일 목록 댓글 수 옆 +N, 새 본문은 제목 NEW 유지, 하단·홈은 새 글+댓글 합계 및 문구 갱신. 성공적으로 렌더된 현재 글 댓글만 보이는 탭에서 읽음 저장. 실패/이전 글의 지연 응답은 제외. 화면·네트워크 복귀 시 목록 댓글 총수와 댓글 상세를 다시 읽고 오래된 목록 응답은 무시한다. 연속 행사 장소 배지 변경도 함께 유지한다.
- 검증: 기존 테스트 확장 및 댓글 렌더/목록 복귀 테스트로 관련 10개 파일 51건 통과. 회원/게스트·신규/레거시·읽기 성공/실패/재시도·본문 전용/댓글 렌더·다중 탭·타 글 ID·숨김/본인/삭제/기간·기존 비공개/익명/글·댓글 UUID·달력 배치/색상 범위를 선택했다. 무관한 수집 전체 검사는 제외. 대상 ESLint 오류 0, 기존 경고와 테스트 타입 경고 있음. 배포 스크립트 구문 및 대상 diff 검사 완료. 운영 SQL 마이그레이션은 실행하지 않았다.
- 화면 검증: Mac 잠금 해제가 필요하다는 이전 판단을 정정했다. 별도 headless Chromium에서 실제 프로덕션 빌드와 로컬 관리자 fixture API를 연결해 화면을 캡처하고 직접 확인했다. 320/390/1024px 목록 +2/+1, 상세 댓글 표시 후 합계 3→1→0, 읽은 본문 NEW 부재, 가로 넘침/통계 겹침 부재를 확인했다. 댓글 API 503 재시도 3회 후에도 읽음 댓글 ID는 비어 있고 +2/+1·합계 3을 유지했다. 390/1024px 캘린더의 주를 넘는 연속 행사 4개 구간에서 장소 배지 선행·긴 장소 말줄임·18px 높이·드래그 금지를 확인했다. 캡처는 /tmp/board-final-*.png 및 /tmp/calendar-final-*.png(임시 파일).
- 화면에서 찾은 인접 원인: c867db26의 고정 168px 통계 칸은 댓글 +N 추가 후 간격이 좁아져 기존 grid 칸을 minmax(168px, max-content)로 바꾸고 통계 항목 축소를 막았다. 원래 제목 가변 폭과 우측 통계를 유지하며 별도 레이아웃을 만들지 않았다.
- 실패 안내: 최초 3d33d060부터 댓글 조회 catch는 콘솔만 기록해 실패가 빈 댓글처럼 보였다. 기존 loadComments 및 상세 재시도 버튼 스타일을 재사용해 실패 문구/수동 재시도를 연결했다. 기존 댓글 표시·실패 시 미읽음 유지·이전 요청 무시를 보존한다. 기존 실패/복귀 테스트를 확장하고 수동 재시도 경계만 추가했다. 마지막 변경 후 영향받는 2개 파일 18개 테스트·빌드·ESLint 오류 0·diff 검사를 통과했다(기존 51개에서 수동 재시도 1개 추가). 브라우저에서 실패 안내 화면을 직접 확인하고 다시 시도로 댓글 2개 표시·합계 3→1·오류 안내 제거까지 확인했다. 운영 원본 사건이 조회 실패였다는 증거는 없다.
- 검증 제외: 실제 MySQL 스키마 이관/다중 연결 및 사용자의 실제 관리자 세션은 미검증. 브라우저 API는 검증용 메모리 데이터이며 운영 변경을 전달하지 않는다. 일반 변경 확인을 위한 운영 배포나 가짜 운영 댓글 작성은 하지 않았다.
- 설계/마이그레이션/롤백: docs/decisions/2026-09-19-free-board-comment-read-snapshots.md. 새 필드를 제거하지 않고 이전 코드로 롤백할 수 있으며 기존 읽음 행과 작성 데이터는 유지한다.

## 2026-09-19 댓글 배지 색상·연속 행사 장소 배지 위치 조정 및 배포 준비

- 요청: 댓글 +N 색상 변경, 장소를 행사 제목 앞 인라인 대신 왼쪽 위 작은 배지로 표시하고 배포.
- 경로/기존 구현: 기존 .free-board-comment-new-badge 및 .calendar-overlay-location을 재사용한다. 인라인 flex 배치가 제목 앞에 붙어 보이는 원인이며 새 데이터·렌더러는 추가하지 않았다. cf2b082e의 18px 막대/20px 레인 및 모바일 날짜 여백은 겹침 방지 목적이므로 배지 8px 돌출을 반영해 레인 간격·본문 예약 높이를 같은 기존 상수 30px로 맞추고 막대 시작점도 8px 내렸다.
- 변경: 댓글 배지는 파란색 #2563eb. 장소 배지는 왼쪽 4px·위 -8px, 높이 12px·글자 8px의 밝은 배경/테두리 배지로 표시. 기존 막대 높이·날짜 범위·색상·상세 클릭·키보드·비드래그·전체 행사 직접 표시를 보존한다.
- 화면 검증: 390/1024px 실제 앱 빌드에서 파란 +2/+1, 첫 주/주 경계/복수 레인/긴 장소명/인접 단일 행사 표시를 캡처·직접 확인. 배지 위쪽 위치와 높이, 레인 겹침 없음, 가로 넘침 없음, 페이지 JS 오류 0 확인. CSS 배치 변경이라 새 문자열 단위검사를 추가하지 않았고 실제 브라우저 경계 검사와 기존 레이아웃/색상 검사를 재사용했다.
- 배포 기준: 현재 작업 폴더 HEAD는 운영보다 오래된 d7f4bfc1. 운영 version 1789529504618 및 서버 SHA-256을 배포 기록 95da66e3과 대조하고 그 후 기록 전용 28036f72를 기준으로 별도 codex/board-comment-calendar-badges-20260919 작업본을 구성했다. 이번 게시판/캘린더 변경만 옮기고 별도 미배포 기능·운영 수집 변경을 섞지 않는다. 동일 책임 파일의 기준 내용을 비교한 뒤 복사하고 혼합 파일은 해당 변경만 연결했다.
- 검증: 별도 배포 작업본의 관련 10개 파일 52개 검사 및 Cafe24 빌드 통과. 배포는 커밋·푸시 이후 기존 deploy:cafe24 명령으로 진행한다. 스키마 적용 및 운영 검증 결과는 아래 후속 기록에 남긴다.
- DB 사전 확인: 운영 원장 읽음 34행 및 기존 필드 해시만 조회했다. 같은 MySQL 연결의 임시 테이블에서 nullable 컬럼 추가·레거시 upsert 보존·FOR UPDATE·rollback 통과. 운영 원본 행은 변경하지 않았으며 임시 테이블은 제거했다. 별도 배포 빌드로 390/1024px 화면 검사도 재실행해 통과했다.

## 2026-09-19 게시판·캘린더 배포 검증 완료

- 구현 986642a9를 origin/codex/board-comment-calendar-badges-20260919에 먼저 푸시한 뒤 기존 deploy:cafe24 성공. 서비스 active, 공개 __health 200/ok, buildTime 1789750324455 일치. 서버 읽음 API와 공용 모듈 SHA-256도 배포 작업본과 일치한다.
- nullable LONGTEXT read_comment_ids 적용 확인. 기존 읽음 34행과 기존 필드 해시가 배포 전과 동일하다. 같은 MySQL 연결의 임시 테이블에서 레거시 upsert 보존·행 잠금·rollback을 확인했다.
- 390/1024px 운영 캘린더 실데이터에서 해피홀(신촌)의 장소 배지가 막대 왼쪽 4px·위 8px·높이 12px로 표시되고 드래그 불가·페이지 오류 0 확인. /tmp/production-calendar-badge-visible.png를 직접 검토했다. 배포된 게시판 번들은 격리된 로컬 관리자 fixture API로 파란 +2/+1·상세 읽음 후 합계 3→1을 확인했다.
- 운영에 가짜 글·댓글을 쓰거나 실제 사용자의 읽음 기록을 변경하지 않았다. 사용자의 당시 세션·원본 새 글 1의 정확한 발생 원인과 실제 MySQL 다중 연결 동시성은 미확정/미검증이며, 다중 탭 합집합은 서버 회귀검사로 검증했다.
