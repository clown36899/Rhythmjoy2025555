...
- `Share`를 누른 뒤에는 업로드가 백그라운드 작업으로 넘어갈 시간을 둔 다음 해당 계정 프로필 딥링크를 다시 열어 게시물 수 증가를 확인한다. 새 릴스 화면이나 게시 직후 권한창에 머문 상태를 게시 실패로 오판하지 않는다.
- 프로필 게시물 수 증가를 확인하지 못하면 `verification-required`로 멈추고 자동 재게시를 금지한다. 다음 실행은 먼저 기존 기준 게시물 수와 현재 게시물 수를 대조해 이미 게시된 건이면 `published`로 복구하며, 증가가 없을 때만 계속 차단한다. 이는 중복 게시 방지 규칙이다.
