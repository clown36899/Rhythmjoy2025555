...
- 먼저 프론트/수집 매핑 기준을 안정화하고, 실제 DB 컬럼 추가는 별도 마이그레이션으로 검토한다.
...
수동 실행용 SQL은 `supabase/db_tasks/calendar_dance_scope/`에 둔다. 이 경로는 자동 마이그레이션 폴더가 아니므로, 커밋되어도 운영 DB에 자동 적용되지 않는다.
...
- `03_dry_run_backfill.sql`의 `unknown` 비율이 납득 가능한 수준
