...
- 미래 회차의 휴무 예외가 제거되면 다음 조정에서 같은 ID가 기본 정규 소셜로 복귀한다.
- 날짜가 지난 뒤에도 근거 원장에 연결된 `automation.exception_type=closure` 표시용 휴무 회차는 공식 운영 이력으로 보존한다.
- 표시용 행이 이미 정리됐더라도 남아 있는 수집 후보 또는 외부 API 휴무 예외에서 다시 물질화한다.

## 유지하는 안전장치

- 수집 후보 원장과 외부 API 예외 원장은 수정하거나 삭제하지 않는다.
- 과거 회차의 포스터를 휴무 회차에 재사용하지 않는다.
- 실제 개별 소셜이 생성 반복본을 대체하는 규칙, 90일 범위, Cron 인증과 동일 ID 재실행 안전성을 유지한다.
- 근거 없는 일반 정규 생성본은 날짜가 지나면 계속 정리하고, 공식 휴무 후보·예외로 물질화된 회차만 과거 정리에서 제외한다.
