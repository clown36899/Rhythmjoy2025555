...
## 2026-09-10 소셜 바로가기 대상 정정

사용자 정책: 소셜 바로가기는 개별 게시물이 아닌 수집 게시판/계정으로 이동한다. 증상→판정→표시→외부 결과: 수집 원문 link1 저장→기존 상세화면(3d33d060)이 그대로 href 사용→개별 글 열림. 기존 구현 일부 있음: collection-registry의 사보이 메뉴 501 주소 및 findSourceByUrl을 재사용하고 상세화면의 표시 연결만 수정했다. 원문 link1은 수집 근거·중복 판정 용도로 보존하므로 DB/필드/마이그레이션을 추가하지 않았다. 기존 정규 소셜의 게시판 링크도 같은 경로를 사용한다. 소셜 외 활동과 등록되지 않은 출처는 기존 링크를 유지한다. 기존 표준 검사에 네이버 글→게시판, Instagram 글→계정 경계를 추가하고 빌드/실제 화면으로 검증한다. 수집 원장과 등록 로직은 변경하지 않으므로 재수집/운영 데이터 수정은 제외한다.

- 검증 완료: 기존 수집 표준·Cafe24 빌드 통과. d6717b9a를 푸시한 뒤 배포, health 정상 및 buildTime=1789019993812 확인. 운영 상세 화면에서 9/10 사보이 굼바의 href가 메뉴 501 게시판, 9/9 소셜클럽 쵸리의 href가 thesocialcluba 계정임을 직접 확인했다. 두 링크 모두 수집 위치 바로가기 표시 및 draggable=false 확인.
