...
8. 오늘 일정과 신규 등록은 서로 다른 계약이다. 오늘 일정은 등록 시점과 관계없이 당일 진행 일정의 요약이고, 신규 등록은 `new_event_enabled_at` 이후 생성된 큐만 전달하는 전진 전용 경로다. Push와 인앱 알림함은 같은 정규 설정·활성화 경계·분류 필터를 사용한다.
9. 알림 클릭의 서버 요청은 항상 안전한 루트 경로를 사용한다. 알림 종류·원본 ID·내부 목적지는 서버에 전송되지 않는 URL fragment로 전달하고, 인증 확인 후 클라이언트 라우터가 내부 목적지로 이동한다.
10. 일정에 `event_dates`가 하나라도 있으면 그 목록을 실제 개최일의 유일한 원본으로 사용한다. `start_date`와 `end_date`의 범위 판정은 명시 개최일이 없는 연속 일정에만 사용한다. 오늘 일정 카드에는 시리즈 시작일이 아니라 요약 대상 날짜를 저장·표시한다.
...
반복 수업·분리된 축제 기간은 캘린더와 오늘 일정 알림이 같은 개최일 계약을 사용한다. 설정 API의 일시 장애가 저장된 ON/OFF 값을 뒤집어 보이게 하지 않는다.
