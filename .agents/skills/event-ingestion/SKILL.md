...
- 실행 명령: `node .Codex/skills/event-ingestion/scripts/check-db-events.js [시작일] [종료일]`
...
- [ ] 과거 데이터 청소(앵커 보존): `node .Codex/skills/event-ingestion/scripts/cleanup-scraped-events.cjs` 실행
- [ ] `node .Codex/skills/event-ingestion/scripts/check-db-events.js YYYY-MM-DD YYYY-MM-DD` 실행
...
- [ ] **[순서 2: 중복 체크]** `node .Codex/skills/event-ingestion/scripts/check-duplicate.js` 실행
