...
- **필터링 규칙**: 검색어에 날짜를 넣는 대신, **검색 엔진의 기간 설정(Custom Range) 기능**을 사용하여 **지난 1달(등록일 기준)** 동안의 게시물이 검색되도록 필터를 적용합니다.
    - **이유**: 검색 엔진의 필터는 게시물의 **등록일(Posted Date)** 기준입니다. 수집 대상은 **수집 시점 기준 향후 3주간(시작일 기준)** 개최되는 이벤트이지만, 이를 찾기 위해서는 최근 1달 내에 올라온 홍보물을 검색해야 하기 때문입니다.
    - **구글**: '도구 > 전체 기간 > 지난 1개월'을 선택합니다.
    - **네이버**: '옵션 > 기간 > 1개월'을 선택합니다.
