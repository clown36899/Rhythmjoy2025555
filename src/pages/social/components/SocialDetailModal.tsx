...
const SocialDetailModal: React.FC<SocialDetailModalProps> = ({
    ...
    const openMap = () => {
        if (schedule.address) {
            window.open(`https://map.kakao.com/link/search/${encodeURIComponent(schedule.address)}`, '_blank');
        } else if (schedule.place_name) {
            window.open(`https://map.kakao.com/link/search/${encodeURIComponent(schedule.place_name)}`, '_blank');
        }
    };
