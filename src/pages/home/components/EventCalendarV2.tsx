...
export default function EventCalendarV2({
        ...
        return days.map((day) => {
            ...
            return (
                <div
                    key={dateString}
                    onClick={(e) => handleDateClick(day, e.nativeEvent as PointerEvent)}
                    className={`calendar-cell ${isOtherMonth ? "calendar-cell-other-month" : ""} ${isSelected ? "calendar-cell-selected" : ""}`}
                    style={{
                ...
                >
                    {/* 날짜 숫자 */}
                    <div className="calendar-date-number-container">
                        <span
                    ...
                    <div className="calendar-event-bars-container">
                        {eventBarsData.map((barData, idx) => (
                            <div
                                key={idx}
                                className="calendar-event-bar-slot"
                            >
                                {barData && (
                                    <div
                                        className={`calendar-event-bar ${barData.categoryColor} ${barData.isStart ? "calendar-event-bar-start" : ""} ${barData.isEnd ? "calendar-event-bar-end" : ""}`}
                                        style={{
                                            opacity: barData.isFaded ? 0.2 : 0.6, // 배경 바는 연하게
                                        }}
                                    />
                                )}
                            </div>
                    ...
                    <div className="calendar-dots-container">
                        {singleDayEvents.slice(0, 4).map((event, index) => {
                            const isFaded = selectedDateEventIds !== null && !selectedDateEventIds.has(event.id);
                            return (
                                <div
                                    key={index}
                                    className={`calendar-dot ${event.category === "class" ? "cal-bg-purple-500" : "cal-bg-blue-500"}`}
                                    style={{
                        ...
                        {singleDayEvents.length > 4 && (
                            <span className="calendar-more-dots">+</span>
                        )}
                    </div>
                </div>
            );
    ...
    return (
        <div
            ...
            style={{
                transform: `translateY(${externalDragOffset}px)`,
                transition: externalIsAnimating ? "transform 0.25s ease-out" : "none",
        ...
        >
                        ...
                        {showMonthDropdown && (
                            <div className="calendar-month-dropdown">
                                {monthNames.map((name, index) => (
                                    <div
                                        key={name}
                                        className={`calendar-month-option ${currentMonth.getMonth() === index ? "selected" : ""}`}
            ...
            {/* 달력 그리드 */}
            <div
                className={`calendar-grid ${isFullscreen ? 'calendar-grid-fullscreen' : ''}`}
                style={{
                    flex: 1, // 남은 공간 모두 차지
                    display: 'grid',
                    gridTemplateColumns: 'repeat(7, 1fr)',
                    gridAutoRows: isFullscreen ? 'minmax(80px, 1fr)' : '1fr', // 전체화면일 때 최소 높이 보장
                    position: 'relative', // 오버레이 위치 기준
                }}
            >
                {/* 오버레이 (일반 모드에서만) */}
                {!isFullscreen && renderMultiDayTitlesOverlay(currentDays, currentMonth)}

                {/* 그리드 셀 */}
                {isFullscreen
                    ? renderFullscreenGrid(currentDays, currentMonth)
                    : renderNormalGrid(currentDays, currentMonth)
                }
            </div>
