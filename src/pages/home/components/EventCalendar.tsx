...
export default function EventCalendar({ selectedDate, onDateSelect, onMonthChange, showHeader = true, currentMonth: externalCurrentMonth, onEventsUpdate }: EventCalendarProps) {
      ...
      <div className="bg-gray-800 rounded-none lg:rounded-lg p-0 lg:p-6 h-full flex flex-col">
            ...
            <h2 className="text-2xl font-bold text-white">
              ...
              <div className="relative inline-block">
                ...
                >
                  ...
                  <i className={`ri-arrow-down-s-line transition-transform ${showMonthDropdown ? 'rotate-180' : ''}`}></i>
                </button>
        ...
        <div className="grid grid-cols-7 gap-0 lg:gap-1 mb-0 lg:mb-4 px-1 lg:px-0 h-4 lg:h-auto pt-2 lg:pt-0">
          {['일', '월', '화', '수', '목', '금', '토'].map(day => (
            <div key={day} className="text-center text-gray-400 font-semibold py-0 lg:py-2 text-[9px] lg:text-sm flex items-center justify-center">
              {day}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-0 lg:gap-1 flex-1 px-1 lg:px-0 pb-2 lg:pb-0">
          {days.map((day, index) => {
            ...
            return (
              <div key={index} className="h-6 lg:aspect-square p-0 lg:p-0">
                {day && (
                  <button
                    onClick={() => handleDateClick(day)}
                    className={`w-full h-full flex flex-col items-center justify-center text-[9px] lg:text-sm rounded lg:rounded-lg transition-all duration-300 cursor-pointer relative overflow-hidden ${
                  ...
                  >
                    ...
                    {eventCount > 0 && (
                      <div className={`flex items-center justify-center relative z-10 ${todayFlag ? 'mt-0' : 'mt-0.5 lg:mt-1'}`}>
                        {eventCount <= 3 ? (
