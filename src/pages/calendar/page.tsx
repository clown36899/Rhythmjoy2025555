...
const AdminBillboardModal = lazy(() => import("../v2/components/AdminBillboardModal"));
...
// import { useCalendarGesture } from "../v2/hooks/useCalendarGesture"; // Not needed for calendar-only page
...
export default function CalendarPage() {
    ...
    // --------------------------------------------------------------------------------
    // DISABLED for calendar-only page - we don't want gesture controls
    // const {
    ...
    // Force fullscreen mode for calendar-only page (no gestures)
    const calendarMode = 'fullscreen';
    const setCalendarMode = (_mode: 'collapsed' | 'expanded' | 'fullscreen') => { }; // No-op
    const dragOffset = 0;
    const isAnimating = false;
    const getTargetHeight = () => {
        if (typeof window === 'undefined') return 600;
        const actualViewportHeight = window.visualViewport?.height || window.innerHeight;
        const bottomNavHeight = 130;
        const filterBarHeight = 34;
        const SAFETY_BUFFER = 30;
        return actualViewportHeight - 50 - bottomNavHeight - filterBarHeight - SAFETY_BUFFER;
    };
