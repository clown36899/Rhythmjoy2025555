...
const fixedSocialLinks: Record<string, { label: string; icon: string }> = {
    instagram: { label: 'Instagram', icon: 'ri-instagram-fill' },
                    ...
                    {user ? (
                        <div className={`SD-userProfile is-theme-${profileTheme}`}>
                            ...
                            <div className={`SD-profileBody ${hasProfileDetails ? '' : 'is-empty'}`.trim()}>
                                ...
                                <div className="SD-profileMetaRow" aria-label="프로필 활동 정보와 외부 링크">
                                    {profileRegion ? (
                                        <span className="SD-profileMetaChip">
                                        ...
                                        <button
                                            type="button"
                                            className="SD-profileMetaChip is-empty"
                                            onClick={openProfileEditor}
                                            aria-label="활동 지역 입력"
                                            title="활동 지역 입력"
                                        >
                                    ...
                                    {profileGenres.map((genre) => (
                                        <span className="SD-profileMetaChip" key={genre}>
                                    ...
                                    {profileSocialLinks.map((link) => (
                                        <a
                                            className="SD-profileSocialIcon"
                                            key={link.key}
                                            href={link.url}
                                            target="_blank"
                                            rel="noreferrer"
                                            data-social={fixedSocialLinks[link.key] ? link.key : 'other'}
                                    ...
                                    {profileSocialLinks.length === 0 && (
                                        <button
                                            type="button"
                                            className="SD-profileSocialIcon is-empty"
                                            onClick={openProfileEditor}
                                            aria-label="SNS 또는 웹사이트 연결"
                                            title="SNS 또는 웹사이트 연결"
                                        >
